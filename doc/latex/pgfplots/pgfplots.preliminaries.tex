...
\begin{enumerate}
  \item \PGFPlots\ 1.11 introduces
  	\begin{itemize}
		\item |\strut| for every default tick label. This allows tick labels to have the same baseline even if they have different heights.
		\item The |axis cs| is now the default coordinate system: if you write |\draw (1,2) -- (2,2);| \PGFPlots\ will automatically treat it as |\draw (axis cs:1,2) -- (axis cs:2,2);|.
	\end{itemize}
...
\begin{pgfplotskey}{compat=\mchoice{1.11,1.10,1.9,1.8,1.7,1.6,1.5.1,1.5,1.4,1.3,pre 1.3,default} (initially default)}
...
\end{codeexample}
	...
	\begin{pgfplotskeylist}{%
		...
		compat/ticklabel struts=\meta{version},%
...
\end{codeexample}
	...
	The |compat/ticklabel struts| key determines if to insert |\strut| for every |xticklabel|, |yticklabel|, and |zticklabel| generated by \PGFPlots\ styles. Note that this affects the bounding box of figures due to enlarged tick labels\index{Bounding Box Control!ticklabel struts}.
