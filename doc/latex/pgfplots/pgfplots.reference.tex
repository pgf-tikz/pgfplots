...
\pageref{page:aftergroupstack:as:list}
\begin{pgfplotskey}{use aftergroup stack as fast list=\mchoice{true,false} (default true)}
	This key enables a quite unusual runtime optimization which is discussed in more detail in section~\ref{sec:pgfplots:optimization}. In short, it uses the \TeX\ save stack as a list-datastructure to collect point coordinates. This has two consequences:
	\begin{enumerate}
		...
		Without aftergroup stacks, \PGFPlots\ requires $O(N^2)$ time to update axis limits, apply all transformation or collect mark positions. This is due to the runtime complexity for appending something to the end of a \TeX\ macro: $N$ appends with each $O(N)$ time lead to $O(N^2)$ time in sum.
	...
	\end{enumerate}
	Please note that |use aftergroup stack as fast list| does \emph{not} apply to the plot table statement. Furthermore, it does \emph{not} apply to stacked plots and to error bars. These cases still require macro-append statements with $O(N^2)$ runtime.
