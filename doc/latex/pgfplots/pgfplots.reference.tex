...
By default the axis lines are drawn as a ``\texttt{box}, but it is possible to change the appearance of the $x$~and~$y$ axis lines.
...
\begin{axis}[
	...
	\addplot[blue,mark=none] plot[id=square,domain=-4:4,samples=20] function{x*x+x+104};
\end{axis}
\end{tikzpicture}
\end{codeexample}
...
\begin{pgfplotsxykey}{minor tick num=\marg{number} (initially 0)}
	Sets the number of minor ticks for linear axis.
...
Default is ``\texttt{inside}''. this setting applies to both $x$~and~$y$ axis.
\end{pgfplotskey}
