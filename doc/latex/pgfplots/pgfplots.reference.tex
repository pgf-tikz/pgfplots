...
\begin{addplotoperation}[]{graphics}{\marg{file name}}
	...
	Of course, one could do this manually by providing proper scales and such. The operation |plot graphics| is intended so simplify this process -- although the user will still have enough to do.
...
print -depsc external1
\end{codeexample}
	\noindent which is then found in |external1.eps| (possibly exported to |.pdf|). The |surf| command of Matlab generates the surface, the following commands disable the axis descriptions, initialise the desired view and export it. Then, we can use
\begin{codeexample}[]
\begin{tikzpicture}
...
\end{tikzpicture}
\end{codeexample}
\noindent to load the graphics just as if we would have drawn it with \PGFPlots. Unsurprisingly, this wasn't enough. We observe that 
\begin{enumerate} 
	\item The bounding box of the graphics is not correct. This is a particular weakness of Matlab as far as I know -- it never produces tight bounding boxes.
	\item The graphics is on top of the axis, it should be the other way.
\end{enumerate}
The second point can be done with |axis on top|. The first needs modifications of the bounding box -- which has to be done manually. There are a couple of tools around which a more or less successful in defining a correct bounding box. One is |pdfcrop|, for example. In most cases, you will need to adjust them manually. Some viewers allow to access the low--level image coordinates (like |gv|). You can use such a viewer to determine the exact bounding box. As soon as we know the correct bounding box, we can do one of the following things:
\begin{enumerate} 
	\item Write the correct bounding box in the |.eps| file with a text editor. That's not difficult.
	\item Tell |\includegraphics| how to modify the existing bounding box. That's even simpler and more portable to |.pdf| documents.
\end{enumerate}
Let's follow the second approach here. Using |gv|, I determined that the bounding box needs to be shifted |1| to the left, |2| down. Furthermore, the right end is |3| too far and the top area has about |21| units space wasted. This can be provided to the |trim| option of |\includegraphics|, and we use |clip| to clip the rest away: 
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}[enlargelimits=false,axis on top]
		\addplot graphics
			...
			includegraphics={trim=1 2 3 21,clip}]
			{external1};
	\end{axis}
\end{tikzpicture}
\end{codeexample}
	...
	Lets' try another example before we turn to the expected key--value options. We use |gnuplot| to draw a (relatively stupid) example data set. The gnuplot script
\begin{codeexample}[code only]
set samples 30000
set parametric
unset border
...
\end{codeexample}
\noindent generates |external2.eps| with a uniform random sample of size $30000$. As before, we import this scatter plot into \PGFPlots\ using |plot graphics|. Again, the bounding box is too large, so we need to adjust it.
\begin{codeexample}[]
\begin{tikzpicture}
...
\paragraph{Details about \texttt{plot graphics}}:
	...
	where the |node| style is a configurable style. The node is placed at the coordinate designated by |xmin|, |ymax|. 
	...
	The scaling will thus ignore the aspect ratio of the external image and prefer the one used by \PGFPlots. You will need to scale the axis if you want to adjust the \PGFPlots\ axis ratio. Use the |scale only axis| key in such a case.
\end{addplotoperation}
\begin{pgfplotsxykeylist}{
...
\begin{stylekey}{/pgfplots/plot graphics/node}
	A predefined style which for the node containing the graphics. The predefined value is
\begin{codeexample}[code only]
\pgfplotsset{
