...
\begin{pgfplotsxykey}{axis \x\ line=\mchoice{box,top,middle,center,bottom,none} (initially box)}
Allows to choose the location of the $x$ (or $y$) axis line(s).
The labels and ticks are placed accordingly.
The choice |bottom| will draw only a line at $y=y_\text{min}$, |middle| will draw a line at $y=0$, and |top| will draw only a line at $y=y_\text{max}$. Finally, |box| is a combination of options |top| and |bottom|.
...
\begin{pgfplotskey}{separate axis lines=\marg{true,false} (default true)}
	Enables or disables separate path commands for every axis line. This option affects only the cases with non-centered axis lines, i.e. it affects only the axis lines which are part of the outer box.

	Both cases have their advantages and disadvantages, I fear there is no reasonable default (suggestions are welcome).

	The case |separate axis lines=true| allows to draw arrow heads on each single axis line, but it can't close edges very will -- in case of thick lines, you will see unsatisfactory edges.
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}[
	separate axis lines,
...
\begin{stylekey}{/pgfplots/hide axis=\mchoice{true,false}}
