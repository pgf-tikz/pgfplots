...
\end{lstlisting}
The first syntax chooses the next unused plot specification of the list specified by \texttt{cycle list=LIST} (see section~\ref{sec:cycle:list}) and the second syntax specifies a plot specification explicitly. This specification will be used inside of the legend (if any). The `\texttt{plot coordinates}', `\texttt{plot file}' and plot expression variants are well--known \Tikz\ plotting methods. The `\texttt{plot table}' command is introduced in \PGFPlots\ and allows to draw specific columns of a text file against each other (see below for more details). The \Tikz--Gnuplot interface and the plot--expressions are not yet supported by \PGFPlots, sorry.
...
\begin{lstlisting}
\addplot plot table[xindex=2,yindex=3] {datafile.dat};
\addplot plot table[x=dof,yindex=2] {datafile.dat};
\end{lstlisting}

Summary and remarks:
\begin{itemize}
	\item Use \texttt{plot table[x=COLNAME,y=COLNAME]} to access column names. Those names are case sensitive and need to exist.
	\item Use \texttt{plot table[xindex=INTEGER,yindex=INTEGER]} to access column indices. Indexing starts with~$0$. You may also use an index for~$x$ and a column name for~$y$.
...
\end{lstlisting}
	\item The accepted input format is as follows:
		\begin{itemize}
			...
			\item Any line starting with `\#' is ignored.
...
\subsubsection{\texttt{log identify minor tick positions=[true$|$false]}}
\subsubsection{\texttt{log number format code/.code=\{\TeX-code\}}}
\subsubsection{\texttt{log base 10 number format code/.code=\{\TeX-code\}}}
...
\item[\texttt{every extra x tick}] FIXME
\item[\texttt{every extra y tick}] FIXME
...
\end{axis}
\end{lstlisting}
	I began using these \lstinline!\listnew!-macros, but I guess it is better to use the `\texttt{cycle list}' option and styles to get the same effect.
\end{enumerate}
Please note that the `\lstinline!\\!' is required to separate each element. Furthermore, the comment characters~`\%' are necessary if the elements are on different lines.
