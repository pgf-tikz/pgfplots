...
\begin{addplotoperation}[]{expression \marg{math expr}}{}
	Similar to |plot gnuplot|, this input method allows to provide expressions which will be sampled. But unlike |plot gnuplot|, the expressions are evaluated using the math parser of \PGF, no external program is required.
...
\paragraph{Remarks}
\begin{enumerate} 
	\item What really goes on is a loop which assigns the current sample coordinate to the macro |\x| (can be customized with |variable=\t| or so). \PGFPlots\ defines a math constant |x| which always has the same value as |\x|. This is no longer true if you redefine |variable|.
...
%\index{y@\texttt{\textbackslash y} In Coordinate Expressions}%
	...
	\item The complete set of math expressions can be found in the \PGF\ manual. Please note, however, that trigonometric functions are defined in degrees. The character `|^|' is used for exponentiation (not `|**|' as in gnuplot).
...
\begin{enumerate}
	\item Use |plot gnuplot|. It uses the external, freely available program |gnuplot| as desktop calculator. \PGFPlots\ as such is capabable of handling large numbers with reasonable accuracy.
...
\message{FIXME: incompatibility with FP introduced here: 485 in reference.tex}%
%--------------------------------------------------
% \begin{codeexample}[]
% % Requires:
% % \usepackage{fp}
...
% \end{codeexample}
%-------------------------------------------------- 
\noindent Please note, however, that |fp| is relatively slow. It may be faster to use |plot gnuplot| which invokes |gnuplot| and leads to the same result.
\end{addplotoperation}
...
\begin{stylekey}{/pgfplots/scatter/classes=\marg{styles for each classname}}
	A scatter plot style which visualizes points using several classes. The style assumes that every point coordinate has a class label attached, that means the choice |scatter src=explicit symbolic| is assumed. A class label can be a number, but it can also be a symbolic constant. Given class labels for every point, \marg{styles for each classname} contains a comma-separated list which associates appearance options to each class label.
\begin{codeexample}[]
\begin{tikzpicture}
...
\begin{tikzpicture}
	\begin{axis}[axis equal=false]
		\addplot[blue] gnuplot[domain=0:2*pi,samples=300] {sin(x)*sin(2*x)};
	\end{axis}
\end{tikzpicture}
\hspace{1cm}
\begin{tikzpicture}
...
\begin{tikzpicture}
	\begin{loglogaxis}[axis equal=false]
		\addplot gnuplot[domain=0:10000] {x**-2};
	\end{loglogaxis}
\end{tikzpicture}
\hspace{1cm}
\begin{tikzpicture}
...
\begin{codeexample}[]
\begin{tikzpicture}[domain=0:6.2832,samples=200]
   \begin{axis}[
      legend style={
         overlay,
         at={(-0.5,0.5)},
