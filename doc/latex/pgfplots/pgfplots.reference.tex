...
\end{codeexample}
	...
	\item The \meta{options} are remembered for the legend. Furthermore, they are available as `|current plot style|' as long as the path is not yet finished or in associated error bars.
...
\begin{addplot3operation}[]{coordinates}{\marg{coordinate list}}
	The |\addplot3 coordinates| method is an extension of the twodimensional variant, |\addplot coordinates| which is described in all detail on page~\pageref{pgfplots:addplot:coordinates}.
...
\begin{pgfplotskey}{colormap=\marg{name}\marg{color specification}}
	...
	The syntax of \marg{color specification} can be either the same as those for \PGF\ shadings described in~\cite[VIII -- Shadings]{tikz}: a series of colors along with a length separated by semicolons or a simplified form.
\begin{codeexample}[code only]
...
Each entry has the form \meta{color model}|(|\meta{length}|)=(|\meta{arguments}|)| where the \meta{length} argument is optional as discussed. The example above means that the left end of the colormap shall have RGB components $1,0,0$, indicating $100\%$ red and $0\%$ green and blue. The next entity starts at |1cm| and describes a color with $100\%$ green. The |rgb255| also expects three RGB components, but in the range $[0,255]$. Finally, |gray| specifies a color in parenthesis with the same value for each, R G and B and |color| accesses predefined colors.
...
FIXME: replicate from page \pageref{pgfplots:scatter:src}. How do I split these (identical) parts of the docs?

\end{pgfplotskey}

\begin{pgfplotskey}{scatter src=\marg{value}}
	This is an alias for |point meta=|\marg{value}.

	See also the scatter--plot specific documentation of this key on page~\pageref{pgfplots:scatter:src}.
\end{pgfplotskey}
...
All axis descriptions are placed usually relative to the axis' rectangle such that |(0,0)| is the lower left corner and |(1,1)| is the upper right corner. In fact, coordinates without unit like |(0,0)| or |(1.03,1)| are interpreted as coordinates in the \declaretext{axis description cs}. This coordinate system is used automatically, so it might never be necessary to use it explicitly. It is described in the following.
...
\end{codeexample}
	...
	\item Since the view does not influence these positions, |axis description cs| might not be a good choice for axis labels in 3D. The \declaretext{ticklabel cs} is used in this case, see page~\pageref{sec:ticklabelcs}.
\end{itemize}
\end{coordinatesystem}


Axis descriptions can be placed using the predefined node \declaretext{current axis}. At the time when axis descriptions are drawn, all anchors which refer to the axis origin (that means the ``real'' point $(0,0)$) or any of the axis corners can be references using |current axis.|\meta{anchor name}. Please see section~\ref{pgfplots:sec:align}, Alignment, for further details.
...
\begin{pgfplotskey}{view=\marg{azimuth}\marg{elongation} (initially \{25\}\{30\})}
	FIXME
\end{pgfplotskey}

\begin{pgfplotskeylist}{%
	xticklabel dist=\marg{dimension} (initially 0cm),%
	yticklabel dist=\marg{dimension} (initially 0cm),%
	zticklabel dist=\marg{dimension} (initially 0cm)}
	FIXME
\end{pgfplotskeylist}
...
\label{option:anchor}%
This option shifts the axis horizontally and vertically such that the axis anchor (a point on the axis) is placed at coordinate $(0,0)$. For users who are familiar with \Tikz: an axis is actually a very special node, so anchors work as in~\cite{tikz}.
...
\begin{command}{\pgfplotspointaxisxy\marg{x coordinate}\marg{y coordinate}}
