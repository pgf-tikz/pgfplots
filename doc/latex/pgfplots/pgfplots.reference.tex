...
\begin{environment}{{axis}[\meta{options}]}
...
\begin{environment}{{semilogxaxis}[\meta{options}]}
...
\begin{environment}{{semilogyaxis}[\meta{options}]}
...
\begin{environment}{{loglogaxis}[\meta{options}]}
...
\end{codeexample}
\noindent Due to its simplicity, |plot file| is faster while |plot table| allows higher flexibility.
\end{addplotoperation}
...
\begin{key}{/tikz/mark options=\marg{options}}
	This \Tikz\ option allows to add \marg{options} to markers, for example to set drawing colors or transformations like scaling or rotation. The keys |mark options| accumulate.
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}[y=2cm]
	\addplot coordinates 
...
\begin{codeexample}[]
\tikzset{mark options={scale=0.4}}
\begin{tikzpicture}
\begin{axis}[y=2cm]
	\addplot+[mark options={scale=2}] 
		coordinates 
		{(-2,0) (-1,1) (0,0) (1,1) (2,0)};
\end{axis}
\end{tikzpicture}
...
\pgfplotsset{
  every axis plot post/.append style={
   mark=triangle,mark options={rotate=90}}}
\begin{tikzpicture}
\begin{axis}[y=2cm]
	\addplot coordinates 
		{(-2,0) (-1,1) (0,0) (1,1) (2,0)};
...
\end{codeexample}
so their position and appearance can be customized. The coordinate \lstinline!(0,0)! denotes the lower left axis corner and \lstinline!(1,1)! the upper right. 
...
The starred versions $\dotsc$|line*| \emph{only} affect the axis lines, without correcting the positions of axis labels, tick lines or other keys which are (possibly) affected by a changed axis line. The non-starred versions are actually styles which also set
\begin{enumerate}
	\item |tick align=center| for the respective axis,
	\item |enlarge limits=false| for the respective axis,
	\item axis label style such that axis labels are placed on the opposite site of tick labels,
	\item arrow heads for the respective axis.
\end{enumerate}
For centered axis lines, axis labels will be placed near the end of the axis. For non-centered axis lines, labels will be centered and placed besides the axis line (possibly rotated).
You will need to overwrite these options if the default doesn't fit your needs. 

The style |default non boxed lines| will be set for each non-boxed line variation to install arrow heads, see below.
...
\begin{stylekey}{/pgfplots/default non boxed lines (initially -stealth)}
	A style which is used by the |axis x line| and |axis y line| options. It is installed for every non-boxed axis line variation.

	The default is to use |-stealth| as arrow heads.
\end{stylekey}

\subsubsection{Two Ordinates ($y$ axis)}
In some applications, more than one $y$ axis is used if the $x$ range is the same. \PGFPlots\ does not support this natively (yet?), sorry. However, it is possible with to realize two $y$ axis without too much work ``by hand''. 

This section demonstrates one possibility to do so. Maybe I find time to realize such a feature natively~-- until then, I don't see a better way.
...
\end{tikzpicture}
\end{codeexample}
The basic idea is to draw two axis ``on top'' of each other -- one, which contains the $x$ axis and the left $y$ axis, and one which has \emph{only} the right $y$ axis. This requires attention in the following possibly non-obvious aspects:
\begin{enumerate}
...
\begin{xystylekey}{/pgfplots/every outer \x\ axis line}
	Installed for every axis line which lies on the outer box.

	There are more options to install axis lines, see below for a list or section~\ref{pgfplots:page:axislines} on page~\pageref{pgfplots:page:axislines} for details.

	If you want arrow heads, you may also need to check the |separate axis lines| boolean key.
\end{xystylekey}

\begin{xystylekey}{/pgfplots/every inner \x\ axis line}
...
\begin{xystylekey}{/pgfplots/every \x\ tick label}
...
\begin{xystylekey}{/pgfplots/every \x\ tick scale label}
 Configures placement and display of the nodes containing the order of magnitude of tick labels, see~\ref{sec:scaled:ticks} for more information about |scaled ticks|.
\end{xystylekey}


\begin{xystylekey}{/pgfplots/every \x\ tick}
	Installed for tick \emph{lines} on either $x$ or $y$ axis.
\end{xystylekey}

\begin{xystylekey}{/pgfplots/every minor \x\ tick}
	Installed for minor tick lines on either $x$ or $y$ axis.
\end{xystylekey}

\begin{xystylekey}{/pgfplots/every major \x\ tick}
...
\begin{xystylekey}{/pgfplots/every extra \x\ tick}
 ...
 The default is to set |log identify minor tick positions=true|.
\end{xystylekey}
...
\begin{xystylekey}{/pgfplots/every axis \x\ grid}
	Used for each grid line in either $x$ or $y$ direction.
\end{xystylekey}

\begin{xystylekey}{/pgfplots/every minor \x\ grid}
	Used for each minor grid line in either $x$ or $y$ direction.
\end{xystylekey}

\begin{xystylekey}{/pgfplots/every major \x\ grid}
...
All these options append their argument to the corresponding `|every ...|'--styles.
...
\begin{pgfplotsxykeylist}{%
	axis line style=\marg{$\dots$},
	inner axis line style=\marg{$\dots$},
	outer axis line style=\marg{$\dots$},
	\x\ axis line style=\marg{$\dots$}}
...
\label{page:aftergroupstack:as:list}
\begin{pgfplotskey}{use aftergroup stack as fast list=\mchoice{true,false} (default true)}
	This key enables a quite unusual runtime optimization. It uses the \TeX\ save stack as a list-datastructure to collect point coordinates. This has two consequences:
	\begin{enumerate}
		\item \PGFPlots\ will run faster if there are a lot of coordinates. Let $N$ be the number of points in a single plot statement. The aftergroup stack will then lead to $O(N)$ time for most cases.
		
		Without aftergroup stacks, \PGFPlots\ requires $O(N^2)$ time for $N$ coordinates. This is due to the runtime complexity for appending something to the end of a \TeX\ macro: $N$ separate appends lead to $O(N^2)$ time in sum because redefining a macro requires time proportional to the macro's length (which is $O(N)$).
		
		For plots with about $10$ data points, this won't make a significant difference.

		\item \PGFPlots\ occupies the save stack, for which \TeX\ has never been designed. To use this key, almost all common \TeX\ distributions will need to be reconfigured to use a larger save stack. Please see section~\ref{sec:pgfplots:optimization} for advice how to change memory limitations.

		Furthermore, the save stack seems to have some hard limits which can't be exceeded.
	\end{enumerate}

	\paragraph{Limitations:} Please note that |use aftergroup stack as fast list| does \emph{not} apply to the plot table statement because this one collects several macros simulatenously. Furthermore, it does \emph{not} apply to stacked plots and to error bars. These cases still require macro-append statements with $O(N^2)$ runtime.

	It appears that the system layer of pgf also uses macro append to assemble long paths. In such a case, this optimization does not use anything, sorry.

	It has been tested successfully with scatter plots.

	Since standard \TeX\ distributions provide a very limited save stack, this option is disabled initially.
\end{pgfplotskey}
