...
\begin{command}{\pgfplotsaxispointxy\marg{x coordinate}\marg{y coordinate}}
	A point convert like |\pgfpointxy| which applies any necessary coordinate transformations to \marg{x coordinate} and \marg{y coordinate}.
...
\begin{commandlist}{%
	...
	Defines |\pgfmathresult| to be the low-level \PGF\ coordinate which is used for \marg{x coordinate of an axis}.

	The command applies any |x coord trafo| keys, data scalings and/or logarithms or whatever \PGFPlots\ does to map input coordinates to internal coordinates.
