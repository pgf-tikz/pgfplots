...
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}[xbar,tick align=outside,enlargelimits=0.15]
\addplot
...
Figure~\ref{fig:bar:and:comb} top shows an example for `\texttt{ybar}' and `\texttt{xbar}' with (fictional) numbers. Users can place more than one bar plot into the same axis using horizontal/vertical shifts.
...
\begin{pgfplotsxykey}{\x tick=\marg{coordinate list}}
The options \texttt{xtick} and \texttt{ytick} assigns a list of \emph{Positions} where ticks shall be placed. The argument \marg{coordinate list} will be used inside of a \lstinline!\foreach \x in !\marg{coordinate list} statement, and \marg{coordinate list} contains one of the following formats:
\begin{itemize}
...
%\begin{codeexample}[width=4cm,pre={\begin{minipage}{8cm}},post={\end{minipage}}]
\begin{codeexample}[]
\fbox{%
\begin{tikzpicture}%
	\begin{pgfinterruptboundingbox}
