...
\begin{figure}
	...
	\pgfplotsset{every axis/.append style={width=6cm,anchor=north west,y label style={text width=0pt}}}%
...
\subsection{Accessing axis coordinates manually}
...
\end{tikzpicture}
\end{codeexample}
The example above uses |legend style| which is quite the same as |\pgfplotsset{every axis legend/.append style={...}}|.
...
instead of
\begin{codeexample}[code only]
\pgfplotsset{<own style name>/.style={<style variables>}}.
\end{codeexample}
...
\begin{codeexample}[code only]
\pgfplotsset{every axis/.append style={line width=1pt,tick style={line width=0.6pt}}}
\end{codeexample}
	or styles like
\begin{codeexample}[code only]
\pgfplotsset{every axis/.append style={thick,tick style={semithick}}}
\end{codeexample}
\end{key}
