...
\begin{codeexample}[code only]
\pgfplotsset{every axis grid/.style={style=help lines}} % applies to major and minor grids,
\pgfplotsset{every minor grid/.append style={color=blue}}     % applies only to minor grid lines,
\pgfplotsset{every major grid/.append style={thick}}          % applies only to major grid lines.
\end{codeexample}
...
\begin{command}{\pgfplotsset\marg{key-value-list}}
	Defines or sets all options in \marg{key-value-list}. It is a shortcut for |\pgfqkeys{/pgfplots}|\marg{key-value-list}, that means it inserts the prefix |/pgfplots| to any option which has no full path.
