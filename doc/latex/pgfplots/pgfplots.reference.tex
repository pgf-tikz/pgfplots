...
\end{codeexample}
	...
	The key |scatter plot| is simply a boolean variable which enables marker modifications. It applies only to markers and it can be combined with any other plot type.
...
\begin{pgfplotskey}{scatter src=\mchoice{none,x,y,z,explicit} (initially none)}
	...
	Please note that |scatter src|$\neq$|none| results in work even if |scatter plot=false|.
\end{pgfplotskey}
...
\begin{codeexample}[]
% Example: use three different marker classes 
% 0% - 30%   : first class
% 30% - 60%  : second class
% 60% - 100% : third class
\begin{tikzpicture}
\begin{axis}[
scatter plot/@pre marker code/.code={%
...
scatter plot/@post marker code/.code={%
   \endscope
}]

\addplot+[scatter plot]
   plot[scatter src=y,samples=40]
   (\x, {sin(\x r)});
