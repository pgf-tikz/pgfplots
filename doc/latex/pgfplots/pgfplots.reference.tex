...
\subsubsection{The Common Interface for Two-Dimensional Plots}
...
\subsubsection{The Common Interface for Three-Dimensional Plots}

\begin{command}{\addplot3\oarg{options} \meta{input data} \meta{trailing path commands};}
	The common interface for all three dimensional plots is |\addplot3|. Basically, it is the same as the two dimensial |\addplot| command: it reads input coordinates from one of the accepted input methods provided as \meta{input data} and prepares coordinates.

	However, there are some differences to the two dimensional variant: there are line, scatter and mesh plots. Line and scatter plots work in exactly the same way as in two dimensions, i.e.\ a sequence of coordinates is processed in the order of appearance.
...
\subsubsection{Three Dimensional Parameterized Plots}
...
\subsubsection{Three Dimensional Mesh Plots}
\begin{plottype}{mesh}
...
\subsubsection{Three Dimensional Surface Plots}
...
\begin{itemize}
	...
	For threedimensional figures, the |auto| mechanism applies only for the $z$ axis. The $x$ and $y$ axis won't be enlarged. 
...
\end{codeexample}
	\noindent in order to avoid number formatting routines on |\tick| or numerics for tick scale methods.
\end{pgfplotsxycodekeylist}

\subsubsection{String Symbols as Input Coordinates}
It is possible to provide a string dictionary to pgfplots. An input coordinate can than use any symbol provided in that dictionary.
\begin{pgfplotsxykeylist}{symbolic \x\space coords=\marg{dictionary}}%
	A styles which sets |x coord trafo| and |x coord inv trafo| (or the respective |y| or |z| variants) such that any element in \marg{dictionary} is a valid input coordinate. The \marg{dictionary} can be a comma separated list or a list terminated with `|\\|'. In both case, withe spaces are considered to be part of the names (use `|%|' at end of lines).
