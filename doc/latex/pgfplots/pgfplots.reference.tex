...
\end{pgfplotsxykeylist}
\begin{pgfplotsxykeylist}{%
	meta=\marg{column name},
	meta index=\marg{column index}}
...
\end{codeexample}
	...
	The value |base 10:|\meta{exponent} allows to adjust the algorithm manually. For example, |base 10:3| will divide every tick label by $10^3$:
\begin{codeexample}[]
\begin{tikzpicture}
...
\end{codeexample}
	...
	The last option, |scaled ticks=manual:|\marg{text}\marg{scale code} allows even more customization. It allows \emph{full control} over the displayed scaling label \emph{and} the scaling code: \marg{text} is used as-is inside of the tick scaling label while \marg{scale code} is supposed to be a one-argument-macro which scales each tick. Example:
\begin{codeexample}[]
\begin{tikzpicture} 
\begin{axis}[
...
The \marg{text} can be arbitrary. It is completely in user control. The second argument, \marg{scale code} is supposed to be a one-argument-macro in which |#1| is the current tick position in floating point representation. The macro is expected to assign |\pgfmathresult|.

This feature may also be used do transform coordinate in case they can't be processed with \PGFPlots: transform them and supply a proper tick scaling method such that tick labels represent the original range.
\end{pgfplotsxykeylist}
