...
\begin{command}{\addplot\oarg{style options} \meta{input data} \meta{trailing path commands};}
...
The `|plot coordinates|' command is like that provided by \Tikz\ and reads its input data from a sequence of point coordinates.
\begin{codeexample}[code only]
...
Please note that this simple example could be generated without any external tool as is shown in the following example.
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}[view={0}{90},axis on top]
		\addplot3[surf,shader=interp,samples=50,domain=-3:3]
			{exp(-(x - y)^2 - x^2 )};
	\end{axis}
\end{tikzpicture}
\end{codeexample}
\noindent However, the number of samples supported by \PGFPlots\ is restricted by \TeX\ and is usually much smaller than those supported by external tools (not talking about timings). Furthermore, external programs have support of real z buffering which is currently only simulated by \PGFPlots. Thus, it may help to consider |plot graphics| for complicated surface plots.
...
\begin{tikzpicture}
	\begin{axis}[
		...
		ylabel=$y$,
		zlabel=$z$,
		every axis x label/.style={at={(rel axis cs:0.5,-0.15,-0.15)}},
		every axis y label/.style={at={(rel axis cs:1.15,0.5,-0.15)}},
		every axis z label/.style={at={(rel axis cs:-0.15,-0.15,0.5)}},
	]
...
\begin{pgfplotskey}{point meta=\mchoice{none,x,y,z,explicit,explicit symbolic} (initially none)}
	FIXME
\end{pgfplotskey}
...
\begin{pgfplotskeylist}{point meta min=\marg{number},point meta max=\marg{number}}
	FIXME
	FIXME: unclear what happens if provided inside of addplot
\end{pgfplotskeylist}
...
\begin{command}{\ref\marg{label name}}
	...
	You may need to use |\protect\ref|\marg{argument} in \LaTeX\ when |\ref| is used inside of |\caption|, |\section| or other commands with moving arguments.
\end{command}
...
\begin{tikzpicture}
	...
	\pgfplotsextra{%
		\pgfpathmoveto{\pgfplotsaxispointxy{1}{2}}%
		\pgfpathlineto{\pgfplotsaxispointxy{2}{4}}%
		\pgfusepath{stroke}%
	}
	\end{axis}
\end{tikzpicture}
...
\begin{command}{\pgfplotsaxispointxy\marg{x coordinate}\marg{y coordinate}}
	A point command like |\pgfpointxy| which applies any necessary coordinate transformations to \marg{x coordinate} and \marg{y coordinate}.
...
\begin{plottype}{mesh}
	Uses the current colormap to determine colors for each fixed line segment. Each line segment will get the same color.
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
While this works very nice for two dimensional plots, it requires considerably more effort than non--graphical documents. This is even more so for three dimensional plots. In other words: \PGFPlots' three dimensional routines are slow. There are reasons for this and some of them may vanish in future versions. But one of these reasons is, that \TeX\ has never been designed for complex visualisation techniques. Consider the image externalization routines mentioned in section~\ref{sec:pgfplots:export} to reduce typesetting time.

Another issue which arises in three dimensional visualization is: \PGFPlots\ doesn't really know anything about depth. Both, the underlying \PGF\ package and the Portable Document Format (|.pdf|) as such are inherently two--dimensional. \PGFPlots\ does its best to maintain a reasonable efficiency combined with routines which yield good quality. It has z buffering techniques. But still, it is just a smart way of drawing lines and polygons on top of each other. You may encounter this problem sometimes. Maybe it will be improved in future versions.

If you decide that you need both, speed and 100\% reliable z buffers (depth information), you should consider using other visualization tools and return to \PGFPlots\ in several years. If you can wait for a complex picture and you don't even see the limitations arising from z buffering defiancies, you should use \PGFPlots.

Enough for now, let's continue.

\subsubsection{Providing Three Dimensional Coordinate Input}
\begin{command}{\addplot3\oarg{style options} \meta{input data} \meta{trailing path commands};}
...
\end{tikzpicture}
\end{codeexample}
	\noindent uses the \declaretext{samples} and \declaretext{samples y} keys to define a mesh.

	For data input as with |coordinates|, |file| or |table|, the input stream can contain empty lines which mark the end of one matrix row.
...
\end{codeexample}
	...
	We could also have used a file in which the the first column contains $x_i$, the second $y_i$ and a third the $z_i$. Empty lines will also mark the end of a matrix row:
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
\end{tikzpicture}
\end{codeexample}
	Since |\addplot file| and |\addplot table| are very similar, the same would also have worked for |\addplot table|.

	In summary, the same coordinate input methods which are usable for two dimensions can be used for three dimensional coordinates as well. The idea is simply to explain how the long sequence of coordinates translates into a mesh -- and one way to do so is to place empty lines at the end of each complete mesh line.

	Alternatively, we can also supply one of \declaretext{mesh/rows} or \declaretext{mesh/cols} manually. Thus, the example above could be written
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
\end{codeexample}
	...
	As already mentioned, expression plotting is different from plain data input. Here, the |rows| and |cols| of the data matrix are implicitly set by the \declaretext{samples} and \declaretext{samples y} keys.
	% FIXME 
\end{command}
...
\begin{pgfplotskey}{z buffer=\mchoice{none,reverse x seq,reverse y seq,reverse xy seq,sort} (initially none)}
	\begin{itemize}
		\item heuristics
...
\begin{tikzpicture}
	...
	\addplot3[surf,samples=40,domain=0:1] 
		{5*x*sin(2*deg(x)) * y};
	\end{axis}
\end{tikzpicture}
\end{codeexample}
...
\begin{pgfplotskey}{shader=\mchoice{flat,interp,faceted,flat corner,flat mean} (initially interp)}
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
\begin{tikzpicture}
	\begin{axis}[view={60}{30}]
		\addplot3+[domain=0:5*pi,samples=60] 
...
\subsection{Markers, Linestyles and Colors}
\label{sec:markers}%
...
\begin{pgfplotskey}{view=\marg{pitch}\marg{yaw} (initially {25}{30})}
	FIXME
\end{pgfplotskey}

\begin{pgfplotskeylist}{%
	xticklabel dist=\marg{dimension} (initially 0.5cm),%
	yticklabel dist=\marg{dimension} (initially 0.5cm),%
	zticklabel dist=\marg{dimension} (initially 0.1cm)}
	FIXME
\end{pgfplotskeylist}
...
\end{codeexample}
\noindent and/or the shorthand macros |grid style=|\marg{keys}, |minor grid style=|\marg{keys} and |major grid style=|\marg{keys}.
\end{pgfplotsxykeylist}
