...
\begin{tikzpicture}
\begin{axis}[view={60}{-30}]
	\addplot3[mesh,scatter,only marks,scatter src=z,
		samples=30,domain=-1:1,y domain=0:2*pi,z buffer=none]
		({sqrt(1-x^2) * cos(deg(y))},{sqrt( 1-x^2 ) * sin(deg(y))},x);
\end{axis}
\end{tikzpicture}
\end{codeexample}
...
\begin{pgfplotskey}{z buffer=\mchoice{none,reverse x seq,reverse y seq,reverse xy seq,sort coordinates} (initially none)}
	\begin{itemize}
		\item heuristics
		\item applies to all 3D plots (including scatter)
		\item only partially implemented
		\item |sort coordinates| is not implemented for mesh yet
