...
\end{tikzpicture}
\end{codeexample}
\noindent to load the graphics just as if we would have drawn it with \PGFPlots. The |axis on top| simply tells \PGFPlots\ to draw the axis on top of any plots (see its description).
...
\begin{enumerate}
	\item The vector graphics program |inkscape| appears to be pretty good at it. Simply import graphics and save as |.eps|. The bounding box will be adjusted automatically.
	\item I know of |pdfcrop| for |.pdf| images -- although |inkscape| may be the better choice.
\end{enumerate}
...
Using |gv|, I determined that the bounding box needs to be shifted |1| to the left and |2| down. Furthermore, the right end is |3| too far and the top area has about |21| units space wasted. This can be provided to the |trim| option of |\includegraphics|, and we use |clip| to clip the rest away: 
\begin{codeexample}[]
\begin{tikzpicture}
