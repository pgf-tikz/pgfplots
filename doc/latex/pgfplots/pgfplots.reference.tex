...
\begin{pgfplotskey}{plot graphics/lowlevel draw=\marg{width}\marg{height}}
	...
	Thus, you can tweak |plot graphics| to place any \TeX\ box of the desired dimensions into an axis between the provided minimum and maximum coordinates.
\end{pgfplotskey}
...
\begin{tikzpicture}
	\begin{axis}
        ...
        % the file may have more columns.
		\addplot+[point meta=explicit]
			table[x=xcolname,y=ycolname,meta expr={0.5*(\thisrow{anything} + sqrt(\thisrow{othercol}))}] 
				{datafile.dat};
	\end{axis}
\end{tikzpicture}
\end{codeexample}
...
\begin{tikzpicture}
	\begin{axis}[
		...
		samples=50,point meta rel=axis wide,point meta=y]
...
\begin{tikzpicture}
	\begin{axis}[
		...
		samples=50,point meta rel=per plot,point meta=y]
...
\end{codeexample}
	...
	notes:
	- axis name 'parent axis'
	- axis name 'current colorbar axis'
	- 'point meta min/max' contains the values
	- list styles which are inherited from parent
	- |parent axis width|
	- |parent axis height|
\end{pgfplotskey}
...
\end{stylekey}
\begin{stylekey}{/pgfplots/colorbar vertical}
\end{stylekey}
