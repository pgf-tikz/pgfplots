...
The first syntax, \declareandlabel{of colormap}, allows to convert the colors of a |colormap| to a |cycle list|. It accepts a |colormap name| as argument and defaults to the most recently assigned |colormap name|. This choice merely converts the colors as they are found in the colormap into the cycle list, i.e.\ there is no interpolation involved. Applying this to the default |colormap name=hot| which has $4$~colors results in the following example:
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}[
...
Finally, there is the choice \declareandlabel{indices of colormap}|=|\marg{list} (or, optionally, |indices of colormap=|\marg{list}| of |\meta{colormap name}). It accepts a list of indices (similar to |index of colormap|). Indices which are out of range are clipped to the nearest index. For example, |viridis| comes with $\pgfplotscolormapsizeof{viridis}$~elements and we can write
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}[
