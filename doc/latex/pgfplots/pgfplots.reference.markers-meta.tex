...
\pgfplotscolorbardrawstandalone[
	...
	colormap access=map]
\end{codeexample}
\noindent The example above shows how curly braces can simplify the color map definition. It also shows that the absolute values of the positions are meaningless as they are mapped to $[0,1000]$ anyway -- but their relative positions are of importance. If you add |point meta min| and |point meta max|, you can see how they are mapped.
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
		samples of colormap={
...
\end{codeexample}
	...
	One thing in this context needs \emph{special attention}: we told the system to use $5$~colors, but the case |colormap access=const| merely shows~$4$ colors! The visualization of colormaps with positions combined with |colormap access=const| is special, see also |colormap access/extra interval width| for details regarding the presence of an artificial interval for |colormap access=const|.
...
\pgfplotscolorbardrawstandalone[
    ...
    colormap access=const]
\end{codeexample}
	\noindent As already mentioned, |colormap access=const| ignores the rightmost color (``|red|'').
