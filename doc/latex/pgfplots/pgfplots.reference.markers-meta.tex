...
\pgfplotsshowcolormapexample{rgb=(0,0,1) color=(black) rgb255=(238,140,238)}
	...
	Here, the three input colors form the left end, middle point, and right end of the interval, respectively. A couple of \meta{color type}s are available, the the \meta{color value} depends on the actual \meta{color type} which is shown below in all detail. Most |colormap| definitions use the simple form and merely list suitable color definitions.
...
\pgfplotsshowcolormapexample{rgb(-500)=(0,0,1) color(0)=(black) rgb255(1500)=(238,140,238)}
	...
	This syntax allows to distribute colors over using non-uniform distances.\footnote{Note that \PGFPlots\ up to and including version $1.13$ offered just rudimentary support for non-uniform color maps.} The positions can be arbitrary numbers\footnote{Note that \PGFPlots\ up to and including version $1.13$ only supported ranges $[0,16300]$.} (or dimensions), but they each new color must come with a larger position than its preceding one. The position can be omitted in which case it will be deduced from the context. If the first two colors have no position, the first will receive position~$0$ and the second will receive position $1\text{cm}$. All following ones receive the last encountered mesh width. Note that non-uniform positions make a real difference in conjunction with |colormap access=piecewise const|.
...
\begin{enumerate}
	\item
	\declareandlabel{index of colormap}|(|\meta{length}|)=(|\meta{index}| of |\meta{colormap name}|)|
...
%\pgfplotsshowcolormapexample{rgb=(1,0,0) rgb=(0,1,0) rgb255=(0,0,255) gray=(0.3) color=(green)}
	...
	\item
	\declareandlabel{color of colormap}|(|\meta{length}|)=(|\meta{value}| of |\meta{colormap name}|)|.
	...
	\item
	\declareandlabel{const source color of colormap}|(|\meta{length}|)=(|\meta{value}| of |\meta{colormap name}|)|.
	\index{const color of colormap!In colormap specification}%

	This key is almost the same as |color of colormap| mentioned above, but it uses the same functionality as |colormap access=piecewise constant| in order to map colors.
	...
	\item 
	\declareandlabel{colors of colormap}|(|\meta{length}|)=(|\meta{list}| of |\meta{colormap name}|)|
	...
	\item 
	\declareandlabel{const source colors of colormap}|(|\meta{length}|)=(|\meta{list}| of |\meta{colormap name}|)|
	\index{const source colors of colormap!In colormap specification}%

	A convenience key which is equivalent to a sequence of |const source color of colormap(|\meta{length}|)|, one for each element in \meta{list}. The \meta{list} is evaluated using |\foreach|. Note that you need round braces around the argument.

	The \meta{length} argument is optional as usual. Note that only the first \meta{length} can be set explicitly, all following ones are deduced automatically.

	Note that |const source colors of colormap| is also available during |cycle list| definitions.

	\item

	\declareandlabel{samples of colormap}|(|\meta{length}|)=(|\meta{number}| of |\meta{colormap name}|)| or
	...
	\item
	\declareandlabel{indices of colormap}|(|\meta{length}|)=(|\meta{list of indices}| of |\meta{colormap name}|)| or
