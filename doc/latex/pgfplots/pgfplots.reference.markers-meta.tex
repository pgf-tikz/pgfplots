...
\pgfplotsshowcolormapexample{rgb(-500)=(0,0,1) color(0)=(black) rgb255(1500)=(238,140,238)}
	...
	This syntax allows to distribute colors over using non-uniform distances. 
...
\noindent	Finally, \meta{option list with `of colormap' prefix} can be nothing but a valid argument of |colormap name|. In this case, the argument is interpreted as source colormap and will be copied.
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
	  of colormap={viridis}
...
\begin{pgfplotskey}{colormap access/extra interval width=\marg{fraction}}
	...
	The default width of this interval is the mesh width of the color map, i.e.\ is will always be as large as the other intervals. If the color map has non-uniform distances, the smallest encountered mesh width is used for the extra interval.
	\index{Color maps!Non uniform}

	This key allows to customize the width of the artifical interval. The value is a fraction of the total width and accepted values are $0\le $\meta{fraction}$\le 0.9$. Here, $0$ disables the extra interval. Any other value creates such an interval and its width is \meta{fraction} percent of the entire color map.
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
		color(0)=(blue)
	...
	colormap access=const]
\end{codeexample}
	Note how the extra interval of |colormap access=const| modifies the positions of the colormap definition: they are all shifted to the left. If you need full control over the positions, you have to disable the extra interval:
...
\pgfplotscolorbardrawstandalone[
	...
	colormap access/extra interval width=0]
\end{codeexample}
	Clearly, such a dedicated |colormap| is only useful in conjunction with |colormap access=const|.
