...
\end{codeexample}
	...
	Technical side-note: |colormap access=piecewise constant| internally ensures that each color in the |colormap| receives its own inteval (i.e.\ is actually visible in the output). To this end, it adds an artifical end-point at the right end of the |colormap|. This operation is done without any user action and it is specific to |colormap access=piecewise constant|.
\end{pgfplotskey}
