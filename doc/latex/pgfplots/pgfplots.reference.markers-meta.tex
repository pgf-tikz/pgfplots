...
\begin{key}{/tikz/mark phase=\marg{integer $p$} (initially 1)}
	This option allows to control which markers are drawn. It is primarily used together with the \Tikz\ option |mark repeat=|$r$: it tells \tikzname\ that the first mark to be drawn should be the $p$th, followed by the $(p + r)$th, then
...
Note that there are \emph{more available choices} of colormaps in the associated libraries, name in the |colorbrewer| library and in the |colormaps| library which need to be loaded by means of |\usepgfplotslibrary{colorbrewer,colormaps}|.
...
\begin{enumerate}
	...
	This method takes a \meta{colormap name} on input, samples \meta{number} colors from it (using a uniform mesh width) and inserts it into the currently built |colormap|. It is equivalent to the choice |colors of colormap||(|\meta{position}|)=(0,h,...,1000)| with |h| chosen such that you get \meta{number} samples positioned at an equidistant grid.
...
\foreach \i 
  in {0,...,\pgfplotscolormaplastindexof{jet}}
{
...
\begin{pgfplotskey}{colormap access=\mchoice{map,direct,const,\\piecewise constant,piecewise const,piecewise linear} (initially map)}
