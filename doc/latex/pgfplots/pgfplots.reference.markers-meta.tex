...
\end{codeexample}
	...
	The next possibility resembles |samples of colormap|: it expects \declaretext{samples of colormap}|=|\marg{number}| of |\meta{colormap name}. It chooses \meta{number} samples of the selected colormap, using the same distance to the next.

    The next possibility is similar to |color of colormap|: it expects
...
\end{codeexample}
	...
	The complete syntax on how to customize |of colormap| is the same as the building block to defined |colormap|s based on other |colormap|s as described in Section~\ref{sec:pgfplots:colormaps:based:on:others} on
    page~\pageref{sec:pgfplots:colormaps:based:on:others} -- with the difference that |cycle list={[of colormap]}| inserts the selected colors into the |cycle list| instead of a |colormap|.
