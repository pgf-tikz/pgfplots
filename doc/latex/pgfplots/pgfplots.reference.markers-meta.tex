...
\begin{stylekey}{/pgfplots/every axis plot post (initially {})}
    The \todosp{shall the curly brackets after ``initially'' be shown? Also
    this style is again defined in Section~4.18.1 (see corresponding index
    entry). Is that intended?} |every axis plot post| style can be used to overwrite parts (or all) of
...
\end{codeexample}
    ...
    Please use the the choice |colormap name=viridis| as this makes uses of the
...
\begin{command}{\pgfplotscolormaptoshadingspec\marg{colormap name}\marg{right end size}\marg{\textbackslash macro}}
    ...
    specification. It can be used in commands like |\pgfdeclare*shading| \todosp{better `...' instead of `*'?} (see
...
\end{codeexample}
    ...
    The second possibility is similar to |color of colormap|: it expects
...
\end{codeexample}
    ...
    selected in this case. In general, the syntax allows to select any
    (defined) colormap.  Note that |indices of colormap| is quite similar to
    the related way to build colormaps based on existing colormaps as outlined
    in Section~\ref{sec:pgfplots:colormaps:based:on:others} on
    page~\pageref{sec:pgfplots:colormaps:based:on:others}.

    A further related choice is
    \declareandlabel{const colors of colormap}|=|\marg{list}. It is about the
    same as |colors of colormap|, except that it uses |const color of colormap|
    as color specification, i.e.\@ the strategy
...
\begin{pgfplotskey}{%
    ...
    This \todosp{shouldn't the value list be surrounded by curly braces? (ff)} is one of two ways to employ more than one |cycle list| in order to
...
\label{pgfplots:pointmeta}
    ...
    \begin{description}
        ...
        \item[\declaretext{explicit}] This choice tells \PGFPlots{} to expect
            ...
            How point meta data is provided for |plot coordinates|,
            |plot table| and the other input methods is described in all detail
...
\end{codeexample}
        ...
        \item[\normalfont\declare{\meta{expression}}] This choice allows to
            ...
            If used together with |plot table|, you may also access other
