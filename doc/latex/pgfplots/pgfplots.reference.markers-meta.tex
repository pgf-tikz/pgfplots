...
\end{tikzpicture}
\end{codeexample}
	\noindent The provided |cycle multiindex list| consists of three lists. The style for a single plot is made up using elements of each of the three lists: the first plot has style |red,solid,mark=*|, the second has |blue,dotted,mark options={solid},mark=x|, the third has |mark=o|. The fourth plot restarts all lists and uses the same as the first plot, i.e.\ |red,solid,mark=*|.
	...
	The argument for |cycle multiindex list| is the very same as the one for |cycle multi list| (including the special |[2 of]mark list| syntax and providing other cycle lists by name):
