...
\begin{pgfplotskey}{colormap access=\mchoice{map,direct} (initially map)}
	...
	Suppose we have the following color definitions:
...
\begin{codeexample}[]
    ...
    \def\showcolor#1{%
    	\tikz \draw[black,fill={#1}] 
			(0,0) rectangle (2em,1.6em);%
    }%
    %
    ...
    \end{minipage}
\end{codeexample}
	and we use these colors in order to define a |colormap|:
...
\begin{axis}[
	colormap={brewer}{
		...
		color=(YlGn-K)},
	anchor=center,enlargelimits=false,
	% FIXME : DEFINE A STYLE
	colorbar sampled={
		surf,
...
\def\showcolor#1{%
	...
	The choice |colormap access=direct| is actually like a color palette for images with indexed colorspaces: suppose we use the famous ``Lena'' image~\cite{lena}, downsampled it to $128\times 128$, and export it to coordinates of the form
\begin{codeexample}[code only]
1 1 194
1 2 194
1 3 170
...
...
\end{codeexample}
	with an indexed colorspace. Furthermore, we import its color palette as \PGFPlots\ |colormap|. To this end, I used the following Matlab script (as outlined in Section~\ref{sec:pgfplots:import:matlab} on page~\pageref{sec:pgfplots:import:matlab}) in order to convert the input |tiff| image to coordinates:
\begin{codeexample}[code only]
...
\end{tikzpicture}
\end{codeexample}
	The files |lena_cm.tex| and |lena.dat| are shipped with \PGFPlots\ and can be downloaded from CTAN. Note that this kind of operation is considerably less efficient than |\includegraphics| in terms of
\begin{itemize} 
