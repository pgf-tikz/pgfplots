...
\pgfplotscolorbardrawstandalone[
	...
	colormap access=const]
\end{codeexample}
	The use of options inside of the argument is discussed in the next sub-section, see |of colormap| and |target pos|. See also |colormap access/extra interval width| for details regarding positions and the artificial interval which is available for |colormap access=const|.
...
\paragraph{Adding flexility using extra keys:}	The building blocks to define color maps use a common mechanism which accepts an \meta{option list with `of colormap' prefix} after the argument (inside of the braces). The following options are available:

\begin{pgfplotskey}{of colormap/target pos=\marg{position(s)} (initially empty)}
	Allows to define the \meta{position} for zero, one, or more of the involved colors. An empty value means to use the argument `|(|\meta{position}|)|' in the definition, for example |color of colormap(100)=(4)|. This is sufficient if there is just one color involved. However, if |target pos| has a non-empty value, it overrides the |(|\meta{position}|)| argument.
...
\end{codeexample}
	...
	The default width of this interval is the mesh width of the color map (the value \declaretext{h}), i.e.\ it will always be as large as the other intervals. If the color map has non-uniform distances, the smallest encountered mesh width is used for the extra interval.
	\index{Color maps!Non uniform}

	This key allows to customize the width of the artifical interval. The value is a fraction of the total width and accepted values are $0\le $\meta{fraction}$\le 0.9$. Here, $0$ disables the extra interval. 
...
\pgfplotscolorbardrawstandalone[
	...
	colormap access=const]
\end{codeexample}
	As outlined in the documentation of |of colormap|, the syntax |of colormap={viridis,target pos=...}| samples as many uniformly drawn samples from |viridis| as it finds positions. Since this uniform sampling already contains the last color, we can merely \emph{replicate} the last color explicitly to add the extra interval, and we can control its width by means of the additional position (which is $1000-800=200$ in our case).
	...
	\paragraph{Attention:} at the time of this writing, uniform color maps only support the default interval width and \meta{fraction}$=0$, further customization is only possible for non-uniform color maps. If you ever need to work around this limitation, you should file a feature requests and move one of your color position until the colormap becomes a non-uniform colormap.
