...
\begin{enumerate}
	\item
	|index of colormap(|\meta{length}|)=(|\meta{index}| of |\meta{colormap name}|)|
...
%\begin{codeexample}[code only]
%XXXX
%rgb=(1,0,0) rgb=(0,1,0) index of colormap=(0,0,255) gray=(0.3) color=(green)
%\end{codeexample}
%\pgfplotsshowcolormapexample{rgb=(1,0,0) rgb=(0,1,0) rgb255=(0,0,255) gray=(0.3) color=(green)}

	The \meta{length} argument is optional as usual.
	
	\item
	|color of colormap(|\meta{length}|)=(|\meta{value}| of |\meta{colormap name}|)|.
	...
	\item 
	|colors of colormap(|\meta{length}|)=(|\meta{list}| of |\meta{colormap name}|)|
	\index{colors of colormap!In colormap specification}%

	A convenience key which is equivalent to a sequence of |color of colormap(|\meta{length}|)|, one for each element in \meta{list}. The \meta{list} is evaluated using |\foreach|. Note that you need round braces around the argument.

	The \meta{length} argument is optional as usual. Note that only the first \meta{length} can be set explicitly, all following ones are deduced automatically.

	\item

	|samples of colormap(|\meta{length}|)=(|\meta{number}| of |\meta{colormap name}|)| or
	\index{samples of colormap!In colormap specification}%

	A convenience key which is equivalent to a sequence of \meta{number} occurences of |color of colormap(|\meta{length}|)|. Note that you need round braces around the argument.

	The \meta{length} argument is optional as usual. Note that only the first \meta{length} can be set explicitly, all following ones are deduced automatically.

	\item
	|indices of colormap(|\meta{length}|)=(|\meta{list of indices}| of |\meta{colormap name}|)| or
	\index{indices of colormap!In colormap specification}%

	A convenience key which is equivalent to a sequence of |index of colormap(|\meta{length}|)|, one for each element in \meta{list}. The \meta{list} is evaluated using |\foreach|. Note that you need round braces around the argument.
...
In addition, \PGFPlots\ supports dedicated formats of \meta{list} which allow to acquire values from a |colormap|. In this case, \meta{list} contains keys enclosed in square brackets:
...
Finally, there is the choice \declareandlabel{indices of colormap}|=|\marg{list}. As above, it accepts an optional `|of|' clause of the form |indices of colormap=|\marg{list}| of |\meta{colormap name}. The main argument is a list of indices $0\le N_i < N$ where $N$ is the number colors in the colormap definition (compare the documentation of |index of colormap|). Indices which are out of range are clipped to the nearest index. For example, |viridis| comes with $\pgfplotscolormapsizeof{viridis}$~elements and we can write
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}[
...
\begin{pgfplotskey}{colormap access=\mchoice{map,direct,piecewise constant,piecewise linear} (initially map)}
	This key configures how point meta data is used to determine colors from a color map. The initial configuration \declaretext{map} performs the linear mapping operation explained above.
	
	The choice \declaretext{direct} does not perform any transformation; it interpretes the value of |point meta| as integer indices into the current color map. If a number is no integer, it will be truncated by throwing away any fraction parts of the number.
...
\begin{axis}[
	colormap={brewer}{
		color=(YlGn-B) color=(YlGn-E) 
		color=(YlGn-G) color=(YlGn-I) 
		color=(YlGn-K)},
	enlargelimits=false,
	colorbar as palette,
]
...
\end{tikzpicture}
\end{codeexample}
	The example has a $3\times 4$ matrix, each with explicitly provided values of |point meta|. The first point has a |point meta| value of~$0$ and uses the first color of the |colormap| (|YlGn-B|, \showcolor{YlGn-B}). The second point has value~$1$ and uses the second color, |YlGn-E| \showcolor{YlGn-E}, and so on. Note that |NaN| is interpreted as `hole' in the matrix and is left empty (you see the background color here). Note furthermore that coordinate |(1,2) [-1]| is mapped to the first color since values outside of the allowed range are automatically clipped to the next correct color. The same holds for the point with |[99]|: it is mapped to the last available color~\showcolor{YlGn-K}. Finally, the point with |[2.99]| is mapped to index $2$~(\showcolor{YlGn-G}) since |colormap access=direct| truncates to the next integer index.
...
\end{itemize}
	...
	The choice |colormap access=|\declaretext{piecewise linear} is an alias for |colormap access=map|.
...
\end{codeexample}
	...
	FIXME : finish docs: mention |samples of colormap| and its friends, mention importance of suitable colormaps, mention the ``extra interval'' at the end
