...
\pgfplotsshowcolormapexample{rgb(-500)=(0,0,1) color(0)=(black) rgb255(1500)=(238,140,238)}
	...
	\PGFPlots\ also offers advanced \meta{color specification}s in order to build color maps from existing color maps. This is explained in the following sub sections.
...
\item[cmyk] which expects \meta{arguments} of the form |(|\meta{cyan}|,|\meta{magenta}|,|\meta{yellow}|,|\meta{black}|)| where each component is in the interval $[0,1]$,
\item[cmyk255] which is the same as |cmyk| but expects components in the interval $[0,255]$,
\item[cmy] which expects \meta{arguments} of the form |(|\meta{cyan}|,|\meta{magenta}|,|\meta{yellow}|)| where each component is in the interval $[0,1]$,
\item[hsb] which expects \meta{arguments} of the form |(|\meta{hue}|,|\meta{saturation}|,|\meta{brightness}|)| where each component is in the interval $[0,1]$,
\item[Hsb] which is the same as |hsb| except that \meta{hue} is accepted in the interval $[0,360]$ (degree),
...
\begin{enumerate}
	...
	A convenience key which is equivalent to |colors of colormap(|\meta{position}|)=(0,h,...,1000)| with |h| chosen such that you get \meta{number} samples positioned at an equidistant grid.
...
\pgfplotscolorbardrawstandalone[
	...
	colormap access=const]
\end{codeexample}
	\noindent One thing in this context needs \emph{special attention}: we told the system to use $5$~colors, but the case |colormap access=const| merely shows~$4$ colors! This is due to the fact that |colormap access=const| defines colors for entire \emph{intervals} whereas |colormap access=map| defines colors for \emph{single positions}. \PGFPlots\ tries hard to get the most out of this difference, and the details are documented for the key |of colormap/sample for|.
	The consequence is that |colormap access=const| requires you to decide:
	\begin{enumerate}
		\item Either accept that a color is dropped (default),
		\item or use |sample for=const| and accept that the colors are somewhat resampled,
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
		samples of colormap={
	...
	colormap access=const]
\end{codeexample}
		\item or use |colormap access/extra interval width=h| and accept that the positions are rescaled:
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
		samples of colormap={
...
\noindent The previous examples above shows how curly braces can simplify the color map definition. They also shows that the absolute values of the positions are meaningless as they are mapped to $[0,1000]$ anyway -- but their relative positions are of importance. Adding  |point meta min| and |point meta max| shows the desired ranges.

		
	Note that a |colorbar| can be configured to show the |target pos|itions if you add |xtick=data| to its option list:
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
		samples of colormap={
...
\end{codeexample}
	...
	\item
	\declareandlabel{index of colormap}|(|\meta{position}|)=(|\meta{index}| of |\meta{colormap name}|)|
...
\end{codeexample}
	...
	All special remarks of |samples of colorbar| (like curly braces, option list support, positions) apply here as well.
	\item
	\declareandlabel{color of colormap}|(|\meta{position}|)=(|\meta{value}| of |\meta{colormap name}|)|.
	...
	All special remarks of |samples of colorbar| (like curly braces, option list support, positions) apply here as well.
	\item 
	\declareandlabel{colors of colormap}|(|\meta{position}|)=(|\meta{list}| of |\meta{colormap name}|)|
...
%-------------
	...
	\item \declaretext{of colormap}|(|\meta{position}|)=(|\meta{options}|)| The building blocks to define color maps use a common mechanism which accepts an \meta{option list with `of colormap' prefix} after the argument (inside of the braces). The following options are available:
...
\begin{pgfplotskey}{of colormap/target pos=\marg{position(s)} (initially empty)}
	...
	The key |target pos| is primarily intended to provide positions for more than one color definition, i.e.\ in conjunction with keys like |samples of colormap| or |colors of colormap|. In this context, as many \meta{position(s)} as specified are used. If there are too few to cover all involved colors, the remaining ones are deduces automatically.

	Attention for |colormap access=const|: A non-empty value for |target pos| implies |colormap access/extra interval width=0| as default in order to keep the precise positions provided by the end user. An empty value means ``automatically compute positions'' and defaults to |colormap access/extra interval width=h|. 
\end{pgfplotskey}

\begin{pgfplotskey}{of colormap/sample for=\mchoice{default,const} (initially default)}
	This key allows to configure |samples of colormap| such that the resulting |colormap| is suitable for a dedicated values of |colormap access|. 
	...
	The strategy |colormap access=const| means that colors are associated with entire \emph{intervals} and no longer with single positions in the |colormap|. That comes with the following consequences:
	\begin{itemize}
...
\pgfplotscolorbardrawstandalone[
	...
	colormap access=const]
\end{codeexample}
		\item As soon as there are positions, \PGFPlots\ has a conflict if the colormap is used in a context with |colormap access=const|: it cannot enlarge the displayed limit, but it cannot respect both the given positions \emph{and} the required number of colors! It has the choice to either omit one color, or to add an artifical position and rescale all other positions.
...
\end{codeexample}
		...
		Technically, this means that if implicitly sets |colormap access/extra interval width=0| (the default is |colormap access/extra interval width=h|).

		Restoring |colormap access/extra interval width=h| results in rescaled positions:
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
		samples of colormap={
...
\pgfplotscolorbardrawstandalone[
	...
	colormap access=const]
\end{codeexample}
		This results in correctly respected positions and a fully respected range of the input colormap (at the expense of a slightly modified sampling procedure).
	
		In this case, the resulting |colormap| looks odd when used in conjunction with |colormap access=map|, but is fine together with |colormap access=const|.
	\end{itemize}

	The key |sample for| applies whenever |of colorbar| involves sampling. It does not apply for explicitly fixed colors.
...
\noindent	The key-value system can also be activated by the color specification
	...
	\declareandlabel{of colormap}|(|\meta{position}|)=|\marg{option list with `of colormap' prefix}.

\noindent In this case, it is allowed that \meta{option list with `of colormap' prefix} contains nothing but a valid argument of |colormap name|. In this case, the argument is interpreted as source colormap and will be copied.
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
	  of colormap={viridis}
...
\noindent Another supported case is that \meta{option list with `of colormap'} prefix contains a valid argument of |colormap name| and some |target pos|. In this case, the provided colormap is sampled using |samples of colormap| where the number of samples is deduced from the number of positions.
\begin{codeexample}[]
\pgfplotscolorbardrawstandalone[
	colormap={example}{%
	  of colormap={viridis, 
...
\end{codeexample}
	\noindent The example defines a style named `|ocean height|' with a suitable list of positions somewhere in the document. Then, it defines a colormap `|CM|' which makes use of these keys -- but only in the range $[-7046,2050]$, and with a special combination of two other |colormap|s. The first |of colormap| specification selects only those target positions which fall in the range $[-7046,0]$ and ensures that $-7046$ actually becomes an element of |target pos|. The second |of colormap| specification selects all in the range $[0.1,2050]$ and ensures that $2050$ becomes an element of |target pos|. Note that the $0.1$ merely serves as indicator to \emph{not} select $0$ again. Thus, the selection is essentially equivalent to 
\begin{codeexample}[code only]
...
target pos={-7046,-6000,-5000,-3000,-1000,-750,-500,-250,-100,-50,0}
...
target pos={10,100,200,500,1000,1100,1200,1500,2000,2050}
\end{codeexample}
