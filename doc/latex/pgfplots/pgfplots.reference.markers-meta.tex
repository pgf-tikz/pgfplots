...
\begin{pgfplotskey}{of colormap/target pos=\marg{position(s)} (initially empty)}
	...
	If you have |samples of colormap| or an explicit list of source positions (like in |colors of colormap|) and provide more elements in |target pos| than samples exists, the last provided sample will be replicated as often as needed.
...
\end{codeexample}
		...
		Technically, this means that it implicitly sets |colormap access/extra interval width=0| (the default is |colormap access/extra interval width=h|).
...
\end{codeexample}
	...
	The key |sample for| applies whenever |of colorbar| involves sampling. It does not apply for explicitly fixed colors. Its effect is that the number of samples is reduced by~$1$ and the last provided sample is replicated once.
