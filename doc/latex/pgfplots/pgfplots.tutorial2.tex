...
\addplot table {plotdata/data_d2.dat};
\end{loglogaxis}
\end{tikzpicture}
...
\addplot table {plotdata/data_d2.dat};
\addplot table {plotdata/data_d3.dat};
\addplot table {plotdata/data_d4.dat};
\end{loglogaxis}
\end{tikzpicture}
\end{codeexample}
...
\PGFPlots\  will only use \meta{options} and will ignore its |cycle list|. If you write a plus sign before the square brackets as in

|\addplot+[|\meta{options}|] ...|, 

\PGFPlots\  will append \meta{options} to the automatically assigned cycle list.
...
\node[pin=0:Special.] at (axis cs:769,1.623e-04) {};
...
Next steps might be how to visualize functions using line plots, how to align adjacent graphics properly (even if the axis descriptions vary), how to employ scatter plots of \PGFPlots\ , or how to draw functions of two variables.

%The steps are explained in all detail in the reference Section~\ref{sec:pgfplots:reference}.
