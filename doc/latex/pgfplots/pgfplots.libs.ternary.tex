...
\end{codeexample}
	...
	\[ 
		...
		\end{bmatrix}
	\]
	where $A=(0,0)$ is the lower left corner of the triangle, $B=(1,0)$ the lower right and $C=(\nicefrac12,\nicefrac{\sqrt3}{2})$ the upper one. The axis range for $A$ is on the \emph{opposite} site of $A$; the ticks for $B$ are on the \emph{opposite} site of $B$ and similarly for $C$.

	The input coordinate $(100\%,0\%,0\%)$ is mapped to $A$, the input coordinate $(0\%,100\%,0\%)$ to $B$ and $(0\%,0\%,100\%)$ to $C$.
