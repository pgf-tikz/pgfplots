...
\end{tikzpicture}%
\end{codeexample}
The \lstinline!plot coordinates! and \lstinline!plot function! commands are two of the several \Tikz\ ways to create plots, see section~\ref{sec:addplot} for more details. The options `\texttt{xlabel}' and `\texttt{ylabel}' define axis descriptions.
...
\begin{tikzpicture}
	\begin{semilogyaxis}[
		xlabel=Index,ylabel=Value,width=7cm]
...
You can use any of the \Tikz\ options to modify the appearance. For example, the ``\texttt{scale}'' transformation takes the picture as such and scales it.
{%
\begin{codeexample}[]
