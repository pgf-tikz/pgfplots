...
\begin{figure}
\tikzstyle{every axis}+=[anchor=outer center]%
...
\begin{tikzpicture}%
	...
	\useasboundingbox (an axis.outer south west) (an axis.above north east); 
\end{tikzpicture}%
\nobreak
\hskip0.5cm%
\nobreak
...
\centering
	...
	\tikzstyle{every picture}+=[baseline]%
	\tikzstyle{every axis}+=[anchor=north west]%
	\begin{tikzpicture}
...
\begin{tikzpicture}[baseline]
	...
	\addplot[color=blue,mark=*] table[x=Cost,y=Error] {pgfplots.testtable};
	%--------------------------------------------------
	% \addplot[color=blue,mark=*] coordinates {
	% 	(7,		8.47178381e-02)
	% 	(31,	3.04409349e-02)
...
\end{axis}
\end{lstlisting}
which is the same as \lstinline!\begin{semilogyaxis}...\end{semilogyaxis}!, see figure~\ref{fig:semilogy} for an example.

\begin{figure}
	\begin{tikzpicture}[baseline]%
		\begin{semilogyaxis}[xlabel=Index,ylabel=Value,width=7cm,anchor=north]
		\addplot[color=blue,mark=*] coordinates {
			(1,8)
			(2,16)
			(3,32)
...
\begin{tikzpicture}
	...
	\addplot coordinates 
		{(5,8.312e-02) ...  (9217,3.261e-06)};

	\addplot coordinates
		{(7,8.472e-02) ... (47103,8.437e-06)};

	\addplot coordinates 
		{(9,7.881e-02) ... (178177,1.751e-05)};

	\addplot coordinates
		{(11,6.887e-02) ... (553983,3.053e-05)};

	\addplot coordinates
		{(13,5.755e-02) ... (1496065,4.670e-05)};
	\legend{$d=2$,$d=3$,$d=4$,$d=5$,$d=6$}
	\end{loglogaxis}
\end{tikzpicture}
\end{lstlisting}
