	...
	\begin{itemize}
		...
		\item Remarks on |shader=interp|:
			\begin{itemize}
				...
				\item The |interp| shader yields the same outcome as |faceted interp,faceted color=none|, although |faceted interp| requires much more ressources -- but it also allows |z buffer=sort|.
				\index{Errors!shader=interp,z buffer=sort not supported}
				...
				okular and evince need a long time and do not display the colors correctly (evince even shows transparency). I hope this message will soon become outdated... if not, provide bug reports to the Linux community to communicate the need to improve support for Type 4 (|patch|) and Type 5 pdf (|surf|) shadings.
			\end{itemize}
	\end{itemize}
...
\begin{plottype}[/pgfplots]{patch}
	...
	As for dimensional |patch| plots, the default input type is a long sequence of triangles, where each consecutive set of three points defines a triangle:
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
...
\end{tikzpicture}
\end{codeexample}
	See the description of |shader=interp| for details and remarks. The example above makes use of the alternative syntax to provide a geometry: the |patch table| input. It allows to provide vertices separate from patch connectivity, where each patch is defined using three indices into the vertex array (see Section~\ref{sec:pgfplots:2d:patch} for details).
