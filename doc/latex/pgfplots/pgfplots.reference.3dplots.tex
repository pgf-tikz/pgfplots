...
Here is a simple approach with the same vertices as the example above, but |mesh/color input=|\declaretext{explicit}:
\begin{codeexample}[]
% \usepgfplotslibrary{patchplots}
\begin{tikzpicture}
...
\begin{pgfplotskey}{%
	...
	If the input color has \emph{no color model}, the color model is supposed to be the value of |mesh/colorspace explicit color input| (which is |rgb| by default).
...
\begin{axis}
    \addplot3[
        ...
        mesh/color input=explicit mathparse,
		%
		% CMYK produces better quality for this example
