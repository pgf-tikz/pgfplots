...
\begin{pgfplotskey}{z buffer=\mchoice{default,none,auto,sort,reverse x seq,reverse y seq,reverse xy seq} (initially default)}
	...
	The choice \declaretext{sort} can be used for scatter, line, mesh, surface and patch plots. It sorts according to the depth of each point (or mesh segment)\footnote{The choice \texttt{sort} is \emph{not} available for \texttt{surf} plots with \texttt{shader=interp} because the low level format requires matrix ordering and doesn't support sorting. Use either \texttt{shader=flat} or, if really necessary, \texttt{shader=faceted interp}. Patch plots do not have this restriction.}. Sorting in \TeX\ uses a slow algorithm and may require a lot of memory (although it has the expected runtime asymptotics $\mathcal O(N \log N)$). The depth of a mesh segment is just \emph{one} number, currently determined as \emph{mean} over the vertex depths. Since |z buffer=sort| is actually just a more intelligent way of drawing mesh segments on top of each other, it may still fail. Failure can occur if mesh segments are large and overlap at different parts of the segment (see Wikipedia ``Painter's algorithm''). If you experience problems of this sort, consider reducing the mesh width (the mesh element size) such that they can be sorted independently.
...
\end{codeexample}
	...
	Another choice is |shader=|\declareandlabel{interp} which uses Goraud shading (bilinear interpolation) to fill mesh segments: 
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
\end{codeexample}
	...
	The |shader=interp| employs a low--level shading implementation which is currently (only) available for the postscript driver \declaretext{pgfsys-dvips.def} and the |pdflatex| driver \declaretext{pgfsys-pdftex.def}. For other drivers, the choice |shader=interp| will result in a warning and is equivalent to |shader=flat mean|. Unfortunately, not all viewers have satisfying support for |shader=interp|\footnote{For |patch| plots, \texttt{shader=interp} generates Type 4 pdf shadings whereas |surf| plots generate Type 5 pdf shadings.}: at the time of this writing, free Linux viewers like xpdf, okular and evince need a long time and do not display the colors correctly (evince even shows transparency). Acrobat Reader displays them both fast and as declared.
...
\end{codeexample}
	...
	The last choice is |shader=|\declareandlabel{faceted interp}. As the name suggests, it is a mixture of |interp| and |faceted| in the sense that each element is shaded using bilinear (trilinear for triangles) interpolation in the same way as for |interp|, but additionally, the edges are colored in |faceted color|:
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
\begin{plottype}[/pgfplots]{patch}
	Three dimensional |patch| plots work exactly like two dimensional |patch| plots (compare Section~\ref{sec:pgfplots:2d:patch}): they consist of geometries made up by triangles. The |\addplot3[patch]| variant allows each corner of type $(x,y,z)$, plus the additional color data.

	As for for dimensional |patch| plots, the input is interpreted as a long sequence of triangles, where each consecutive set of three points defines a triangle:
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
...
\end{codeexample}
	...
	Patch plots are (almost) the same as |mesh| or |surf| plots, they only have more freedom in their input range (and a more complicated geometry). Actually, ``|patch|'' is just a style for |surf,mesh/type=triangle|. In other words, |patch| \emph{is} the same; and it shares the same internal implementation. Thus, most of the keys to configure |mesh| or |surf| plots apply to |patch| as well, especially |shader| and |z buffer|. Only the |mesh/| keys which are related to the lattice (matrix) input like |mesh/ordering| or |mesh/rows| have no effect on |patch| plots. In fact, |\addplot3[patch]| automatically activates |z buffer=sort| to ensure a good drawing sequence. The |shader| can be used to modify the appearance:
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}
...
\end{tikzpicture}
\end{codeexample}
	Unfortunately, many viewers, especially have problems viewing Type 4 pdf shadings as the one above (resulting in extraordinary long loading times, occasionally wrong colors or even transparency). Acrobat reader displays everything as specified in the pdf specification.
...
\end{codeexample}
	...
	As already mentioned, the initial configuration for three dimensional |patch| plots uses |z buffer=sort|, i.e.\ it sorts the triangles according to their view depth and draws them on top of each other. This \textbf{may fail occasionally} if triangles are cyclic or otherwise quite large. Consider to reduce the triangle sizes to reduce or avoid the problem\footnote{Perhaps I can add such a refinement feature to \PGFPlots\ when I find time.}.
\end{plottype}
}
