...
\begin{itemize}
	...
	\item The only way to select individual contour levels is to use a suitable |colormap| definition, for example using |colors of colormap| as outlined in Section~\ref{sec:pgfplots:colormaps:based:on:others} on page~\pageref{sec:pgfplots:colormaps:based:on:others}. However, this approach only accepts \emph{mapped} numbers. These are typically in the range $[0,1000]$ (maybe up to $[0,16000]$) on input.
