...
\begin{addplot3operation}[]{(\meta{$x$ expression},\meta{$y$ expression},\meta{$z$ expression})}{}
	...
	Note further that since the complete point expression is surrounded by round braces, you can't use round braces inside of \meta{$x$ expression}, \meta{$y$ expression} or \meta{$z$ expression} directly -- surround the expressions (which contain round braces) with curly braces additionally:
