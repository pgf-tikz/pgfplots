...
\begin{pgfplotskey}{z buffer=\mchoice{default,none,auto,sort,reverse x seq,reverse y seq,reverse xy seq} (initially default)}
	...
	The choice \declaretext{sort} can be used for scatter, line, mesh, surface and patch plots. It sorts according to the depth of each point (or mesh segment)\footnote{The choice \texttt{sort} is \emph{not} available for surface plots with \texttt{shader=interp} because the low level format requires matrix ordering and doesn't support sorting.}. Sorting in \TeX\ uses a slow algorithm and may require a lot of memory (although it has the expected runtime asymptotics $\mathcal O(N \log N)$). The depth of a mesh segment is just \emph{one} number, currently determined as \emph{mean} over the vertex depths. Since |z buffer=sort| is actually just a more intelligent way of drawing mesh segments on top of each other, it may still fail. Such cases occur if mesh segments are large and overlap at different parts of the segment (see Wikipedia ``Painter's algorithm''). If you experience problems of this sort, consider reducing the mesh width (the mesh element size) such that they can be sorted independently.
...
\end{codeexample}
	...
	Another choice is |shader=interp| which uses Goraud shading (bilinear interpolation) to fill mesh segments. 
...
\end{codeexample}
	...
	The last choice (and perhaps also the large you should consider) is |shader=faceted interp|. As the name suggests, it is a mixture of |interp| and |faceted| in the sense that each element is shading using bilinear (trilinear for triangles) interpolation in the same way as for |interp|, but additionally, the edges are colored in |faceted color|.
\pgfplotsexpensiveexample
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
