...
\end{codeexample}
	...
	The alternative \declaretext{patch table*} is almost the same as |patch table| -- but it allows to provide |point meta| (color data) per patch instead of per vertex. If you write |patch table*|, a further column of the |patch table*| argument is interpreted as color data:
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
\end{tikzpicture}
\end{codeexample}
	\noindent The `|*|' always prefers |point meta| data from the provided |patch table*| argument. However, it is still supported to write |point meta=\thisrow|\marg{colname} or similar constructs -- but now, \meta{colname} refers to the |patch table|, not the input table (if any). More precisely, |point meta| is evaluated in a context where the patch connectivity has been resolved and the |patch table*| is loaded.
