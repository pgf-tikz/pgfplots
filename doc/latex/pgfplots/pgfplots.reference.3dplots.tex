...
We continue with a brief summary over the different choices, along with their properties.
...
Here is a simple approach with the same vertices, but |mesh/color input=|\declaretext{explicit}:
\begin{codeexample}[]
% \usepgfplotslibrary{patchplots}
\begin{tikzpicture}
...
\end{codeexample}
	...
	The precise definition how colors can be provided is subject of the next sections.
\end{pgfplotskey}
...
\begin{pgfplotskey}{%
	mesh/colorspace explicit color input=\mchoice{rgb,rgb255,cmy,cmyk,cmyk255,gray,wave,hsb,Hsb,HTML} (initially value of \texttt{colorspace explicit color output})%
}
