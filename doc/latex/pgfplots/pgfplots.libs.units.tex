...
\begin{pgfplotsxykey}{\x\ unit prefix=\marg{prefix} (initially |empty|)}
  ...
  Notice that if the \marg{axis}| unit| isn't set the entire unit will not be typeset.
\end{pgfplotsxykey}
...
\begin{pgfplotskey}{unit code/.code 2 args=\marg{typeset code} (initially |\textbackslash mathrm\{\#1\#2\}|)}
...
\begin{codeexample}[code only]
  \pgfkeys{unit code/.code 2 args={#1#2}}
\end{codeexample}
