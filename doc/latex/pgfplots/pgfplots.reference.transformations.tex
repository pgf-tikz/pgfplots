...
\begin{pgfplotskey}{data cs=\meta{name} (initially cart)}
...
\begin{command}{\pgfplotsaxistransformcs\marg{fromname}\marg{toname}}
	...
	On input, the coordinates are stored in |/data point/x|, |/data point/y|, and |/data point/z| (the latter one may be empty). The macro will test if there is a declared coordinate transformation from \meta{fromname} to \meta{toname} and invoke it. If there is none, it will attempt to convert to |cart| first and then from |cart| to \meta{toname}. If that does not exist as well, the operation fails.
\end{command}
...
\begin{command}{\pgfplotsdefinecstransform\marg{fromname}\marg{toname}\marg{code}}
	...
	The documentation of this system is beyond the scope of this manual\footnote{Which is quite large even without API documentation, as you will certainly agree...}. Please consider reading the source-code comments and the source of existing transformations if you intent to write own transformations.
\end{command}
