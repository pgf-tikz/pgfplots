...
\end{codeexample}
    ...
    \item Some of the coordinate input routines use the powerful
        ...
        \todosp{should these (three) better be \textbackslash addplot
        coordinates, etc.\@ like on the next page in ``empty lines''? (ff)}
    ...
    \item The accepted input format of tables is as follows:
        ...
        \begin{itemize}
            ...
            \item The accepted number format is the same as for
                `|plot coordinates|', see above.
            \item If you omit column selectors, the default is to plot the
                first column against the second. That means |plot table|
                does exactly the same job as |plot file| for this case.
...
\begin{addplotoperation}[]{expression}{\marg{math expr}}
    ...
    |\addplot expression |\marg{math expression}|;|
        \todosp{should this `expression' also be blue?}
\end{addplotoperation}
...
\begin{pgfplotskeylist}{%
    ...
    Sets the function's domain(s) for |plot expression| and |plot gnuplot|. Two
    ...
    |plot expression|. In case you'd like to plot only a subset of other
...
\begin{addplotoperation}[]{gnuplot}{\oarg{further options}\marg{gnuplot code}}
    ...
    that \texttt{plot gnuplot} is actually a re-implementation of the |plot
    function| method known from \PGF{}. It also invokes \PGF{} basic layer
    commands.} employs the external program |gnuplot| to compute coordinates.
    The resulting coordinates are written to a text file which will be plotted
    with |plot file|. \PGF{} checks whether coordinates need to be regenerated
    and calls |gnuplot| whenever necessary (this is usually the case if you
    change the number of samples, the argument to |plot gnuplot| or the plotted
    domain).\footnote{Please note that \PGFPlots{} produces slightly different
    files than \Tikz{} when used with \texttt{plot gnuplot} (it configures high
    precision output). You should use different \texttt{id} for \PGFPlots{} and
    \Tikz{} to avoid conflicts in such a case.}
...
\begin{addplotoperation}[]{graphics}{\marg{file name}}
    ...
    The operation |plot graphics| is intended so simplify this process. However
...
\end{codeexample}
    ...
    |plot graphics| for complicated surface plots.
...
\end{codeexample}
    ...
    |plot graphics|. Again, the bounding box is too large, so we need to adjust
...
\end{codeexample}
    ...
    So, |plot graphics| takes a graphics file along with options which can be
    ...
    A legend for |plot graphics| uses the current plot handler and the current
    plot |mark|:
    \todosp{here seems to be a bug: move `red' to a new line and it will raise
        an error}
    %
\begin{codeexample}[]
\begin{tikzpicture}
...
\begin{pgfplotskey}{plot graphics/lowlevel draw=\marg{width}\marg{height}}
    A low-level interface for |plot graphics| which actually invokes
    ...
    Thus, you can tweak |plot graphics| to place any \TeX{} box of the desired
...
The |plot graphics| tool of \PGFPlots{} allows to include three-dimensional
