...
\begin{key}{/pgfplots/table/ignore chars=\marg{comma-separated-list} (initially empty)}
	Allows to silently remove a set of single characters from input files. The characters are separated by white spaces. The documentation for this command, including special like |\%| or binary character codes like |\^^ff| can be found in the manual for \PGFPlotstable.
\end{key}
\begin{key}{/pgfplots/table/white space chars=\marg{comma-separated-list} (initially empty)}
	...
	This setting applies to |\adplot file| as well.
\end{key}
