...
\end{tikzpicture}
\end{codeexample}
It is also possible to provide the data inline:
\begin{codeexample}[code only]
\begin{tikzpicture}
\begin{loglogaxis}[
	xlabel=Dof,
...
\paragraph{Legends in \texttt{plot graphics}:} A legend for |plot graphics| uses |sharp plot| in the initial configuration. Furthermore, the legend will the actual plot |mark|. If you'd like to change the legend appearance, consider using the |every legend image post| style:

	|\addplot[..., every legend image post/.append style={const plot}] graphics ...|

	The example activates |const plot| for the legend image.
\end{addplotoperation}
