...
\begin{pgfplotskey}{plot graphics/points=\marg{list of coordinates} (initially empty)}
	This key allows to provide the external data range. It constitutes an alternative to |plot graphics/xmin| (and its variants): simply provide at least two coordinates in \marg{list of coordinates}. Their bounding box is used to determine the external data range, and the graphics is squeezed between these coordinates. 
...
\end{tikzpicture}
\end{codeexample}
	\noindent The \meta{list of coordinates} is a sequence of the form |(x,y)| for two--dimensional plots and |(x,y,z)| for three--dimensional ones, the ordering is irrelevant.
...
\end{tikzpicture}
\end{codeexample}
\noindent \PGFPlots\ computes the correct |unit aspect ratio| out of the provided canvas coordinates. Consequently, you may not be able to re-define |unit aspect ratio| without getting complaints of \PGFPlots.


\PGFPlots\ detects if the input data has a non--standard |unit vector ratio|. For example, the following figure uses |unit vector ratio=1 1 1| (i.e.\ |axis equal|) in matlab:

{\setlength{\fboxsep}{0pt}%
\fbox{\includegraphics[width=6cm]{plotdata/3drisingdrop}}%
}%
...
\begin{axis}[
	view={-37.5}{30},
	enlargelimits=false,
	%plot box ratio={1.0000}{1.0015}{1.0014},
]
	\addplot3 graphics[
		points={
			...
			(0,-0.00263,0.00263)   => (366,276)% to avoid low-rank matrix! document
			(0.002625,-0.002625,0) => (455,213)
		}
	]
	{plotdata/3drisingdrop.png};
...
{\setlength{\fboxsep}{0pt}%
\fbox{\includegraphics[width=6cm]{plotdata/plotgraphics3d.png}}%
}%
...
\begin{axis}[
	view={-37.5}{30},
	enlargelimits=false,
	plot box ratio ={2.00325}{1.32796}{1},
]
	\addplot3 graphics[
		points={
			(70,60,16)     => (87, 292)
			(134.3,60,16)  => (366,399)
			(70.19,60,0)   => (87, 139)
			(134.3,17.5,16)=> (508,307)
		}
	]
	{plotdata/plotgraphics3d.png};
