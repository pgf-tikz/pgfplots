...
If one wishes to have an easier approach of aligning one should refer to the library |groupplots| on page~\pageref{sec:group:plot}.
...
\end{enumerate}
Note that all these methods are more or less incompatible with image externalization to |.pdf|. The problem is that |pdflatex| crops everything outside of the bounding box away. The only safe way to ``restrict'' bounding boxes of external |.pdf| images is to use negative |\hspace| or |\vspace| commands (or options to |\includegraphics|).
