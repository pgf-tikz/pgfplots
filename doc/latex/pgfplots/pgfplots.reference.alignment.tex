...
The idea is simple: use a \LaTeX\ table and provide one |tikzpicture| for every cell. It has worked for many years together with |\includegraphics| and it works in the very same way for \PGFPlots. The approach is the simplest one since it doesn't need special knowledge. Its disadvantage, however, is control over positions \emph{inside} of the image (like differently sized axis descriptions).

Is is strongly recommended to employ the |baseline| option for each cell picture, which simplifies vertical alignment considerably. Horizontal alignment may need changes to the bounding boxes of the individual cells which is discussed in Section~\ref{sec:bb} below.
...
Note that the |external| library works on each picture separately (unlike the alternative array alignment techniques). If you want to use this alignment form together with the |external| library \emph{and} you need truncated bounding boxes, you should consider using the |trim left| and |trim right| options of \Tikz. These options are the \emph{only} way to externalize a graphics with reduced bounding box. Unfortunately, |trim left| and |trim right| are not shipped with \PGFPlots\ and will be part of the next stable \Tikz\ release.
