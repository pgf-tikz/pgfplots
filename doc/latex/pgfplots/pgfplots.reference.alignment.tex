...
Sometimes multiple alignmed axes in array form are desired. \PGFPlots\ supports this task in several ways which are described in the following. There are basically three related, yet different, approaches:
\begin{enumerate}
	...
	This allows considerably simpler alignment! Alas, it needs special handling for |legend entries| due to a weakness of |\matrix|.
...
The idea is simple: use a \LaTeX\ table and provide one |tikzpicture| for every cell. It has worked for many years together with |\includegraphics| and it works in the very same way for \PGFPlots. The approach is the simplest one since it doesn't need special knowledge. Its disadvantage, however, is more difficulty to control positions \emph{inside} of the image (like differently sized axis descriptions).

Is is strongly recommended to employ the |baseline| option for each cell picture, which simplifies vertical alignment considerably. The author believes that the most simple and most effective way to realize horizontal alignment of adjacent, yet isolated, pictures is the |trim left| and |trim right| feature which will be part of the next stable \PGF\ release (or available as part of the experimental \PGF\ version from |texample.net|). The |trim left| feature can be used to exclude axis descriptions on the left from the bounding box, and the |trim right| can exclude axis descriptions on the right from the bounding box. Thus, alignment is done using the vertical axis lines.
...
Note that the |external| library works on each picture separately (unlike the alternative array alignment techniques).  An advantage of this |trim left| / |trim right| feature is that it works even together with the |external| library. These options are the \emph{only} way to externalize a graphics with reduced bounding box. Unfortunately, |trim left| and |trim right| are not shipped with \PGFPlots\ and will be part of the next stable \Tikz\ release.
...
\begin{enumerate}
	...
	\item The \declareandlabel{trim left} and \declareandlabel{trim right} feature. 
\end{enumerate}
