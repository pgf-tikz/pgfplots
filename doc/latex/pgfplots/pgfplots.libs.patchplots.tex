...
\end{codeexample}
	...
	A patch with type |biquadratic| and |shader=interp| can produce a bounding box which is much larger than the one induced by the interpolation points. The shape is transformed to a tensor bezier spline and the control points of that shape determine the bounding box. This holds for all advanced patch types; it is necessary in order to display the shading. Use |\useasboundingbox| with custom paths to restrict the image's bounding box if this yields unsatisfying results.
...
\end{codeexample}
	...
	Just as described for |biquadratic|, a patch with type |bicubic| and |shader=interp| can produce a bounding box which is much larger than the one induced by the interpolation points. The shape is transformed to a tensor bezier spline and the control points of that shape determine the bounding box. 
