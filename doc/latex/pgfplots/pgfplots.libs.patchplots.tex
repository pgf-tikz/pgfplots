...
\PGFPlots\ supports automatic patch refinement. There are mainly two purposes for patch refinement: to increase the quality of |z buffer=sort| and to improve color interpolation for high--order patches. 
...
\begin{pgfplotskey}{patch refines=\marg{levels} (initially 0)}
	...
	A positive \marg{levels} enables (recursive) patch refinement: each patch is refined individually. 
...
\foreach \level in {0,1,2} {%
	...
	\addplot3[patch,patch type=triangle quadr,
		shader=interp,patch refines=\level]
	coordinates {
		(0,0,0) (5,4,0) (0,7,0)
		(2,3,0) (3,6,1) (-1,4,0)
	};
...
\end{codeexample}
	\noindent The example is a highly distorted mesh of coarse mesh width, so the effects have large impact. For practical examples, you probably won't need many refinement levels.
	
	The refined patches reproduces the geometry's shape exactly. In addition, they improve color interpolation. Note that its purpose is just visualization, therefor hanging nodes are allowed (and will be generated by |patch refine| for most |patch type|s).
...
\begin{pgfplotskey}{patch to triangles=\mchoice{true,false} (initially false)}
	Occasionally, one has complicated a |patch type| on input and would like to visualize it as a |triangle| mesh. \PGFPlots\ supports automatic triangulation of patches. Triangulation means to replace each individual input patch by one or more triangles. Triangulation can be combined with |patch refines| in which case |patch refines| is applied first and the resulting refined patches are then triangulated.
\begin{codeexample}[]
\foreach \level in {0,1,2} {%
	\begin{tikzpicture}
	\begin{axis}[
		nodes near coords={(\coordindex)},
		footnotesize,
		title={Triangulation + patch refines=\level}]
...
The |patchplots| library has been optimized for use with interpolated shadings, i.e.\ for |shader=interp|. \PGFPlots\ also supports |shader=flat| and |shader=faceted| by stroked and/or filled paths. This works if the \emph{mesh width is small enough} such that patches do not fold over themselves.
