...
\begin{tikzpicture}
	\begin{axis}[legend columns=-1,legend entries={$(x+0)^k$,$(x+1)^k$,$(x+2)^k$,$(x+3)^k$},legend to name=named]
	\addplot {x};
	\addplot {x+1};
	\addplot {x+2};
	\addplot {x+3};
...
\end{codeexample}
	...
	It is also possible to re--use the legend multiple times, for example here at this place: \pgfplotslegendfromname{named}.

	\begin{command}{\pgfplotslegendfromname\marg{name}}
		Retrieves a stored legend (one defined by |legend to name|) and draws it.

		The command can occur inside of a picture or just in the text (including figure captions). It can even occur \emph{before} the axis as such (it is handled like a |\ref| command, so you may need to run \LaTeX\ twice).
...
\subsubsection{Colorbars Outside Of an Axis}
...
\pgfplotsset{
	footnotesize/.style={
		...
		tick label style={font=\footnotesize},
		label style={font=\small},
		every axis title/.style={font=\small,at={(0.5,1)},above},
...
\pgfplotsset{
	tiny/.style={
		...
		every axis title/.style={font=\footnotesize,at={(0.5,1)},above},
