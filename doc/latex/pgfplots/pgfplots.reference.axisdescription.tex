...
\end{codeexample}
    ...
    which starts at |(name.north west)| and has the same direction as the $y$
    axis. Note that the red line and the $y$ axis are parallel. It also
...
\begin{pgfplotskeylist}{%
    ...
    These \todosp{`axis z line' and `axis z line*' are not listed above. Is that intended?} keys allow to choose the locations of the axis lines. The last one,
...
\end{codeexample}
    ...
    \todosp{this style doesn't exist (so). Better use `axis ... line*' or as
        above: `...line*'? (Make both, i.e.\@ this and the above the same.)}
...
\end{codeexample}
    ...
    a normal axis with one |plot graphics| command and it is invoked with a
...
\end{codeexample}
    ...
    |plot expression| and a surface plot to visualize the |colorbar|. Use
