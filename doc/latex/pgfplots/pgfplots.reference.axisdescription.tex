...
\begin{tikzpicture}
	\begin{axis}[legend entries={$e_1$,$e_2$},legend to name=named]
	\addplot {x};
	\addplot {x+1};
	\end{axis}
\end{tikzpicture}
...
And the legend is \pgfplotslegendfromname{named}.
\end{codeexample}

	Note that only the \emph{first} plot has |legend entries|. Thus, its legend will be assembled, and stored under the name `|named|', but it won't be drawn. The stored legend is drawn with |\pgfplotslegendfromname{named}| below the three plots. Since there is no picture, a |\tikz| picture is created and a |\matrix[/pgfplots/every axis legend]| path is drawn inside of it, resulting in the legend as if it had been placed inside of the axis.
	...
	It is also possible to re--use the legend multiple times, for example here: \pgfplotslegendfromname{named}.
