...
\end{codeexample}
<<<<<<< HEAD:doc/latex/pgfplots/pgfplots.reference.axisdescription.tex
...
>>>>>>> stable:doc/latex/pgfplots/pgfplots.reference.axisdescription.tex
...
\end{codeexample}
	...
	Note that |\label| information, even the small \Tikz\ pictures here, can be combined with the |external| library for image externalization, see section~\ref{sec:pgfplots:export} for details (in particular, the |external/mode| key).
\end{commandlist}
...
\label{key:legend:to:name}
	Enables a legend export mode: instead of drawing the legend, a self--contained, independent set of drawing commands will be stored as \meta{name}. The definition is done globally. The name can be referenced using 

	|\pgfplotslegendfromname|\marg{name}.

Thus, typing |\pgfplotslegendfromname|\marg{name} somewhere outside of the axis, maybe even outside of any picture, will cause the legend to be drawn.
...
\end{codeexample}
	...
	Note that only the \emph{first} plot has |legend entries|. Thus, its legend will be created as usual, and stored under the name `|named|', but it won't be drawn. The stored legend can then be drawn with |\pgfplotslegendfromname{named}| below the three plots. Since there is no picture in this context, a |\tikz| picture is created and a |\matrix[/pgfplots/every axis legend]| path is drawn inside of it, resulting in the legend as if it had been placed inside of the axis.
	...
	\begin{command}{\pgfplotslegendfromname\marg{name}}
		Retrieves a stored legend (one defined by |legend to name|) and draws it.

		|\pgfplotslegendfromname{named}: | \pgfplotslegendfromname{named}

		The command can occur inside of a picture or just in the text (including figure captions). It can be provided multiple times and even \emph{before} the axis as such (it is handled like a |\ref| command, so you may need to run \LaTeX\ twice).

		If |\pgfplotslegendfromname| encounters an undefined argument, it issues an ``undefined reference'' warning just like an undefined |\ref| does.

		Note that this |.aux| file related stuff is (currently) only supported for \LaTeX, sorry.

		If you want the legend to be exported \emph{and} drawn, consider using |extra description/.append code={\pgfplotslegendfromname|\marg{name}|}|.

		Note that |\pgfplotslegendfromname| can be combined with the |external| library for image externalization. In other words, the legend will work even if the defining axis has been externalized, see section~\ref{sec:pgfplots:export} for details (in particular the |external/mode| key).
	\end{command}

	\begin{stylekey}{/pgfplots/every legend to name picture}
		A style which is installed when |\pgfplotslegendfromname| is used outside of a picture: a new picture will be created with |\tikz[/pgfplots/every legend to name picture]|. 
...
\end{codeexample}
	...
	This is mainly useful in conjunction with |point meta rel=per plot|.
...
\end{codeexample}
	...
	Furthermore, the axis appearance is changed using |axis y line*=right| (or |axis x line*=bottom| for |colorbar horizontal|).
...
\begin{pgfplotskey}{colorbar to name=\marg{name} (initially empty)}
	Enables to detach a |colorbar| from its parent axis: instead of drawing the |colorbar|, a self--contained, independent set of drawing commands will be stored as \meta{name}. The definition is done globally. The name can be referenced using 

	|\pgfplotscolorbarfromname|\marg{name}.

Thus, typing |\pgfplotscolorbarfromname|\marg{name} somewhere outside of the axis, maybe even outside of any picture, will cause the |colorbar| to be drawn.
\begin{codeexample}[vbox]
...
\pgfplotscolorbarfromname{storedcolorbar}
\end{center}
\end{codeexample}
	...
	\begin{stylekey}{/pgfplots/every colorbar to name picture}
		A style which is installed when |\pgfplotscolorbarfromname| is used outside of a picture: a new picture will be created with |\tikz[/pgfplots/every colorbar to name picture]|. 
