...
\begin{anchorlist}{near xticklabel,near yticklabel,near zticklabel,near ticklabel}
	These anchors can be used to align at the part of a node (for example, an axis description) which is \emph{nearest} to the tick labels of a particular axis.

	These anchors are used for axis labels, especially for three dimensional axes. In fact, these anchors are used for every tick label by default.
...
\end{codeexample}
	...
	The style |line legend| can be used in conjunction with the |area legend| key. See in the example under that key.
\end{stylekey}
...
Note that three dimensional axes only support to use the same value for every axis, i.e.\ three dimensional axes support only the |axis lines| key. See section~\ref{sec:pgfplots:axislines:3d} for examples of three dimensional axis line variations.
\end{pgfplotskeylist}
...
\pgfplotsset{
	...
	/pgfplots/colorbar vertical/lowlevel/.style={%
		plot graphics/lowlevel draw/.code 2 args={%
			\if\pgfkeysvalueof{/pgfplots/y dir/value}n% check for reversed axes
...
\pgfplotsset{
	...
	/pgfplots/colorbar horizontal/lowlevel/.style={%
		plot graphics/lowlevel draw/.code 2 args={%
			\if\pgfkeysvalueof{/pgfplots/x dir/value}n% check for reversed axes
