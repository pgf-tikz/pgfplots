...
Occasionally, one has multiple adjacent plots, each with the same legend -- and just \emph{one} legend suffices. But where shall it be placed? And how? One solution is to use the |overlay| key to exclude the legend from bounding box computations, and place it absolutely such that it fits. Another is the |legend to macro| feature:

\begin{pgfplotskey}{legend to macro=\marg{\textbackslash macro} (initially empty)}
	Enables a legend export mode: instead of drawing the legend, the \meta{\textbackslash macro} will be defined to be a self--contained legend. The definition is done globally. Thus, typing \meta{\textbackslash macro} somewhere outside of the axis, maybe even outside of any picture, will cause the legend to be typeset.
\begin{codeexample}[vbox]
\pgfplotsset{footnotesize}
\begin{tikzpicture}
	\begin{axis}[legend entries={$e_1$,$e_2$},legend to macro=\storedlegend]
	\addplot {x};
	\addplot {x+1};
	\end{axis}
\end{tikzpicture}
...
And the legend is \storedlegend.
\end{codeexample}

	Note that only the \emph{first} plot has |legend entries|. Thus, its legend will be assembled, stored into |\storedlegend|, but it won't be drawn. Typing |\storedlegend| below the three plots detects that it is not currently inside of a picture, so it starts a new one and places a |\matrix[/pgfplots/every axis legend]| path inside of it, resulting in the legend as if it had been placed inside of the axis.

	The stored legend will contain the currently active values of legend- and plot style related options as |legend image code|. The algorithm works in the same way as for |\label| and |\ref|, i.e.\ it keeps any options with |/tikz/| prefix and expands those with |/pgfplots/| prefix.

	Note that the legend uses the style |every axis legend|, even though its placement options might be inadequate. You may want to adjust the style in the same axis in which the stored legend has been defined (the value will be copied and restored as well).

	It is also possible to re--use the legend multiple times, for example here: \storedlegend.

	If the macro is used outside of any picture, a new picture is created with |\tikz[/pgfplots/every legend macro picture]|. Thus, you can re-define this style to set alignment options (such as |baseline|).
	\begin{stylekey}{/pgfplots/every legend macro picture}
		A style which is installed when |legend to macro| is used outside of a picture.

		For example, the initialisation
\begin{codeexample}[code only]
\pgfplotsset{
	legend style={matrix anchor=west,at={(0pt,0pt)}},
	every legend macro picture/.style={baseline},
}
...
\end{codeexample}
		\noindent will cause the legend to be positioned such that its |west| anchor is at |y=0pt|. The |baseline| option will align this point of the legend with the text baseline.
	\end{stylekey}
\end{pgfplotskey}
