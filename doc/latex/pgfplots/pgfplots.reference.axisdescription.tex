...
\end{codeexample}
	...
	Note that only the \emph{first} plot has |legend entries|. Thus, its legend will be assembled, and stored under the name `|named|', but it won't be drawn. The stored legend is drawn with |\pgfplotslegendfromname{named}| below the three plots. Since there is picture, a new one is created and a |\matrix[/pgfplots/every axis legend]| path is drawn inside of it, resulting in the legend as if it had been placed inside of the axis.
