...
\draw [blue,fill] (2,0) circle (2pt) node [right] {(2,0)};

% this here will always work inside of an axis:
\draw [blue,fill] (axis cs:-1,0) circle (2pt) node [right] {(-1,0)};
\end{axis}
\end{tikzpicture}
\end{codeexample}
...
Here is an example which inserts a \PGFPlots\ graphical correctly into a |tikzpicture|:
