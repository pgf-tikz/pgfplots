...
\begin{pgfplotskey}{visualization depends on=\marg{data} (initially empty)}
	...
	Suppose you want a scatter plot, which depends on the $(x,y)$ coordinates (of course), the |point meta| data to draw individual colors and furthermore data which influences the |mark size|. Thus, you need a total of~$4$ coordinates for every data point. But: \PGFPlots, in its initial configuration, supports only $3$ for a two--dimensional plot (coordinates and |point meta|). A solution is proposed by 
	
	|visualization depends on={|\marg{content}|\as |\meta{\textbackslash macro}|}|:
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}
	\addplot+[
		...
		scatter src=y,
		samples=40,
		visualization depends on={expr{5*cos(deg(x))}\as \perpointmarksize},
