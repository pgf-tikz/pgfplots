...
\end{tikzpicture}
\end{codeexample}
	The \marg{content of nodes} is, if nothing else has been specified, the content of the ``point meta'', provided by |\pgfmathprintnumber{\pgfplotspointmeta}|. The macro |\pgfplotspointmeta| contains whatever has been selected by the |point meta| key, it defaults to the $y$ coordinate for two dimensional plots and the $z$ coordinate for three dimensional plots.
