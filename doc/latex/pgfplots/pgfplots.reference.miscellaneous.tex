...
\begin{pgfplotskey}{clip bounding box=\mchoice{default tikz,upper bound} (initially controlled by compat key)}
	...
	More precisely, the choice \declaretext{default tikz} installs the clip path induced by the axis (|\pgfplotspathaxisoutline|) as ordinary \tikzname\ path. That means its bounding box essentially contributes to the picture's bounding box, irrespective of the size of contained paths.
