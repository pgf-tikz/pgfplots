...
\end{pgfplotskey}
\begin{pgfplotskey}{clip has bounding box=\mchoice{true,false} (initially true)}
	Controls whether the path generated by |clip=true| contributes to the bounding box. This has a consequence for |axis lines|$\neq$|box|, in particular, for |hide axis|: if the value is |true|, hiding (parts of) the axis will not reduce the bounding box because the clip path is as large as before. The value |false| allows to reduce the bounding box also in case of |hide axis|.

	This key is controlled by the |compat| level. Its default is |true|. Since |compat=1.8|, it is set to |false|.
\end{pgfplotskey}
