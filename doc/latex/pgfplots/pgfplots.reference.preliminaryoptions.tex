...
\begin{handler}{{.cd}}
	Each key has a (often very long) name, like |/pgfplots/xmin|. However, if the ``current directory'' is |/pgfplots|, it suffices to write just |xmin|. The |.cd| key handler changes the ``current directory'' in this way.
\end{handler}
