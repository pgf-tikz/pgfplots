...
A key usually has a value (like a number, a string, or perhaps some macro code). You can assign values for keys (``set keys'') in many places in a \LaTeX\ document. The value will remain effective until it is changed or until the current \TeX\ scope ends (which happens after a curly brace `|}|', after |\end|\marg{name} or, for example, after |\addplot|).
...
\begin{command}{\pgfplotsset\marg{key-value-list}}
	Defines or sets all options in \meta{key-value-list}. The \meta{key-value-list} can contain any of the options in this manual which have the prefix |/pgfplots/| (you do not need to type the prefix).
...
\end{codeexample}
	can be used to set document--wise styles for line specifications, the legend's style and axis labels. The settings remain in effect until the end of the actual environment (like |\end{figure}|) or until you redefine them or until the next closing curly brace `|}|' (whatever comes first).
