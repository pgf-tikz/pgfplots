...
\begin{pgfplotskey}{clickable coords code=\marg{\TeX\ code which defines {\normalfont\ttfamily\textbackslash pgfplotsretval}}}
	...
	to provide number printing. The |\pgfmathprintnumberto[verbatim]| doesn't use math mode to format a number, and it writes its result into |\pgfmathresult|. The |\edef| means ``expanded definition'' and has the effect of expanding all macros to determine the value, in our case ``Number \meta{the value}''. The following example uses it twice to pretty--print the data:
{
	% allow to typeset the example. I couldn't get the \catcodes
	% correctly...
	\pgfplotstableset{
