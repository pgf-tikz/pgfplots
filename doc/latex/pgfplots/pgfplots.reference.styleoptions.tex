...
\begin{stylekey}{/pgfplots/every semilogx axis (initially empty)}
    Installed at the beginning of every plot with linear $x$-axis and
    logarithmic $y$-axis, but after `|every axis|'.
\end{stylekey}
...
\begin{stylekey}{/pgfplots/every axis plot no \# (initially empty)}
	...
	This style is install before the argument list of |\addplot|\meta{options}, i.e.\ \meta{options} are preferred. Note that this also means that any items in a |cycle list| have higher priority.
\end{stylekey}
