...
\begin{enumerate}
	\item \PGFPlots\ 1.5.1 now interpretes circle- and ellipse radii as \PGFPlots\ coordinates. In other words: it allows to provide |\draw circle[radius=5];| inside of an axis. This requires |\pgfplotsset{compat=1.5.1}| or higher. 

	Without this compatibility setting, circles and ellipses uses low--level canvas units of \pgfname\ as in earlier versions.

	\item \PGFPlots\ 1.5 uses |log origin=0|. Older versions keep |log origin=infty|. This requires |\pgfplotsset{compat=1.5}| or higher.
...
\end{codeexample}
	...
	Although typically unnecessary, it is also possible to activate only selected fixes and keep compatibility to older versions in general:
	\begin{pgfplotskeylist}{compat/path replacement,compat/labels,compat/scaling,compat/general}
	Say, you have a document with |\pgfplotsset{compat=1.3}| and you want to keep it this way.
...
\end{codeexample}
	...
	Each of the specified keys accepts the possible values of the |compat| key.
	\end{pgfplotskeylist}
\end{pgfplotskey}
