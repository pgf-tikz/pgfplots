...
\section{About this Version of \PGFPlots}
...
\begin{enumerate}
	\item Starting with \PGFPlots\ $1.2$, the |sample|, |domain| and |samples at| keys are interpreted slightly differently than before.
	  The good thing is: |domain| accepts the full data range of \PGFPlots\ now. But: there is an |/tikz/domain| key and an |/pgfplots/domain| key now -- and only the latter will be used by \PGFPlots.

	  Use |\begin{axis}[domain=|$\dotsc$|]| instead of |\begin{tikzpicture}[domain=|$\dotsc$|]| and everything will be fine.
