...
\begin{pgfplotsxykeylist}{%
    \x tick pos=\mchoice{left,right,both} (initially both),
...
\begin{pgfplotsxykeylist}{%
    \x ticklabel pos=\mchoice{default,lower,upper} (initially default),
       ticklabel pos=\mchoice{default,lower,upper} (initially default)%
}
	...
	Note that the key accepts a couple of equivalent keys: all of \declaretext{lower}, \declaretext{left}, \declaretext{bottom} have the same meaning as |lower|. Similarly, all of \declaretext{upper}, \declaretext{right}, \declaretext{top} have the same meaning as |upper|.
\end{pgfplotsxykeylist}
