...
\end{codeexample}
It is advisable to prefer the `|/.append style|' versions whenever possible to ensure compatibility with future versions.
...
\begin{itemize}
	\item \declareandlabel{\axisdefaultticklabel} for normal plots and 
	\item \declareandlabel{\axisdefaultticklabellog} for logplots, see below.
\end{itemize}
...
\begin{pgfplotsxykeylist}{\x tick pos=\mchoice{left,right,both} (initially both),tickpos=\mchoice{left,right,both}}
Allows to choose where to place the small tick lines. In the default configuration, this does also affect tick \emph{labels}, see below. The |tickpos| style sets all of them to the same value.
...
Allows to choose where to place tick \emph{labels}. The choices |left| and |right| place tick labels either at the left or at the right side of the complete axis. The choice |default| uses the same setting as |xtick pos| (or |ytick pos|). This option is only useful for boxed axis -- keep it to |default| for non-boxed figures. The |ticklabel pos| style sets all three of them to the same value.
...
\end{codeexample}
	...
	In other words: usually, you see log axes base $10$ and that's it. It is only intersting for coordinate filters: 
