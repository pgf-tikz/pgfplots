...
\begin{command}{\pgfplotstablecolname}
	During the evaluation of column options, this command expands to the current column' name. It is valid while |\pgfplotstabletypeset| processes the column styles, prepares the output cell content and checks row predicates.
\end{command}
...
\begin{command}{\pgfplotstablename}
	During |\pgfplotstabletypeset|, this macro contains to the table's macro name as top-level expansion (the \emph{contents} of this macro can be handed over to |\pgfplotstabletypeset| using |\expandafter|). For tables which have been loaded from disk, this name is a temporary macro.
\end{command}
