...
\pgfkeys{%
	...
	/pdflinks/search key prefixes in/.add={/pgf/number format/,}{},
...
\begin{key}{/pgfplots/table/column name=\marg{\TeX\ display column name}}
	...
	|columns/|\marg{lowlevel colname}|/.style={column name={|\meta{\TeX\ display column name}|}| .
\end{key}
...
\end{codeexample}
	...
	\begin{itemize}
		\item \declareandlabel{table}: either a file name or an already loaded table where to get the data points,
		\item \declareandlabel{x}: the column name of the $x$ axis,
		\item \declareandlabel{y}: the column name of the $y$ axis.
	\end{itemize}
