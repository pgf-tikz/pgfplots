...
\begin{stylekey}{/pgfplots/table/create col/linear regression=\marg{key-value-config}}%
	Computes a linear (least squares) regression $y(x) = a \cdot x + b$ using the sample data $(x_i,y_i)$ which has to be specified inside of \meta{key-value-config} (see below).
...
\pgfplotstablecreatecol[
	linear regression={table=\loadedtbl}
]
	{regression}
	{\loadedtbl}
...
\begin{tikzpicture}
	...
	\addplot table \loadedtbl;
	\addplot table[y=regression] \loadedtbl;
	\addlegendentry{$y(x)$}
	\addlegendentry{% 
...
\pgfkeys{
	...
	\begin{key}{%
		...
		Provides the table from where to load the |x| and |y| columns.
	\end{key}
	\begin{keylist}{%
		/pgfplots/table/create col/linear regression/x=\marg{column} (initially [index]0),
		/pgfplots/table/create col/linear regression/y=\marg{column} (initially [index]1)}
		Provides the source of $x_i$ and $y_i$ data, respectively. The argument \meta{column} can be any of the usual column sources, i.e.\ |[index]|\meta{integer} to designate column indizes (starting with $0$), column names, |create on use| specifications or |alias|es.

		The initial configuration uses the first and second columns of the input |table|.
	\end{keylist}

	\begin{keylist}{%
		/pgfplots/table/create col/linear regression/xmode=\mchoice{linear,log} (initially linear),
		/pgfplots/table/create col/linear regression/ymode=\mchoice{linear,log} (initially linear)}
		...
		The |log basis x| and |log basis y| keys determine the basis.

\begin{codeexample}[]
\pgfplotstableset{
	% Configure how to create the 'regression' column:
	create on use/regression/.style={
		create col/linear regression={
			table=pgfplotstable.example1.dat,
			x=dof,y=error2,
			xmode=log,ymode=log}}}
\begin{tikzpicture}
	\begin{loglogaxis}
	...
	\addplot table[x=dof,y=regression,read completely] 
		{pgfplotstable.example1.dat};	
	%\xdef\slope{\pgfplotstableregressiona} %<-- might be handy occasionally
...
\end{codeexample}
	...
	The |variance| key allows to load values from a table \meta{column name} name. Such a column name is (initially, see below) loaded from the same table where data points have been found. The \meta{column name} may also be a |create on use| name.
\begin{codeexample}[]
\pgfplotstableset{
	% same example as above, but with variance list:
	create on use/regression/.style={
		create col/linear regression={
			table=pgfplotstable.example1.dat,
			x=dof,y=error2,
...
\end{codeexample}
	...
	\begin{key}{/pgfplots/table/sort cmp=\marg{less than routine} (initially float <)}
		...
		\begin{pgfplotskeylist}{%
			...
			The |date <| compares dates of the form |YYYY-MM-DD|. The |string <| uses lexicographical string comparison based on the ASCII character codes of the sort keys.
\begin{codeexample}[]
