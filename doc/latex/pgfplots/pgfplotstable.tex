...
\pgfplotstabletypesetfile[ % local config, applies only for this table
	1000 sep={\,},
	columns/info/.style={
...
\begin{key}{/pgfplots/table/typeset=\mchoice{true,false} (initially true)}
	A boolean which disabled the final typesetting stage. Use |typeset=false| in conjunction with |write to macro| if only the generated code is of interest.	
\end{key}
...
\begin{codeexample}[]
\pgfplotstabletypesetfile[columns={level,dof}]
	{pgfplotstable.example1.dat}
...
\pgfplotstablenew[
	% define how the 'new' column shall be filled:
	create on use/new/.style={create col/expr={(15+\pgfplotstablerow)!}},
	columns={new}]
	{11}
	\table
% show it:
...
\end{codeexample}
	...
	\end{itemize}
	The keys |xmode| and |ymode| can take either |log| or |linear|.

	I needed such a thing as part of my postprocessing routines and decided to put it here -- if it proves to be useful for anyone, feel free to use it.
\end{stylekey}
