...
\pgfkeys{/pgfmanual/gray key prefixes={/pgfplots/table/create col/linear regression}}
	\begin{key}{%
...
\end{codeexample}
	...
	\begin{key}{/pgfplots/table/create col/linear regression/variance=\marg{column name} (initially empty)}
	Associates variances loaded from \meta{column name} to each data point. High (relative) variance means an unreliable data point. The \meta{column name} may also be a |create on use| name.
	\end{key}
