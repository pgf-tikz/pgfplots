...
\begin{stylekey}{/pgfplots/table/create col/linear regression=\oarg{options}\marg{\textbackslash table {\normalfont or} file name}\marg{x col}\marg{y col}}
	Computes a linear regression $y(x) = a \cdot x + b$ using the sample data $x_i$ from column \meta{x col} and $y_i$ from column \meta{y col}. Both columns are loaded from the first argument which can be either a loaded table, a file name or an inline table (as usual).
...
\pgfplotstablecreatecol[
	linear regression={\loadedtbl}{x}{y}
]
	{regression}
	{\loadedtbl}
...
\end{codeexample}
	...
	The following \oarg{options} are accepted:
