...
\begin{stylekey}{/pgfplots/table/create col/linear regression=\marg{\textbackslash table {\normalfont or} file name}\marg{x col}\marg{y col}\marg{options}}
	Computes a linear (least squares) regression $y(x) = a \cdot x + b$ using the sample data $x_i$ from column \meta{x col} and $y_i$ from column \meta{y col}. Both columns are loaded from the first argument which can be either a loaded table, a file name or an inline table (as usual).

	The last parameter, \meta{options}, can be omitted.
...
\end{codeexample}
	...
	The following \meta{options} are accepted as comma--separated list:
