...
\end{codeexample}
		...
		\item As option which affects just a single table. This is provided as optional argument to the respective table typesetting command, for example |\pgfplotstabletypesetfile|\oarg{options}\marg{file}.
	\end{enumerate}
...
The following extract of \cite{tikz} explains how to configure number formats. The common prefix |/pgf/number format| can be omitted; it will be recognised automatically.
...
\subsection{The Postprocessing Framework}
...
End users probably don't need to use |\pgfplotstablecreatecol| directly at all -- there is the high--level framework |create on use| which invokes it internally and can be used with simple key--value assignments (see below).


This command is admittedly technical, but it offers a flexible framework to generate new columns. It has been designed to create new columns using the already existing values -- for example using logical or numerical methods to combine existing values. It provides fast access to a row's value, the previous row's value and the next row's value.
...
\begin{codeexample}[]
% requires \usepackage{array}
\pgfplotstabletypesetfile[% here, configuration options apply only to this single statement:
