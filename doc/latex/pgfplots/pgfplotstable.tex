...
\begin{command}{\pgfplotstablename}
	...
	Advances users may benefit from expressions like |\expandafter\pgfplotstabletypeset\pgfplotstablename|.
...
\begin{stylekey}{/pgfplots/table/preproc/expr=\marg{math expression}}
	...
	\begin{itemize}
		...
		Note that |\thisrow{}| is not as powerful as in the context of column creation routines: the argument must match exactly the name of the currently processed column name. You can also use the shorthand
...
\end{codeexample}
	...
	As in |assign cell contents|, the code can evaluate helper macros like |\pgfplotstablerow| to change only particular entries. Furthermore, the postprocessor may depend on the unprocessed cell input (as it has been found in the input file or produced by the loading procedure) and/or the preprocessed cell value. These values are available as
	\begin{itemize}
		\item the key \declaretext{@unprocessed cell content} which stores the raw input,
		\item the key \declaretext{@preprocessed cell content} which stores the result of the preprocessor,
		\item the key \declaretext{@cell content} which contains the result of the typesetting routine,
...
\end{codeexample}
	...
	\begin{itemize}
		\item \declaretext{table}: either a file name or an already loaded table where to get the data points,
		\item \declaretext{x}: the column name of the $x$ axis,
		\item \declaretext{y}: the column name of the $y$ axis.
	\end{itemize}
	The keys \declaretext{xmode} and \declaretext{ymode} can take either |log| or |linear|. All mentioned keys have the common key path 
...
\pgfplotstableforeachcolumn\table\as\col{%
	The column name is `\col'; its index \pgfplotstablecol;\par
}
\end{minipage}
\end{codeexample}
