...
\begin{key}{/pgfplots/table/header=\mchoice{true,false,numeric} (initially true)}
	...
	The choice |header=|\declaretext{false} is identical to this last case, i.e.\ even if the first line contains strings, they won't be recognised as column names.

	Finally, the choice \declaretext{numeric} is the opposite of |false|: it assumes that the first non--comment line \emph{contains} column names. In other words: even if only numbers are contained in the first line, they are considered to be column \emph{names}.
\end{key}
...
\begin{commandlist}{%
	\pgfplotstabletranspose\marg{\textbackslash outtable}\marg{\textbackslash table or filename},%
