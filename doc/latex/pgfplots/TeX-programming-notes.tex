...
\end{codeexample}
	...
	It boils down to a suitable |\csname ... \endcsname|. Consequently, it expands to |\relax| if the key happens to be undefined.
\end{command}
...
The first thing which is strange when inspecting the actual code is that \PGF Keys has some suffix `|/.initial|'. This is, in fact, a consistent new system of \PGF Keys: these strange suffixes allow to configure and modify the keys to which them apply. These suffixes are called ``key handlers''. Whenever you encounter \meta{key path/key name} followed by `|/.|\meta{handler}', you can safely assume that \meta{key path/key name} is about to be reconfigured or modified.
