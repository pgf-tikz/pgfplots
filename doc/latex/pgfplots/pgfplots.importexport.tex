...
eachnth = 1;
I = 1:eachnth:size(C,1); % this is nonsense for eachnth=1 -- but perhaps you don't want each clor.
CC = C(I,:);
fid = fopen('tempfile.tex','w'); % write TeX statements to temp file
fprintf(fid,'\\pgfplotsset{colormap={matlab}{\n');
fprintf(fid,'rgb=(%f,%f,%f)\n',CC');
fprintf(fid,'}}\n');
fclose(fid); % ok, now inspect 'tempfile.tex' or use '\input tempfile.tex' in LaTeX
\end{codeexample}
