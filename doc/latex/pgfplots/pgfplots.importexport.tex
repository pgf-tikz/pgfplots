...
\begin{key}{/tikz/external/mode=\mchoice{convert with system call,list and make,$\dotsc$} (initially convert with system call)}
	...
	It is not required to use |make|: the library expects you to generate the images somehow and it doesn't care about the ``how''. Using |make -f |\meta{name-of-tex-file}|.makefile -j 2| allows parallel execution which might, indeed be an option. 
	
	\PGFPlots\ provides special support for makefiles: it provides automatical prerequisites to update the graphics if any of the data files required for the external graphics has been changed.	
