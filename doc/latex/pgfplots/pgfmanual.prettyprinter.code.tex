...
\def\pgfmanual@pretty@after@cs{\pgfmanual@pretty@mainloop}%
...
\def\pgfmanual@pretty@collectcs@loop@END{%
	\pgfkeysifdefined{/codeexample/prettyprint/cs arguments/\pgfmanual@loc@TMPa}{%
		...
		\def\pgfmanual@pretty@collectcs@loop@END@next{%
			\pgfmanualprettyprintercollectargcount{\pgfkeysvalueof{/codeexample/prettyprint/cs arguments/\pgfmanual@loc@TMPa}}%
		}%
	}{%
		...
		\let\pgfmanual@pretty@collectcs@loop@END@next=\pgfmanual@pretty@after@cs
	}%
...
% 	invoked. Just defined \pgfmanualprettyprinterhandlecollectedargs
% 	properly.
%
% ATTENTION:
...
%   arg.
\def\pgfmanualprettyprintercollectargcount#1{%
	\begingroup
...
% 	properly.
%
% @see \pgfmanualprettyprintercollectargcount
% 	
% @remark This macro checks for \pgfmanual@EOI. In case it encounters
% \pgfmanual@EOI, it stops collecting, leaves the \pgfmanual@EOI as
% next token to be read and invokes the finish routine.
% If you need to check for this case, use
% \pgfutil@ifnextchar\pgfmanual@EOI{}{}
\def\pgfmanualprettyprintercollectupto#1{%
	\begingroup
...
\def\pgfmanual@pretty@pgfkeys@collectvalue#1{%
	...
	\else
		...
		\if\pgfmanual@pretty@isconsumed0%
			\let\pgfmanualprettyprinterhandlecollectedargs=\pgfmanual@pretty@collectargs@finish@value
			\def\pgfmanual@pretty@pgfkeys@collectvalue@next{%
				\pgfmanualprettyprintercollectupto,#1%
			}%
		\fi
	\fi
