...
\begin{handler}{{.define layer set}=\marg{ordered layer names}\marg{style definitions}}
    ...
    seems that the `key' has to be `layers/key' and otherwise will not work.
    To prove that have a look at \url{} and delete `layers/' there which will
    cause an error. $\to$ How to modify the `handler' or the describing text
    to make that more clear?} \meta{key}.
