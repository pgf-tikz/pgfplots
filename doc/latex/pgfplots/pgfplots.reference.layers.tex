...
\begin{handler}{{.define layer set}=\marg{ordered layer names}\marg{style definitions}}
    ...
    Afterwards, \meta{key} can be specified as argument to |set layers| as follows:
	\begin{itemize}
		\item if \meta{key} has the type |/pgfplots/layers/|\meta{name}, you can write |set layers=|\meta{name}.
		...
		\item if \meta{key} is an arbitrary key path, |set layers| expects the fully qualified \meta{key} name. 
