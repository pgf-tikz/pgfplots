...
\begin{handler}{{.define layer set}=\marg{ordered layer names}\marg{style definitions}}
    Allows to define a new layer set configuration named \todosp{in fact it
    seems that the `key' has to be `layers/key' and otherwise will not work. To
    prove that have a look at \url{http://tex.stackexchange.com/a/350514/95441}
    and delete `layers/' there which will cause an error. $\to$ How to modify
    the `handler' or the describing text to make that more clear?} \meta{key}.
