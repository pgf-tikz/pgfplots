...
\begin{axis}[
	...
	\draw[black] (axis cs:0,0)
		ellipse [
			x radius=transformdirectionx(1),
			y radius=transformdirectiony(2)];

	\draw[red]   (axis cs:0,0)
		ellipse [rotate=90,
			x radius=transformdirectionx(1),
			y radius=transformdirectiony(2)];
	% see \pgfplotspointaxisdirectionxy for arbitrary ellipses

	\addplot [only marks,mark=*] coordinates { (0,0) };
\end{axis}
\end{tikzpicture}
\end{codeexample}
	Here, the \declareandlabel{transformdirectionx} math command does the same as |axis direction cs|, but only in $x$ direction. The result of |transformdirectionx| is a dimensionless unit which can be interpreted relative to the current \pgfname\ $x$ unit vector $e_x$ (see the documentation of |\pgfplotstransformdirectionx| for details). Since the |x radius| key of \tikzname\ converts dimensionless input variables $s$ to $s \cdot e_x$, the expression |x radius=transformdirectionx(1)| leads to a first radius of $1$~axis unit and |y radius=transformdirectiony(2)| leads to a second radius of $2$~axis units.

	There are the math commands |transformdirectionx|, \declareandlabel{transformdirectiony}, and \declareandlabel{transformdirectionz}. Each of them defines |\pgfmathresult| to contain the result of |\pgfplotstransformdirectionx| (or its variants for $y$ and $z$, respectively).
