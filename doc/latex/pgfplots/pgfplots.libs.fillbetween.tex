...
\begin{tikzkey}{fill between/on layer=\marg{layer name} (initially pre main)}
	...
	As soon as you type |\addplot fill between|, \PGFPlots\ will activate layered graphics using |set layers| such that this works automagically. \PGFPlots\ will also install a clip path the first time it encounters this layer.
...
\draw [name path=red,red,thick]
	(axis cs:\verticalbar,-1) -- (axis cs:\verticalbar,2);
...
\begin{tikzpicture}
	...
	\draw[-stealth,thick,name path=A,red]  (0,0) -- (1,1);
	\draw[-stealth,thick,name path=B,blue] (0,-1) -- (1,0);
