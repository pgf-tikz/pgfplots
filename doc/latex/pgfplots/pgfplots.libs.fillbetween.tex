...
\end{codeexample}
	...
	The feature |soft clip| is a part of |fill between| in the sense that it determines the fill path. However, there is also a separate module which allows to apply soft--clipping to individual paths. To this end, a |decoration=soft clip| is available. A use--case could be to highlight parts of the input path:
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}
...
\begin{pgfplotskey}{fill between/on layer=\marg{layer name} (initially pre main)}
...
\begin{pgfplotskey}{fill between/inner moveto=\mchoice{connect,keep} (initially connect)}
...
\begin{command}{\pgfplotsfillbetween\oarg{options}\marg{draw style}}
...
\begin{tikzpicture}
	...
	\pgfplotsfillbetween[of=first and second,
		split, 
...
\end{codeexample}
	...
	Note that |\pgfplotsfillbetween| is \emph{no} typically |\path| statement: it generates one or more of \tikzname\ |\path| statements (each with their own, individual \emph{draw style}).
\end{command}
...
\begin{tikzpicture}[line join=round,x=3cm,y=3cm]
	...
	\pgfplotsgetnamedpath{first}
	\let\A=\pgfplotsretval
	\pgfplotsgetnamedpath{second}
...
\begin{command}{\pgfplotsgetnamedpath\marg{string name}}
	Defines |\pgfplotsretval| to contains the softpath associated with \meta{string name}. The \meta{string name} is supposed to be the value of |name path| or |name path| global.
