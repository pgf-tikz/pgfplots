...
\end{codeexample}
	...
	\begin{itemize}
		\item It can be of the form \declaretext{domain}|=|\meta{xmin}|:|\meta{xmax}. This choice is equivalent to
			...
			|(axis cs:|\meta{xmin}|,\pgfkeysvalueof{/pgfplots/ymin}) rectangle |
			|(axis cs:|\meta{xmax}|,\pgfkeysvalueof{/pgfplots/ymax})|.
		\item It can be of the form \declaretext{domain y}|=|\meta{ymin}|:|\meta{ymax}. This choice is equivalent to

			|axis cs:(\pgfkeysvalueof{/pgfplots/xmin},|\meta{ymin}|) rectangle |
			|(axis cs:\pgfkeysvalueof{/pgfplots/xmax},|\meta{ymax}|)|.
...
\begin{axis}[domain=-3:12,samples=25,smooth,width=12cm,height=7cm]

% Draw curves
\addplot[name path=g3,thin] {dnorm(3,1.5)};
\addplot[name path=g6,thin] {dnorm(6,1.5)};

% Draw vertical bar:
\draw [name path=red,red,thick]
	({rel axis cs:0,0}-|{axis cs:6,0}) -- ({rel axis cs:0,1}-|{axis cs:6,0});
...
\pgfextra
	\tikzpathintersectionsegments[of=g3 and g6,
		sequence=B0 -- A1,
	]%
\endpgfextra;
...
\tikzfillbetween[
	of=axis and lower,
	soft clip={domain=-6:6}]
	{pattern=north west lines, pattern color=orange}
