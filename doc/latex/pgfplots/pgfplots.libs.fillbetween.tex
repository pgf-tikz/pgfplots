...
\begin{tikzkeylist}{%
	fill between/soft clip=\meta{path name},
	fill between/soft clip first=\meta{path name},
	fill between/soft clip second=\meta{path name}%
}

	The |soft clip| keys accepts either an argument of the form \meta{point}| rectangle |\meta{point} or, alternatively, the name of some \tikzname\ path.

	Here, \meta{path name} refers to a path which has been labelled by means of |name path|.

	The path cannot be arbitrary, though: it should be \emph{reasonable simple}. In particular, it should be convex (vague form of a rectangle or circle or ``cloud''). 
