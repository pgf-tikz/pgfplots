...
\begin{tikzpicture}[shorten >=4pt,shorten <=4pt]
  \begin{groupplot}[group style={group size=2 by 2},
    height=3.5cm,width=3.5cm,font=\footnotesize]
    \nextgroupplot%1
...
\begin{pgfplotskey}{group style=\marg{group/options}}
...
\begin{pgfplotskeylist}{group/group size=\meta{columns} by \meta{rows} (initially 1 by 1),
      group/columns=\marg{columns} (initially 1),
      group/rows=\marg{rows} (initially 1)}
...
\begin{pgfplotskey}{group/group name=\meta{name} (initially group)}
...
\begin{tikzpicture}
  ...
  \draw (my plots c2r1.north) circle (3pt) node {north};
  \draw (my plots c1r2.center) circle (3pt) node {center};
  \draw (my plots c2r2.north west) circle (3pt) node {North west};
\end{tikzpicture}
\end{codeexample}
\end{pgfplotskey}

\begin{pgfplotskey}{group/empty plot=\marg{style} (initially /pgfplots/hide axis)}
  ...
  name. Consider the same example as before.
\begin{codeexample}[]
\begin{tikzpicture}
  \begin{groupplot}[group style={
        group name=my plots,group size=2 by 2},
