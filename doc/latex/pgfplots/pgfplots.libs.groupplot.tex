...
\begin{pgfplotskeylist}{%
  ...
  These are simply a short hand for both using both |xticklabels at| and |xlabels at| simultaneously, i.e.

\begin{codeexample}[code only]
\pgfplotsset{group/x descriptions at=edge bottom,group/y descriptions at=edge right}
\end{codeexample}
yields that $x$ descriptions (|xlabel| and |xticklabel|) are only used for the lowest row whereas $y$ descriptions are only used for the rightmost column.
