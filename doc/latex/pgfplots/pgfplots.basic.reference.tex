...
\begin{commandlist}{\pgfplotspointgetcoordinates,\pgfplotspointgetcoordinates\marg{point}}
	A command which returns high-level coordinate values. After invoking the macro, the following keys will be set:
	...
	\begin{enumerate}
		\item Without arguments. In this case, it returns values of the point which is about to be processed by the current plot handler.
		\item With an argument in curly braces. In this case, it expects a coordinate and assigns the keys accordingly. Note that this command merely supports two-dimensional axes and assigns only |/data point/x| and |/data point/y|.
    ...
    |nodes near coords|). This allows to access \emph{all} coordinate values at once:
    %
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}
...
\end{codeexample}
    %
	The example works because |\pgfplotspointgetcoordinates| is part of the standard implementation of |nodes near coords|; the resulting values are directly available.
...
\end{tikzpicture}
\end{codeexample}
	The example computes and names intersections of |A| and |HelperLine|. Afterwards, it places |node|s and |pin|s at the intersections and shows the logical coordinates values by means of |\pgfplotspointgetcoordinates|\marg{point}. Since the macro merely computes values without actually displaying anything, we also need to read the results from |/data point/x| and |/data point/y| and format them using |\pgfmathprintnumber|.

	In its second mode, |\pgfplotspointgetcoordinates|\marg{point} expects one of two things as \meta{point}:
	\begin{enumerate}
		\item a basic-level PGF point like |\pgfpointanchor{A}{center}| or
		\item a TikZ point like |(A.center)| or |(3,5)|.
	\end{enumerate}
\end{commandlist}
