...
\begin{command}{\pgfplotspointgetcoordinates}
	Makes high-level values of the ``current'' point accessable. The ``current'' point is the one which has is about to be processed by the current plot handler. The macro is supposed to be called during the visualization phase.
