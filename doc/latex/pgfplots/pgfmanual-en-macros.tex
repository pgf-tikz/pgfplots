...
%\showzeug#1\endshowzeug
        \pgfmanualprettyprintcode{#1}%
	}
    \end{minipage}}%
  \code@end%
  \par%
...
% \showzeug argument pattern\endshowzeug
\def\showzeug#1{%
	\def\temp{#1}%
	\def\tempb{\endshowzeug}%
	\ifx\temp\tempb
		\let\showzeug@next=\relax
	\else
		#1\_{\the\catcode`#1 }%
		\let\showzeug@next=\showzeug
	\fi
	\showzeug@next
}%
...
\long\def\pgfmanualprettyprintpgfkeys#1{%
	\begingroup
	\pgfmanualprettyprintstyles
	\pgfmanual@pretty@pgfkeys@loop#1,\pgfmanual@EOI% the extra comma is handled in \pgfmanual@pretty@pgfkeys@foundone!
	\endgroup
}
		...
		\else
			...
			\else
				\ifnum10=\catcode`#1
					...
					#1%
				\else
					% found the beginning of a key! Handle it:
					% FIXME what if we found an opening brace!?
					\def\pgfmanual@pretty@pgfkeys@loop@NEXT{\pgfmanual@pretty@pgfkeys@foundone#1}%
				\fi
			\fi
		\fi
	\fi
...
\def\pgfmanual@pretty@pgfkeys@checkforEOI{%
...
% #1 is a complete key.
\def\pgfmanual@pretty@pgfkeys@foundone#1,{%
	\pgfmanual@pretty@pgfkeys@unpack#1=\pgfkeysnovalue=\pgfkeys@stop
	\pgfutil@ifnextchar\pgfmanual@EOI{%
		% drop the comma!
		\pgfmanual@pretty@pgfkeys@loop
	}{%
...
\def\pgfmanual@pretty@pgfkeys@unpack#1=#2=#3\pgfkeys@stop{%
	\begingroup
	...
	\ifx\pgfmanual@glob@TMPb\pgfkeysnovalue
		% ... and we are ready; there is no value.
	\else
		% ... and the key's value!
		=%
