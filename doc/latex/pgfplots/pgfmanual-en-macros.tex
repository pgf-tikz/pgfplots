...
\long\def\pgfmanual@prettyprint@code#1{#1}%
...
\pgfkeys{%
	...
	/codeexample/prettyprint/key value/.code={\textcolor{yellow}{#1}},
}%

\long\def\pgfmanual@prettyprint@code#1{%
\tracingmacros=2 \tracingcommands=2
	\pgfmanual@pretty@loop#1[\pgfmanual@EOI
}%
% DISABLE:
\long\def\pgfmanual@prettyprint@code#1{#1}%
...
\long\def\pgfmanual@pretty@loop#1[#2{%
	#1%
	\def\pgfmanual@loc@TMPa{#2}%
	\ifx\pgfmanual@loc@TMPa\pgfmanual@EOI
	\else
		\expandafter\pgfmanual@pretty@loop@collectoarg\expandafter#2%
	\fi
}%
\long\def\pgfmanual@pretty@loop@collectoarg#1]{%
	[\pgfmanual@pretty@syntax@keys{#1}]%
	\pgfmanual@pretty@loop
}%
\long\def\pgfmanual@pretty@syntax@keys#1{%
	\pgfmanual@pretty@syntax@keys@loop#1,\pgfmanual@EOI% the extra comma is handled in \pgfmanual@pretty@syntax@akey!
}%
\long\def\pgfmanual@pretty@syntax@keys@loop#1{%
	\def\pgfmanual@loc@TMPa{#1}%
	\ifx\pgfmanual@loc@TMPa\pgfmanual@EOI
		\let\pgfmanual@pretty@syntax@keys@loop@next=\relax
	\else
		\let\pgfmanual@pretty@syntax@keys@loop@next=\pgfmanual@pretty@syntax@keys@loop
		\ifnum13=\catcode`#1
			#1%
		\else
			\def\pgfmanual@pretty@syntax@keys@loop@next{\pgfmanual@pretty@syntax@akey#1}%
		\fi
	\fi
	\pgfmanual@pretty@syntax@keys@loop@next
}%
\def\pgfmanual@pretty@syntax@akey#1,{%
	\pgfmanual@pretty@syntax@akey@unpack#1=\pgfkeysnovalue=\pgfkeys@stop
	\pgfutil@ifnextchar\pgfmanual@EOI{%
		% drop the comma!
		\pgfmanual@pretty@syntax@keys@loop
	}{%
		,%
		\pgfmanual@pretty@syntax@keys@loop
	}%
}%
{\catcode`\^^M=13 \gdef\pgfmanual@pretty@restorespaces{\def^^M{\space}}}

\def\pgfmanual@pretty@syntax@akey@unpack#1=#2=#3\pgfkeys@stop{%
	\begingroup
		\pgfmanual@pretty@restorespaces
		\xdef\pgfmanual@glob@TMPa{#1}%
		\xdef\pgfmanual@glob@TMPb{#2}%
	\endgroup
	\def\pgfmanual@loc@TMPa{\pgfkeysvalueof{/codeexample/prettyprint/key name/.@cmd}}%
	\expandafter\pgfmanual@loc@TMPa\pgfmanual@glob@TMPa\pgfeov
	\ifx\pgfmanual@glob@TMPb\pgfkeysnovalue
	\else
		=%
		\def\pgfmanual@loc@TMPa{\pgfkeysvalueof{/codeexample/prettyprint/key value/.@cmd}}%
		\expandafter\pgfmanual@loc@TMPa\pgfmanual@glob@TMPb\pgfeov
	\fi
}
