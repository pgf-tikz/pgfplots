...
\begin{bug}[-]
	...
	\begin{itemize}
		\item compute precise BB (using necessary condition on derivative)
		\item or keep it as-is, but restrict BB of shading if \verb|clip=true|. 
		\item or keep it as-is, but introduce \verb|overlay| to the \verb|\addplot| command(s) if \verb|clip=true| is in effect. Might be useful anyway.

		ATTENTION: \verb|clip has bounding box| is inactive for more recent versions. Disabling the bounding box might be bad idea.

		\item IDEA: compute the pdf Xform's bounding box using the method here (i.e. the bounding box derived from control points). BUT: update the picture's BB using the interpolation points (not the control points). This is more natural anyway, and it will probably result in a correct BB. If not, the user can easily extend it.

	\end{itemize}
	There is a unit test \verb|unittests/unittest_libs_patchplots_BB_1.tex| which reproduces the issue
\end{bug}
...
\begin{feature}
	...
	\begin{itemize}
		...
		\item shader=flat
		\item patch plots lib and its refinement strategies
		\item improve input syntax
			\begin{itemize}
				\item accept both xcolor and normalized RGB / CMYK expressions
				 \item accept only normalized expressions -- without colorsspace if the color space is fixed in advance
				 \item accept math expressions for individual components which map to 0,1
\begin{verbatim}
possibilities for explicit cdata input formats:
...
color=blue
rgb=1,0,0
1,0,0 % uses hard-coded colorspace
\end{verbatim}
			\end{itemize}
		\item communicate auto-detected input colorspace to the shader. Hm; ok, it could also lazily use the first encountered one and assume that all have the same. might work.
