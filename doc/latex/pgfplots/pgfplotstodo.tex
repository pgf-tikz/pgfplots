...
\begin{feature}
 ...
 \url{http://www.mathworks.com/access/helpdesk/help/toolbox/rf/f2-999699.html}
	...
	\begin{itemize}
		\item write support for ticklabel lines which are not axis lines (or for centered axis lines)
		
		$\rightarrow$ place ticklabels on the $y\equiv0$ instead of $y_{\hbox{min}}$!

		\item provide support for partial grid lines
			\begin{itemize}
				\item \ok for the arcs together with an \texttt{arc limits}
				\item perhaps also interesting: min $y$ value which can't be penetrated?
				\item needs to be done for $x$ grid as well (for the circles)
				\item idea: provide the end value EXPLICITLY for INDIVIDUAL grid lines syntax

				\verb|value => limit|

				idea: lookup with fixed format and round/truncate to 2 digits after comma

				this should allow the desired output, even if it is complicated

			\end{itemize}
		\item default tick positions should have nonlinear distance (?)
			\begin{itemize}
				\item idea: allow non-uniform minor ticks (depending on the magnitude)
			\end{itemize}
		\item fine tuning
		\item check anchors
	\end{itemize}
