...
\begin{feature}[-,prio=9]
  ...
  Remaining sub-items:
  \begin{itemize}
  	\item modifiable start/ends of the filled region. something like "trim start" and "trim end"
	\item somehow, the space after \verb|\addplot fill between[|  is not being processed properly
	\item extract tikz module?
	\item legend
	\item coordinate-based styles of sorts \verb|every positive segment, every negative segment|
	\item better handling of closed paths: we do not need two closed path segments (think about contour plots!)
