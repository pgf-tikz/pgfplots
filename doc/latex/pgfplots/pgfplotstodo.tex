...
\begin{doctodo}
	layers: 
	\begin{enumerate}
		\item motivation and use-cases
		\item simple example
		\item multi-axis discussion
		\item tikz integration
		\item explain how to merge custom layers and pgfplots layers (and say that pgfplots overwrites layers of tikz)
		\item specialties: defining own layer sets
		\item limitations: show list of supported anchors and explain implications of cell picture
	\end{enumerate}
	...
	TODO:
	\begin{itemize}
		\item optimize background paths;
	 	\item communicate pgf@layerlist to tikz if and only if layered graphics is active and cell picture=false
		\item implement 'clip mode=individually' for axis paths
		\item document 'mark layer'
	\end{itemize}
\end{doctodo}
...
\begin{bugtracker}
\begin{bug}
	\verb|\pgfplotsforeachungrouped| cannot be combined with three or more arguments like |\foreach|
\end{bug}
