...
regressiontests		&	for 1.9	&	for 1.9		& for 1.8 (7\% fail)		&2009-12-30					\\
unittests			&	for 1.9	&   for 1.9     &               &     \\
manual				&	for 1.9	&	for 1.9		& for 1.5		&	\\
pgfplotstable.pdf	&	for 1.9	&	for 1.9		& for 1.5\\
example	latex		&	for 1.9	&	for 1.9     & 2009-12-30					\\
example	context		&	for 1.9	&	for 1.9     & 2009-12-30				\\
example	plain tex	&	for 1.9	&	for 1.9     & 2009-12-30				\\
tests context		&	for 1.9	&   for 1.9\\
\end{tabular}
...
\begin{feature}
	add option to select whether trigonometric functions should be radians or degrees.
\end{feature}
...
\begin{feature}
	allow to rotate bar plots and fix rotation of tick labels.
...
\begin{feature}[epic=Bar plots,prio=10]
...
\begin{feature}[-,prio=9]
  ...
  \begin{itemize}
	...
	\item extract tikz module?
	\item provide tikz-level command to generate a fill-between
	\item legend
	...
	\item make layers configurable

	layering: this might be a mess as soon as someone tries to add scopes etc...
	\item bad test coverage and known issues around non-standard combinations of different coordinate sequences
	\item NO. perhaps -- different path joins: line-to, |-, -|, rounded corner ?
	\item automatically label the coordinates as in \verb|name intersections|

	access the high-level coordinates of some intersection point
\begin{verbatim}
%        % nicht unbedingt eine 'fillbetween' Frage, aber:
%        % Gibt es eine Möglichkeit die Koordinaten des Schnittpunktes abzufragen?
%        % (Der folgende Code führt zu einer Fehlermeldung)
%        \node [label=left:{$y=\pgfmathprintnumber{\pgfkeysvalueof{/data point/y}}$}]
%            at (intersection-1) {};
\end{verbatim}
	...
	\item add compatibility layer for bug fix for accuracy improvements of smooth segments

	\item replace ``inner'' moveto segments by linetos, i.e. close the path in a ``different'' way. At least as option.

	Exceptions might be useful in the presence of closepaths

	\item answer related questions: google returns lots of hits for ``pgfplots fill between''
\begin{minimal}

% -*- TeX:NoWrap:UTF-8 -*-

% ----- minimal example -----
\begin{filecontents}{test_data.txt}
x y z
0 2 3
0.5 -1 0
1 3 4
...
\begin{tikzpicture}
    \begin{axis}[
        ...
        \addplot [name path=A,domain=-2:2] {x^2};
        \addplot [name path=B,domain=-\X:\X,samples=2] {x-1};

		% FAIL: reason: smooth has end-point on -1.5
        \addplot [fill=black!10] fill between [of=A and B,soft clip first={
			(axis cs:-\X,\pgfkeysvalueof{/pgfplots/ymin}) rectangle
...
\end{minimal}
	...
	\item Adding pins after the main path affects the name path:
\begin{minimal}
\documentclass{standalone}
    \usepackage{tikz}
    \usepackage{pgfplots}
...
\end{document}
\end{minimal}
	\item allow to replace move-to instructions in the middle of a path automatically by lineto.
