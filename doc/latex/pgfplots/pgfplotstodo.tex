...
\begin{feature}[-,prio=9]
  ...
  \begin{itemize}
	...
	\item layering: this might be a mess as soon as someone tries to add scopes etc...
	...
	\item access the high-level coordinates of some intersection point
\begin{verbatim}
...
%            at (intersection-1) {};
\end{verbatim}
	\item allow high-level access to soft-clip-feature such that one can draw the soft-clipped path separately.

		Idea: install it as a decoration.

	\item ensure that \verb|\usetikzlibrary{pgfplots.fillbetween}| cannot be loaded before pgfplots
	\item bug in soft clip + smooth
\begin{minimal}
