...
\begin{bug}
Bei folgendem Balkendiagram sind die Werte ueber den versetzten Balken
zu weit versetzt. Jeweils nach recht oder links je nachdem ob der
Balken sich rechts oder links von der Markierung befindet. Ich habe
das Gefuehl dass die Shift-werte nicht korrekt sind.
\begin{verbatim}
\begin{tikzpicture}
  \begin{axis}[
    ybar,
    ...
    ylabel={\#participants},
    symbolic x coords={trash,save,close},
    xtick=data,
    nodes near coords,
    nodes near coords align={vertical},
    ]
    \addplot+[ybar] plot coordinates {(trash,2) (save,8) (close,4)};
    \addplot+[ybar] plot coordinates {(trash,8) (save,5) (close,8)};
    \addplot+[ybar] plot coordinates {(trash,1) (save,1) (close,1)};
    \legend{used,understood,not understood}
  \end{axis}
\end{tikzpicture}
\end{verbatim}
Wenn ich die "\addplot+[ybar] plot coordinates {(trash,2) (save,8)
(close,4)};" mit  "\addplot coordinates {(trash,2) (save,8)
(close,4)};" ersetze ist das Problem verschwunden. Was ist denn hier
genau der Unterschied?

VERMUTUNG: das liegt an \verb|/pgfplots/ybar| vs \verb|/tikz/ybar| innerhalb von \verb|\addplot|! Deshalb geht auch \verb|xbar| nicht!

VERMUTUNG: der fuehrt fuer die scatter sachen \verb|/pgfplots/ybar| aus und verdoppelt so den shift!
\end{bug}

\begin{bug}
	The xbar plot handler doesn't work correctly together with \verb|nodes near coords|! 

	Possible solution: use \verb|0.5*bar shift|. But why!?
\begin{minimal}
\documentclass{article}
\usepackage{pgfplots}
\begin{document}
...
\begin{tikzpicture}
  \begin{axis}[
    xbar,
	%/pgf/bar shift=0pt, % this has no effect on nodes near coords!? Why?
	xmin=0,
    ...
    \addplot+[xbar] plot coordinates {(7,0) (2,1)};
    \addplot+[xbar] plot coordinates { (5,0) (2.5,1)};
    \addplot+[xbar] plot coordinates { (0.4,0) (3,1)};
  \end{axis}
\end{tikzpicture}
\end{document}
\end{minimal}
...
\end{tikzpicture}
\end{verbatim}
>>>>>>> stable:doc/latex/pgfplots/pgfplotstodo.tex
\end{bug}
\begin{bug}[-]
