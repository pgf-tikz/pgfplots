...
\section{Unstable version}
list of features which have "pending" state:
\begin{itemize}
\item contours: 97\% ready.
  	\item add "labels=if less than X"
...
\item smith charts: 0\% ready, but implementation should be simple with what I have
  now
\item patch plots: 100\% ready for linear patches
  	90\% for high order ones
\item internal coordmath framework: 100\% ready, but not used everywhere and
  undocumented
...
\begin{feature}[-]
  Konnodalplots fuer Ternary Axes
  	$\leadsto$ siehe notizen von Stefan Pinnow
\end{feature}
