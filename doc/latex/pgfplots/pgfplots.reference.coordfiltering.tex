...
\label{key:restrict:x:to:domain}
	These keys append $x$ (or $y$ or $z$) coordinate filters which to restrict the respective coordinate to a domain. The versions without star, |restrict x to domain|, will assign the value |-inf| if $x$ is below \meta{min} and |+inf| if $x$ is above \meta{max}. The starred versions, |restrict x to domain*|, will truncate coordinates to $[\hbox{\meta{min}}, \hbox{\meta{max}}]$, i.e.\ they assign the value \meta{min} if the coordinate falls outside of the lower limit and \meta{max} if the value falls outside of the upper limit.
