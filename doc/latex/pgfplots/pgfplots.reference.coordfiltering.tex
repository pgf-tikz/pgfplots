...
\begin{pgfplotsxycodekeylist}{\x\ filter,filter point}
    The code keys |x filter| and |y filter| allow coordinate filtering which
    are based on a \emph{single} coordinate. A coordinate filter gets an input
    coordinate as |#1| (on input, the same value is stored in
    |\pgfmathresult|), applies some operation and writes the result into the
...
\end{codeexample}
    ...
    engine of \PGF{}. Please keep in mind that the math engine works with
    limited \TeX{} precision.
...
\end{codeexample}
    ...
    If key filters are invoked for |plot table|, access to the current row's
    ...
    |/data point y| and |/data point z|. \todosp{shouldn't this be `/data point/x' etc.?} It may change its coordinates using
    |\pgfkeyssetvalue{/data point x}|\marg{new value}; access to variables can
    be accessed with |\pgfkeysvalueof{/data point/x}| or, if the argument shall
