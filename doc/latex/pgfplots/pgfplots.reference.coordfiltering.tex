...
\begin{pgfplotskey}{@restrict to domain=\marg{filter name}\marg{expression}\marg{\meta{min}:\meta{max}}\mchoice{0,1}}
	...
	For example,
	|@restrict to domain={hist/data}{}{0:1}{0}| does the same job as |restrict x to domain=0:1|, but it is applied to |hist/data|. Similarly, 
	|@restrict to domain={hist/data}{}{0:1}{1}| does the same job as |restrict x to domain*=0:1|.
