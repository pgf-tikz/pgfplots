...
\begin{plottype}[/pgfplots]{hist=\normalfont\marg{options}}
...
\begin{axis}[
	...
	xticklabel=
		...
		\pgfmathprintnumber\nexttick]
	\addplot+[hist={data=\thisrow{data},bins=3}]
...
\end{codeexample}
	...
	\begin{pgfplotskey}{hist/data=\marg{expression} (initially y)}
		...
		It is also possible to avoid invocations of the math parser. Use |hist/data value| instead to do so.
	\end{pgfplotskey}
...
\begin{tikzpicture}
	\begin{axis}[
		title={$x \exp(-x^2-y^2)$ and its gradient field},
		domain=-2:2,
		xmin=-2,xmax=2,
		ymin=-2,ymax=2,
		view={0}{90},
...
\end{codeexample}
	...
	\begin{pgfplotskeylist}{%
		quiver/u=\meta{expression},
		quiver/v=\meta{expression},
		quiver/w=\meta{expression}}
		These keys define how the vector directions $(u,v)$ (or, for three dimensional plots, $(u,v,w)$) shall be set.

		The \meta{expression} can be a constant expression like |quiver/u=1| or |quiver/u=42*5|. It may depend on the final base point values using the values |x|, |y| or |z| as in the example above. In this context, |x| yields the $x$ coordinate of the point where the vector starts, |y| the $y$ coordinate and so on. 
		
		Another important application is to use \emph{table column references} like |quiver/u=\thisrow{column_for_u}| in conjunction with |\addplot table|. 
	\end{pgfplotskeylist}
	\begin{pgfplotskeylist}{%
		quiver/u value=\meta{value},
		quiver/v value=\meta{value},
		quiver/w value=\meta{value}}
	...
	\begin{stylekey}{/pgfplots/quiver/every arrow (initially empty)}
		...
		The style can contain any \Tikz\ drawing option. It might depend upon |\pgfplotspointmetatransformed| or the |mapped color| (provided |point meta| has been set).
	...
	\begin{pgfplotsxycodekeylist}{%
		quiver/before arrow,%
		quiver/after arrow}%
		Advanced keys for mor fine tuning of the display. They allow to install some \TeX\ code manually before or after the drawing operations for single arrows. Both are empty initially.
