...
\begin{plottype}[/pgfplots]{patch}
	...
	However, |patch| plots are defined by \emph{explicitly} providing the triangulation: they expect a sequence of triangles which make up the mesh.

	There are two dimensional and three dimensional patch plots, see Section~\ref{sec:pgfplots:3d:patch} for the three dimensional ones. Both, however, have the same interface.

	The standard input format is to provide a sequence of two--dimensional coordinates as usual. Each consecutive set of three points makes up a triangle:
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
	\addplot[patch]
...
\end{tikzpicture}
\end{codeexample}
	Patch plots make use the |mesh| configuration, including the |shader|. Thus, the example above uses the initial |shader=flat| (which uses the \emph{mean} color data to determine a triangle's color). The |shader=interp| yields the following result\footnote{At the time of this writing, many free readers, especially those for Linux, have difficulties displaying the correct color maps and take a long time to render. Acrobat reader displays the shading according to the pdf standard.}:
\begin{codeexample}[]
\begin{tikzpicture}
	\begin{axis}
...
\end{codeexample}
	...
	Two dimensional |patch| plots simply draw triangles in their order of appearance. Consider using a three dimensional |patch| plot in order to apply |z buffer| (i.e.\ to modify the sorting sequence).
...
\end{tikzpicture}
\end{codeexample}
	\noindent The example consists of \emph{two separate} tables. The |patch table| argument is a table, provided inline where rows are separated by |\\| (which is the purpose of the |row sep=\\| key as you guessed). The |patch table| here declares thee triangles: the triangle made up by vertex $\#0$, $\#1$ and $\#2$, the triangle made up by $\#1$, $\#2$ and $\#3$ and finally the one using the vertices $\#4$, $\#3$ and $\#5$. The vertices as such are provided using the standard input methods of \PGFPlots; in our case using a table as well. The standard input simply provides coordinates (and |point meta|) which are stored in the vertex array.
\end{pgfplotskeylist}

	Please refer to the documentation of three dimensional patch plots in Section~\ref{sec:pgfplots:3d:patch} for details about |shader| and its parameters and |z buffer| techniques.
\end{plottype}
}
