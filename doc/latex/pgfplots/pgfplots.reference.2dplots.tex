...
Here |ybar| yields |/pgfplots/ybar| because it is an argument to the axis, not to a single plot.
	...
	Sometimes it is useful to write the $y$ directly near the bars. This can be realized using the |nodes near coords| method:
\begin{codeexample}[]
\begin{tikzpicture}
\begin{axis}[
	x tick label style={
	...
	bar width=9pt,
	nodes near coords,
	point meta=y *10^-7 % this is displayed number.
]
\addplot 
	coordinates {(1930,50e6) (1940,33e6)
		 (1950,40e6) (1960,50e6) (1970,70e6)};
...
\label{pgfplots:scatter:src}
	This key is necessary for any scatter plot. It needs to be provided as \marg{option} for |\addplot| to configure the value used to determine marker appearances. Actually, |scatter src| is nothing but an alias for |point meta|, so the main documentation for this key is on page~\pageref{pgfplots:pointmeta}. However, we summarize the choices here together with scatter plot examples.
	...
	A mathematical expression for |scatter src| can also depend on an explicitly provided numerical value, that means it is possible to combine the choice \declaretext{explicit} and \meta{expression}. To do so, provide |scatter src| twice: 
