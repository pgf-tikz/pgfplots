...
\newif\ifpgfplotsmanualautolinks
\pgfplotsmanualautolinkstrue


\ifpgfplotsmanualautolinks
	...
	\pgfkeys{%
		...
		/codeexample/prettyprint/cs/pgfplotsset/.code 2 args={#1\{\pgfmanualprettyprintpgfkeys{#2}\}},
		/codeexample/prettyprint/cs arguments/pgfplotstableset/.initial=1,
		/codeexample/prettyprint/cs/pgfplotstableset/.code 2 args={#1\{\pgfmanualprettyprintpgfkeys{#2}\}},
		%
		/codeexample/prettyprint/autolinks/.style={%
			/codeexample/prettyprint/key name/.code={\pgfplotshyperref{##1}{##1}},
			/codeexample/prettyprint/key value display only/.code={##1},
			/codeexample/prettyprint/cs/.code={\pgfplotshyperref{##1}{##1}},
			/codeexample/prettyprint/cs with args/.code 2 args={\pgfplotshyperref{##1}{##1}\{##2\}},
			/codeexample/prettyprint/cs arguments/pgfkeys/.initial=1,
			/codeexample/prettyprint/cs/pgfkeys/.code 2 args={\pgfplotshyperref{##1}{##1}\{\pgfmanualprettyprintpgfkeys{##2}\}},
			/codeexample/prettyprint/cs arguments/begin/.initial=1,
			/codeexample/prettyprint/cs/begin/.code 2 args={##1\{\pgfplotshyperref{##2}{##2}\}},
			/codeexample/prettyprint/cs arguments/end/.initial=1,
			/codeexample/prettyprint/cs/end/.code 2 args={##1\{\pgfplotshyperref{##2}{##2}\}},
			/codeexample/prettyprint/cs arguments/usepgfplotslibrary/.initial=1,
			/codeexample/prettyprint/cs/usepgfplotslibrary/.code 2 args={##1\{\pgfplotshyperref{##2}{##2}\}},
			/codeexample/prettyprint/cs/usetikzlibrary/.code 2 args={##1\{\pgfplotshyperref{##2}{##2}\}},
			/codeexample/prettyprint/cs/usepgflibrary/.code 2 args={##1\{\pgfplotshyperref{##2}{##2}\}},
		},%
...
%\def\declaretext#1{\pgfplotshyperref{#1}{\texttt{\declare{#1}}}}
\def\v|#1|{\pgfplotshyperref{#1}{\texttt{#1}}}%
...
\def\pgfplotsmanualexternalexpensivetrue{%
	\pgfplotsiffileexists{tikzlibraryexternal.code.tex}{%
		...
		\pgfplots@warning{Sorry, the manual compilation CAN'T BE OPTIMIZED because \string\usetikzlibrary{external} is not available.}%
	}%
}%
...
\newif\ifpgfplotshyper@hasxyzoption

\def\pgfplotshyper@installreplacements{%
	\def\marg##1{{##1}}%
	\def\oarg##1{[##1]}%
	\def\meta##1{<##1>}%
	\def\x{x}%
...
% Defines a new pdf cross ref label for use with \pgfplotshyperref.
%
% #1: the label.
% The text #2 will be shown in the resulting pdf.
\def\pgfplotshyperlabel#1#2{%
	\begingroup
	\pgfplotshyper@installreplacements%
	%
	\pgfutil@in@\x{#1}%
	\ifpgfutil@in@
		\pgfplotshyper@hasxyzoptiontrue
	\fi
	\edef\pgfplotshyperlabel@@{#1}%
	\pgfutil@ifundefined{pgfplotsmanual@wrotehyperlabel@\pgfplotshyperlabel@@}{%
		\pgfplotshyperlabel@generate{#1}{#2}%
		\ifpgfplotshyper@hasxyzoption
			\def\x{y}%
			\pgfplotshyperlabel@generate{#1}{#2}%
			\def\x{z}%
			\pgfplotshyperlabel@generate{#1}{#2}%
...
\def\pgfplotshyperlabel@generate#1#2{%
	\immediate\write\@auxout{%
		\noexpand\expandafter\noexpand\gdef
		\noexpand\csname pgfplotsmanual@hashyperlabel@\pgfplotshyperlabel@@\noexpand\endcsname{1}%
	}%
	\expandafter\gdef\csname pgfplotsmanual@hashyperlabel@\pgfplotshyperlabel@@\endcsname{1}%
	\expandafter\gdef\csname pgfplotsmanual@wrotehyperlabel@\pgfplotshyperlabel@@\endcsname{1}%
	\edef\pgfplotshyperlabel@@{pgfplots.\pgfplotshyperlabel@@}%
	\expandafter\hypertarget\expandafter{\pgfplotshyperlabel@@}{#2}%
}%

%\def\declarewithhyperref#1{%
%	\pgfplotshyperlabel{#1}{{\color{red!75!black}#1}}%
%}%

\def\pgfplotshyperref#1#2{%
	\begingroup
	\pgfplotshyper@installreplacements
	%
	%
	...
	% inside of codeexamples where \pgfplotshyperref will be invoked - 
	...
	\ifpgfutil@in@
		...
		\def\pgfplotshyperref@substitute@backslash##1##2\relax{%
			\edef\pgfplotshyperlabel@@{\textbackslash ##2}%
		}%
		\pgfplotshyperref@substitute@backslash#1\relax
	\else
		\edef\pgfplotshyperlabel@@{#1}%
	\fi
	%
	\pgfutil@ifundefined{pgfplotsmanual@hashyperlabel@\pgfplotshyperlabel@@}{%
		% try the /pgfplots/, /tikz/ prefixes...
		\pgfutil@ifundefined{pgfplotsmanual@hashyperlabel@/pgfplots/\pgfplotshyperlabel@@}{%
			\pgfutil@ifundefined{pgfplotsmanual@hashyperlabel@/tikz/\pgfplotshyperlabel@@}{%
				\begingroup
					\toks0={#1}%
					\pgfplots@warning{pgfplotshyperref{\the\toks0 }: target label does not exist.}%
				\endgroup
				#2%
				\let\pgfplotshyperlabel@@=\pgfutil@empty
			}{%
				\edef\pgfplotshyperlabel@@{/tikz/\pgfplotshyperlabel@@}%
			}%
		}{%
			\edef\pgfplotshyperlabel@@{/pgfplots/\pgfplotshyperlabel@@}%
		}%
	...
	\ifx\pgfplotshyperlabel@@\pgfutil@empty
	\else
		\expandafter\pgfplotshyperref@\expandafter{\pgfplotshyperlabel@@}{#2}%
	\fi
	\endgroup
}%
\def\pgfplotshyperref@#1#2{%
	\hyperlink{pgfplots.#1}{#2}%
}%
...
\newenvironment{addplot+}{
  \begin{pgfmanualentry}
	...
	\pgfplotshyperlabel{\textbackslash addplot+}{}%
	}%
    \pgfmanualbody
}
{
...
\newenvironment{addplot3generic}{
  \begin{pgfmanualentry}
	...
	\pgfplotshyperlabel{\textbackslash addplot3}{}%
	}%
    \pgfmanualbody
}
{
...
\newenvironment{addplot3operation}[3][]{
  \begin{pgfmanualentry}
    ...
    \ifx\pgfmanualtest\@empty%
      \index{#2@\protect\textcolor{gray}{\protect\texttt{plot}}\protect\texttt{ #2}}%
      \index{Plot operations!addplot3 #2@\protect\texttt{plot #2}}%
    \fi%
	}%
    \pgfmanualbody
}
...
\def\extractcommand#1#2\@@{%
  ...
  \pgfplotshyperlabel{\textbackslash\strippedat}{}%
}
