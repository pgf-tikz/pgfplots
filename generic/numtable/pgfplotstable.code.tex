...
\pgfkeys{%
	...
	/pgfplots/table/create col/assign last/.style={
		/pgfplots/table/create col/assign next%
	},
	/pgfplots/table/create col/assign next/.style={
		/pgfplots/table/create col/next content={}%
	},
	/pgfplots/table/create col/next content/.initial={},
	/pgfplots/table/create col/expr/.style={%
		/pgfplots/table/create col/assign next/.code={%
			\pgfmathparse{#1}%
	...
	/pgfplots/table/create col/gradient/.style 2 args={%
		...
		/pgfplots/table/create col/assign next/.code={%
			\pgfmathfloatparsenumber{\thisrow{#1}}%
			\let\pgfmathcur@x=\pgfmathresult
			\pgfmathfloatparsenumber{\thisrow{#2}}%
			\let\pgfmathcur@y=\pgfmathresult
			\pgfmathfloatparsenumber{\nextrow{#1}}%
...
% 'create col/assign next' 
% 'create col/assign last'
%
% The key 'create col/assign next' will be invoked for every row of table #2.
% It is supposed to assign the key 'create col/next content'.
% During evaluation of 'create col/assign next', the macro '\thisrow{<col name>}' 
...
\def\pgfplotstablecreatecol@opt[#1]#2#3{%
	...
	\ifpgfplots@loop@CONTINUE
		...
		\ifpgfplots@loop@CONTINUE
			...
			\else
				\pgfkeysvalueof{/pgfplots/table/create col/assign next/.@cmd}\pgfeov
			\fi
			\pgfkeysgetvalue{/pgfplots/table/create col/next content}{\pgfplotstable@entry}%
			\global\let\pgfplotstable@TMP=\pgfplotstable@entry
			\endgroup
			\let\pgfplotstable@entry=\pgfplotstable@TMP
