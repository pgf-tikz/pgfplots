...
\def\pgfplots@domain@to@foreach#1:#2\relax{%
	...
	\pgfmathparse{\pgfplots@loc@TMPa+(\pgfplots@loc@TMPb-\pgfplots@loc@TMPa)/(\tikz@plot@samples-1)}%
...
% \pgfplotsforeachlogarithmicungrouped \x in {a:b} {#3}
%
% samples \x between a and b using a logarithmic scale.
\def\pgfplotsforeachlogarithmicungrouped#1{\pgfplotsforeachlogarithmicungrouped@{#1}}%
\long\def\pgfplotsforeachlogarithmicungrouped@#1in #2#3{%
	% define
	...
	\let\pgfplots@foreach@loc@TMPa=\pgfmathresult
	\pgfmathfloatparsenumber{\tikz@plot@samples}%
	...
	\long\def\pgfplots@foreach@loc@TMPf{#3}%
	\c@pgf@counta=0
	\pgfutil@loop
	\ifnum\c@pgf@counta<\tikz@plot@samples
		...
		\expandafter\pgfmathfloatexp@\expandafter{\pgfmathresult}%
		\pgfmathfloattosci@{\pgfmathresult}% just to stay compatible ...
		\let#1=\pgfmathresult
		%
		% ok, invoke it!
