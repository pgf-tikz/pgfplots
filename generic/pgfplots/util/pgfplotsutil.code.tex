...
% Usage:
% \pgfplotsutil@search@and@replace{<token>}{<replacement>}{<string>}
%
% -> will assign the modified string into \pgfplotsretval.
%
% #1: the token to search for (only a SINGLE token is allowed here!)
% #2: zero, one or more tokens which will be inserted instead of '#1'.
% #3: the string to search for
\long\def\pgfplotsutil@search@and@replace#1#2#3{%
	\def\pgfplotsretval{}%
