...
%
% Example: \pgfplots@createcolormap{my map}{rgb(0cm)=(1,0,0); rgb(1cm)=(0,1,0); rgb(1cm)=(0,0,1); gray(1cm)=(3);  color(1cm)=(green); }
...
\def\pgfplots@createcolormap#1#2{%
	...
	\expandafter\def\csname\pgfplots@createcolormap@name @max\endcsname{1000}%
	\pgfmathdivide@{\csname\pgfplots@createcolormap@name @max\endcsname}{\pgfplots@createcolormap@MAX}%
		...
		{%
			...
			{%
				...
				{%
					...
					{\pgfplots@error{Illformed shading colormap specification}}%
				}%
			}%
		}%
	}%
...
\def\pgfplots@colormap@showdebuginfofor#1{%
	...
	\message{(Transformed) Range: [0:\csname pgfpl@cm@#1@max\endcsname];  }%
...
\def\pgfplotscolormaptoshadingspec#1#2#3{%
	\begingroup
	\pgfmathparse{#2}%
	\expandafter\pgfmathdivide@\expandafter{\pgfmathresult}{\pgfplotscolormaprange{#1}}%
	\let\pgfplots@loc@TMPb=\pgfmathresult
...
% Expands to the transformed range's right end of a colormap. The left
% end is fixed to '0'.
\def\pgfplotscolormaprange#1{\csname pgfpl@cm@#1@max\endcsname}
...
\def\pgfplotscolormapfind@precomputed[#1:#2][#3]#4#5{%
	...
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
		\pgfmathsubtract@{#2}{#1}%
		\let\pgfplots@loc@TMPb=\pgfmathresult
		\pgfmathdivide@{\pgfplotscolormaprange{#5}}{\pgfplots@loc@TMPb}%
...
%\message{mapping [#1,#2] -> [0,\pgfplotscolormaprange{#5}]  yielded phi(#4) = \pgfplotscolormapfind@transformedx, situated in interval no \pgfplotscolormapfind@intervalno.}%
