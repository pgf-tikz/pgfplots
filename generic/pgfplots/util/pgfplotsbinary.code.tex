%--------------------------------------------
% $Header: /cvsroot/pgfplots/pgfplots/generic/pgfplots/util/pgfplotsbinary.code.tex,v 1.1 2009/02/25 21:48:28 ludewich Exp $
%
% Package pgfplots
%
...
\pgfqkeys{/pgfplots/bin}{%
	bytes/.store in=\pgfplotsbinary@bytes,%
	bytes=4,
...
\newcount\c@pgfplotsbin@tmpa
\newcount\c@pgfplotsbin@tmpb
\newcount\c@pgfplotsbin@tmpc
...
% - \c@pgfplotsbin@tmpc=0
\def\pgfplotsbinarydecimaltoFF@#1{%
	\ifnum#1>0
		\c@pgfplotsbin@tmpa=#1
		\divide\c@pgfplotsbin@tmpa by256
		\c@pgfplotsbin@tmpb=\c@pgfplotsbin@tmpa\relax
		\multiply\c@pgfplotsbin@tmpa by256
		\advance#1 by -\c@pgfplotsbin@tmpa\relax
		\pgfplotsbinary@apphighorderbytes{\csname pgfp@bin@\the#1 \endcsname}%
		#1=\c@pgfplotsbin@tmpb
		\advance\c@pgfplotsbin@tmpc by1
		\def\pgfplotsbinary@loc@TMPa{\pgfplotsbinarydecimaltoFF@{#1}}%
	\else
		% pad with zeros:
		\c@pgfplotsbin@tmpb=\pgfplotsbinary@bytes\relax
		\advance\c@pgfplotsbin@tmpb by-\c@pgfplotsbin@tmpc
		\ifcase\c@pgfplotsbin@tmpb
			% ok.
		\or
			% one byte missing.
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO}%
		\or
			% two bytes missing.
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO}%
		\or
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO}%
		\or
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO}%
		\or
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO}%
		\or
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO}%
		\or
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO}%
		\or
			\pgfplotsbinary@apphighorderbytes{\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO\pgfplotsbinary@ZERO}%
		\fi
		\let\pgfplotsbinary@loc@TMPa=\relax
	\fi
%\message{RESULT SO FAR: \pgfplotsbinaryresult}%
	\pgfplotsbinary@loc@TMPa
}%

% High level routine which accepts an register containing an unsigned
% integer.
\def\pgfplotsbinarydecimaltoFF#1{%
	\begingroup
	\global\let\pgfplotsbinaryresult=\pgfplotsbinaryempty
	\c@pgfplotsbin@tmpc=0
	\pgfplotsbinarydecimaltoFF@{#1}%
	\endgroup
}%

% Defines \pgfplotsbinaryresult to be the binary representation of a
% count register.
%
% The representation will use unsigned dual number representation.
%
% The assignment to \pgfplotsbinaryresult will be globally.
% #1: an integer register containing an UNSIGNED integer.
\let\pgfplotsbinaryunsignedcount=\pgfplotsbinarydecimaltoFF
