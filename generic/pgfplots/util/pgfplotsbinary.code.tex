%--------------------------------------------
% $Header: /cvsroot/pgfplots/pgfplots/generic/pgfplots/util/pgfplotsbinary.code.tex,v 1.8 2009/03/03 12:46:56 ludewich Exp $
%
% Package pgfplots
%
...
% linear maps anyway.
%
% More precislely, we have the following.
% - We assume that signed number ranges are symmetric. A signed
%   integer with N bits has the number range
%   [ - (2^(N-1) -1), + 2^(N-1) -1 ].
%   For 3 Bit, this is 
%    -3 -2 -1 0 1 2 3
%   and the number of different integers is 2^N - 1  (one is lost due
%   to symmetry).
% - The idea is now to map this number range to [0,2^N -1] LINEARLY.
%   For the example with 3 Bit, there are two possibilities to do so:
%
%    -3 -2 -1 0 1 2 3 
% ->  0  1  2 3 4 5 6
% or
% ->  1  2  3 4 5 6 7
%   
%   I've chosen the the second possibility because the addition of
%   2^N-1 for non-negative numbers can be realized easily.
%
% - thus, negative numbers are mapped using
%    [ - 2^(N-1) -1, 0[  -> [1,2^(N-1)-1]
%   and non-negative numbers using
%    [ 0, 2^(N-1)-1 ] -> [ 2^(N-1),  2^N-1 ]
%
% This doesn't need TeX register arithmetics on the whole range.
%
% As a consequence, the binary pattern 00 00 00 00 does never occur.
% The smallest binary pattern will be 00 00 00 01.
%
...
% bytes,
%   [-16383.99998, 16383.99998 ] -> [ - ( 2^{k*8-1} -1 ),  + 2^{k*8-1} -1 ] 
%   							 -> [ 1, 2^{k*8} ]
% where the second mapping is performed using
% \pgfplotsbinaryencodesignedmaplinearly.
%
% The first step has a quantization error unless bytes=4 (which is
% exact). The second step is lossless.
%
% If your numbers have a restricted data range, you should quantize
% them manually as you may get better quality in that case.
%
% REMARKS:
% -  due to the symmetry of TeX, one looses one number. The binary
%    sequence 00 00 00 00 will never be used.
% -  Technical background:
% TeX uses only **31** bit for dimen registers. To be more precise, it
% uses 14 bits before the decimal point and 16 afterwards, which leads
% to the real data range
% [ - (2^30 -1) / 2^16,  (2^30-1) / 2^16 ] = [ -16383.99998, +16383.99998 ]
% thus, there are 2* (2^30 -1) +1 = 2^31 -1 numbers - instead of the
% possible 2^32 numbers. The 2^16 = 65536 is the scaling factor: 1pt = 65536sp.
%
% Therefor, the implementation here converts a dimen to a count
% register and multiplies by 2 before doing anything else.
%
