%--------------------------------------------
% $Header: /cvsroot/pgfplots/pgfplots/generic/pgfplots/util/pgfplotsbinary.code.tex,v 1.7 2009/03/02 22:45:18 ludewich Exp $
%
% Package pgfplots
%
	...
	bytes/.code={%
		...
		\or
			\def\pgfplotsbinary@add@signed@largest@absolute{%
				...
				\advance\c@pgfplotsbin@input by 1 % but need need to add one more... (the sum is in TeX range!)
			}%
...
%
% So, what we do is to setup a linear map
% phi: [-16384.9999, 16384.9999] -> [ - 2^{k*8-1} -1,  + 2^{k*8-1} -1 ]  % +- 1 somewhere
% and another linear map which maps into
% [0,2^{k*8}].
