...
\def\pgfkeysalsofilteredfrom#1{%
	\expandafter\pgfkeyspred@TMP\expandafter{#1}%
}
...
\pgfkeys{%
	...
	/pgf/key filters/active families or no family/.code 2 args={%
		\ifnum\pgfkeyscasenumber=0
%\message{[pgfkeyshasactivefamilyornofamily(\pgfkeyscurrentkey, \pgfkeyscasenumber) invoking unknown handler '#2']}%
			\pgfkeys@eval@key@filter@subroutine{#2}%
		\else
			\ifnum\pgfkeyscasenumber=3
				\pgfkeysgetfamily\pgfkeyscurrentpath\pgfkeyspred@TMP
			\else
...
% \message{[pgfkeyshasactivefamilyornofamily(\pgfkeyscurrentkey, \pgfkeyscasenumber) family is ACTIVE]}%
% \else
% \message{[pgfkeyshasactivefamilyornofamily(\pgfkeyscurrentkey, \pgfkeyscasenumber) family is NOT active.]}%
% \fi
%-------------------------------------------------- 
			\else% Ok, it does not belong to any family.
%\message{[pgfkeyshasactivefamilyornofamily(\pgfkeyscurrentkey, \pgfkeyscasenumber) invoking has-no-family-handler '#1']}%
				\pgfkeys@eval@key@filter@subroutine{#1}%
			\fi
		\fi
	},
	%
...
% \def\pgfkeys@eval@key@filter@subroutine#1{%
...
% \pgfkeys@eval@key@filter@subroutine{/pgf/key filters/equals=/tikz}
%
% \pgfkeys@eval@key@filter@subroutine works only if key filtering is
% active.
%
