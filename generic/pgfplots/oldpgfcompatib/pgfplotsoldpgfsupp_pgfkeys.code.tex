...
\def\pgfkeys@case@three{%
  ...
  {\pgfkeysgetvalue{/handlers/\pgfkeyscurrentname/.@cmd}{\pgfkeys@code}%
  	\pgfkeys@case@three@modifydefaultpath@if@needed
    \expandafter\pgfkeys@code\pgfkeyscurrentvalue\pgfeov}
  {\pgfkeys@unknown}%
}
...
\def\pgfkeys@case@three@handle@restricted{%
  ...
  \pgfkeysifdefined{/handlers/\pgfkeyscurrentname/.@cmd}{%
    \pgfkeys@ifexecutehandler{%
      ...
      % -> it should be one directory higher!
...
% Allows to change the default path for handled keys.
%
% Effect:  if a handled key was not fully qualified, its key path will
% be set to '#1/'.
%
% Example:
%
% \pgfkeyssetdefaultpathforhandled{/another/path}
% \pgfqkeys{/path/to/key}{a style/.append style={}}
%
% will actually change the style
% /another/path/a style.
%
% MOTIVATION: the use of different key paths usually fails for handled
% keys. This method allows to define priorities for handled keys.
%
% COST: overhead if disabled:
% 	eiter \pgfkeysaddeddefaultpathtrue or
% 	\pgfkeysaddeddefaultpathfalse and one \relax.
% The overhead if enabled is a check and one string replacement.
%
% #1 : the replacement key path for handled keys (without trailing
% '/').
% Use '#1={}' to disable this feature.
\def\pgfkeyssetdefaultpathforhandled#1{%
	\def\pgfkeys@temp{#1}%
	\ifx\pgfkeys@temp\pgfkeys@empty
		% disable modification.
		\let\pgfkeys@case@three@modifydefaultpath@if@needed=\relax
	\else
		% install a replacement path!
		\def\pgfkeyssetdefaultpathforhandled@{#1/}%
		\let\pgfkeys@case@three@modifydefaultpath@if@needed=\pgfkeys@replacedefaultpath@ifnecessary
	\fi
}%
\let\pgfkeys@case@three@modifydefaultpath@if@needed=\relax
\def\pgfkeys@replacedefaultpath@ifnecessary{%
	\ifpgfkeysaddeddefaultpath
		\expandafter\def\expandafter\pgfkeys@replacedefaultpath@\pgfkeysdefaultpath##1\relax{%
			\pgfkeys@pathtoks=\expandafter{\pgfkeyssetdefaultpathforhandled@##1}%
		}%
		\expandafter\pgfkeys@replacedefaultpath@\the\pgfkeys@pathtoks\relax
	\fi
}%
...
\def\pgfkeys@ifexecutehandler@handleonlyexisting#1#2{%
  ...
  \else
     ...
     \pgfkeysifdefined{\pgfkeyscurrentpath}{#1}{%
		\pgfkeysifdefined{\pgfkeysifdefined/.@cmd}{#1}{#2}%
     }%
  \fi%
}%
...
%\noexpand%
%\message{Found \noexpand\pgfkeys@searchalso@name\space in #1: \noexpand\ifpgfkeyssuccess true!\noexpand\else false.\noexpand\fi}%
		\noexpand\fi}%
	\expandafter\gdef\expandafter\pgfkeys@global@temp\expandafter##\expandafter1\expandafter\pgfeov\expandafter{\pgfkeys@global@temp}%
  }%
