...
\def\pgfplotstablegetcolumnbyname#1\of#2\to#3{%
	\pgfutil@ifundefined{\string#2@#1}{%
		...
		% features...
		% 1. create on use:
		\pgfplotstable@checkspecialchars@pgfkeys#1\pgfplotstable@EOI
		%
		\ifpgfutil@in@
			\def\pgfplotstable@TMP{/pgfplots/table/create on use/{#1}}%
		\else
			\def\pgfplotstable@TMP{/pgfplots/table/create on use/#1}%
		\fi
		\pgfkeysifdefined{\pgfplotstable@TMP/.@cmd}{%	
			% aah - a 'create on use' style exists. Apply it!
			\expandafter\pgfplotstablecreatecol\expandafter[\pgfplotstable@TMP]{#1}{#2}%
			...
			% existant.
			\ifpgfutil@in@
				\def\pgfplotstable@TMP{/pgfplots/table/alias/{#1}}%
			\else
				\def\pgfplotstable@TMP{/pgfplots/table/alias/#1}%
			\fi
			\pgfkeysifdefined{\pgfplotstable@TMP}{%
				\pgfkeysgetvalue{\pgfplotstable@TMP}{\pgfplotstable@TMPB}%
				\let\pgfplotstable@TMP=\pgfplotstable@TMPB
				\pgfutil@ifundefined{\string#2@\pgfplotstable@TMP}{%
					\pgfplots@error{Sorry, could not retrieve aliased column '\pgfplotstable@TMP' from table '\pgfplotstablenameof{#2}'. The original request was for '#1', which does not exist either.}%
					\pgfplotslistnewempty#3
				}{%
					\expandafter\let\expandafter#3\csname\string#2@\pgfplotstable@TMP\endcsname
				}%
			}{%
...
\def\pgfplotstablecopy#1\to#2{%
	\let#2=#1%
	\pgfplotstablegetname#2\pgfplotstable@TMP
	\expandafter\let\csname\string#1@@table@name\endcsname=\pgfplotstable@TMP
...
%\pgfplots@message{ATTEMPTING TO READ #2}%
	...
	%\pgfplots@message{ASSIGNING COLUMN NO \the\pgfplotstableread@curcol / \the\pgfplotstableread@numcols}%
			% pgfp@numtable@TMP := column list content
			\expandafter\global\expandafter\let\expandafter\pgfplotstable@TMP\csname pgfp@numtable@col@\the\pgfplotstableread@curcol\endcsname
			% global := pgfp@numtable@TMP
			\expandafter\global\expandafter\let\csname pgfp@numtable@col@\the\pgfplotstableread@curcol\endcsname=\pgfplotstable@TMP
	...
	\pgfplotslistforeachungrouped\pgfplotstable@colnames\as\pgfplotstable@TMP{%
		...
		\expandafter\pgfplotstable@TMPB\csname pgfp@numtable@col@\the\c@pgfplotstable@counta\endcsname
%\message{Column '\pgfplotstable@TMP' has entries: \expandafter\meaning\csname pgfp@numtable@col@\the\c@pgfplotstable@counta\endcsname}%
		\expandafter\global\expandafter\let\csname pgfp@numtable@col@\the\c@pgfplotstable@counta\endcsname=\pgfutil@empty
...
\def\pgfplotstableread@create@column@lists{%
	\pgfutil@loop
	\ifnum\pgfplotstableread@curcol<\pgfplotstableread@numcols
		\expandafter\pgfplotslistnewempty\csname pgfp@numtable@col@\the\pgfplotstableread@curcol\endcsname
		\advance\pgfplotstableread@curcol by1\relax
	\pgfutil@repeat
}
...
%\pgfplots@message{Inserting '#1' at (\the\pgfplotstableread@lineno, \the\pgfplotstableread@curcol).}%
	...
	\expandafter\pgfplotstableread@TMP\csname pgfp@numtable@col@\the\pgfplotstableread@curcol\endcsname
	\advance\pgfplotstableread@curcol by1\relax
}
...
\def\pgfplotstabletypeset@postproc@separately#1&#2\pgfplotstable@EOI{%
	...
	\expandafter\pgfplotstable@postproccellcontent\pgfplotstabletypeset@rawinput\pgfeov
	\pgfkeysgetvalue{/pgfplots/table/@cell content}\pgfmathresult
	\pgfplotslist@TOK@a=\expandafter{\pgfplotstable@entry@a}%
	\pgfplotslist@TOK@b=\expandafter{\pgfmathresult}%
	\edef\pgfmathresult{\the\pgfplotslist@TOK@a&\the\pgfplotslist@TOK@b}%
	\pgfkeyslet{/pgfplots/table/@cell content}\pgfmathresult
}
...
\def\pgfplotstabletypeset@opt[#1]#2{%
	...
	\pgfplotslistforeach\pgfplotstable@colnames\as\pgfplotstable@colname{%
		...
		\else
			\edef\pgfplotstable@TMP{\the\pgfplotslist@TOK@a,columns/\the\pgfplotslist@TOK@b/.try}%
		\fi
		\pgfplotslist@TOK@a=\expandafter{\pgfplotstable@TMP}%
		\pgfplotslist@TOK@b=\expandafter{\pgfplotstable@displaycolkey}%
		\edef\pgfplotstable@TMP{\the\pgfplotslist@TOK@a,\the\pgfplotslist@TOK@b}%
			...
			\ifpgfplotstableuserow
				...
				\else
					\ifnum\c@pgfplotstable@counta=\pgfplotstable@firstnumrows
						...
						\pgfplots@warning{Unbalanced cell with content '\the\pgfplotslist@TOK@a' of column '\the\pgfplotslist@TOK@b' has been skipped: row count \the\c@pgfplotstable@counta+1 > \pgfplotstable@firstnumrows (which is the number of rows in the first column)}%
						\pgfplotstableuserowfalse
					\fi
				\fi
				\ifpgfplotstableuserow
...
\def\pgfplotstablecreatecol@opt[#1]#2#3{%
	...
	\pgfplotslist@TOK@a=\expandafter{#2}% this should handle macro names in '#2'.
	\edef\pgfplotstable@newcolname{\the\pgfplotslist@TOK@a}%
	...
	\ifpgfplots@loop@CONTINUE
		...
		\pgfplotslistforeachungrouped\pgfplotstable@colnames\as\pgfplotstable@colname{%
			\ifpgfplotstable@isfirstrow
				\expandafter\pgfplotslistcheckempty\csname\string#3@\pgfplotstable@colname\endcsname
				\ifpgfplotslistempty
					% the table is completely empty. break.
					\pgfplots@loop@CONTINUEfalse
				\else
					\expandafter\pgfplotslistpopfront\csname\string#3@\pgfplotstable@colname\endcsname\to\pgfplotstable@entry
		...
		\ifpgfplots@loop@CONTINUE
			...
			\pgfplotslist@TOK@a=\expandafter{\pgfplotstable@entry}%
			\pgfplotslist@TOK@b=\expandafter{\pgfmathaccuma}%
			\xdef\pgfplotstable@TMP{%
				\noexpand\def\noexpand\pgfplotstable@entry{\the\pgfplotslist@TOK@a}%
				\noexpand\def\noexpand\pgfmathaccuma{\the\pgfplotslist@TOK@b}%
			}%
		...
		\advance\c@pgfplotstable@rowindex by1\relax
	\pgfutil@repeat
	\global\let\pgfplotstable@TMP=\pgfplotstable@newcol
	\global\let\pgfplotstable@TMPB=\pgfplotstable@newcolname
	\endgroup
	\ifpgfplotstablecreatecol@isreallynew
		\expandafter\pgfplotslistpushback\expandafter{\pgfplotstable@TMPB}\to#3
	\fi
	\expandafter\let\csname\string#3@\pgfplotstable@TMPB\endcsname=\pgfplotstable@TMP
}%
