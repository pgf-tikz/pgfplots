...
% tick options:
	...
	/pgfplots/log basis x/.initial=e,
	/pgfplots/log basis y/.initial=e,
	/pgfplots/log basis z/.initial=e,
...
% FIXME: simplify this code! It is just a replication of the default error stuff together with an xshift!
	...
	/pgfplots/mesh/true/.code={%
		...
		\if\pgfplots@perpointmeta@choice0%
			...
			\fi
		\fi
		\pgfkeysalso{/pgfplots/every mesh,/pgfplots/every mesh legend,/tikz/color=mapped color}%
	},
...
%\message{Color for current point is RGB '\pgfmathresult' (determined using meta 'phi(\pgfplotspointmeta) = \pgfplotspointmetatransformed')}%
	...
	/pgfplots/compat/anchors/pre 1.3/.code={\global\pgfplots@deprecated@anchorstrue},%
	/pgfplots/compat/anchors/newest/.code= {\global\pgfplots@deprecated@anchorsfalse},%
...
\def\pgfplots@BEGIN@prepare@description@styles{%
	\ifpgfplots@threedim
		\pgfplotsset{every 3d description}%
	\fi
...
\def\pgfplots@prepare@coord@filtering@for#1{%
	...
	\else
		...
		\pgfplots@if{pgfplots@disablelogfilter@#1}{\relax}{%
			...
			\expandafter\let\csname pgfplotsmath@log@#1@logten\endcsname=\logten
			\expandafter\let\csname pgfplotsmath@log@#1@logten@inv\endcsname=\reciproclogten
			...
			\else
				...
				\else
					...
					% The 10^e notation hide the underlying log.
					% 
					...
					\expandafter\pgfmath@basic@multiply@\expandafter{\pgfmathresult}{\logten}%
					\expandafter\let\csname pgfplotsmath@log@#1@logten\endcsname=\pgfmathresult
					\pgfmath@basic@multiply@{\reciproclogten}{\pgfplots@loc@TMPa}%
					\expandafter\let\csname pgfplotsmath@log@#1@logten@inv\endcsname=\pgfmathresult
					\expandafter\edef\csname pgfplotsmathlogtologten@#1@\endcsname##1{%
						\noexpand\pgfmath@basic@multiply@{##1}{\csname pgfplotsmath@log@#1@logten@inv\endcsname}%
					}%
					%
					\begingroup
				...
				\expandafter\def\csname pgfplots@prepare@#1coord\endcsname##1{%
					...
					\else
						\pgfplots@if{pgfplots@disablelogfilter@#1}{%
							...
							\csname pgfplotsmathlog@#1@\endcsname{##1}%
						}%
					\fi
				}%
			\else
...
\def\pgfplots@create@axis@descriptions{%
	\begingroup
		%
		\pgfplots@change@pgfpoints@to@relaxiscs
		%
		\ifpgfplots@hide@x
		\else
