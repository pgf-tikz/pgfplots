...
% log=1
	...
	/pgfplots/error bars/mark/.store in=				\pgfplots@errorbars@mark,
	/pgfplots/error bars/mark={-},
	/pgfplots/error bars/mark options/.store in=		\pgfplots@errorbars@markopts,
	/pgfplots/error bars/mark options={rotate=90},
...
\long\def\pgfplots@addplotimpl@expression#1#2;{%
	...
	\pgfplots@addplot@enqueue@coords{}{}{\draw[#1] plot #2;}{}%
	\pgfplots@end@plot
}%
...
\long\def\pgfplots@addplotimpl@table@START#1#2#3{%
	...
	\expandafter\pgfplots@TMPB\pgfplots@TMP% this will automatically continue the path statement until the trailing ';'
}
...
%\pgfplots@message{processing plots coords with trailing path '#3'}%
	...
	\ifpgfplots@filtercoords
		\pgfplots@filtercoords@and@up@limits plot coordinates {#2};\newplotcmdto\pgfplots@filteredcoordinateplotcmd
		\ifpgfplots@errorbars@enabled
		...
		\ifx\pgfplots@recordederrorbar\pgfutil@empty
			\edef\pgfplots@TMP{%
				...
				{
					\noexpand\def\noexpand\pgfplots@plotstyleopts{\the\pgfplotslist@TOK@a}%
				}%
				{%
	...
	\else
		...
		\fi
		\pgfplots@addplot@enqueue@coords{}{\draw[every axis plot,#1]}{ plot coordinates {#2}#3;}{}%
	\fi
	\pgfplots@end@plot
...
\def\pgfplots@filter@one@coord@pair#1#2#3#4{%
	...
	\else
		...
		\else
			...
			\edef\pgfplots@filter@output{\the\pgfplots@toka (\pgfplots@filter@tmp@x,\pgfplots@filter@tmp@y)\pgfplots@errorspec}%
...
\long\def\pgfplots@finish@datatrafo@@generate@errorbars plot coordinates #1#2;\to#3{%
	\let#3=\pgfutil@empty
	...
	plot coordinates {#1};%
	\pgfplots@EOI
	\edef#3{plot coordinates {\pgfplots@data@scaletrafo@result}#2;}%
}
...
\message{PROCESSING ERROR BARS FOR (#1,#2) +- (#3,#4)...}%
	...
	\or
		...
		\pgfplots@process@errorbar@@for{y}{-}%
	\fi
\message{FINISHED ERROR BARS FOR (#1,#2) +- (#3,#4)...}%
%	\endgroup
}
...
\def\pgfplots@process@errorbar@@for#1#2{%
	...
	\else
		...
		\fi
\message{errorbar@@for{#1}{#2}: streaming '\pgfplots@TMP'...}%
...
\long\def\pgfplots@path@enqueue@coords#1#2#3#4{%
	\ifpgfplots@draw@at@end
		\begingroup
		\globaldefs=1
		\pgfplotslistpushback{#1}\to\pgfplots@stored@plot@precommands
		\pgfplotslistpushback{#2}\to\pgfplots@stored@plot@cmds
		\pgfplotslistpushback{#3}\to\pgfplots@stored@plot@data
		\pgfplotslistpushback{#4}\to\pgfplots@stored@plot@postcommands
		\endgroup
	\else
		\begingroup
		#1#2#3#4%
...
\def\pgfplots@environment@opt[#1]{%
	...
	\global\pgfplotslistnewempty\pgfplots@stored@plot@precommands
	\global\pgfplotslistnewempty\pgfplots@stored@plot@postcommands
	\global\pgfplotslistnewempty\pgfplots@stored@plot@cmds
	\global\pgfplotslistnewempty\pgfplots@stored@plot@data
...
\def\endpgfplots@environment@opt{%
	...
	\ifpgfplots@draw@at@end
		\def\pgfplots@nextcommand{%
			...
			\loop
			\pgfplotslistcheckempty\pgfplots@stored@plot@cmds
			\ifpgfplotslistempty
				\pgfplots@loop@CONTINUEfalse
			\else
				\pgfplots@loop@CONTINUEtrue
			...
			\ifpgfplots@loop@CONTINUE
				...
				\begingroup
					\globaldefs=1
					\pgfplotslistpopfront\pgfplots@stored@plot@cmds\to\pgfplots@plotcmd
					\pgfplotslistpopfront\pgfplots@stored@plot@precommands\to\pgfplots@plotprecmd
					\pgfplotslistpopfront\pgfplots@stored@plot@postcommands\to\pgfplots@plotpostcmd
					\pgfplotslistpopfront\pgfplots@stored@plot@data\to\pgfplots@plotdata
				\endgroup
\message{Processing stored plot with precommand '\meaning\pgfplots@plotprecmd';  pgfplots@plotcmd '\meaning\pgfplots@plotcmd' postcommand '\meaning\pgfplots@plotpostcmd'}%
				\pgfplots@plotprecmd
				\ifx\pgfplots@plotcmd\pgfutil@empty
					\pgfplots@plotdata%
				\else
					...
					\ifpgfplots@float@numerics@mode
						...
						\expandafter\pgfplots@finish@datatrafo@@generate@errorbars\pgfplots@plotdata\to\pgfplots@plotdata
						%
						\ifpgfplots@errorbars@enabled
							\pgfplots@streamerrorbarend
							\expandafter\pgfplots@errorbars@finishwithstyleoptions\expandafter[\pgfplots@plotstyleopts]{\pgfplots@recordederrorbar}%
						\fi
					\fi
\message{calling plotcmd and plotdata: pgfplots@plotcmd '\meaning\pgfplots@plotcmd' data '\meaning\pgfplots@plotdata'}%
					\expandafter\pgfplots@plotcmd\pgfplots@plotdata
				\fi
				\pgfplots@plotpostcmd
				\endgroup
			\repeat
		}%
	\else
