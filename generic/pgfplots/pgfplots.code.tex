...
\pgfkeys{%
	...
	/pgfplots/colorbar vertical/.style={%
...
% FIXME: simplify this code! It is just a replication of the default error stuff together with an xshift!
	...
	/pgfplots/mesh/true/.code={%
		...
		\if\pgfplots@perpointmeta@choice0
			...
			\fi
		\fi
		\pgfkeysalso{/pgfplots/every mesh,/pgfplots/every mesh legend,/tikz/draw=mapped color,/tikz/fill=none}%
	},
...
\def\pgfplots@environment@opt[#1]{%
	...
	\else
		...
		\global\let\pgfplots@axiswide@metamin=\pgfplots@loc@TMPa
	\fi
	...
	\else
		...
		\global\let\pgfplots@axiswide@metamax=\pgfplots@loc@TMPa
	\fi
	%
	% --------------------
	% Start axis:
...
%\message{Processing stored plot with precommand '\meaning\pgfplots@stored@current@precmd';  pgfplots@plotcmd '\meaning\pgfplots@stored@current@cmd' postcommand '\meaning\pgfplots@stored@current@postcmd'}%
	...
	\ifpgfplots@colorbar
		...
		%
		% FIXME : THIS DOES NOT WORK YET -- '#' characters are
		% expanded too often!
		\pgfkeysgetvalue{/pgfplots/every colorbar/.@cmd}\pgfplots@loc@TMPa
		\toks1=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\pgfkeysgetvalue{/pgfplots/every tick/.@cmd}\pgfplots@loc@TMPa
		\toks2=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\pgfkeysgetvalue{/pgfplots/every minor tick/.@cmd}\pgfplots@loc@TMPa
		\toks3=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\pgfkeysgetvalue{/pgfplots/every major tick/.@cmd}\pgfplots@loc@TMPa
		\toks4=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\pgfkeysgetvalue{/pgfplots/every axis grid/.@cmd}\pgfplots@loc@TMPa
		\toks5=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\pgfkeysgetvalue{/pgfplots/every minor grid/.@cmd}\pgfplots@loc@TMPa
		\toks6=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\pgfkeysgetvalue{/pgfplots/every major grid/.@cmd}\pgfplots@loc@TMPa
		\toks7=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\pgfkeysgetvalue{/pgfplots/every tick label/.@cmd}\pgfplots@loc@TMPa
		\toks8=\expandafter{\pgfplots@loc@TMPa####1\pgfeov}%
		%
		\xdef\pgfplots@glob@TMPa{%
			\noexpand\begingroup
			\the\toks0 % restore colormap definition
			%  restore relevant styles:
			%--------------------------------------------------
			% \noexpand\pgfkeysdef{/pgfplots/every colorbar}{\the\toks1 }%
			% \noexpand\pgfkeysdef{/pgfplots/every tick}{\the\toks2 }%
			% \noexpand\pgfkeysdef{/pgfplots/every minor tick}{\the\toks3 }%
			% \noexpand\pgfkeysdef{/pgfplots/every major tick}{\the\toks4 }%
			% \noexpand\pgfkeysdef{/pgfplots/every axis grid}{\the\toks5 }%
			% \noexpand\pgfkeysdef{/pgfplots/every minor grid}{\the\toks6 }%
			% \noexpand\pgfkeysdef{/pgfplots/every major grid}{\the\toks7 }%
			% \noexpand\pgfkeysdef{/pgfplots/every tick label}{\the\toks8 }%
			%-------------------------------------------------- 
