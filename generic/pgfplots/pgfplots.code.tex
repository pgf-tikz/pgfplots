	...
	\def\pgfplots@coord@stream@end@@{%
		...
		\else
			...
			\else
				\pgfplots@toka={}%
			\fi
			\pgfplotslist@TOK@b=\expandafter{\pgfplots@coord@stream@recorded #2;}%
			\begingroup% allow usage of more temporary token registers FIXME
			\ifx\pgfplots@recordederrorbar\pgfutil@empty
				\let\pgfplots@TMPB=\pgfutil@empty%
			\else
				\toks255=\expandafter{\pgfplots@recordederrorbar}%
				\def\pgfplots@TMPB{%
					\noexpand\pgfplots@errorbars@finishwithstyleoptions[current plot style]{\the\toks255 }%
				}%
			\fi
			\xdef\pgfplots@TMP{%
