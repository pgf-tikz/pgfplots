...
\def\pgfplots@start@plot@with@options#1#2{%
	...
	\fi
	\pgfplots@rememberplotspec{#1}% also increases \pgfplots@numplots
...
\def\pgfplots@filter@one@coord@pair#1#2#3#4{%
	...
	\else
		...
		\else
			\ifpgfplots@draw@at@end
				\ifpgfplots@stackedmode
					% will use float numerics if necessary:
					\pgfplots@stacked@preparepoint@inmacro{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}%
				\fi
				% update also axis limits:
				\pgfplots@update@limits@for@one@point{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}{}{}%
				\ifpgfplots@errorbars@enabled
					\pgfplots@process@errorbar@for@with@unfiltered{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}{#3}{#4}{#1}{#2}%
				\fi
			\else
				...
				\ifpgfplots@stackedmode
					% works with pgfmath:
					\pgfplots@stacked@preparepoint@inmacro{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}%
					\pgfplots@stacked@finishpoint{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}%
				\fi
				\pgfplots@process@errorbar@for@with@unfiltered{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}{#3}{#4}{#1}{#2}%
			\fi
			\pgfplots@toka=\expandafter{\pgfplots@coord@stream@recorded}%
			\edef\pgfplots@coord@stream@recorded{\the\pgfplots@toka (\pgfplots@filter@tmp@x,\pgfplots@filter@tmp@y)}%
...
\def\pgfplots@apply@data@scaletrafo@to@one@point#1#2#3#4{%
	...
	\ifpgfplots@stackedmode
		...
		\pgfplots@stacked@finishpoint{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}%
		\pgfplots@stacked@rememberzerolevelpoint@for@next@plot{(\pgfplots@filter@tmp@x,\pgfplots@filter@tmp@y)}%
	\fi
...
% or log( y + e_y ).
\def\pgfplots@process@errorbar@for@with@unfiltered#1#2#3#4#5#6{%
