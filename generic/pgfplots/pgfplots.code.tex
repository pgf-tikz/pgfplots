...
\long\def\pgfplots@addplotimpl@expression#1(#2,#3)#4;{%
	...
	\pgfplots@plot@data{%
		...
		\t@pgfplots@tokc=\expandafter{\pgfplots@TMP}%
		\xdef\pgfplots@TMP{\the\t@pgfplots@tokc(\pgfplots@current@point@x,\pgfplots@current@point@y)}%
	}%
...
\long\def\pgfplots@PREPARE@COORD@STREAM#1#2{%
	\def\pgfplots@coord@stream@start@{%
		\pgfplotsapplistnewempty\pgfplots@coord@stream@recorded
		\ifpgfplots@errorbars@enabled
		...
		\else
			...
			\xdef\pgfplots@TMP{%
				...
				{% precommand:
					...
					\noexpand\tikzstyle{current plot style}=[\the\t@pgfplots@toka]%
			...
			\pgfplotsapplistedefcontenttomacro\pgfplots@coord@stream@recorded\to\pgfplots@coord@stream@recorded
...
\def\pgfplots@process@one@point#1#2#3#4{%
	...
	\else
		...
		\else
			...
			\expandafter\pgfplotsapplistpushback\pgfplots@TMP\to\pgfplots@coord@stream@recorded
			%\t@pgfplots@tokc=\expandafter{\pgfplots@coord@stream@recorded}%
...
\def\pgfplots@coord@stream@INIT@finalize@storedcoords#1{%
	...
	\def\pgfplots@coord@stream@start@{%
		...
		\ifpgfplots@record@marker@stream
			\pgfplotsapplistnewempty\pgfplots@recorded@marker@stream@@%
			\pgfplotsapplistpushback\noexpand\pgfplotstreamstart\to\pgfplots@recorded@marker@stream@@%
	...
	\def\pgfplots@coord@stream@end@{%
		...
		\pgfplotstreamend
		\ifpgfplots@record@marker@stream
			\pgfplotsapplistpushback\noexpand\pgfplotstreamend\to\pgfplots@recorded@marker@stream@@%
			\pgfplotsapplistxdefcontenttomacro\pgfplots@recorded@marker@stream@@\to#1%
	...
	\fi
	%
	% this here is a helper method to allow \xdef of the append-list.
	\def\pgfplots@fin##1##2{%
	...
	\def\pgfplots@coord@stream@finalize@currentpt{%
		...
		\ifpgfplots@record@marker@stream
			\edef\pgfmathresult{\noexpand\pgfplots@fin{\the\pgf@x}{\the\pgf@y}}%
			\expandafter\pgfplotsapplistpushback\pgfmathresult\to\pgfplots@recorded@marker@stream@@%
			%--------------------------------------------------
			...
			\ifnum0<\pgfplots@numplots\relax
				...
				\else
					...
					\ifnum\curcolumnNum=\maxcolumnCount
						\curcolumnNum=0
					\fi
					\expandafter\t@pgfplots@toka\expandafter{\pgfplots@TMP}%
				\fi
			\repeat
...
%\message{#1- display limits after data transformation: [\csname pgfplots@#1min\endcsname:\csname pgfplots@#1max\endcsname]}%
	...
	\else
		...
		\ifx\pgfplots@TMP\pgfplots@TMPB
			...
			\t@pgfplots@toka=\expandafter{\pgfplots@TMP}%
			\edef\pgfplots@TMP{{\the\t@pgfplots@toka}\the\t@pgfplots@tokc}%
...
%\message{Converting #1tick='\csname pgfplots@#1tick\endcsname'}%
	...
	\else
		\t@pgfplots@tokc=\expandafter{\csname pgfplots@datascaletrafo@#1\endcsname}%
		\edef\pgfplots@TMP{{\csname pgfplots@extra@#1tick\endcsname}\the\t@pgfplots@tokc}%
		\expandafter\pgfplots@apply@data@scale@trafo@to@user@ticks\pgfplots@TMP\to\pgfplots@TMPC
