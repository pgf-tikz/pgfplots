...
\newdimen\pgfplots@zcoordminTEX
\newdimen\pgfplots@zcoordmaxTEX
% FIXME : at least ONE of the dimen sets for axis limits is
% unnecessary with the change to logical coords!
...
% right   : 3
	...
	/pgfplots/zticklabel pos/.is choice,
	/pgfplots/zticklabel pos/.belongs to family=/pgfplots/tick,
	/pgfplots/zticklabel pos/default/.code	={\def\pgfplots@zticklabelposnum{0}},
	/pgfplots/zticklabel pos/default/.belongs to family=/pgfplots/tick,
	/pgfplots/zticklabel pos/left/.code	={\def\pgfplots@zticklabelposnum{1}},
	/pgfplots/zticklabel pos/left/.belongs to family=/pgfplots/tick,
	/pgfplots/zticklabel pos/right/.code	={\def\pgfplots@zticklabelposnum{3}},
	/pgfplots/zticklabel pos/right/.belongs to family=/pgfplots/tick,
	/pgfplots/zticklabel pos/top/.style={/pgfplots/zticklabel pos/right},
	/pgfplots/zticklabel pos/bottom/.style={/pgfplots/zticklabel pos/left},
...
% axis options:
	...
	/pgfplots/stack plots/y/.belongs to family=/pgfplots, % FIXME : z version missing!
...
% 4:  mi = given explicitly somehow
	...
	/pgfplots/point meta/explizit/.code={
	...
	/pgfplots/scatter plot/.is if=pgfplots@scatterplotenabled,
	/pgfplots/scatter plot/.default=true,
	/pgfplots/scatter src/.style={/pgfplots/point meta=#1},
	/tikz/scatter plot/.style={/pgfplots/scatter plot=#1},
	%
	...
	% '#1' is empty.
	/pgfplots/scatter plot/@pre marker code/.code={},
	/pgfplots/scatter plot/@post marker code/.code={},
	/pgfplots/scatter plot/use mapped color/.style={
		/pgfplots/scatter plot/@pre marker code/.code={
...
%\message{Color for current point is RGB '\pgfmathresult' (determined using meta 'phi(\pgfplotspointmeta) = \pgfplotspointmetatransformed')}%
		...
		/pgfplots/scatter plot/@post marker code/.code={\endscope}
	},
	/pgfplots/scatter plot/use mapped color/.default={draw=mapped color!80!black,fill=mapped color},
	/pgfplots/scatter plot/use mapped color,
}
...
\def\pgfplots@drawaxis@innerlines@onorientedsurf#1#2#3{%
	\ifnum\csname pgfplots@#1axislinesnum\endcsname=2
		...
		\pgfextra
		\csname pgfplotspointonorientedsurfaceabsetupforset#3\endcsname{\csname pgfplots@logical@ZERO@#3\endcsname}%
...
\def\pgfplots@check@invalid@range{%
	...
	\ifpgfplots@limits@are@computed
		...
		\else
			...
			%
			\ifpgfplots@threedim
				\pgfmathfloatmin@{\pgfplots@zmin}{\pgfplots@zmax}%
				\global\let\pgfplots@zmin=\pgfmathresult
				\pgfmathfloatmax@{\pgfplots@zmin}{\pgfplots@zmax}%
...
% \message{Axis scaling x=\pgfplots@tmpXscale, y=\pgfplots@tmpYscale\ yields lower-left-corner (\pgfplots@xmin,\pgfplots@ymin) = (\the\pgfplots@xcoordminTEX,\the\pgfplots@ycoordminTEX) and upper right (\pgfplots@xmax,\pgfplots@ymax) = (\the\pgfplots@xcoordmaxTEX,\the\pgfplots@ycoordmaxTEX).   Axis dimensions are width=\the\pgf@x, height=\the\pgf@y. Inverse unit vector length are x:\pgfplots@x@inverseveclength; y:\pgfplots@y@inverseveclength.}%
% \endgroup
%-------------------------------------------------- 
}
...
\def\pgfplots@draw@axis{%
	...
	\ifpgfplots@threedim
		...
		\ifcase\pgfplots@zaxislinesnum\relax
			\pgfplotspointonorientedsurfaceabsetupforsetz{\pgfplots@zmin}%
			\pgfplots@draw@axis@insurface@symmetric xyz0
			%
			\pgfplotspointonorientedsurfaceabsetupforsetz{\pgfplots@zmax}%
			\pgfplots@draw@axis@insurface@symmetric xyz1
		\or
			\pgfplotspointonorientedsurfaceabsetupforsetz{\pgfplots@zmin}%
			\pgfplots@draw@axis@insurface@symmetric xyz2
		\or
			\pgfplotspointonorientedsurfaceabsetupforsetz{\pgfplots@logical@ZERO@z}%
			\pgfplots@draw@axis@insurface@symmetric xyz2
		\or
			\pgfplotspointonorientedsurfaceabsetupforsetz{\pgfplots@zmax}%
			\pgfplots@draw@axis@insurface@symmetric xyz2
		...
		\ifcase\pgfplots@xaxislinesnum\relax
			\pgfplotspointonorientedsurfaceabsetupforsetx{\pgfplots@xmin}%
			\pgfplots@draw@axis@insurface@onlyticksandgrid yzx
			\pgfplots@draw@axis@insurface zyx0
			%
			\pgfplotspointonorientedsurfaceabsetupforsetx{\pgfplots@xmax}%
			\pgfplots@draw@axis@insurface@onlyticksandgrid yzx
		...
		\or
			...
			\pgfplots@draw@axis@insurface zyx0
		\or
			\pgfplotspointonorientedsurfaceabsetupforsetx{\pgfplots@logical@ZERO@x}%
			\pgfplots@draw@axis@insurface@onlyticksandgrid yzx
			\pgfplots@draw@axis@insurface zyx1
		\or
			\pgfplotspointonorientedsurfaceabsetupforsetx{\pgfplots@xmax}%
		...
		\ifcase\pgfplots@yaxislinesnum\relax
			\pgfplotspointonorientedsurfaceabsetupforsety{\pgfplots@ymin}%
			\pgfplots@draw@axis@insurface@onlyticksandgrid xzy
			\pgfplots@draw@axis@insurface@onlyticksandgrid zxy
			%
			\pgfplotspointonorientedsurfaceabsetupforsety{\pgfplots@ymax}%
			\pgfplots@draw@axis@insurface@onlyticksandgrid xzy
			\pgfplots@draw@axis@insurface@onlyticksandgrid zxy
		\or
			\pgfplotspointonorientedsurfaceabsetupforsety{\pgfplots@ymin}%
			...
			\pgfplots@draw@axis@insurface@onlyticksandgrid zxy
		\or
			\pgfplotspointonorientedsurfaceabsetupforsety{\pgfplots@logical@ZERO@y}%
			\pgfplots@draw@axis@insurface@onlyticksandgrid xzy
			\pgfplots@draw@axis@insurface@onlyticksandgrid zxy
		\or
			\pgfplotspointonorientedsurfaceabsetupforsety{\pgfplots@ymax}%
...
\def\pgfplots@draw@axis@insurface#1#2#3#4{%
	\pgfplots@if{pgfplots@hide@#1}{\relax}{%
		...
		%
		\ifcase#4
			\ifnum\csname pgfplots@#3ticklabelposnum\endcsname=0
				\pgfplots@drawticklabels@onorientedsurf
			\fi
		\or
			\ifnum\csname pgfplots@#3ticklabelposnum\endcsname=3
				\pgfplots@drawticklabels@onorientedsurf
			\fi
		\fi
		%
...
\def\pgfplots@BEGIN@init@and@draw@axis{%
	...
	\setbox\pgfnodepartimagebox=\hbox\bgroup\bgroup
		...
		\else
			\pgfpathmoveto{\pgfplotsqpointxy{\pgfplots@xmin}{\pgfplots@ymin}}%
			\pgfpathlineto{\pgfplotsqpointxy{\pgfplots@xmax}{\pgfplots@ymin}}%
			\pgfpathlineto{\pgfplotsqpointxy{\pgfplots@xmax}{\pgfplots@ymax}}%
			\pgfpathlineto{\pgfplotsqpointxy{\pgfplots@xmin}{\pgfplots@ymax}}%
			\pgfusepath{clip}%
		\fi
}
...
%\message{every axis is now '\meaning\pgfplots@loc@TMPa'}%
	...
	\ifpgfplots@collect@firstplot@astick
		...
		\let\pgfplots@firstplot@coords@z=\pgfutil@empty
	\fi
