...
%\message{Got T^{-1}(H#1) = \Hmacro}%
			...
			\else
				...
				\fi
			\fi
\pgfplots@message{Computed H#1=\the\H [unscaled]; transforming ... }%
			\edef\Hmacro{\the\pgfmathfloat@a@S\pgf@sys@tonumber{\H}e\the\pgfmathfloat@a@E}%FIXME
			\ifpgfplots@is@datascaled
			...
			\fi
			\let\Hmacro=\pgfmathresult
\pgfplots@message{got H#1=\Hmacro\ [transformed]}%
			...
			\else
				\ifdim\max@d=0pt
					\if@cur@is@scaled
						...
						\csname pgfplots@datascaletrafo@#1\endcsname{21.0e0}% FIXME : call createfloat method
						\let\min=\pgfmathresult
...
\def\pgfplots@inverse@datascaletrafo@x@onlyshift#1{%
	\pgfmathadd@{#1}{\pgfplots@data@scale@trafo@SHIFT@x}%
}
...
\def\pgfplots@inverse@datascaletrafo@y@onlyshift#1{%
	\pgfmathadd@{#1}{\pgfplots@data@scale@trafo@SHIFT@y}%
}
