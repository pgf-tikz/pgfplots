			...
			\else
				...
				\else
					...
					\ifx\pgfplots@loc@TMPa\pgfplots@loc@TMPb
						\expandafter\def\csname pgfplots@#1ticklabel@pos\endcsname{u}%
					\else
						\pgfplots@error{Sorry, the key '#1ticklabel pos' does not know what to do with '#2'... accepted values are 'lower' or 'upper' and some aliases. Maybe you misspelled it?}%
					\fi
				\fi
			\fi
		\fi
...
% upper (alias right alias top)      : u
	...
	/pgfplots/xticklabel dist/.initial=,
	/pgfplots/yticklabel dist/.initial=,
	/pgfplots/zticklabel dist/.initial=,
	/pgfplots/ticklabel dist/.style={
		/pgfplots/xticklabel dist={#1},
		/pgfplots/yticklabel dist={#1},
		/pgfplots/zticklabel dist={#1}
	},%
...
% axis options:
	...
	/pgfplots/view/.code 2 args={%
		...
		\else
			\pgfmathparse{#1}\let\pgfplots@view@az=\pgfmathresult
			\pgfmathparse{#2}\let\pgfplots@view@el=\pgfmathresult
		\fi
	},
	/pgfplots/view={25}{30},
...
% FIXME: simplify this code! It is just a replication of the default error stuff together with an xshift!
	...
	/pgfplots/mesh/true/.code={%
		...
		\if\pgfplots@perpointmeta@choice0
			...
			\fi
		\fi
		\pgfkeysalso{/pgfplots/every mesh,/tikz/draw=mapped color,/tikz/fill=none}%
	},
	/pgfplots/mesh/false/.code={
		\pgfplots@meshmodefalse
	...
	/pgfplots/every mesh/.code={%
		\if\pgfplotsplothandlermesh@zbuffer@choice0%
			\pgfkeysalso{/pgfplots/z buffer=reverse xy seq}%
		\fi
	},%
	...
	% RULES:
	%   1. fill=none 
	%   	=> shader must not be 'interp' because we want ONLY a mesh
	%   	plot and that's not supported by 'interp'
	%   2. draw != none 
	%   	=> shader must not be 'interp' because 'interp' does not support meshes.
	%   	Provide a warning and switch silently to 'flat mean'.
	%   3. 'shader!=interp' and fill!=none 
	%   	=>  'draw=none' is forbidden because that leaves 'holes'.
	/pgfplots/shader/.is choice,
	/pgfplots/shader/flat corner/.code={\def\pgfplotsplothandlermesh@shader{0}},
	/pgfplots/shader/flat mean/.code={\def\pgfplotsplothandlermesh@shader{1}},
	/pgfplots/shader/interp/.code={\def\pgfplotsplothandlermesh@shader{2}},
	/pgfplots/shader/flat/.style={/pgfplots/shader/flat mean},%
	/pgfplots/shader/faceted/.style={/pgfplots/shader/flat mean,/tikz/draw={#1}},
	/pgfplots/shader/faceted/.default=black,
	/pgfplots/shader/faceted,
	/pgfplots/surf/.code={%
		\pgfkeysalso{/pgfplots/mesh=#1,/tikz/draw=none,/tikz/fill=mapped color}%
	},%
...
\pgfdeclareshape{pgfplots@low@level@shape}{%
	...
	% 	\pgfqpoint{\pgfplots@savedanchor@minminmin@x\relax}{\pgfplots@savedanchor@minminmin@y\relax}%
...
\def\pgfplots@decide@which@figure@surfaces@are@drawn{%
	\ifpgfplots@threedim
		\ifdim\pgfplots@view@dir@threedim@x>0pt
		...
		\fi
		\ifdim\pgfplots@view@dir@threedim@y>0pt
		...
		\fi
		\ifdim\pgfplots@view@dir@threedim@z>0pt
...
\def\pgfplots@init@ticklabelaxisspecfor#1#2{%
	\if#1x%
		\def\pgfplots@tmp@@##1##2{v##1##2}%
		\def\pgfplots@point@on@middle@of##1##2{%
			\pgf@process{%
				\pgfpointscale{0.5}{\pgfplotspointxaxis}%
				\pgf@xa=\pgf@x
				\pgf@xb=\pgf@y
				\if 1##1%
					\pgfplotspointyaxis
	...
	\else
		\if#1y%
			\def\pgfplots@tmp@@##1##2{##1v##2}%
			\def\pgfplots@tmp@@@{x}%
			\def\pgfplots@point@on@middle@of##1##2{%
				\pgf@process{%
					\pgfpointscale{0.5}{\pgfplotspointyaxis}%
					\pgf@xa=\pgf@x
					\pgf@xb=\pgf@y
					\if 1##1%
						\pgfplotspointxaxis
		...
		\else
			\def\pgfplots@tmp@@##1##2{##1##2v}%
			\def\pgfplots@tmp@@@{x}%
			\def\pgfplots@point@on@middle@of##1##2{%
				\pgf@process{%
					\pgfpointscale{0.5}{\pgfplotspointzaxis}%
					\pgf@xa=\pgf@x
					\pgf@xb=\pgf@y
					\if 1##1%
						\pgfplotspointxaxis
		...
		\fi
	\fi
	\pgfplotsifaxislineisonconvexhull{\pgfplots@tmp@@ 00}{%
		...
		\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 00}%
		...
		\ifdim\csname pgf@#2\endcsname<\csname pgfplots@tmp@center#2\endcsname\relax
			\if u\csname pgfplots@#1ticklabel@pos\endcsname
				\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 11}%
				\pgfplots@point@on@middle@of11%
			\fi
		\else
		...
		%--------------------------------------------------
		% \pgfplotsifaxislineisonconvexhull{\pgfplots@tmp@@ 01}{% FIXME : eliminate sanity assertion if possible
		% }{%
		...
		\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 10}%
		% compute the middle point on \pgfplots@tmp@@ 10
		\pgfpointadd
			{\csname pgfplotspoint\pgfplots@tmp@@@ axis\endcsname}
			{\pgfpointscale{0.5}{\csname pgfplotspoint#1axis\endcsname}}%
		%
		\ifdim\csname pgf@#2\endcsname<\csname pgfplots@tmp@center#2\endcsname\relax
			\if u\csname pgfplots@#1ticklabel@pos\endcsname
				\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 01}%
				\pgfplots@point@on@middle@of01%
			\fi
		\else
			\if l\csname pgfplots@#1ticklabel@pos\endcsname
				\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 01}%
	...
	%
	% this here makes use of the already prepared point
	% '\pgf@x,\pgf@y'. See the documentation in
	% \pgfplots@init@ticklabelaxisspecfor@anchor.
	\pgfplots@init@ticklabelaxisspecfor@anchor#1%
}%
\def\pgfplots@init@ticklabelaxisspec@twodim@for#1#2{%
	\if#1x%
		\def\pgfplots@tmp@@##1{v##10}%
		\def\pgfplots@tmp@@@{y}%
		\def\pgfplots@point@on@middle@of##1{%
			\pgf@process{%
				\pgfpointscale{0.5}{\pgfplotspointxaxis}%
				\pgf@xa=\pgf@x
				\pgf@xb=\pgf@y
	...
	% point. This '<' operation is based on the '#1ticklabel pos'
	% key, the 2D projection of the axis and the argument '#2'.
	\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 0}%
	\pgfpointscale{0.5}{\csname pgfplotspoint#1axis\endcsname}%
	\ifdim\csname pgf@#2\endcsname<\csname pgfplots@tmp@center#2\endcsname\relax
		\if u\csname pgfplots@#1ticklabel@pos\endcsname
			\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 1}%
			\pgfplots@point@on@middle@of1%
		\fi
	\else
		\if l\csname pgfplots@#1ticklabel@pos\endcsname
			\expandafter\edef\csname pgfplots@#1ticklabelaxisspec\endcsname{\pgfplots@tmp@@ 1}%
...
\def\pgfplots@init@ticklabelaxisspecfor@anchor#1{%
	% now, determine the suggested anchor for tick labels.
	% We need to align them such that they point OUTSIDE of the convex
	% hull of the 2D projection.
	% Idea:
	% compute the angle of V := (point on middle) - (center of figure) and
	% the canvas-x and canvas-y axes.
	%
	% REMARK: at this point, (\pgf@x,\pgf@y)  is a point on the middle
	% of the selected axis.
	\advance\pgf@x by-\pgfplots@tmp@centerx\relax
	\advance\pgf@y by-\pgfplots@tmp@centery\relax
	% normalise. I need angles:
	\pgfpointnormalised{}% simply take pgf@x,pgf@y
	% now, 
	% 	veclength(V) = 1,
	% and thus
	%   \pgf@x = cos( V,x ),
	\ifdim\pgf@y>0pt
		\ifdim\pgf@x>0pt
			\ifdim\pgf@x>0.707pt %=cos(45)
				\expandafter\def\csname pgfplots@#1ticklabel@suggested@anchor\endcsname{west}%
			\else
				\expandafter\def\csname pgfplots@#1ticklabel@suggested@anchor\endcsname{south}%
			\fi
		\else
			\ifdim\pgf@x<-0.707pt
				\expandafter\def\csname pgfplots@#1ticklabel@suggested@anchor\endcsname{east}%
			\else
				\expandafter\def\csname pgfplots@#1ticklabel@suggested@anchor\endcsname{south}%
			\fi
		\fi
	\else
		\ifdim\pgf@x>0pt
			\ifdim\pgf@x>0.707pt
				\expandafter\def\csname pgfplots@#1ticklabel@suggested@anchor\endcsname{west}%
			\else
				\expandafter\def\csname pgfplots@#1ticklabel@suggested@anchor\endcsname{north}%
			\fi
		\else
			\ifdim\pgf@x<-0.707pt
				\expandafter\def\csname pgfplots@#1ticklabel@suggested@anchor\endcsname{east}%
	...
	\def\pgfplots@clippath@use@{%
		\ifx\pgfplotspoint@bb@@@\relax
			...
			\begingroup
				...
				\xdef\pgfplots@glob@TMPa{{\the\pgf@pathminx}{\the\pgf@pathminy}{\the\pgf@pathmaxx}{\the\pgf@pathmaxy}{\the\pgf@xa}{\the\pgf@ya}}%
			\endgroup
...
\def\pgfplotspointbbupperright{%
	\pgfplotspointbb@assertvaluesexist
	\expandafter\pgfplotspointbbupperright@\pgfplotspoint@bb@@@
}%
...
\def\pgfplotspointbblowerleft@#1#2#3#4#5#6{\pgf@x=#1 \pgf@y=#2 }%
\def\pgfplotspointbbupperright@#1#2#3#4#5#6{\pgf@x=#3 \pgf@y=#4 }%
\def\pgfplotspointbbdiagonal@#1#2#3#4#5#6{\pgf@x=#5 \pgf@y=#6 }%
...
\def\pgfplotsaxispointxy#1#2{\pgfplots@evalute@tikz@coord@system@interface#1,#2\pgfplots@coord@end}%
\def\pgfplotsaxispointxyz#1#2#3{\pgfplots@evalute@tikz@coord@system@interface#1,#2,#3\pgfplots@coord@end}%

\tikzdeclarecoordinatesystem{rel axis}{\pgfplots@evalute@tikz@coord@system@interface@rel#1\pgfplots@coord@end}
\def\pgfplotsrelaxispointxy#1#2{\pgfplots@evalute@tikz@coord@system@interface@rel#1,#2\pgfplots@coord@end}%
\def\pgfplotsrelaxispointxyz#1#2#3{\pgfplots@evalute@tikz@coord@system@interface@rel#1,#2,#3\pgfplots@coord@end}%
...
\newif\ifpgfplots@autocompute@metamin
\newif\ifpgfplots@autocompute@metamax
...
\def\pgfplots@environment@opt[#1]{%
	...
	\else
		...
		\pgfplots@autocompute@all@limitsfalse
	\fi
	\pgfkeysgetvalue{/pgfplots/point meta min}\pgfplots@axiswide@metamin
	\ifx\pgfplots@axiswide@metamin\pgfutil@empty
		\pgfplots@autocompute@metamintrue
		\global\let\pgfplots@axiswide@metamin=\pgfplots@invalidrange@metamin
	\else
		\pgfplots@autocompute@metaminfalse
		\pgfmathfloatparsenumber{\pgfplots@axiswide@metamin}%
		\let\pgfplots@axiswide@metamin=\pgfmathresult
	\fi
	\pgfkeysgetvalue{/pgfplots/point meta max}\pgfplots@axiswide@metamax
	\ifx\pgfplots@axiswide@metamax\pgfutil@empty
		\pgfplots@autocompute@metamaxtrue
		\global\let\pgfplots@axiswide@metamax=\pgfplots@invalidrange@metamax
	\else
		\pgfplots@autocompute@metamaxfalse
		\pgfmathfloatparsenumber{\pgfplots@axiswide@metamax}%
		\let\pgfplots@axiswide@metamax=\pgfmathresult
	\fi
	%
	% --------------------
	% Start axis:
...
%\message{Processing stored plot with precommand '\meaning\pgfplots@stored@current@precmd';  pgfplots@plotcmd '\meaning\pgfplots@stored@current@cmd' postcommand '\meaning\pgfplots@stored@current@postcmd'}%
				...
				\else
					...
					% them.
					% The re-activation is done in 'current plot style'.
					% FIXME : is that correct and portable!?
	...
	\begingroup
		...
		\pgftransformshift{\pgfplotspointbblowerleft}%
		...
		\let\pgfplotspointminminmin@orig=\pgfplotspointminminmin
		\def\pgfplotspointminminmin{%
			\pgfpointdiff
				{\pgfplotspointbblowerleft}%
				{\pgfplotspointminminmin@orig}%
		}%
		\pgfplotspointbbdiagonal
		\pgf@xa=\pgf@x
		\pgf@xb=\pgf@y
		\pgfsetxvec{\pgf@x=\pgf@xa \pgf@y=0pt }%
		\pgfsetyvec{\pgf@x=0pt \pgf@y=\pgf@xb}%
		\pgfsetzvec{\pgfpointorigin}%
		%
	...
	\begingroup
		...
		\pgf@xa=\pgf@x
		\advance\pgf@xa by-\pgf@picminx
		\xdef\pgfplots@savedanchor@minminmin@x{\the\pgf@xa}%
		%
		\pgf@xa=\pgf@y
		\advance\pgf@xa by-\pgf@picminy
		\xdef\pgfplots@savedanchor@minminmin@y{\the\pgf@xa}%
		%
