...
\newif\ifpgfplots@use@aftergroup@as@fast@list
...
% axis options:
	...
	/pgfplots/default non boxed lines/.style={-stealth},
...
% top=3
	...
	/pgfplots/axis x line/middle/.code	={%
		\pgfkeysalso{
			...
			xtick align=center,
			enlarge x limits=false,
			every inner x axis line/.append style={/pgfplots/default non boxed lines}}%
		...
		\else
			\pgfkeysalso{every axis x label/.style={at={(current axis.right of origin)},anchor=south east}}%
		\fi
	},
	/pgfplots/axis x line/center/.style	={/pgfplots/axis x line/middle},
	/pgfplots/axis x line/top/.code	={
		...
		\ifnum\pgfplots@yaxislinesnum=2 % if 'axis y line==center' then
			\pgfkeysalso{
				every axis y label/.style={at={(current axis.below origin)},anchor=south west}}%
		\fi
	},
...
% right=3
	...
	/pgfplots/axis y line/center/.code	={
		\pgfkeysalso{%
			...
			ytick align=center,
			enlarge y limits=false,
			every inner y axis line/.append style={/pgfplots/default non boxed lines}}%
...
%\message{/pgfplots/error bars/draw error bar:  working with '#1' -- '#2'.}%
	...
	/pgfplots/use aftergroup stack as fast list/.is if=pgfplots@use@aftergroup@as@fast@list,
	/pgfplots/use aftergroup stack as fast list/.default=true,
...
\def\pgfplots@start@plot@with@behavioroptions#1{%
	...
	\pgfqkeys{/tikz}{/pgfplots/every axis plot,/pgfplots/every axis plot no \the\pgfplots@numplots/.try}%
}
...
\long\def\pgfplots@PREPARE@COORD@STREAM#1#2{%
	\def\pgfplots@coord@stream@start@{%
		\ifpgfplots@use@aftergroup@as@fast@list
			\begingroup
			\aftergroup\pgfplotsaftergroupcollectinto
			\aftergroup\pgfplots@coord@stream@recorded
		\fi
		\ifpgfplots@errorbars@enabled
		...
		\else
			...
			\xdef\pgfplots@TMP{%
				...
				{% precommand:
					...
					\the\pgfplots@toka% named start/end points (if already available)
			...
			\ifpgfplots@use@aftergroup@as@fast@list
				\aftergroup\pgfplots@EOI%
				% WARNING: this artificial group destroys
				%  \pgfplots@currentplot@firstcoord@[xy]
				%  \pgfplots@currentplot@lastcoord@[xy]
				% and other intermediate values (which is ok)
				\ifpgfplots@collect@firstplot@astick
					\ifnum\pgfplots@numplots=0\relax
						% safe this information outside of the group:
...
\def\pgfplots@process@one@point#1#2#3#4{%
	...
	\else
		...
		\else
			...
			%
			\ifpgfplots@use@aftergroup@as@fast@list
				\xdef\pgfplots@TMP{(\pgfplots@current@point@x,\pgfplots@current@point@y)}%
				\expandafter\pgfplotsqaftergroupeach\expandafter{\pgfplots@TMP}%
			\else
				\pgfplots@toka=\expandafter{\pgfplots@coord@stream@recorded}%
...
\def\pgfplots@coord@stream@INIT@finalize@storedcoords#1{%
	...
	\def\pgfplots@coord@stream@start@{%
		...
		\ifpgfplots@record@marker@stream
			\ifpgfplots@use@aftergroup@as@fast@list
				\begingroup
				\aftergroup\pgfplotsaftergroupcollectinto
				\aftergroup\pgfplots@recorded@marker@stream@@%
				\aftergroup\pgfplotstreamstart
			\else
	...
	\def\pgfplots@coord@stream@end@{%
		...
		\ifpgfplots@record@marker@stream
			\ifpgfplots@use@aftergroup@as@fast@list
				\aftergroup\pgfplotstreamend
				\aftergroup\pgfplots@EOI%
				\endgroup
				%^^^^^^^^
				% this issues a
				%  \def\pgfplots@recorded@marker@stream@@{recorded stream data}
				% command HERE
				\global\let\pgfplots@recorded@marker@stream=\pgfplots@recorded@marker@stream@@
			\else
	...
	\ifpgfplots@use@aftergroup@as@fast@list
		\def\pgfplots@coord@stream@finalize@currentpt{%
			...
			\ifpgfplots@record@marker@stream
				\aftergroup\pgfplotstreampoint
				\aftergroup{%
				\aftergroup\pgfqpoint
				\aftergroup{%
				\expandafter\pgfplotsqaftergroupeach\expandafter{\the\pgf@x}%
				\aftergroup}%
				\aftergroup{%
				\expandafter\pgfplotsqaftergroupeach\expandafter{\the\pgf@y}%
				\aftergroup}%
				\aftergroup}%
			\fi
	...
	\else
		\def\pgfplots@coord@stream@finalize@currentpt{%
			...
			\ifpgfplots@record@marker@stream
				...
				\pgfplotslist@TOK@a=\expandafter{#1}%
				\pgfplotslist@TOK@b=\expandafter{\pgfmathresult}%
				\xdef#1{\the\pgfplotslist@TOK@a
					\noexpand\pgfplotstreampoint{\noexpand\pgfqpoint\the\pgfplotslist@TOK@b}}%
			\fi
			\pgfplotstreampoint{}% it will simply take \pgf@x and \pgf@y!
		}%
	\fi
...
\long\def\pgfplots@coord@stream@finalize@storedcoords@START plot coordinates #1#2;\pgfplots@EOI{%
	...
	\endpgfextra
	#2;
	% FIXME: inefficient! Use \scope to set variables:
...
\def\pgfplots@enlarge@limit@for#1{%
	...
	\ifdim\pgf@xb>0.001pt
		...
		\fi
	\fi
	\xdef\pgfplots@TMP{\pgfplots@@min}%
	\xdef\pgfplots@TMPB{\pgfplots@@max}%
	\endgroup
...
%\pgfplots@message{ -> #1 = \pgfplots@TMP : \pgfplots@TMPB;}%
	\else
		\global\let\pgfplots@TMP=\pgfplots@@min%
		\global\let\pgfplots@TMPB=\pgfplots@@max%
	\fi
	\endgroup
	\expandafter\global\expandafter\let\csname pgfplots@#1min\endcsname=\pgfplots@TMP
	\expandafter\global\expandafter\let\csname pgfplots@#1max\endcsname=\pgfplots@TMPB
...
\def\pgfplots@create@axis@descriptions{%
	...
	\fi
	%
	\pgfkeysgetvalue{/pgfplots/title}\pgfplots@TMP
	\ifx\pgfplots@TMP\pgfutil@empty
	\else
		\expandafter\pgfplots@show@title\expandafter{\pgfplots@TMP}%
	\fi
