...
% log=1
	...
	/pgfplots/error bars/fixed/.code=				\def\pgfplots@errorbars@fixed{#1}\def\pgfplots@errorbars@mode{0},
	/pgfplots/error bars/fixed relative/.code=		\def\pgfplots@errorbars@rel{#1}\def\pgfplots@errorbars@mode{1},
	/pgfplots/error bars/explicit/.code=			\def\pgfplots@errorbars@mode{2},
	/pgfplots/error bars/explicit relative/.code=	\def\pgfplots@errorbars@mode{3},
	/pgfplots/error bars/fixed=0,
	/pgfplots/error bars/x/.is choice,
	/pgfplots/error bars/x/none/.code=				\def\pgfplots@errorbars@xmode{0},
	/pgfplots/error bars/x/plus/.code=				\def\pgfplots@errorbars@xmode{1},
	/pgfplots/error bars/x/minus/.code=				\def\pgfplots@errorbars@xmode{2},
	/pgfplots/error bars/x/both/.code=				\def\pgfplots@errorbars@xmode{3},
	/pgfplots/error bars/x=none,
	/pgfplots/error bars/y/.is choice,
	/pgfplots/error bars/y/none/.code=				\def\pgfplots@errorbars@ymode{0},
	/pgfplots/error bars/y/plus/.code=				\def\pgfplots@errorbars@ymode{1},
	/pgfplots/error bars/y/minus/.code=				\def\pgfplots@errorbars@ymode{2},
	/pgfplots/error bars/y/both/.code=				\def\pgfplots@errorbars@ymode{3},
	/pgfplots/error bars/y=none,
}
...
\long\def\pgfplots@addplotimpl@expression#1#2;{%
	...
	\pgfplots@path@enqueue{\draw[#1] plot #2;}%
	\pgfplots@end@plot
}%
...
%\pgfplots@message{processing plots coords with trailing path '#3'}%
	...
	\let\pgfplots@errorbar@paths=\relax
	\ifnum\pgfplots@errorbars@xmode=0
		\ifnum\pgfplots@errorbars@ymode=0
		\else
			\pgfplots@errorbars@enabledtrue
		\fi
	\else
		\pgfplots@errorbars@enabledtrue
	\fi
	\ifpgfplots@filtercoords
		...
		\ifpgfplots@errorbars@enabled
			\expandafter\pgfplots@path@enqueue\expandafter{\pgfplots@errorbar@paths}%
		\fi
		\def\pgfplots@TMP{\pgfplots@path@enqueue@coords{\draw[every axis plot,#1]}}%
		\expandafter\pgfplots@TMP\pgfplots@filteredcoordinateplotcmd #3;%
	\else
		...
		\ifpgfplots@errorbars@enabled
			\expandafter\pgfplots@path@enqueue\expandafter{\pgfplots@errorbar@paths}%
		\fi
		\pgfplots@path@enqueue@coords{\draw[every axis plot,#1]} plot coordinates {#2}#3;%
	\fi
	\pgfplots@end@plot
...
\def\pgfplots@update@errorbar@paths#1#2#3#4{%
}
...
\def\pgfplots@filter@one@coord@pair#1#2#3#4{%
	...
	\else
		...
		\else
			\ifpgfplots@errorbars@enabled
				\pgfplots@update@errorbar@paths{\pgfplots@filter@tmp@x}{\pgfplots@filter@tmp@y}{#3}{#4}%
			\fi
			\ifpgfplots@draw@at@end
...
\long\def\pgfplots@apply@data@scaletrafo@to@plot@data plot coordinates #1#2;\to#3{%
	\let#3=\pgfutil@empty
...
% Remembers the plotting command #1 and '#2=plot coordinates {...} ...';
...
%
% #2: see above.
\long\def\pgfplots@path@enqueue@coords#1#2;{%
	\ifpgfplots@draw@at@end
...
\long\def\pgfplots@path@enqueue#1{%
	\ifpgfplots@draw@at@end
		\pgfplotslistpushback{}\to\pgfplots@stored@plot@cmds
		\pgfplotslistpushback{#1}\to\pgfplots@stored@plot@data
	\else
		#1%
	\fi
}
...
\newif\ifpgfplots@checkuniform@two@are@equal
...
\def\pgfplots@checkisuniformLOGtick#1{%
	...
	\foreach \x in {#1}{%
		...
		\else
			...
			\pgfmathapproxequaltomacro\pgfmathresult{1.0}{pgfplots@checkuniform@two@are@equal}%
			\ifpgfplots@checkuniform@two@are@equal
			\else
