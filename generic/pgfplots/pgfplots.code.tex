...
\def\pgfplots@show@label#1#2{%
	\begingroup
	% declare the 'ticklabel cs' for the current axis.
	% It takes 1-2 arguments where the first is the coordinate on the
	% ticklabel axis. The second is an optional shift orthogonal to
	% the axis in direction of the outer normal. The minimum shift is
	% the largest' ticklabels dimension.
	\tikzdeclarecoordinatesystem{ticklabel}{%
		\pgfutil@in@{,}{##1}%
		\ifpgfutil@in@
			\edef\pgfplots@loc@TMPa{##1}%
		\else
			\edef\pgfplots@loc@TMPa{##1,0pt}%
		\fi
		\def\pgfplots@loc@TMPb####1,####2\relax{%
			% invoke
			% \pgfplotsqpointoutsideofticklabelaxisrel{#1}{####1}{ticklabel dimen + ####2}:
			\begingroup
				\afterassignment\pgfplots@gobble@until@relax
				\pgf@xa=####2pt\relax
				\advance\pgf@xa by\pgfplotsvalueoflargesttickdimen{#1} %<- keep this space!
	...
	\pgfplotsdeclareborderanchorforticklabelaxis{#1}{ticklabel}%
	\node 
...
\iffalse
		...
		%
		% Make sure the API point commands are still correct:
		% -> undo the shift for all point commands!
		%  It is enough to do that for some low level commands since
		%  others are build on top of them.
		\let\pgfplotspointbblowerleft@orig=\pgfplotspointbblowerleft
		\let\pgfplotspointbblowerleft=\pgfpointorigin
		%
		\let\pgfplotspointaxisorigin@orig=\pgfplotspointaxisorigin
		\def\pgfplotspointaxisorigin{%
			\pgfpointdiff
				{\pgfplotspointbblowerleft@orig}%
				{\pgfplotspointaxisorigin@orig}%
		}%
		%
		...
		%
		% To allow \pgfplotsqpointxy and \pgfplotsqpointxyz here,
		% I need to store the unit vectors before I overwrite them,
		% then I need to re-implement \pgfplotsqpointxy[z].
		\edef\pgfplots@xx{\the\pgf@xx}%
		\edef\pgfplots@xy{\the\pgf@xy}%
		\edef\pgfplots@yx{\the\pgf@yx}%
		\edef\pgfplots@yy{\the\pgf@yy}%
		\edef\pgfplots@zx{\the\pgf@zx}%
		\edef\pgfplots@zy{\the\pgf@zy}%
		\def\pgfplotsqpointxyz##1##2##3{%
			\pgfpointdiff
				{\pgfplotspointbblowerleft@orig}%
				{%
					\pgfplots@tmpa=\pgfplots@xx\relax
					\pgf@x=##1\pgfplots@tmpa
					\pgfplots@tmpa=\pgfplots@yx\relax
					\advance\pgf@x by##2\pgfplots@tmpa
					\pgfplots@tmpa=\pgfplots@zx\relax
					\advance\pgf@x by##3\pgfplots@tmpa
					%
					\pgfplots@tmpa=\pgfplots@xy\relax
					\pgf@y=##1\pgfplots@tmpa
					\pgfplots@tmpa=\pgfplots@yy\relax
					\advance\pgf@y by##2\pgfplots@tmpa
					\pgfplots@tmpa=\pgfplots@zy\relax
					\advance\pgf@y by##3\pgfplots@tmpa
				}%
		}%
		\def\pgfplotsqpointxy##1##2{\pgfplotsqpointxyz{##1}{##2}{0}}%
		%
...
\fi
		%
		\pgfplotspointbbdiagonal
		\edef\pgfplots@install@description@xyzvec{%
			\pgf@xx=\the\pgf@x\space
			\pgf@xy=0pt
			\pgf@yx=0pt
			\pgf@yy=\the\pgf@y\space
			\pgf@zx=0pt
			\pgf@zy=0pt
		}%
		%
