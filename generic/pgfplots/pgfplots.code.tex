...
% log=1
	...
	/pgfplots/error bars/error mark/.store in=				\pgfplots@errorbars@mark,
	/pgfplots/error bars/error mark={-},
	/pgfplots/error bars/error mark options/.store in=		\pgfplots@errorbars@markopts,
	/pgfplots/error bars/error mark options={rotate=90},
...
%\message{/pgfplots/error bars/draw error bar:  working with '#1' -- '#2'.}%
		\draw #1 -- #2 node[pos=1,sloped,allow upside down] {%
			\expandafter\tikz\expandafter[\pgfplots@errorbars@markopts]{%
				\expandafter\pgfuseplotmark\expandafter{\pgfplots@errorbars@mark}%
				\pgfusepath{stroke}}%
		};
	},
}
...
\long\def\pgfplots@PREPARE@COORD@STREAMS#1#2{%
	...
	\else
		\ifpgfplots@errorbars@enabled
			\pgfplots@error{Sorry, the case pgfplots@draw@at@end =false and error bars = true not yet implemented FIXME}%
		\fi
...
%\pgfplots@message{processing plots coords with trailing path '#3'}%
	...
	% \fi
	% \ifpgfplots@filtercoords
	% 	\pgfplots@coord@stream@INIT@process@filtered\pgfplots@filteredcoordinateplotcmd
	% 	\pgfplots@coord@stream@foreach{#2}%
	% %	\pgfplots@processinputcoords@filtered plot coordinates {#2};\newplotcmdto\pgfplots@filteredcoordinateplotcmd
	% 	\ifpgfplots@errorbars@enabled
	% 		\pgfplots@streamerrorbarend
	% 	\fi
	% 	% Idea: use
	...
	% 	% coordinates. Unfortunately, it is NOT sufficient to use
	% 	% \tikzset{#1}
	% 	\pgfplotslist@TOK@a={every axis plot,#1}%
	% 	\ifx\pgfplots@recordederrorbar\pgfutil@empty
	% 		\edef\pgfplots@TMP{%
	% 			\noexpand\pgfplots@addplot@enqueue@coords
	% 			{
	...
	% 			}%
	% 			{%
	% 				\noexpand\draw[every axis plot,#1]%
	% 			}%
	% 		}%
	% 	\else
	% 		\pgfplotslist@TOK@b=\expandafter{\pgfplots@recordederrorbar}%
	...
	% 		}%
	% 	\fi
	% 	\expandafter\pgfplots@TMP\expandafter{\pgfplots@filteredcoordinateplotcmd #3;}%
	% 		{}%
	% 		{every axis plot,#1}%
	% \else
	% 	\ifpgfplots@errorbars@enabled
	...
	% 	\fi
	% 	\ifpgfplots@draw@at@end
	% 		\pgfplots@update@limits{plot coordinates {#2};}%
	% 	\fi
	% 	\pgfplotslist@TOK@a={every axis plot,#1}%
	...
	% 			\noexpand\tikzstyle{current plot style}=[\the\pgfplotslist@TOK@a]%
	% 		}%
	% 		{\draw[every axis plot,#1]}{ plot coordinates {#2}#3;}{}{every axis plot,#1}%
	% \fi
	% \pgfplots@end@plot
	%-------------------------------------------------- 
...
%	\def\pgfplots@stored@current@style{#5}%
}
