...
\def\pgfplots@drawticklabels@onorientedsurf@prepareanchor#1{%
	...
	\or
		...
		%
		% step 1: 
		%    define \pgfplotspointonorientedsurfbN :
		\ifpgfplots@threedim
			...
			\fi
		\else
			% z == (0,0) anyway. Use that here.
			\if x\pgfplotspointonorientedsurfaceB
				% N == z:
				\def\pgfplotspointonorientedsurfbN##1##2{\pgfplotsqpointxy{##1}{0}}%
			\else
				% B == y,
				% N == z
				\def\pgfplotspointonorientedsurfbN##1##2{\pgfplotsqpointxy{0}{##1}}%
			\fi
		\fi
		%
		% 
		...
		%
		% Step 2: determine how to place tick labels OUTSIDE:
		%
		% Prepare auto anchors and the 'tick distance' feature:
		% FIXME : this here is NOT fool proof. I suppose it works
		% reasonably for the default tick placement...
		% I am not really sure WHY it works.
		\if0\pgfplots@ticklabelside
			% this means : the 'b' axis of the surface is on the lower axis
			% limit. Since I need a vector pointing to the INSIDE of
			% the axis, I need sign = +1
			\def\pgfplots@loc@baxissign{+}%
		\else
			% in this case, the INSIDE area requires a minus sign - the b
		...
		\pgfplotsmath@ifzero{\csname pgfplots@\pgfplotspointonorientedsurfaceB @veclength\endcsname}{%
			...
			\edef\pgfplots@loc@baxisscale{\pgfplots@loc@baxissign\csname pgfplots@\pgfplotspointonorientedsurfaceB @inverseveclength\endcsname}%
		}%
		\ifpgfplots@threedim
			\pgfplotsmath@ifzero{\csname pgfplots@\pgfplotspointonorientedsurfaceN @veclength\endcsname}{%
				...
				\else
					\def\pgfplots@loc@naxissign{-}%
				\fi
				\edef\pgfplots@loc@naxisscale{\pgfplots@loc@naxissign\csname pgfplots@\pgfplotspointonorientedsurfaceN @inverseveclength\endcsname}%
			}%
		\else
			% z == (0,0) anyway. Use that here.
			\def\pgfplots@loc@naxissign{0}%
		...
		\fi
		%
		% Ok, we want to compute (a+b)/2 . BUT: if a==0 or b==0, we
		...
		\pgfplotspointonorientedsurfbN{\pgfplots@loc@baxisscale}{\pgfplots@loc@naxisscale}%
