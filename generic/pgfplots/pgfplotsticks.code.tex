...
\def\pgfplots@drawticklabels@onorientedsurf@#1{%
	...
	\else
		\pgfplots@if{pgfplots@#1majorticks}{%
			...
			%
			% FIXME: instead of styles, I could set them directly.
			% Whould be faster.
...
\def\pgfplots@drawticklabels@onorientedsurf@prepareanchor#1{%
	\ifcase\csname pgfplots@ticklabel@anchor@#1\endcsname%
		% 0: doesn't happen.
		\pgfplots@error{Internal logic error: #1ticklabel anchor=auto should be processed already!}%
	\or
		% 1: paraxial.
		...
		\else
			...
			\if x\pgfplotspointonorientedsurfaceB
				% N == z:
				\def\pgfplotspointonorientedsurfbN##1##2{\pgfplotsqpointxy{##1}{0}{0}}%
			\else
				% B == y,
				% N == z
				\def\pgfplotspointonorientedsurfbN##1##2{\pgfplotsqpointxyz{0}{##1}{0}}%
			\fi
		\fi
		%
		% 
		...
		%
		% Step 2.2: the 'tick distance' feature:
		...
		\pgfplotsmath@ifzero{\csname pgfplots@\pgfplotspointonorientedsurfaceB @veclength\endcsname}{%
			...
			\edef\pgfplots@loc@TMPa{\pgfplots@loc@TMPa\csname pgfplots@\pgfplotspointonorientedsurfaceB @inverseveclength\endcsname}%
		}%
		\ifpgfplots@threedim
			\pgfplotsmath@ifzero{\csname pgfplots@\pgfplotspointonorientedsurfaceN @veclength\endcsname}{%
				...
				\else
					\def\pgfplots@loc@TMPb{}%
				\fi
				\edef\pgfplots@loc@TMPb{\pgfplots@loc@TMPb\csname pgfplots@\pgfplotspointonorientedsurfaceN @inverseveclength\endcsname}%
			}%
		\else
			% z == (0,0) anyway. Use that here.
			\def\pgfplots@loc@TMPb{0}%
		...
		\fi
		\if\pgfplots@loc@TMPb0%
			\def\pgfplots@loc@TMPc{1}%
		\fi
		\if\pgfplots@loc@TMPc0%
			% ok, we can divide by 2.
			\pgfmathmultiply@
				{\pgfkeysvalueof{/pgfplots/#1ticklabel dist}}
				{0.5}%
			\let\pgfplots@loc@TMPc=\pgfmathresult
		\else
			\pgfkeysgetvalue{/pgfplots/#1ticklabel dist}\pgfplots@loc@TMPc
		\fi
%\message{pgfplots@drawticklabels@onorientedsurf@prepareanchor{#1}: Installing a SHIFT TRANSFORMATION of \pgfplots@loc@TMPc * pgfplotspointonorientedsurfbN{\pgfplots@loc@TMPa}{\pgfplots@loc@TMPb} where b = \pgfplotspointonorientedsurfaceB; N = \pgfplotspointonorientedsurfaceN.\MessageBreak pgfplotspointonorientedsurfbN = \meaning\pgfplotspointonorientedsurfbN\MessageBreak}%
		\pgftransformshift{%
			\pgfpointscale
				{\pgfplots@loc@TMPc}
				{\pgfplotspointonorientedsurfbN{\pgfplots@loc@TMPa}{\pgfplots@loc@TMPb}}}%
		%
...
% Sets the values of '#1ticklabel anchor' to one of 'paraxial' (1) or
% 'tikz' (2).
% This does only change anything for the choice 'auto'.
%
% #1: one of [xyz]
\def\pgfplots@prepare@ticklabel@anchors@for#1{%
	\if\csname pgfplots@ticklabel@anchor@#1\endcsname0%
		...
		\else
			\expandafter\def\csname pgfplots@ticklabel@anchor@#1\endcsname{1}% non-orthogonal axis => paraxial
			\if1\pgfplots@tmp@xisaxisparallel%
				\if1\pgfplots@tmp@yisaxisparallel%
					\expandafter\def\csname pgfplots@ticklabel@anchor@#1\endcsname{2}% orthogonal axis => tikz
				\fi
			\fi
		\fi
	\fi
