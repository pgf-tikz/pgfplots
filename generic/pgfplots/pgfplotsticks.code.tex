...
\def\pgfplots@show@ticklabel@@#1#2{%
	\pgfinterruptboundingbox
		% Typeset the label!
		\begingroup
		\pgfkeysalso{/tikz/every node/.append code=\pgf@relevantforpicturesizetrue}%
		\pgf@process{#2}%
...
\def\pgfplots@prepare@tick@coordlists@for#1#2{%
	...
	\else
		...
		\foreach \x in {#2} {%
			...
			\pgfplots@checktickminmax
			%
\message{Checking #1 tick at `\the\pgfplots@tmpa' (min=\csname pgfplots@#1min\endcsname; max=\csname pgfplots@#1max\endcsname) ... }%
			\ifpgfplots@tickshow
