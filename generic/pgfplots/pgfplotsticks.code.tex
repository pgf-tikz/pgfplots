...
\def\pgfplots@show@ticklabel@@#1#2{%
	\pgfinterruptboundingbox
		...
		\begingroup
		\pgf@process{#2}%
		% Remember the position '#2':
		\edef\pgfplots@ticklabel@at@x{\the\pgf@x}%
		\edef\pgfplots@ticklabel@at@y{\the\pgf@y}%
		\pgftransformshift{}%<- simply take pgf@x and pgf@y
		%
		%
		% We collects its bounding box separately.
		%
		\node at (0pt,0pt) {\csname pgfplots@#1ticklabel\endcsname};%
		%
		\pgfplots@ticklabel@maxtickdimen@updateforcurrentpath{#1}{\pgf@x=\pgfplots@ticklabel@at@x\space\pgf@y=\pgfplots@ticklabel@at@y\space}%%
	...
	\begingroup
		\pgfplots@glob@TMPa
		\pgf@protocolsizes{\pgf@xa}{\pgf@xb}%
		\pgf@protocolsizes{\pgf@ya}{\pgf@yb}%
	\endgroup
}%
\def\pgfplots@ticklabel@maxtickdimen@reset#1{%
	\expandafter\gdef\csname pgfplots@maxtick@wd@#1\endcsname{0pt}%
	\expandafter\gdef\csname pgfplots@maxtick@ht@#1\endcsname{0pt}%
}%
...
\def\pgfplots@ticklabel@maxtickdimen@updateforcurrentpath#1#2{%
	\begingroup
\message{checking #1ticklabel pic dimen: MIN=(\the\pgf@picminx,\the\pgf@picminy) MAX = (\the\pgf@picmaxx,\the\pgf@picmaxy). }%
	\advance\pgf@picmaxx by-\pgf@picminx
	\advance\pgf@picmaxy by-\pgf@picminy
	\def\pgfplots@loc@islarger{0}%
	\ifdim\pgf@picmaxx>\csname pgfplots@maxtick@wd@#1\endcsname\relax
		\def\pgfplots@loc@islarger{1}%
	\else
		\ifdim\pgf@picmaxy>\csname pgfplots@maxtick@ht@#1\endcsname\relax
			\def\pgfplots@loc@islarger{1}%
		\fi
	\fi
	\if1\pgfplots@loc@islarger
		\expandafter\xdef\csname pgfplots@maxtick@wd@#1\endcsname{\the\pgf@picmaxx}%
		\expandafter\xdef\csname pgfplots@maxtick@ht@#1\endcsname{\the\pgf@picmaxy}%
	\fi
\pgfplotspointlargesttickdiagonal#1%
\message{The largest #1tick dimen is (\the\pgf@x,\the\pgf@y)}%
	\endgroup
}%

% A point command which yields a vector whose x and y coordinates are
% the largest width and heights of the largest tick label in direction
% #1. The term "largest" is measured in the maximum norm of its
% bounding box.
%
% If not all tick labels have been seen so far, only those which have
% been typeset are included in this coordinate.
% If no tick labels have been seen, the point (0,0) is returned.
%
% #1: either x,y or z. It denotes the axis for which the ticks are
% requested.
\def\pgfplotspointlargesttickdiagonal#1{%
	\pgf@x=\csname pgfplots@maxtick@wd@#1\endcsname\relax
	\pgf@y=\csname pgfplots@maxtick@ht@#1\endcsname\relax
}%
...
\def\pgfplots@drawgridlines@onorientedsurf@#1{%
	\pgfplots@if{pgfplots@hide@\pgfplotspointonorientedsurfaceB}{%
		\relax
	}{%
		\begingroup
...
\def\pgfplots@drawticklabels@onorientedsurf@#1{%
	...
	\else
		\pgfplots@if{pgfplots@#1majorticks}{%
			...
			\def\pgfplots@auto@anchor@mode{1}%
			\begingroup
		%	\pgfkeysalso{/tikz/anchor/.append code={\def\pgfplots@auto@anchor@mode{0}}}%
...
\def\pgfplots@drawticklabels@onorientedsurf@prepareanchor#1{%
	\if\csname pgfplots@ticklabel@anchor@#1\endcsname0%
		\ifpgfplots@threedim
			\expandafter\def\csname pgfplots@ticklabel@anchor@#1\endcsname{1}% 3D => paraxial
		\else
			\expandafter\def\csname pgfplots@ticklabel@anchor@#1\endcsname{1}% use the general paraxial mode!
			\if0\pgfplots@auto@anchor@mode
				\expandafter\def\csname pgfplots@ticklabel@anchor@#1\endcsname{2}% use only tikz mode.
			\fi
		\fi
	\fi
	...
	\or
		...
		%
		% Process anchors.
		% see \pgfplots@init@ticklabelaxisspecfor
		%
		% FIXME : perhaps this implementation here is better than the
		% auto-anchor thing below:
		%\edef\pgfplots@loc@TMPa{anchor=\csname pgfplots@#1ticklabel@suggested@anchor\endcsname}%
		%\expandafter\tikzset\expandafter{\pgfplots@loc@TMPa}%
		% ADVANTAGE of the fixed anchor: all labels appear on the same
		% line. 
		%
		% My ideas are as follows:
		% - the CENTER of tick labels should be on a line starting at
		%   the tick position in direction
		%   \pgfplots@loc@vector@to@outside.
		% - the ANCHOR should be on the boundary of the shape.
		%   
		% Implementation: create a new, temporary anchor and invoke
		% the respective 'border' anchor processing.
		% Unfortunately, this is relatively low-level and may not be
		% portable.
		%
		% 1. initialise the direction vector for tick label anchors:
		\pgfpointscale{-1}{\pgfplotspointouternormalvectorofticklabelaxis{#1}}%
		\edef\pgfplots@loc@vector@to@inside{\noexpand\pgf@x=\the\pgf@x\space\noexpand\pgf@y=\the\pgf@y\space}%
		% 2. init the anchors.
		\pgfkeysalso{/tikz/every node/.append code={%
				% define and use an "auto" anchor:
				\expandafter\def\csname pgf@anchor@\tikz@shape @pgfplotsautoanchor\endcsname{%
					\csname pgf@anchor@\tikz@shape @border\endcsname{\pgfplots@loc@vector@to@inside}%					
				}%
			},%
			/tikz/anchor=pgfplotsautoanchor%
		}%
		%
		% process the (optional) ticklabel distance:
		\begingroup
			\pgfkeysgetvalue{/pgfplots/#1ticklabel dist}\pgfmathresult
			\ifx\pgfmathresult\pgfutil@empty
			\else
				\afterassignment\pgfplots@gobble@until@relax
				\pgf@xa=\pgfkeysvalueof{/pgfplots/#1ticklabel dist}pt\relax
				% make sure we don't have it twice, see above:
...
%\message{pgfplots@drawticklabels@onorientedsurf@prepareanchor{#1}: Installing a SHIFT TRANSFORMATION of \pgfplots@loc@TMPc * pgfplotspointonorientedsurfbN{\pgfplots@loc@baxisscale}{\pgfplots@loc@naxisscale} where b = \pgfplotspointonorientedsurfaceB; N = \pgfplotspointonorientedsurfaceN. pgfplotspointonorientedsurfbN = \meaning\pgfplotspointonorientedsurfbN.}%
		\ifx\pgfplots@loc@TMPc\pgfutil@empty
		\else
			\pgftransformshift{%
				\pgfpointscale
					{\pgfplots@loc@TMPc}
					{\pgfplots@loc@vector@to@inside}%
			}%
		\fi
		%
	...
	\or
		\if1\pgfplots@auto@anchor@mode
			...
			% 'paraxial' implementation is much more general, however.
