...
\def\pgfplots@ticklabel@maxtickdimen@finish#1{%
	...
	\else
		...
		\endgroup
	\fi
\message{The largest #1tick dimen is `\pgfplotsvalueoflargesttickdimen#1'}%
}%
% 
% #1: the axis (x,y or z)
% #2: the normal vector
...
\pgfpathmoveto{#2}
\pgfpathlineto{\pgfplots@loc@ticklabel@bb@corner}
\pgfusepath{stroke}
	\pgfplotsscalarproductofvectors
		{\pgfpointnormalised\pgfplots@loc@vector@to@outside} %% FIXME the unit vector is NOT normalised!
		{\pgfpointdiff
			{#2}%
			{\pgfplots@loc@ticklabel@bb@corner}}%
\message{corner{#1} = \meaning\pgfplots@loc@ticklabel@bb@corner.}%
%\message{checking #1ticklabel pic dimen: MIN=(\the\pgf@picminx,\the\pgf@picminy) MAX = (\the\pgf@picmaxx,\the\pgf@picmaxy).  }%
\message{(n,(corner - pos) ) = \the\pgf@x. }%
