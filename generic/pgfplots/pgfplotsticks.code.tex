...
\def\pgfplots@ticklabel@maxtickdimen@updateforcurrentpath#1#2{%
	\pgfplotsscalarproductofvectors
		{\pgfpointnormalised\pgfplots@loc@vector@to@outside}%
		{\pgfpointdiff
			{#2}%
...
%
% It does also include the value of the 'ticklabel dist' key.
%
...
%\message{pgfplots@drawticklabels@onorientedsurf@prepareanchor{#1}: Installing a SHIFT TRANSFORMATION of \pgfplots@loc@TMPc * pgfplotspointonorientedsurfbN{\pgfplots@loc@baxisscale}{\pgfplots@loc@naxisscale} where b = \pgfplotspointonorientedsurfaceB; N = \pgfplotspointonorientedsurfaceN. pgfplotspointonorientedsurfbN = \meaning\pgfplotspointonorientedsurfbN.}%
	...
	\or
		\if1\pgfplots@auto@anchor@mode
			...
			\fi
		\fi
		\pgfplots@ticklabel@maxtickdimen@prepare@for@normalvec{#1}{\pgfplotspointouternormalvectorofticklabelaxis{#1}}%
	\fi
}%
...
%\message{Got T^{-1}(H#1) = \Hmacro}%
		...
		\else
			...
			\pgfmathmultiply@{\Hmacro}{\csname pgfplotsmath@log@#1@logten@inv\endcsname}%
...
%\message{final H=\pgf@sys@tonumber{\H} * log(10)}%
			\H=\csname pgfplotsmath@log@#1@logten\endcsname\H\relax
			...
			\MIN=\csname pgfplotsmath@log@#1@logten@inv\endcsname\MIN\relax
			...
			\ifdim\MIN<0pt
				...
				\advance\c@pgf@counta by-1
			\fi
			\MIN=\csname pgfplotsmath@log@#1@logten\endcsname pt
			...
			\else
				...
				% 
				\ifdim\MINH<0pt
					\advance\MIN \csname pgfplotsmath@log@#1@logten\endcsname pt
				\fi
			\fi
		\fi
		\MINH=\MIN
		...
		\ifdim\MAX<\pgfplots@tmpa
			...
			\H=\MAX
			\advance\H by-\MIN
			\H=\csname pgfplotsmath@log@#1@logten@inv\endcsname\H
...
%\message{determined step size 10^{\Hmacro}}%
			...
			\MIN=\csname pgfplotsmath@log@#1@logten\endcsname\MIN\relax
			\H=\csname pgfplotsmath@log@#1@logten\endcsname\H\relax
			\MINH=\MIN\relax
...
% processed. The anchor will be named '#2'. It is placed on the
% intersection of the node's border and a line in direction of the
% tick label axis' inwards normal vector of the #1 ticklabel axis.
%
% #1: either x,y or z
% #2: an achor name.
\def\pgfplotsdeclareborderanchorforticklabelaxis#1#2{%
	%
	% Define it inside of 'every node' - hoping this yields the
	% correct shape. 
	\pgfkeysalso{/tikz/every node/.append code={%
			% define "#2" anchor for the current shape:
			\expandafter\def\csname pgf@anchor@\tikz@shape @#2\endcsname{%
				\csname pgf@anchor@\tikz@shape @border\endcsname{%
					\pgf@process{%
						%
						...
						% everything is fine.
						\pgftransformreset
						% FIXME : which transformation matrix needs to be
						...
						\pgfqpointscale{-1}{\pgfplotspointouternormalvectorofticklabelaxis{#1}}%
						%
