		...
		\E\else
			\if\pgfplots@unbounded@handler d% unbounded coords=discard
				\ifpgfplots@warn@for@filter@discards
					\E\pgfplots@message{NOTE: coordinate (\E\pgfplots@current@point@x@unfiltered,\E\pgfplots@current@point@y@unfiltered\ifpgfplots@curplot@threedim,\E\pgfplots@current@point@z@unfiltered\fi) has been dropped because of the \E\pgfplots@unbounded@dir-coordinate filter.}%
				\fi
			\else
...
\def\pgfplots@PREPARE@COORD@STREAM@start@{%
	...
	\or
		...
		% information, no math.
		\def\pgfplots@set@perpointmeta{}%
	% the 'point meta/expr cases is handled below.
	\fi
	% check 
...
\def\pgfplots@PREPARE@COORD@STREAM@end@determinecoordsorting#1{%
	\pgfplots@if{pgfplots@#1islinear}{%
		\pgfmathfloatlessthan@
			{\pgfplots@currentplot@firstcoord@x}{\pgfplots@currentplot@lastcoord@x}%
	}{%
		...
		\def\pgfplots@perpointmeta@unboundedwarning##1{%
			\ifx\pgfplots@perpointmeta@unboundedwarning@stop\relax
				\begingroup
					...
					\pgfplots@warning{The per point meta data `\pgfmathresult' (and probably others as well) is unbounded - using the minimum value instead.}%
				\endgroup
...
\def\pgfplots@perpointmeta@trafo@APPLY{%
	\ifx\pgfplots@current@point@meta\pgfutil@empty%
		\pgfplots@error{could not access the data source for color maps. Maybe you need to add '\string\addplot\space plot[point meta=y]' or something like that?}%
		\pgfmathfloatcreate{1}{1.0}{0}%
...
\def\pgfplots@addplotimpl@function@opt@#1#2#3#4#5{%
	\pgfplots@start@plot@with@behavioroptions{#1,#2,#3,/pgfplots/mesh/ordering/rowwise}%
	%
	...
	\if\pgfplots@unbounded@handler j% unbounded coords=jump
		...
		\edef\pgfplots@coord@stream@coord@{%
			\noexpand\ifx\noexpand\pgfplots@current@point@x\noexpand\pgfutil@empty
				\noexpand\pgfplots@plot@handler@visualize@jump
			\noexpand\else
				\the\t@pgfplots@tokb
			\noexpand\fi
		}%
...
\def\pgfplots@plot@handler@visualize@jump{%
	\pgfplotstreamend
	\pgfplotstreamstart
}%
...
\def\pgfplots@apply@zbuffer#1{%
	...
	\or
		...
		\else
			...
			\fi
		\fi
\message{z buffer=auto mode chose  z buffer=%
	\ifcase\pgfplotsplothandlermesh@zbuffer@choice 
		NONE
	\or
		reverse x seq
	\or
...
% Defines \pgfplotsretval (globally) to be a partial reversion of the
...
% collected into \pgfplotsretval.
%
% This implements the 'z buffer=reverse x seq' feature for
% ordering=rowwise and, likewise, the same for reverse y seq and
% ordering=colwise.
%
% PRECONDITION:
% 	\pgfplotsscanlinelength contains the scanline length
% POSTCONDITION:
% 	\pgfplotsretval contains the partial reversion.
\def\pgfplots@apply@zbuffer@reversescanline#1{%
	...
	\def\pgfplots@coord@stream@end{%
		...
		\ifx\pgfplots@loc@TMPa\pgfutil@empty
		\else
			\pgfplots@error{An internal error occured during z buffer reorderings: the rows/cols where not balanced!}%
		\fi
		\pgfplotsapplistXlet\pgfplots@loc@TMPa={\resultlist}%
		\global\let\pgfplotsretval=\pgfplots@loc@TMPa
	}%
	\pgfplots@coord@stream@foreach@NORMALIZED{#1}%
%\message{I have performed partial reversion of '#1' and got     '\pgfplotsretval'!}%
	\endgroup
}%
