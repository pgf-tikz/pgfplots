...
\long\def\pgfplots@addplotimpl@expression@#1#2#3#4#5{%
	...
	% FIXME : that's wrong here! I need to 
	% provide \pgfmathresult IN THE CORRECT NUMBER FORMAT!
	...
	\else
		...
		%
		%
		% For log-axes, samples logarithmically (unless the user
		% provides its own sampling routine)
...
\def\pgfplots@addplotimpl@function@opt@#1#2#3#4#5{%
	\pgfplots@start@plot@with@behavioroptions{%
		...
		/tikz/domain/.estore in=\tikz@plot@domain,
		#2,#3}%
	...
	\else%
		...
		\def\pgfplots@plot@data{\pgfplotgnuplot[\pgfplots@plot@filename]{%
			...
			\ifpgfplots@curplot@threedim 
				splot [x=\tikz@plot@domain] [y=\pgfplots@plot@ydomain] #4;%
			\else
				plot [x=\tikz@plot@domain] #4;%
			\fi
			}}%
	\fi%
