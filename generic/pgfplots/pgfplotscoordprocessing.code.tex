...
% which is moved in the direction of the outer normal vector. 
%
...
% axis cube.
%
% @see \pgfplotspointoutsideofticklabelaxis
\def\pgfplotspointoutsideofaxis#1#2#3{%
	\def\pgfplotspointoutsideofaxis@trafo##1##2{\csname pgfplotstransformcoordinate##1\endcsname{##2}}%
	\let\pgfplotspointoutsideofaxis@addtolimit=\pgfplotspointoutsideofaxis@addtolimit@
	\edef\pgfplots@loc@TMPa{#1}%
	\expandafter\pgfplotspointoutsideofaxis@\pgfplots@loc@TMPa\relax{#2}{#3}%
}%

% A variant of \pgfplotspointoutsideofaxis which accepts transformed
% values for '#2' (i.e. any data transformations and logs are already
% applied).
\def\pgfplotspointoutsideofaxistransformed#1#2#3{%
	\def\pgfplotspointoutsideofaxis@trafo##1##2{\def\pgfmathresult{##2}}%
	\let\pgfplotspointoutsideofaxis@addtolimit=\pgfplotspointoutsideofaxis@addtolimit@
	\edef\pgfplots@loc@TMPa{#1}%
	\expandafter\pgfplotspointoutsideofaxis@\pgfplots@loc@TMPa\relax{#2}{#3}%
}%
...
% \pgfplotspointoutsideofaxis and its variants (and, in fact, it used
% the same code).
\def\pgfplotspointouternormalvectorofaxis#1{%
	\def\pgfplotspointoutsideofaxis@trafo##1##2{\def\pgfmathresult{##2}}%
	\def\pgfplotspointoutsideofaxis@addtolimit##1##2##3{\def\pgfmathresult{##3}}%
	\edef\pgfplots@loc@TMPa{#1}%
	\expandafter\pgfplotspointoutsideofaxis@\pgfplots@loc@TMPa\relax{0}{1pt}%
}%
\def\pgfplotspointoutsideofaxis@#1#2#3\relax#4#5{%
	\if v#1%
		\def\pgfplots@loc@point@orthogonal@to@v##1##2{%
			\pgfplotspointoutsideofaxis@trafo{x}{#4}\let\pgfplots@loc@TMPa=\pgfmathresult
			\pgfplotspointoutsideofaxis@addtolimit{y}{#2}{##1}\let\pgfplots@loc@TMPb=\pgfmathresult
			\pgfplotspointoutsideofaxis@addtolimit{z}{#3}{##2}\let\pgfplots@loc@TMPc=\pgfmathresult
	...
	\else
		\if v#2%
			\def\pgfplots@loc@point@orthogonal@to@v##1##2{%
				\pgfplotspointoutsideofaxis@trafo{y}{#4}\let\pgfplots@loc@TMPa=\pgfmathresult
				\pgfplotspointoutsideofaxis@addtolimit{x}{#1}{##1}\let\pgfplots@loc@TMPb=\pgfmathresult
				\pgfplotspointoutsideofaxis@addtolimit{z}{#3}{##2}\let\pgfplots@loc@TMPc=\pgfmathresult
		...
		\else
			\def\pgfplots@loc@point@orthogonal@to@v##1##2{%
				\pgfplotspointoutsideofaxis@trafo{z}{#4}\let\pgfplots@loc@TMPa=\pgfmathresult
				\pgfplotspointoutsideofaxis@addtolimit{x}{#1}{##1}\let\pgfplots@loc@TMPb=\pgfmathresult
				\pgfplotspointoutsideofaxis@addtolimit{y}{#2}{##2}\let\pgfplots@loc@TMPc=\pgfmathresult
	...
	%
	% Ok, we want to compute (b+n)/2 . BUT: if b==0 or n==0, we
	% don't want to divide by 2. Check that here:
	\def\pgfplots@loc@TMPc{0}%
	\if\pgfplots@loc@baxissign0%
		\def\pgfplots@loc@TMPc{1}%
	\fi
	\if\pgfplots@loc@naxissign0%
		\def\pgfplots@loc@TMPc{1}%
	\fi
	\begingroup
	\pgf@process{%
		\pgf@x=\pgfplots@loc@baxisscale pt%
		\pgf@y=\pgfplots@loc@naxisscale pt%
		%
		% parse dimen argument #5:
		% FIXME : use math parser for argument!?
		\afterassignment\pgfplots@gobble@until@relax
		\pgf@xa=#5pt\relax
		\edef\pgfplots@loc@TMPa{\pgf@sys@tonumber\pgf@xa}%
		\pgf@x=\pgfplots@loc@TMPa\pgf@x
		\pgf@y=\pgfplots@loc@TMPa\pgf@y
		%
		\if\pgfplots@loc@TMPc0%
			% ok, we need to divide by 2.
			\divide\pgf@x by2
			\divide\pgf@y by2
		\fi
	}%
	\endgroup
	\edef\pgfplots@loc@baxisscale{\pgf@sys@tonumber\pgf@x}%
	\edef\pgfplots@loc@naxisscale{\pgf@sys@tonumber\pgf@y}%
	\pgfplots@loc@point@orthogonal@to@v{\pgfplots@loc@baxisscale}{\pgfplots@loc@naxisscale}%
}%

% Helper method for \pgfplotspointoutsideofaxis and its variants.
% #1: an axis (x,y or z)
...
% 	2 == add nothing.
% #3: the value to add.
\def\pgfplotspointoutsideofaxis@addtolimit@#1#2#3{%
	\if#20%
		\pgfmath@basic@add@{\csname pgfplots@#1min\endcsname}{#3}%
	\else
		\if#21%
			\pgfmath@basic@add@{\csname pgfplots@#1max\endcsname}{#3}%
		\fi
	\fi
}%

% Just like \pgfplotspointoutsideofaxis, but this one here uses the
...
%
% @see \pgfplotspointoutsideofaxis
\def\pgfplotspointoutsideofticklabelaxis#1#2#3{%
	\pgfplotspointoutsideofaxis{\csname pgfplots@#1ticklabelaxisspec\endcsname}{#2}{#3}%
}%
\def\pgfplotspointoutsideofticklabelaxistransformed#1#2#3{%
	\pgfplotspointoutsideofaxistransformed{\csname pgfplots@#1ticklabelaxisspec\endcsname}{#2}{#3}%
}%
...
\def\pgfplotsscanlinecomplete{%
	\ifnum\pgfplots@scanlinelength>0
		...
		\else
			...
			\else
				% special marker which means 'inconsistent scan line
				% length found'
...
%\message{Prepared macro \string\pgfplots@process@one@point: {\meaning\pgfplots@process@one@point}}%
	...
	\def\pgfplots@coord@stream@end@{\pgfplots@PREPARE@COORD@STREAM@end@{#1}{#2}}%
}%
...
% invocation works.
\def\pgfplots@PREPARE@COORD@STREAM@end@#1#2{%
...
\def\pgfplots@autocomplete@meshkeys#1{%
	...
	\ifpgfplots@curplot@threedim
		...
		\if1#1%
			...
			\else
				\pgfplots@error{Error: the arguments of [mesh/rows=\pgfplotsplothandlermesh@rows,mesh/cols=\pgfplotsplothandlermesh@cols] assume  \the\c@pgfplots@scanlineindex\space points, but I got actually N = \pgfplotsplothandlermesh@numpoints\space points! The data matrix appears to be incomplete or overcomplete!?}%
			\fi
		\fi
	\else
...
\long\def\pgfplots@addplotimpl@expression@#1#2#3#4#5{%
	...
	\ifpgfplots@curplot@threedim
		...
		\else
			\ifnum\pgfplots@plot@samples@y=0
				\def\pgfplots@addplotimpl@expression@samplesLINE{1}%
			\fi
		\fi
	\else
	...
	\pgfplotsmathredeclarepseudoconstant{x}{\edef\pgfmathresult{\tikz@plot@var}}%
	\if0\pgfplots@addplotimpl@expression@samplesLINE
		% surface:
		\pgfplotsmathredeclarepseudoconstant{y}{\let\pgfmathresult=\y}%
	\else
		\pgfplotsmathredeclarepseudoconstant{y}{\pgfplots@noy@error\def\pgfmathresult{0.0}}%
	\fi
	% remember them here:
...
%
% The distinction between <\macr> and <filename> is done
% automatically.
%
...
%\message{executing MODE '\meaning\pgfplots@drawmodes'}%
		...
		\if\pgfplotsplothandlermesh@zbuffer@choice4%
			\if\pgfplotsplothandlermesh@shader2%
				\pgfplots@error{Sorry, 'shader=interp' does not support 'z buffer=sort'. If you continue now, 'shader=flat mean' will be used.}%
				\def\pgfplotsplothandlermesh@shader{1}%
			\fi
		\fi
		%
...
%\message{executing \meaning\pgfplots@drawoptions'}%
			...
			\or
				...
				%
				% This won't be used for 1d mesh plots, see above.
		...
		\if\pgfplotsplothandlermesh@ONEDIMMODE0
			...
			% Init memory:
			\c@pgfplots@scanlineindex=\pgfplotsplothandlermesh@cols\relax
			\advance\c@pgfplots@scanlineindex by1
