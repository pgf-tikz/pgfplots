...
%\message{Assembled update-limits macro to {\meaning\pgfplots@update@limits@for@one@point}}%
	...
	\xdef\pgfplots@process@one@point{%
		...
		%
		\ifpgfplots@xislinear
			% NOTE: this handline of unbounded coords in the case of
				...
				\ifpgfplots@warn@for@filter@discards
					\E\pgfplots@message{%
						...
						\E\ifx\E\pgfplots@unbounded@dir\E\pgfutil@empty
							of the \E\pgfplots@unbounded@dir-coordinate filter.
						\E\else
							it is unbounded.
						\E\fi
					}%
...
\def\pgfplots@addplotimpl@function@opt@#1#2#3#4#5{%
	...
	\else%
		...
		\ifpgfplots@curplot@threedim 
			\ifx\pgfplots@plot@ydomain\pgfutil@empty
				\pgfplots@error{Sorry, '\string\addplot3 gnuplot{}' can't be processed correctly because 'y domain' is empty.}%
...
\def\pgfplots@apply@zbuffer#1{%
	...
	\or
		...
		\else
			...
			\fi
		\fi
%\message{z buffer=auto mode chose  z buffer= \ifcase\pgfplotsplothandlermesh@zbuffer@choice NONE \or reverse x seq \or reverse y seq \or reverse xy seq \or sort \fi}%
