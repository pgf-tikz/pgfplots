...
\def\pgfplotsifaxislineisonconvexhull#1#2#3{%
	...
	\if0\csname pgfplots@surfenabled@\pgfplots@loc@TMPb\endcsname
		\if1\csname pgfplots@surfenabled@\pgfplots@loc@TMPc\endcsname
			\pgfplots@loc@tmptrue
		\fi
	\fi
	\if1\csname pgfplots@surfenabled@\pgfplots@loc@TMPb\endcsname
		\if0\csname pgfplots@surfenabled@\pgfplots@loc@TMPc\endcsname
			\pgfplots@loc@tmptrue
		\fi
	\fi
...
%		\else
			...
			\xdef\pgfplots@glob@TMPa{%
				...
				{% precommand(s):
					...
					%
					% ... especially the rows/cols arguments which
					% might have been reported by the coord input
					% sources (from \pgfplots@validate@plot@domain@arguments):
					\ifpgfplots@curplot@threedim
					...
					\else
						\noexpand\pgfkeyssetvalue{/pgfplots/mesh/rows}{\pgfplots@current@point@coordindex}%
						\noexpand\pgfkeyssetvalue{/pgfplots/mesh/cols}{1}%
					\fi
					%
...
\long\def\pgfplots@addplotimpl@expression@#1#2#3#4#5{%
	...
	\else
		\let\pgfplots@loc@TMPa=\pgfutil@empty
	\fi
	\expandafter\pgfplots@start@plot@with@behavioroptions\expandafter{\pgfplots@loc@TMPa,#1,#2,/pgfplots/mesh/ordering/rowwise}%
	%
...
%\message{##\the\c@pgfplots@coordindex: prepare rectangle with corner \pgfplotsplothandlermesh@i@j.}%
	...
	\ifnum\c@pgfplots@coordindex>0
		\if\pgfplotsplothandlermesh@zbuffer@choice4%
			% z buffer=sort:
			\pgfplotsmathcomputeviewdepthfixed{\pgfplots@current@point@x}{\pgfplots@current@point@y}{\pgfplots@current@point@z}% FIXME relies on pgfplots macros
...
\def\pgfplots@apply@zbuffer@SORT@iflessthan@#1,#2,#3,#4;{%
	\pgfplotsmathcomputeviewdepth{#1}{#2}{#3}%
}%
...
%
% @see \pgfplotsmathcomputeviewdepthfixed
\def\pgfplotsmathcomputeviewdepth#1#2#3{%
...
% Similar to \pgfplotsmathcomputeviewdepth, but this always relies on
% fixed point arithmetics.
\def\pgfplotsmathcomputeviewdepthfixed#1#2#3{%
