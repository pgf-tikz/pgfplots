...
\message{pgfplots@plot@data = \meaning\pgfplots@plot@data'}%
	\else
		% Assemble a 
	...
	\pgfplots@coord@stream@start
	\pgfplots@plot@data{%
\message{running with (#3,\pgfplots@addplotimpl@expression@yEXPR,\ifpgfplots@curplot@threedim\pgfplots@addplotimpl@expression@zEXPR\fi).}%
...
\def\pgfplots@process@behavioroptions#1{%
	...
	\else
		...
		\pgfkeysinstallkeyfilter{/pgf/key filters/not}{/pgf/key filters/is descendant of=/tikz}%
		\pgfkeysinstallkeyfilterhandler{/pgf/key filter handlers/ignore}{}%
		\pgfkeysalso{/handler config=full or existing}%
		\pgfqkeysfiltered{/pgfplots}{#1}%
		\pgfplots@validate@plot@domain@arguments
		\edef\pgfplots@loc@TMPa{%
			/pgfplots/samples={\pgfplots@plot@samples},%
		...
		\edef\pgfplots@addplot@nonlegend@options{%
			\the\t@pgfplots@toka,\pgfplots@loc@TMPa}%
	\fi
}%
...
%		\scope
		...
		\ifx\pgfplotsplothandlermesh@rows\pgfutil@empty
			\ifx\pgfplotsplothandlermesh@cols\pgfutil@empty
				\pgfplots@error{Sorry, 'plot mesh' needs either 'plot[mesh/rows=<N>]' or 'plot[mesh/cols=<N>]' set - it didn't find either...}%
			...
			\else
				...
				\ifx\pgfplotsplothandlermesh@numpoints\pgfutil@empty
					\pgfplots@error{Sorry, 'plot mesh' needs a valid 'plot[mesh/num points=<N>]' variable if only one of 'plot[mesh/rows=<N>]' or 'plot[mesh/cols=<N>]' is given.}%
		...
		\else
			\ifx\pgfplotsplothandlermesh@cols\pgfutil@empty
				...
				\ifx\pgfplotsplothandlermesh@numpoints\pgfutil@empty
					\pgfplots@error{Sorry, 'plot mesh' needs a valid 'plot[mesh/num points=<N>]' variable if only one of 'plot[mesh/cols=<N>]' or 'plot[mesh/rows=<N>]' is given.}%
					\def\pgfplotsplothandlermesh@numpoints{100}%
				\fi
				\begingroup
				\c@pgf@counta=\pgfplotsplothandlermesh@numpoints\relax
		...
		\ifnum\pgfplotsplothandlermesh@cols=1
			\pgfplots@error{Sorry, mesh/cols=1 is not supported. I can't draw a mesh in this case.}%
		\fi
		\ifnum\pgfplotsplothandlermesh@rows=1
			\pgfplots@error{Sorry, mesh/rows=1 is not supported. I can't draw a mesh in this case.}%
		\fi
		\c@pgfplots@coordindex=\pgfplotsplothandlermesh@rows\relax
		\multiply\c@pgfplots@coordindex by\pgfplotsplothandlermesh@cols\relax
		\ifnum\c@pgfplots@coordindex=\pgfplotsplothandlermesh@numpoints\relax
		\else
			\pgfplots@error{Error: the arguments of plot[mesh/rows=\pgfplotsplothandlermesh@rows,mesh/cols=\pgfplotsplothandlermesh@cols] assumes \the\c@pgfplots@coordindex\space points, but I got actual N = \pgfplotsplothandlermesh@numpoints\space points! The data matrix appears to be incomplete...}%
		\fi
...
%				\def\pgfplotsplothandlermesh@setcolor####1{\pgfsetstrokecolor{####1}\pgfsetfillcolor{####1}}%
			\else
				\let\pgfplotsplothandlermesh@usepath=\pgfusepathqfill
%				\def\pgfplotsplothandlermesh@setcolor####1{\pgfsetfillcolor{####1}}%
			\fi
		\else
			\let\pgfplotsplothandlermesh@usepath=\pgfusepathqstroke
%			\def\pgfplotsplothandlermesh@setcolor####1{\pgfsetstrokecolor{####1}}%
		\fi
		\ifnum\pgfplots@perpointmeta@choice=0
...
\def\pgfplotsplothandlermesh@fillscanline#1{%
	\pgf@process{#1}%
	\edef\pgfplotsplothandlermesh@i@j{\noexpand\pgf@x=\the\pgf@x\noexpand\relax\noexpand\pgf@y=\the\pgf@y\noexpand\relax}%
	\expandafter\pgfplotsdequepushback\pgfplotsplothandlermesh@i@j\to{lastscanline}%
	\advance\c@pgfplots@coordindex by1
	\ifnum\c@pgfplots@coordindex=\pgfplotsplothandlermesh@cols\relax
		% second line!
...
%\message{##\the\c@pgfplots@coordindex: prepare rectangle with corner \pgfplotsplothandlermesh@i@j.}%
	...
	\ifnum\c@pgfplots@coordindex>0
		...
		\pgfpathmoveto{\pgfplotsplothandlermesh@im@jm}%
		\pgfpathlineto{\pgfplotsplothandlermesh@im@j}%
		\pgfpathlineto{\pgfplotsplothandlermesh@i@j}%
		\pgfpathlineto{\pgfplotsplothandlermesh@i@jm}%
		\pgfpathclose
