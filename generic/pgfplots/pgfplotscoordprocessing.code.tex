...
\def\pgfplots@addplotimpl@plot@withoptions#1[#2]{
	...
	%
	% Invoke this here as soon as the coordinate input type is known.
	% I want to redefine user interface keys accordingly (see below):
	\def\pgfplots@addplotimpl@INIT{\pgfplots@start@plot@with@behavioroptions{#2}}%
	%
	\pgfutil@ifnextchar c{%
		\pgfplots@addplotimpl@INIT
			...
			\pgfutil@ifnextchar t{%
				...
				\pgfutil@ifnextchar ({%
					...
					\pgfutil@ifnextchar\bgroup{%
						\pgfplots@addplotimpl@INIT
						\pgfplots@addplotimpl@expression@curly{#1}%
					}{%
						\pgfutil@ifnextchar e{%
							\pgfplots@addplotimpl@INIT
							\pgfplots@addplotimpl@expression@e{#1}%
						}{%
							\pgfutil@ifnextchar g{%
								\pgfplots@addplotimpl@gnuplot{#1}%
							}{%
...
\def\pgfplots@addplotimpl@f#1f{%
	\pgfutil@ifnextchar i{%
		\pgfplots@addplotimpl@INIT
		\pgfplots@addplotimpl@file{#1}%
	}{%
		% do NOT call \pgfplots@addplotimpl@INIT here!
		\pgfplots@addplotimpl@function{#1}%
	}%
}%
...
%
%
\long\def\pgfplots@addplotimpl@expression#1(#2,#3)#4;{\pgfplots@addplotimpl@expression@{#1}{#2}{#3}{#4}}%
% \addplot[#1] (#2,#3) #4;
...
% \addplot[#1] {#2} #4;
\long\def\pgfplots@addplotimpl@expression@curly#1#2#3;{\pgfplots@addplotimpl@expression@curly@{#1}{#2}{#3}}%
\long\def\pgfplots@addplotimpl@expression@curly@#1#2#3{%
	\pgfplots@gettikzinternal@keyval{variable}{tikz@plot@var}{\x}%
	\def\pgfplots@loc@TMPa{\pgfplots@addplotimpl@expression@{#1}}%
	\expandafter\pgfplots@loc@TMPa\expandafter{\tikz@plot@var}{#2}{#3}%
}%
% \addplot[#1] expression[#2] {#3} #4;
\def\pgfplots@addplotimpl@expression@e#1expression{%
	\pgfutil@ifnextchar[{%
		\pgfplots@addplotimpl@expression@e@{#1}%
	}{%
		\pgfplots@addplotimpl@expression@e@{#1}[]%
...
\def\pgfplots@addplotimpl@expression@e@#1[#2]{%
	\pgfplots@process@behavioroptions{#2}%
	\pgfplots@addplotimpl@expression@curly{#1}%
}%
...
\def\pgfplots@addplotimpl@gnuplot#1gnuplot{\pgfplots@addplotimpl@function{#1}unction}%
...
\def\pgfplots@addplotimpl@function#1unction{%
	% Overwrite the implementation of domain here!
	% Reason: 'plot function' allows MUCH higher data domains than
	% the standard implementation of 'domain'.
	%
	% That's an ugly hack, but it works at least.
	\pgfkeysdef{/tikz/domain}{\edef\tikz@plot@domain{##1}}%
	\pgfplots@addplotimpl@INIT
	%
	\pgfutil@ifnextchar[{%
...
\def\pgfplots@addplotimpl@function@opt#1[#2]#3#4;{\pgfplots@addplotimpl@function@opt@{#1}{#2}{#3}{#4}}%
% \addplot[#1] function[#2] {#3} #4;
\def\pgfplots@addplotimpl@function@opt@#1#2#3#4{%
	...
	\else%
		...
		\def\pgfplots@plot@data{\pgfplotgnuplot[\pgfplots@plot@filename]{%
			...
			\ifpgfplots@curplot@threedim 
				splot [x=\tikz@plot@domain] [y=\pgfplots@plot@ydomain] #3;%
			\else
				plot [x=\tikz@plot@domain] #3;%
			\fi
			}}%
	\fi%
...
\def\pgfplots@addplotimpl@file@opt#1[#2]#3#4;{\pgfplots@addplotimpl@file@opt@{#1}{#2}{#3}{#4}}%
...
\long\def\pgfplots@addplotimpl@table@fromstructure#1#2from#3#4;{\pgfplots@addplotimpl@table@fromstructure@{#1}{#2}{#3}{#4}}%
...
% #3: the argument of plot table{...}
% #4: trailing path arguments after plot table{...}#4;
\long\def\pgfplots@addplotimpl@table@fromfile#1#2#3#4;{\pgfplots@addplotimpl@table@fromfile@{#1}{#2}{#3}{#4}}%
...
\long\def\pgfplots@addplotimpl@coordinates#1plot coordinates#2#3;{\pgfplots@addplotimpl@coordinates@{#1}{#2}{#3}}%
% \addplot[#1] coordinates {#2} #3;
\long\def\pgfplots@addplotimpl@coordinates@#1#2#3{%
...

{
	...
	\long\def\pgfplots@addplotimpl@expression@AS#1(#2,#3)#4;{\pgfplots@addplotimpl@expression@{#1}{#2}{#3}{#4}}%
	\long\def\pgfplots@addplotimpl@expression@curly@AS#1#2#3;{\pgfplots@addplotimpl@expression@curly@{#1}{#2}{#3}}%
	\def\pgfplots@addplotimpl@function@opt@AS#1[#2]#3#4;{\pgfplots@addplotimpl@function@opt@{#1}{#2}{#3}{#4}}%
	\def\pgfplots@addplotimpl@file@opt@AS#1[#2]#3#4;{\pgfplots@addplotimpl@file@opt@{#1}{#2}{#3}{#4}}%
	\long\def\pgfplots@addplotimpl@table@fromstructure@AS#1#2from#3#4;{\pgfplots@addplotimpl@table@fromstructure@{#1}{#2}{#3}{#4}}%
	\long\def\pgfplots@addplotimpl@table@fromfile@AS#1#2#3#4;{\pgfplots@addplotimpl@table@fromfile@{#1}{#2}{#3}{#4}}%
	\long\def\pgfplots@addplotimpl@coordinates@AS#1plot coordinates#2#3;{\pgfplots@addplotimpl@coordinates@{#1}{#2}{#3}}%
	%
