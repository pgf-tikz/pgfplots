...
\def\pgfplotsconvertunittocoordinate#1#2{%
	\begingroup
	\pgf@xa=#2\relax
	\multiply\pgf@xa by\csname pgfplots@#1@inverseveclength\endcsname\relax
...
\def\pgfplotsqpointxyzabsolutesize#1#2#3{%
	\begingroup
	\pgf@xa=#1\relax
	\multiply\pgf@xa by\pgfplots@x@inverseveclength\relax
	\pgf@xb=#2\relax
	\multiply\pgf@xb by\pgfplots@y@inverseveclength\relax
	\pgf@ya=#3\relax
	\multiply\pgf@ya by\pgfplots@z@inverseveclength\relax
...
% is
%   n = v * [0 0 -1]' = - (siny sinp, -sinp cos y, cosp )'
% because the normal view point was the XY plane with -z as its normal
% vector.
...
%   are set properly
%   \pgfplots@view@normalvec@[xyz] will contain the three components
%   of 'n'.
\def\pgfplotssetaxesfrompitchyaw#1#2#3{%
	...
	%
	% n (3D!)
	\pgfmathmultiply@{-\siny}{\sinp}%
	\let\pgfmathresultNx=\pgfmathresult
	\pgfmathmultiply@{\sinp}{\cosy}%
	\xdef\pgfplots@glob@TMPd{{\pgfmathresultNx pt}{\pgfmathresult pt}{-\cosp pt}}%
	\endgroup
%\message{Setting x,y and z from {#1}{#2} to x = \meaning\pgfplots@glob@TMPa, y = \meaning\pgfplots@glob@TMPb, z = \meaning\pgfplots@glob@TMPc...n = \pgfplots@glob@TMPd.}%
	...
	\def\pgfplots@loc@TMPa##1##2##3{%
		\def\pgfplots@view@normalvec@x{##1}%
		\def\pgfplots@view@normalvec@y{##2}%
		\def\pgfplots@view@normalvec@z{##3}%
	}%
...
%\message{Assembled update-limits macro to {\meaning\pgfplots@update@limits@for@one@point}}%
	...
	\xdef\pgfplots@process@one@point{%
		...
		\E\pgfplotsutil@advancestringcounter\E\pgfplots@current@point@coordindex
	}%
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\endgroup
...
\long\def\pgfplots@addplotimpl@expression@#1#2#3#4#5{%
	\ifpgfplots@usefpu
		...
		% FIXME : enable or disable FPU in current plot style!?
		%\def\pgfplots@loc@TMPa{/pgf/fpu=true}%
		\let\pgfplots@loc@TMPa=\pgfutil@empty
	\else
		\let\pgfplots@loc@TMPa=\pgfutil@empty
	\fi
	\expandafter\pgfplots@start@plot@with@behavioroptions\expandafter{\pgfplots@loc@TMPa,#1,#2}%
	%
	\ifpgfplots@meshmode
		\ifpgfplots@curplot@threedim
			\pgfplots@PREPARE@COORD@STREAM{#1,#2,%
				/pgfplots/mesh/rows=\pgfkeysvalueof{/pgfplots/samples},%
				/pgfplots/mesh/cols=\pgfkeysvalueof{/pgfplots/samples}}%
				{#5}%
		\else
			% the mesh data will be communicated automatically for 1D
			% meshes.
			\pgfplots@PREPARE@COORD@STREAM{#1,#2}{#5}%
			%
		\fi
	\else
		\pgfplots@PREPARE@COORD@STREAM{#1,#2}{#5}%
	...
	\gdef\pgfplots@noy@error{%
		\pgfplots@error{Sorry, you can't use 'y' in this context. Please use the [mesh] option to indicate a twodimensional input domain.}%
	...
	\pgfutil@ifundefined{pgfmathdeclarefunction}{%
		...
		\else
			\def\pgfmath@parsefunction@y{\pgfplots@noy@error\def\pgfmathresult{0.0}\pgfmath@postfunction}%
		\fi
	}{%
		% We have PGF > 2.00 :
		...
		\else
			\pgfmathdeclarefunction{y}{0}{\pgfplots@noy@error\def\pgfmathresult{0.0}}%
		\fi
	}%
	%
...
\def\pgfplots@addplotimpl@function@opt@#1#2#3#4#5{%
	\pgfplots@start@plot@with@behavioroptions{#2,#3}%
...
\def\pgfplots@addplotimpl@graphics@@#1#2#3#4#5{%
	\pgfplots@start@plot@with@behavioroptions{#1,#2,/pgfplots/plot graphics/.cd,#3,/pgfplots/plot graphics/src={#4}}%
	\pgfplots@PREPARE@COORD@STREAM{#1,#2,/pgfplots/plot graphics,/tikz/mark=}{#5}%
	\pgfplots@coord@stream@start
...
%\message{In \string\addplot[#1]: I am silently ignoring key `\pgfkeyscurrentkeyRAW' during the preparation phase.}%
		...
		\edef\pgfplots@loc@TMPa{%
			/pgfplots/samples={\pgfplots@plot@samples},% FIXME : samples 2
			\ifpgfplots@meshmode
				\ifpgfplots@curplot@threedim
				\else
					/pgfplots/mesh/rows=\noexpand\pgfkeysvalueof{/pgfplots/mesh/num points},%
					/pgfplots/mesh/cols=1,%
				\fi
			\fi
			/pgfplots/samples at={\pgfplots@plot@samples@at},%
			/pgfplots/domain={\pgfplots@plot@domain}%
		}%
		\t@pgfplots@toka=\expandafter{\pgfplots@addplot@nonlegend@options}%
		\t@pgfplots@tokb=\expandafter{\pgfplots@loc@TMPa}%
		\edef\pgfplots@addplot@nonlegend@options{%
...
%\pgfplots@message{processing plots coords with trailing path '#4'}%
	\pgfplots@start@plot@with@behavioroptions{#1,#2}%
	\pgfplots@PREPARE@COORD@STREAM{#1,#2}{#4}%
	\ifpgfplots@curplot@threedim
...
\long\def\pgfplots@coord@stream@finalize@storedcoords@START normalized coordinates #1#2;\pgfplots@EOI{%
	\ifpgfplots@threedim
		\pgfplotsplothandlermesh@apply@zbuffer{#1}%
...
% b) the current set of options in /pgfplots/plot mesh/.
%
%
% ATTENTION:
...
%		\scope
		...
		\ifx\pgfplotsplothandlermesh@rows\pgfutil@empty
				...
				\else
					...
					\if\pgfplotslocretval1
						\pgfplots@warning{%
							...
							as sqrt(num points) = sqrt(\pgfplotsplothandlermesh@numpoints).}%
					\else
			...
			\else
				...
				\ifx\pgfplotsplothandlermesh@numpoints\pgfutil@empty
					\pgfplots@error{Sorry, 'plot mesh' needs a valid '[mesh/num points=<N>]' variable if only one of '[mesh/rows=<N>]' or '[mesh/cols=<N>]' is given.}%
		...
		\else
			\ifx\pgfplotsplothandlermesh@cols\pgfutil@empty
				...
				\ifx\pgfplotsplothandlermesh@numpoints\pgfutil@empty
					\pgfplots@error{Sorry, 'plot mesh' needs a valid '[mesh/num points=<N>]' variable if only one of '[mesh/cols=<N>]' or '[mesh/rows=<N>]' is given.}%
					\def\pgfplotsplothandlermesh@numpoints{100}%
				\fi
				\begingroup
				\c@pgf@counta=\pgfplotsplothandlermesh@numpoints\relax
		...
		\fi
		\ifx\pgfplotsplothandlermesh@numpoints\pgfutil@empty
			\c@pgfplots@coordindex=\pgfplotsplothandlermesh@rows\relax
			\multiply\c@pgfplots@coordindex by \pgfplotsplothandlermesh@cols\relax
			\edef\pgfplotsplothandlermesh@numpoints{\the\c@pgf@counta}%
		\fi
		...
		\c@pgfplots@coordindex=\pgfplotsplothandlermesh@rows\relax
		\multiply\c@pgfplots@coordindex by\pgfplotsplothandlermesh@cols\relax
		\ifnum\c@pgfplots@coordindex=\pgfplotsplothandlermesh@numpoints\relax
		\else
			\pgfplots@error{Error: the arguments of [mesh/rows=\pgfplotsplothandlermesh@rows,mesh/cols=\pgfplotsplothandlermesh@cols] assume  \the\c@pgfplots@coordindex\space points, but I got actually N = \pgfplotsplothandlermesh@numpoints\space points! The data matrix appears to be incomplete or overcomplete!?}%
		\fi
		%
		\if\pgfplotsplothandlermesh@ONEDIMMODE0
			% ok, normal 2D mesh processing:
			% Init memory:
			\c@pgfplots@coordindex=\pgfplotsplothandlermesh@cols\relax
			\advance\c@pgfplots@coordindex by1
			\pgfplotsdequenewempty{lastscanline}\capacity{\the\c@pgfplots@coordindex}%
			%
		...
		%
		% acuiqre the \iftikz@mode@fill \iftikz@mode@draw flags:
%\message{executing MODE '\meaning\pgfplots@drawmodes'}%
		% Prepare path usage commands:
		\pgfplots@drawmodes
		% Now, we also need to respect the shader rules - the
		% 'shader=interp' case is a little bit special here.
		%
		\iftikz@mode@fill
			% Rule 3: fill!=none and draw==none is forbidden. Always
			% stroke.
			\let\pgfplotsplothandlermesh@usepath=\pgfusepathqfillstroke
		\else
			\if\pgfplotsplothandlermesh@shader2
				% Rule 1: fill=none => shader=interp is forbidden.
				% Switch to 'flat mean'.
				\def\pgfplotsplothandlermesh@shader{1}%
			\fi
			\let\pgfplotsplothandlermesh@usepath=\pgfusepathqstroke
		\fi
		\iftikz@mode@draw
			% Rule 2: draw != none => shader must not be interp
			% because 'interp' does not support meshes.
			\if\pgfplotsplothandlermesh@shader2
				% Rule 1: fill=none => shader=interp is forbidden.
				% Switch to 'flat mean'.
				\def\pgfplotsplothandlermesh@shader{1}%
		...
		\if\pgfplotsplothandlermesh@ONEDIMMODE1
			% no filling in onedimensional mesh mode.
			\let\pgfplotsplothandlermesh@usepath=\pgfusepathqstroke
			\if\pgfplotsplothandlermesh@shader2
				\def\pgfplotsplothandlermesh@shader{1}%
			\fi
...
%\message{executing \meaning\pgfplots@drawoptions'}%
			...
			% Now, process the 'mesh/surf mode=corner|mean  key here:
...
\def\pgfplotsplothandlermesh@fillscanline#1{%
	...
	\advance\c@pgfplots@coordindex by1
	\ifnum\c@pgfplots@coordindex=\pgfplotsplothandlermesh@scanlinelength\relax
		% second line!
...
\def\pgfplotsplothandlermesh@draw#1{%
	...
	\pgf@process{#1}%
	\edef\pgfplotsplothandlermesh@i@j{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}}%
%\message{##\the\c@pgfplots@coordindex: prepare rectangle with corner \pgfplotsplothandlermesh@i@j.}%
	\pgfplotsdequepopfront{lastscanline}\to\pgfplotsplothandlermesh@im@j
	\expandafter\pgfplotsdequepushback\pgfplotsplothandlermesh@i@j\to{lastscanline}%
	\ifnum\c@pgfplots@coordindex>0
		\pgfpathmoveto{\expandafter\pgfplotsplothandlermesh@unpack\pgfplotsplothandlermesh@im@jm}%
		\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack\pgfplotsplothandlermesh@im@j}%
		\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack\pgfplotsplothandlermesh@i@j}%
		\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack\pgfplotsplothandlermesh@i@jm}%
...
\def\pgfplotsplothandlermesh@ONEDIM@draw#1{%
	...
	\ifnum\c@pgfplots@coordindex>0
		\pgfpathmoveto{\expandafter\pgfplotsplothandlermesh@unpack\pgfplotsplothandlermesh@i}%
		\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack\pgfplotsplothandlermesh@im}%
		\pgfplotsplothandlermesh@preparedrawforcurrent
		\pgfplotsplothandlermesh@usepath
	\fi
...
\def\pgfplotsplothandlermesh@apply@zbuffer#1{%
	\let\pgfplotsplothandlermesh@apply@zbuffer@start@BACKUP=\pgfplots@coord@stream@start
	\let\pgfplotsplothandlermesh@apply@zbuffer@end@BACKUP=\pgfplots@coord@stream@end
	\let\pgfplotsplothandlermesh@apply@zbuffer@coord@BACKUP=\pgfplots@coord@stream@coord
	\let\pgfplots@current@point@x=\pgfutil@empty
	\let\pgfplots@current@point@y=\pgfutil@empty
	\let\pgfplots@current@point@z=\pgfutil@empty
	\let\pgfplots@current@point@meta=\pgfutil@empty
	...
	\fi
	\let\pgfplots@coord@stream@start=\pgfplotsplothandlermesh@apply@zbuffer@start@BACKUP
	\let\pgfplots@coord@stream@end=\pgfplotsplothandlermesh@apply@zbuffer@end@BACKUP
	\let\pgfplots@coord@stream@coord=\pgfplotsplothandlermesh@apply@zbuffer@coord@BACKUP
}%
