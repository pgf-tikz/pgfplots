...
\pgfkeys{/pgfplots/array/iflessthan/.code args={#1#2#3#4}{\ifnum#1<#2\relax#3\else#4\fi}}%
...
\message{merge sort start=#2, n=#3.   }%
	\ifnum#3<2
		% ready!
	\else
		...
		\edef\pgfplotsarray@mergesort@@{%
			{#2}{\the\c@pgfplotsarraysort@i}% start1, N_1
			{\the\c@pgfplotsarraysort@m}{\the\c@pgfplotsarraysort@q}% start2, N_2
		}%
		\expandafter\pgfplotsarray@mergesort@recurse@\pgfplotsarray@mergesort@@
		%
		% Merge:
\message{Merge step for mergesort(start=#2,n=#3) running...}%
		% restore 'm':
		\c@pgfplotsarraysort@m=#3\relax
		\divide\c@pgfplotsarraysort@m by2
		\c@pgfplotsarraysort@q=#3
...
\message{direct comparisons ... looping!}%
		\pgfutil@loop
		\pgfplots@loop@CONTINUEtrue
...
\message{comparing \the\c@pgfplotsarraysort@j=\pgfplotsarray@x@mpl\space and}%
			\c@pgfplotsarraysort@j=\c@pgfplotsarraysort@k
			\advance\c@pgfplotsarraysort@j by#2
			\pgfplotsarrayselect{\c@pgfplotsarraysort@j}\of{#1}\to\pgfplotsarray@x@k
\message{\the\c@pgfplotsarraysort@j=\pgfplotsarray@x@k...}%
