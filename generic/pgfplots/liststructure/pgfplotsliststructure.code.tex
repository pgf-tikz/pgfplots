...
\def\pgfplotslistnew@backslash#1#2{%
...
\def\pgfplotslistset#1\of#2\to#3{%
	\ifcase#1\relax
		% change first:
	...
	\else
		...
		\ifnum\count1 =\count0
		\else
			\pgfplots@error{ERROR: \string\pgfplotslistset{\the\count0} failed because there are only \the\count1\space elements in the list. Use pushback to resize it.}%
		\fi
		\endgroup
