...
\testsection{Tests and Debugging of near ticklabel anchors}
{
	\pgfplotsset{
		...
		extra description/.add code={%
			...
			\fill[red] (xticklabel cs:0.5,0) circle (2pt);
			\fill[red] (yticklabel cs:0.5,0) circle (2pt);
			...
			\fill[blue] (xlabel.near xticklabel*nuranker) circle (2pt);
			\fill[blue] (ylabel.near yticklabel*nuranker) circle (2pt);
		},
	}
	\begin{tikzpicture}
		\begin{axis}[
	...
	\begin{tikzpicture}
		\begin{axis}[
			title={near [xyz]ticklabel NO STAR},
			xlabel=$x$ label,
			ylabel=$y$ label,
			xlabel style={anchor=near xticklabel},
			ylabel style={anchor=near yticklabel},
