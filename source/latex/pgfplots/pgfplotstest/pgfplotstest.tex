...
% This tests the command param 'pgfplotstestautoregression':
\expandafter\ifx\csname pgfplotstestautoregression\endcsname\relax	
	% it is not provided. Ok.
\else
	% AAAH -- enable regression tests by means of the pgfregressiontest.sty!
	\usepackage{pgfregressiontest}
	\pgfregressiontestset{
		rev checkout=cd $$OLDPWD && git checkout 1.4.1,
	}
	\tikzexternalize[
		mode=list and make,
