...
\testsubsection{Anchortest}
{
	...
	\testsubsection{Using anchors before axis is finished}
	{
		...
		\testsubsubsection{Reversed axes}
		{
			\pgfplotsset{x dir=reverse,y dir=reverse}
			\TESTPLOTS
		}

		\testsubsubsection{non-unit vectors}
		{
			\pgfplotsset{every axis/.append style={
					x={(30pt,9pt)},
					y={(-15pt,45pt)},
...
\testsection{sloped like XXX axis}
\begin{tikzpicture}
...
\testsection{xyshift in axis and contained nodes}
\begin{tikzpicture}
  \draw (0,0) rectangle (6,4);
  \begin{axis}[at={(0,0)}, anchor=outer south west, width=6cm, height=4cm, xlabel=$x$, ylabel=$y$, title={title}]
...
\begin{tikzpicture}
  \draw (0,0) rectangle (6,4);
  \begin{axis}[at={(2.5cm,0pt)}, anchor=outer south west, width=6cm, height=4cm, xlabel=$x$, ylabel=$y$, title={title}]
    \addplot function[domain={-3.1415:3.1415}]{sin(x)};
    \addlegendentry{$\sin(x)$}
  \end{axis}
\end{tikzpicture}
