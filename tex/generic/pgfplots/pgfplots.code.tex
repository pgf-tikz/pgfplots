...
\pgfdeclareshape{pgfplots@low@level@shape}{%
	...
	%
	\anchor{none}{%
		% this anchor is VERY special: it does not introduce any
		% further anchor. It does NOT work without special handling by
		% the axis.
		\pgfpointorigin
	}%
	%
...
\def\pgfplots@prepare@cell@picture{%
	...
	\ifx\pgfplots@loc@TMPa\pgfplots@loc@TMPb
		...
		% fact that we do not want cell pictures
...
\def\pgfplots@END@cell@picture{%
		...
		\begingroup
			...
			\fi%
			%
			\gloxdef\pgfplots@saveddimen@picminx{\the\pgf@picminx}%
...
\def\pgfplots@END@cell@picture@DISABLED{%
		...
		\begingroup
			...
			\fi%
			%
			\xdef\pgfplots@saveddimen@picminx{0pt}%
			\xdef\pgfplots@saveddimen@picminy{0pt}%
			%
			\ifpgfplots@deprecated@anchors
				\pgfplotspointminminmin
			\else
				\pgfplotspointbblowerleft
			\fi
			\xdef\pgfplotspointlowerleftinnercorner{\global\pgf@x=\the\pgf@x\space\global\pgf@y=\the\pgf@y\space}%
			%
			\global\pgf@x=\pgf@picmaxx
			\global\pgf@y=\pgf@picmaxy
			\global\advance\pgf@x by-\pgf@picminx
