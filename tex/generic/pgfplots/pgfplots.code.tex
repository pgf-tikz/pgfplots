...
\pgfkeys{%
	...
	/pgfplots/use layers/.initial=,
	/pgfplots/use layers/.default={axis background,main,axis foreground,unclipped axis foreground},
...
% 'axis' options:
	...
	% note that there is a special 'anchor=internal' which does not introduce cell pictures
...
% axis options:
	...
	/pgfplots/clip/.is choice,
	/pgfplots/clip/false/.code=	\def\pgfplots@clipmode{0},
	/pgfplots/clip/true/.code=	\def\pgfplots@clipmode{1},
	/pgfplots/clip/plots individually/.code=	\def\pgfplots@clipmode{2},
	/pgfplots/clip=true,
...
\def\pgfplots@prepare@layered@graphics{%
	\pgfkeysgetvalue{/pgfplots/use layers}\pgfplots@loc@TMPa
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
		% DEFAULT.
...
\def\pgfplots@prepare@layered@graphics@marks{%
	\ifpgfplots@layered@graphics
		...
		\def\pgfplots@loc@TMPb{auto}%
		\ifx\pgfplots@loc@TMPa\pgfplots@loc@TMPb
			% COMBINATION: 'use layers,mark layer=auto' --> keep
...
% The purpose of this method is to check for the (only) case where no cell picture is necessary: anchor=internal.
% The idea behind this anchor is to insert the axis DIRECTLY into the embedding tikzpicture, without
% a posteriori shifting. 
%
% A posteriori shifting == anchors. Thus, 'anchor=internal' effectively DISABLES shifts based on the axis' anchor.
% It will still prepare the anchors correctly, but it does not shift this axis (in other words: other node can align
% using the anchors of this axis, but this axis cannot be aligned based on its anchors).
%
% The motivation for this feature was to implement multi-axis support together with layered graphics.
% But the approach failed, I did not complete it:
% The problem is that the origin of an axis can be arbitrary; it depends strongly on the data scaling trafo. It may
% not even be part of the picture. Thus, one would need a data (!) shift transformation such that the lower left
% corner is (0,0) -- this would allow for multi-axis alignment without a cell picture.
%
% The 'anchor=internal' feature may be interesting for SVG export, however - so
% I decided to keep it here (it should not hurt).
\def\pgfplots@prepare@cell@picture{%
	\edef\pgfplots@loc@TMPa{\pgfplots@anchorname}%
	\def\pgfplots@loc@TMPb{internal}
	\ifx\pgfplots@loc@TMPa\pgfplots@loc@TMPb
		% anchor=internal
		%
		% This is a very special case: it is only used to express the
		% fact that we want to disable cell pictures
		...
		\expandafter\def\expandafter\pgfplots@beforenode\expandafter{\pgfplots@beforenode
			\pgftransformshift{\pgfqpointscale{1}{\pgfplotspointorigininternal}}%
		}%	
