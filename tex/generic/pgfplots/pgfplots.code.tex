	...
	/pgfplots/set layers/.code={%
		...
		\else
			\pgfkeysalso{%
				...
				/pgfplots/cell picture=if necessary,
				/pgfplots/set layers*={#1},
			}%
		\fi
	},
	% same as 'set layers=none':
...
\def\pgfplots@replacement@for@onlayer@#1{%
	\pgfplotsextra{\pgfplotsonlayer{#1}{axis environment content}}%
}%
...
%
% POSTCONDITION:
%   - a \pgfsetlayers{<list>} is issued
%   - '/pgfplots/@layer set' is defined such that any following axes can initialized their layers.
...
\def\pgfplots@prepare@layered@graphics{%
	\global\let\pgfplots@layer@active=\pgfutil@empty
	...
	\ifpgfplots@layered@graphics
		\pgfutil@ifundefined{pgf@layerlist}{%
			...
			\expandafter\pgfplotsutilforeachcommasep\expandafter{\pgf@layerlist}\as\pgfplots@loc@TMPa{%
				\t@pgfplots@toka=\expandafter{\pgfplots@layer@active}%
				\t@pgfplots@tokb=\expandafter{\csname pgfplots@layer@active@\pgfplots@loc@TMPa\endcsname}%
				\xdef\pgfplots@layer@active{\the\t@pgfplots@toka\the\t@pgfplots@tokb}%
			}%
		}%
	\fi
}%
...
\def\pgfplots@assert@layer@is@active#1#2{%
	\pgfplotsutilifstringequal{#1}{discard}{}{%
		\begingroup
		\def\b@foundit{0}%
		\expandafter\def\csname pgfplots@layer@active@#1\endcsname{\def\b@foundit{1}}%
		\pgfplots@layer@active
		\if0\b@foundit
			\pgfplotsthrow{inactive layer}{#1}{#2}\pgfeov%
		\fi
		\endgroup
	}%
}%
...
% #2: context information (for improved error recovery)
\def\pgfplotsonlayer#1#2{%
	...
	\ifpgfplots@layered@graphics
		...
		\else
			...
			\pgfplots@assert@layer@is@active{#1}{#2}%
			\pgfonlayer{#1}%
		\fi
	\fi
}%
...
\def\pgfplots@BEGIN@init@and@draw@axis{%
		...
		\else
			\pgfplotsgetlayerforstyle{/pgfplots/axis background}%
...
\def\pgfplots@set@options#1{%
	...
	%
	% retrieve layer configurations.
	\pgfkeysgetvalue{/pgfplots/@layer set}\pgfplots@loc@TMPa
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
	\else
