...
\pgfkeys{%
	...
	/pgfplots/layer family/.is family,
	/pgfplots/error bars/.is family,
	/pgfplots/set layers/.initial=,
	/pgfplots/set layers/.default={axis background,main,axis foreground,unclipped axis foreground},
...
\def\pgfplots@replacement@for@onlayer#1{\pgfplotsextra{\pgfplotsonlayer{#1}}}%
\def\pgfplots@replacement@for@endonlayer{\pgfplotsextra{\endpgfplotsonlayer}}%
...
%\pgfplots@message{Vor legende: \meaning\pgfplots@already@computed@legend@node^^J}%
	\ifx\pgfplots@createlegend@toname\pgfutil@empty
		% DRAW:
		\pgfplotsonlayerforstyle{every axis legend}%
		\pgfplots@already@computed@legend@node
		\endpgfplotsonlayerforstyle
	\else
...
% want it this way).
% 
% The other axis are scaled such that the desired unit vector ratios
% are fulfilled.
%
...
\def\pgfplots@getscale@unit@vector@reltoreference#1#2{%
	...
	% between e_ref and E_#1, which is available as #3.
	%
	% So: T^{-1} E_#1 :=  s* T^{-1} e_#1 where 
	%  s = #3 * ||T^{-1} e_ref|| / || T^{-1} e_#1 || 
	%    = |10^{scale_ref}| / |10^{scale_#1}| * #3 * || e_ref|| / ||e_#1||.
	% 
...
\def\pgfplotsdefineaxistype#1#2{%
	...
	\t@pgfplots@toka=\expandafter{\csname #1axis\endcsname}%
	\t@pgfplots@tokb=\expandafter{\csname end#1axis\endcsname}%
	\expandafter\edef\csname start#1axis\endcsname{\the\t@pgfplots@toka}%
	\expandafter\edef\csname stop#1axis\endcsname{\the\t@pgfplots@tokb}%
}%
...
\def\pgfplots@prepare@layered@graphics{%
	\pgfkeysgetvalue{/pgfplots/set layers}\pgfplots@loc@TMPa
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
		% DEFAULT.
...
\def\pgfplotsonlayerforstyle#1{%
	\pgfplotsgetlayerforstyle{#1}%
	\ifx\pgfplotsretval\pgfutil@empty
		\expandafter\pgfplotsonlayer\expandafter{\pgfplotsretval}%
	\fi
}%
\def\endpgfplotsonlayerforstyle{%
	\ifpgfplots@layered@graphics
		\endpgfplotsonlayer
	\fi
}
% Returns the 'on layer' argument for '\pgfplotsonlayerforstyle'.
%
% @see \pgfplotsonlayerforstyle
...
\def\pgfplots@END@init@and@draw@axis{%
	...
	\else
		% Note that layer changes are expensive. That's why I collect
		% the pgfplots@stored@markerlist here and assign the layer
		% just one time:
		\pgfplotsonlayer{unclipped axis foreground}%
...
%\message{DATA: '\meaning\pgfplots@stored@current@data';}%
		...
		\pgfplots@stored@current@precmd
		\pgfplotsonlayer{\pgfkeysvalueof{/pgfplots/on layer}}%
