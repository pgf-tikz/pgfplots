...
\def\pgfplots@set@optimal@datatrafos@allaxes{%
	...
	\if3\pgfplots@scale@mode@choice
		...
		\edef\pgfplots@loc@TMPa{%
			\noexpand\pgfplotscoordmath{\pgfplots@glob@TMPb}{datascaletrafo set params}
				\pgfplots@glob@TMPa% <- this already contains braces
		}%
		\pgfplots@loc@TMPa
		%
		\xdef\pgfplots@glob@TMPa{\expandafter\pgfutil@firstoftwo\pgfplots@glob@TMPa}%
\message{using datascaletrafo of axis '\pgfplots@glob@TMPb' for each axis.^^J}%
		%
		\def\pgfplots@loc@TMPd##1{%
			\if ##1\pgfplots@glob@TMPb
			\else
\message{Initialising the data scale transformation in direction ##1 to 10^\pgfplots@glob@TMPa*##1 - 0...^^J}%
				\pgfplotscoordmath{##1}{datascaletrafo set params}{\pgfplots@glob@TMPa}{0}%
			\fi
		}%
		\pgfplots@loc@TMPd x%
		\pgfplots@loc@TMPd y%
