	...
	\else
		...
		\pgfplots@ifcellpicture@required{%
			\if2\pgfplots@cell@picture@mode
				% cell picture=if necessary: OK, proceed as for 'cell picture=true'
			\else
			...
			\expandafter\def\expandafter\pgfplots@beforenode\expandafter{\pgfplots@beforenode
				...
				\iffalse
				\pgftransformshift{%
					\pgfqpointscale{1}{\pgfplotspointorigininternal}%
				}%
				\pgftransformshift{%
					\pgfpointdiff{%
						% This is necessary in order to prepare the
						% outer anchors (which assume that '(0,0)' is at 'outer
						% south west') or 'current axis':
						\pgfplotspointorigininternal%
					}{%
						\begingroup
						% we need to respect the transformation taken
						% in \pgfplots@BEGIN@cell@picture@DISABLED
						% here (do not take it twice):
						\pgfqpointscale{-1}{\pgfpointanchor{current axis}{south west}}%
						\pgf@process{}%
						\endgroup
					}%
				}%
...
\def\pgfplots@BEGIN@cell@picture@DISABLED[#1]{%
	% FIXME : This is NOT correct up to now. But the approach appears
	% to be correct.
	%
	% It seems that
	% a) the tick label size required for the auto-label-placement
	% appears to be incorrect, causing the labels to be shifted
	% incorrectly
	% b) the shifts of the axis' node are incorrect
	%
	%
	%
	\begingroup
...
\def\pgfplots@END@cell@picture@DISABLED{%
	...
	%
	{%
		\pgftransformshift{\pgfplotspointorigininternal}%
		\node[rectangle,draw] at (0pt,0pt) {origininternal};
		\pgfpathcircle{\pgfpointorigin}{2pt}\pgfusepath{fill}% FIXME
		\draw[->] \pgfextra \pgfpathmoveto\pgfpointorigin \pgfpathlineto\pgfplotspointorigininternal\endpgfextra;
	}%
}%
...
%\message{DATA: '\meaning\pgfplots@stored@current@data';^^J}%
	...
	%
	\begingroup
\pgfgettransform\pgfplots@loc@TMPa\message{TRANSFORMATION before beforenode \meaning\pgfplots@loc@TMPa^^J}%
	\pgfplots@beforenode
	%
\pgfgettransform\pgfplots@loc@TMPa\message{TRANSFORMATION before node\meaning\pgfplots@loc@TMPa^^J}%
	\ifpgfplots@deprecated@anchors
