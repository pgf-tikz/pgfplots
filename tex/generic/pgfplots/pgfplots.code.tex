...
\pgfkeys{%
	...
	/pgfplots/colorbar as legend/.style={
		...
		/pgfplots/colorbar/draw/.add code={%
			...
			% variable:
			\def\pgfplots@loc@TMPc{}%
			\pgfplotsforeachungrouped \x in {0,...,\pgfplotscolormaplastindexof{\pgfplots@CM}} {%
			...
			\xdef\pgfplots@glob@TMPa{%
				\noexpand\pgfplotsset{%
					...
					colormap={@temp@CM}{indices of colormap={0,...,\pgfplotscolormaplastindexof{\pgfplots@CM} of \pgfplots@CM}},
					% ... and normalizes the number range.
					point meta min=0,point meta max=1000,
					colorbar style={
						surf,shader=interp,
						colormap access=const,
						\noexpand\pgfplots@colorbar@as@legend@orientation tick=data,
						% here we add the positions back into the
						% tick label:
						\noexpand\pgfplots@colorbar@as@legend@orientation ticklabels={\the\t@pgfplots@tokb},
						%extra y ticks={\MAX},
					},
				}%
			}%
	...
	\ifx\pgfplots@loc@TMPa\pgfplots@colorbar@draw@legacyimpl
		...
		\if c\pgfplots@colormap@access
			...
			\def\pgfplots@colorbar@extra@options{%
				...
				samples=2,%
				% ... but: just in case someone uses 'ytick={data}',
				% we want the precise positions. Check that case right
				% before 'colorbar addplot', and compute the positions
				% if needed:
				/utils/exec={%
					\ifpgfplots@collect@firstplot@astick
						% Oh! Someone wrote 'ytick=data'! Compute
						% positions. This won't change the appearance
						% anyway due to 'shader=interp'.
						\pgfplots@colorbar@positions@to@samplesat
					\fi
				}%
			}%
		...
		\if 1\b@pgfplots@colorbar@fallback@to@sampled
			\t@pgfplots@toka=\expandafter{\pgfplots@colorbar@extra@options}%
			\edef\pgfplots@loc@TMPa{%
				\noexpand\pgfplotsset{%
					colorbar sampled={%
						surf,%
