...
% axis options:
	...
	/pgfplots/x post scale/.initial=,% EXPERIMENTAL
...
% FIXME: simplify this code! It is just a replication of the default error stuff together with an xshift!
	...
	/pgfplots/plot graphics/squeeze tol/.initial=1pt,
	%
	...
	/pgfplots/patch/.code={%
		\pgfkeysalso{/pgfplots/surf=#1,/pgfplots/mesh input=patches}%
	},%
	/pgfplots/patch table/value/.initial=,
	/pgfplots/patch table/.style={
		/pgfplots/patch table/value={#1},
		/pgfplots/patch table cdata=none%
	},
	/pgfplots/patch table*/.code={%
		\pgfkeyssetvalue{/pgfplots/patch table/value}{#1}%
		\if\pgfplotsplothandlermesh@extratbl@cdatachoice0%
			\pgfkeysalso{/pgfplots/patch table cdata=auto}%
		\fi
	},
	/pgfplots/patch table cdata/.is choice,
	/pgfplots/patch table cdata/none/.code	={\def\pgfplotsplothandlermesh@extratbl@cdatachoice{0}},
	/pgfplots/patch table cdata/auto/.code	={\def\pgfplotsplothandlermesh@extratbl@cdatachoice{1}},
	/pgfplots/patch table cdata/per vertex/.code	={\def\pgfplotsplothandlermesh@extratbl@cdatachoice{2}},
	/pgfplots/patch table cdata/per patch/.code	={\def\pgfplotsplothandlermesh@extratbl@cdatachoice{3}},
	/pgfplots/patch table cdata=none,
...
\else
	...
	\or decorate [ydiscont,decoration={pre length=\ydisstart, post length=\ydisend}] 
		%{ -- (\pgfplots@xmin,	\pgfplots@ymax) }
		{ \pgfextra\pgfpathlineto{\pgfplotsqpointxy{\pgfplots@xmin}{\pgfplots@ymax}}\endpgfextra }
	\or 
		%{ (\pgfplots@xmin,	\pgfplots@ymax) }
...
\else
	...
	\or %{ (\pgfplots@xmax,	\pgfplots@ymax) }
		{ \pgfextra\pgfpathmoveto{\pgfplotsqpointxy{\pgfplots@xmax}{\pgfplots@ymax}}\endpgfextra }
	\or decorate [xdiscont,decoration={pre length=\xdisstart, post length=\xdisend}] 
		%{ -- (\pgfplots@xmax,	\pgfplots@ymax) }
		{ \pgfextra\pgfpathlineto{\pgfplotsqpointxy{\pgfplots@xmax}{\pgfplots@ymax}}\endpgfextra }
	\fi
\fi
...
\else
	...
	\or %{ (\pgfplots@xmax,	\pgfplots@ymin) }
		{ \pgfextra\pgfpathmoveto{\pgfplotsqpointxy{\pgfplots@xmax}{\pgfplots@ymin}}\endpgfextra }
	\or decorate [ydiscont,decoration={pre length=\ydisend, post length=\ydisstart}] 
		%{ -- (\pgfplots@xmax,	\pgfplots@ymin) }
		{ \pgfextra\pgfpathlineto{\pgfplotsqpointxy{\pgfplots@xmax}{\pgfplots@ymin}}\endpgfextra }
	\fi
\fi
...
\else
	...
	\or decorate [xdiscont,decoration={pre length=\xdisend, post length=\xdisstart}] 
		%{ -- (\pgfplots@xmin,	\pgfplots@ymin) }
		{ \pgfextra\pgfpathlineto{\pgfplotsqpointxy{\pgfplots@xmin}{\pgfplots@ymin}}\endpgfextra }
		...
		\begingroup
			\ifx\pgfplots@createlegend@toname\pgfutil@empty
				...
				\def\pgfplots@createlegend@prepareimage@to##1{%
					...
					\edef##1{\noexpand\pgfplots@show@small@legendplots{\the\t@pgfplots@tokb}{\the\t@pgfplots@tokc}}%
				}%
			\else
				...
				\def\pgfplots@createlegend@prepareimage@to##1{%
					\t@pgfplots@tokb=\expandafter{\pgfplots@curlegend@opts}%
					\t@pgfplots@tokc=\expandafter{\pgfplots@curplotlist}%
					\edef\pgfplots@loc@TMPa{\noexpand\pgfplots@export@legend@image@to@macro{\the\t@pgfplots@tokb,\the\t@pgfplots@tokc}{}}%
					\pgfplots@loc@TMPa
					\let##1=\pgfplotsretval
				}%
			\fi
			...
			\pgfplotsloop{%
				...
				\else
					...
					\pgfplots@createlegend@prepareimage@to{\pgfplots@loc@TMPa}%
					%
...
\def\pgfplots@apply@unit@ratio{%
	...
	\else
		...
		\expandafter\pgfplots@apply@unit@ratio@getratios\pgfplots@unit@vector@ratio\pgfplots@EOI
		%
		% Step 1: compute the unit vector which STAYS CONSTANT.
		% I choose the smallest unit vector.
		%
		% All other ones will be scaled to fit its length.
		%
		% Choosing the smallest unit vector allows to maintain the
		% original axis size without further attention.
		%
		% More precisely, 'axis equal' will *shrink* all unit vectors.
		% Then, it enlarges the axis limits of the other axes.
		% Consequently, the axis rectangle has the same size as before,
		% only the limits are different.
		%
		% For 'axis equal image', choosing the smallest unit vector as
		% reference means that the resulting image can only become smaller
		% as the original axis. I guess that's also quite a good choice.
		%
		\ifpgfplots@threedim
...
\def\pgfplots@apply@unit@ratio@getratios#1 #2 #3 #4\pgfplots@EOI{%
	\def\pgfplots@unit@ratio@x{#1}%
	\def\pgfplots@unit@ratio@y{#2}%
	\def\pgfplots@unit@ratio@z{#3}%
}%
\def\pgfplots@apply@unit@ratio@prepareratios{%
	\pgfutil@ifnextchar[{\pgfplots@apply@unit@ratio@prepareratios@opt}{\pgfplots@apply@unit@ratio@prepareratios@}
}%
\def\pgfplots@apply@unit@ratio@prepareratios@opt[#1]{%
	\edef\pgfplots@apply@unit@ratio@reference{#1}%
	\pgfplots@apply@unit@ratio@prepareratios@
}%
\def\pgfplots@apply@unit@ratio@prepareratios@#1 #2 #3 #4\pgfplots@EOI{%
	...
	\ifpgfplots@threedim
		...
		\else
			\pgfmathdivide@{\pgfplots@unit@ratio@z}{\csname pgfplots@unit@ratio@\pgfplots@apply@unit@ratio@reference\endcsname}%
	...
	\else
		...
		\else
			\pgfmathdivide@{\pgfplots@unit@ratio@y}{\csname pgfplots@unit@ratio@\pgfplots@apply@unit@ratio@reference\endcsname}%
			\let\pgfplots@loc@TMPa=\pgfmathresult
			%
			\pgfmathmultiply@{\pgfplots@loc@TMPa}{\pgfplots@unit@ratio@x}%
			\let\pgfplots@unit@ratio@x=\pgfmathresult
	...
	\fi
	%
	% Normalize such that the largest weight is 1:
	\let\pgfplots@unit@ratio@max\pgfplots@unit@ratio@x
	\ifdim\pgfplots@unit@ratio@y pt>\pgfplots@unit@ratio@max pt
		\let\pgfplots@unit@ratio@max=\pgfplots@unit@ratio@y
	\fi
	\ifpgfplots@threedim
		\ifdim\pgfplots@unit@ratio@z pt>\pgfplots@unit@ratio@max pt
			\let\pgfplots@unit@ratio@max=\pgfplots@unit@ratio@z
		\fi
	\fi
	\ifdim\pgfplots@unit@ratio@max pt=1pt
		\def\pgfmathresult{1}%
	\else
		\pgfmath@basic@reciprocal@\pgfplots@unit@ratio@max
	\fi
	\let\pgfplots@loc@TMPa=\pgfmathresult
	\pgfmath@basic@multiply@{\pgfplots@unit@ratio@x}{\pgfplots@loc@TMPa}%
	\let\pgfplots@unit@ratio@x=\pgfmathresult
	\pgfmath@basic@multiply@{\pgfplots@unit@ratio@y}{\pgfplots@loc@TMPa}%
	\let\pgfplots@unit@ratio@y=\pgfmathresult
	\ifpgfplots@threedim
		\pgfmath@basic@multiply@{\pgfplots@unit@ratio@z}{\pgfplots@loc@TMPa}%
		\let\pgfplots@unit@ratio@z=\pgfmathresult
	\else
...
\message{Rescaling '#1' by \pgfplots@glob@TMPa.^^J}%
		...
		\pgfplotsmathmultiplypowten@{\csname pgfplots@\pgfplots@apply@unit@ratio@reference @veclength\endcsname}{\pgfplots@glob@TMPc}%
		\expandafter\let\csname pgfplots@#1@veclength\endcsname=\pgfmathresult
		\pgfplotsmathmultiplypowten@{\csname pgfplots@\pgfplots@apply@unit@ratio@reference @inverseveclength\endcsname}{-\pgfplots@glob@TMPc}%
		\expandafter\let\csname pgfplots@#1@inverseveclength\endcsname=\pgfmathresult
		%
		%
...
% Invokes #3 if ||D_#1||  < ||D_#2||  and #4 if that is not the case.
...
% #1: a new value for 'output cs'
% #2: initialization code which should define/modify
% 	\pgfplotsqpointxy
% 	\pgfplotsqpointxyz
% 	\pgfplotsqpointxy@orthogonal
% #3: code to update global axis limits \pgfplots@[xyz][minmax] by
% actual data limits, given by
% \pgfplots@data@[xyz][minmax].
% Arithmetics are to be performed in \pgfplotscoordmath!
\def\pgfplotsdefineoutputcs#1#2#3{%
	\expandafter\def\csname pgfplots@outputcs@#1\endcsname{#2}%
	\expandafter\def\csname pgfplots@outputcs@#1@upd@limits\endcsname{#3}%
}%

\pgfplotsdefineoutputcs{}{}{}% empty, the default.

\def\pgfplotsactivateoutputcs#1{%
	\pgfutil@ifundefined{pgfplots@outputcs@#1}{%
		\pgfplots@error{Sorry, 'output cs={#1}' is not known to pgfplots}%
	}{%
		\csname pgfplots@outputcs@#1\endcsname
	}%
}%
\def\pgfplotsupdatelimitsfordatainoutputcs#1{%
	\pgfutil@ifundefined{pgfplots@outputcs@#1}{%
		\pgfplots@error{Sorry, 'output cs={#1}' is not known to pgfplots}%
	}{%
		\csname pgfplots@outputcs@#1@upd@limits\endcsname
	}%
}%
...
\def\pgfplotsdefineaxistype#1#2{%
	\expandafter\def\csname pgfplots@axistype@#1\endcsname{#2}%
	\expandafter\def\csname pgfplots@environment@#1axis\endcsname{%
		\pgfutil@ifnextchar[%
			{\csname pgfplots@environment@#1axis@\endcsname}%
			{\csname pgfplots@environment@#1axis@\endcsname[]}%
	}%
	\expandafter\long\expandafter\def\csname pgfplots@environment@#1axis@\endcsname[##1]{%
		\begin{axis}[##1,axis type=#1]%
	}%
	\expandafter\def\csname endpgfplots@environment@#1axis\endcsname{\end{axis}}%
	\edef\pgfplots@glob@TMPc{\expandafter\noexpand\csname #1axis\endcsname\expandafter\noexpand\csname pgfplots@environment@#1axis\endcsname}%
	\expandafter
	\tikzaddtikzonlycommandshortcutlet\pgfplots@glob@TMPc
	%
	\edef\pgfplots@glob@TMPc{\expandafter\noexpand\csname end#1axis\endcsname\expandafter\noexpand\csname endpgfplots@environment@#1axis\endcsname}%
	\expandafter
	\tikzaddtikzonlycommandshortcutlet\pgfplots@glob@TMPc
}%
...
\def\endpgfplots@environment@opt{%
	...
	\pgfplotssurveyphase@setactiveplothandlers
	%
\message{processing late options \meaning\pgfplots@late@options.^^J}%
...
%\message{DATA: '\meaning\pgfplots@stored@current@data';}%
		...
		\else
			...
			%
			\pgfplotsactivateoutputcs{\pgfkeysvalueof{/pgfplots/output cs}}%
			%
