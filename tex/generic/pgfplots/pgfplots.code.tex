...
% Defines the class name \pgfplotspointmetainputhandler:
	/pgfplots/point meta/.code={%
		...
		% characters, so 'point meta=\thisrow{xcol}' won't cause 'unknown control sequence'
		% if it is set in a context where \thisrow is not available:
		\def\pgfplots@loc@TMPa{#1}%
		\pgfplots@command@to@string\pgfplots@loc@TMPa\pgfplots@loc@TMPb
		% FIXME : invoke that stuff in the point meta key path with
		% pgfkeys! Use 'expr' as '.unknown' handler.
		% Problem: the check about is difficult; need to search for
		% '=' inside of '#1' first.
