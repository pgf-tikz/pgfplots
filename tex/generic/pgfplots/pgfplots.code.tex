...
% FIXME: simplify this code! It is just a replication of the default error stuff together with an xshift!
	...
	/pgfplots/patch/.code={%
		\pgfkeysalso{/pgfplots/surf=#1,/pgfplots/mesh/type=triangle}%
	},%
	...
	/pgfplots/patch table cdata=none,
	/pgfplots/mesh/type/.is choice,
	/pgfplots/mesh/type/lattice/.code=	{\def\pgfplotsplothandlermesh@type{5}\def\pgfplotsplothandlermesh@matrixinput{1}},
	/pgfplots/mesh/type/triangle/.code=	{\def\pgfplotsplothandlermesh@type{4}\def\pgfplotsplothandlermesh@matrixinput{0}},
	/pgfplots/mesh/type/lattice,
...
%\pgfplots@message{Vor legende: \meaning\pgfplots@already@computed@legend@node}%
	...
	\else
		...
		\expandafter\xdef\csname pgfplots@legend@to@name@\pgfplots@createlegend@toname\endcsname{%
			...
			\noexpand\pgfplots@ifintikzpicture
				{\noexpand\pgfplots@legend@to@name}
				{\noexpand\tikz[/pgfplots/every legend to name picture/.try]{\noexpand\pgfplots@legend@to@name}}%
			\noexpand\endgroup
		}%
...
\def\pgfplots@colorbar@move@outside@scope@with@globalmacro#1{%
	...
	\xdef#1{%
		...
		\noexpand\pgfplots@ifintikzpicture
			{\noexpand\pgfplots@colorbar@draw}
			{\noexpand\tikz[/pgfplots/every colorbar to name picture/.try]{\noexpand\pgfplots@colorbar@draw}}%
		\noexpand\endgroup
	}%
	\ifx\pgfplots@colorbar@toname\pgfutil@empty
	\else
