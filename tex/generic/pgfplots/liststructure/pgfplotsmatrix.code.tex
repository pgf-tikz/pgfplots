...
%
% The algorithm is a 1:1 conversion from the LU decomposition found in
% numerical recipes in C; that's why it is hardly documented.
%
% It is to be used together with \pgfplotsmatrixLUsolve.
%
\def\pgfplotsmatrixLUdecomp#1\perm#2\signs#3{%
...
\def\pgfplotsmatrixLUdecomp@scalingloop{%
	\ifnum\pgfplotsmat@i<\pgfplotsmat@n
		...
		\pgfplotscoordmath{default}{if is}{\pgfplotsmat@big}{0}{%
			\pgfplots@error{Singular matrix in \string\pgfplotsmatrixLUdecomp}%
		}{}%
...
\def\pgfplotsmatrixLUdecomp@mainloop@j{%
	\ifnum\pgfplotsmat@j<\pgfplotsmat@n
\message{main loop no \the\pgfplotsmat@j^^J}%
		%
		\pgfplotsmat@i=0
		\pgfplotsmatrixLUdecomp@mainloop@j@i
\message{STEP j@i finished^^J}%
		%
		\pgfplotscoordmath{default}{zero}%
		\let\pgfplotsmat@big=\pgfmathresult
		%
...
\message{STEP j@i@second finished^^J}%
		%
\message{checking for row interchange \the\pgfplotsmat@j == \the\pgfplotsmat@imax? (j==imax?)^^J}%
		...
		\fi
\message{STEP INTERCHANGe finished^^J}%
		%
...
\def\pgfplotsmatrixLUdecomp@mainloop@j@i@second{%
	\ifnum\pgfplotsmat@i<\pgfplotsmat@n
		...
		\let\pgfplotsmat@dum=\pgfmathresult
\message{if( (dum = vv[i] * fabs(sum)) >= big ):  [ dum= \pgfplotsmat@dum ; big = \pgfplotsmat@big] :}%
		\pgfplotscoordmath{default}{if less than}{\pgfplotsmat@dum}{\pgfplotsmat@big}{%
\message{FALSE^^J}%
		}{%
\message{TRUE. -> imax = \the\pgfplotsmat@i ^^J}%
...
\message{II LOOP: \the\pgfplotsmat@j^^J}%
		%
		\pgfplotsmat@select\the\pgfplotsmat@i,\the\pgfplotsmat@j\to\pgfplotsmat@Aij
...
\def\pgfplotsmatrixLUbacksubst@loop@i@backw{%
	...
	\else
\message{BACKW LOOP: \the\pgfplotsmat@i^^J}%
		%
...
\def\pgfplotsmatrixLUbacksubst@loop@i@backw@j{%
	\ifnum\pgfplotsmat@j<\pgfplotsmat@n
\message{LOOP: \the\pgfplotsmat@j^^J}%
		%
		\pgfplotsmat@select\the\pgfplotsmat@i,\the\pgfplotsmat@j\to\pgfplotsmat@Aij
		\pgfplotsmat@selectb\pgfplotsmat@j\to\pgfplotsmat@temp
		\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsmat@temp}{\pgfplotsmat@Aij}}%
