...
\pgfkeys{
	...
	/pgfplots/warning/illegal user argument in log axis/.code args={#1#2}{%
		\pgfplots@warning{Ignoring illegal input argument #1=#2: cannot apply log.}%
	},
	...
	/pgfplots/warning/approx empty range enlarged/.code args={#1#2#3}{%
		\pgfplots@warning{Axis range for axis #1 is approximately empty; enlarging it (it is [#2:#3])}%
	},
	/pgfplots/warning/lossy colormap rgb conversion/.code 2 args={%
		\pgfplots@warning{I am about to convert colormap '#1' from #2\space to rgb (which might be lossy). Perhaps the choice 'colorbar sampled={surf,shader=interp}' yields better results since it supports #2.}%
	},
	...
	/pgfplots/warning/scatter classes no such class/.code={%
		\pgfplots@warning{scatter/classes: can't find class for '#1'!? Please make sure you have specified 'scatter src=explicit symbolic'. Ignoring class '#1' (this message will not come again).}%
	},
	...
	/pgfplots/warning/tick computation failed/.code 2 args={%
		\pgfplots@warning{Tick computation for direction #1 failed; there are always too few tick labels (try min ticks has already been advanced to #2)}%
	},
	...
	/pgfplots/warning/unit vector ratio axis undetermined/.code={%
		\pgfplots@warning{The algorithm to implement 'unit vector ratio' failed! It could not determine the axis which shall be scaled and decided to use 'unit vector ratio axis=#1'.}%
	},
