...
\def\pgfplots@prepare@tick@coordlists@for@checkdatalimits#1{%
	% FIXME : should we use that here as well!?
	%\pgfplots@axis@lines@getstartend{#1}%
...
% POSTCONDITION:
%   \pgfplots@line@beg contains the start of the axis line and
%   \pgfplots@line@end the end, both in transformed coords
\def\pgfplots@axis@lines@getstartend#1{%
	\ifcase\csname pgfplots@#1@line@limit\endcsname
		% axis B line limits=axis:
		\expandafter\let\expandafter\pgfplots@line@beg\csname pgfplots@#1min\endcsname
		\expandafter\let\expandafter\pgfplots@line@end\csname pgfplots@#1max\endcsname
	\or
		% axis B line limits=data:
		\expandafter\let\expandafter\pgfplots@line@beg\csname pgfplots@data@#1min@transformed\endcsname
		\expandafter\let\expandafter\pgfplots@line@end\csname pgfplots@data@#1max@transformed\endcsname
	\else
		\pgfplots@error{Illegal internal state encountered: unsupported choice}%
	\fi
}%
...
\def\pgfplots@drawgridlines@onorientedsurf@fromto#1{%
	\begingroup
	\pgfplots@axis@lines@getstartend{\pgfplotspointonorientedsurfaceB}%
	\pgfpathmoveto{\pgfplotspointonorientedsurfaceab{#1}{\pgfplots@line@beg}}%
	\pgfpathlineto{\pgfplotspointonorientedsurfaceab{#1}{\pgfplots@line@end}}%
	\endgroup
}%
