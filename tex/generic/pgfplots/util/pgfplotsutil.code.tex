...
\def\pgfplots@alloc@catcodetable{%
	...
	%
	% FIXME : try to make a portable approach to to it
	% luatexbase? luatex.sty ? what for context/ plain tex!? Isn't
	% there a useful default? After all, this catcodetable can safely
	% by that of the latex format file!?
	\def\pgfplotsglobalretval{4242}%
}%
\pgfutil@IfUndefined{directlua}{}{%
	\directlua{tex.enableprimitives("",tex.extraprimitives())}%
	\directlua{require("pgfplots")}%
	%
	\pgfplots@alloc@catcodetable
	\c@pgf@counta=\pgfplotsglobalretval
	\savecatcodetable\c@pgf@counta
	\directlua{pgfplots.LOAD_TIME_CATCODETABLE = \pgfplotsglobalretval}%
}%
