...
\def\pgfplots@logfileopen#1{%
	...
	\pgfutil@ifundefined{tikzpicturedependsonfile}{%
	}{%
		\tikzpicturedependsonfile{#1}%
	}%
}%
...
% <macro>##1    -> currently, this is the same as the '@' variant:
...
% <macro>logofbase -> log(#2)
% <macro>invlogofbase -> 1/log(#2)
\def\pgfplotsmathdefinemacrolnbase#1#2{%
	\expandafter\edef\csname #1logbase\endcsname{#2}%
	\pgfmathlog@{#2}%
	\expandafter\edef\csname #1logofbase\endcsname{\pgfmathresult}%
	\expandafter\pgfmath@basic@reciprocal@\expandafter{\pgfmathresult}%
	\expandafter\edef\csname #1invlogofbase\endcsname{\pgfmathresult}%
	%
	\expandafter\edef\csname #1@tofixed\endcsname##1{%
	...
	\expandafter\edef\csname #1@\endcsname##1{%
		\expandafter\noexpand\csname #1@tofixed{##1}%
		\noexpand\ifx\noexpand\pgfmathresult\noexpand\pgfutil@empty
			\noexpand\pgfmathfloatcreate{3}{0.0}{0}%
		\noexpand\else
