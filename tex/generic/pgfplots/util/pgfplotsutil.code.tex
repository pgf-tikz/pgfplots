...
\def\pgfplots@logfileopen#1{%
	...
	\pgfutil@ifundefined{tikzexternalifwritesmakefile}{%
	}{%
		\pgfplots@write@makefile@deps@for@input@file{#1}%
	}%
}%

\def\pgfplots@write@makefile@deps@for@input@file#1{%
	\tikzifexternalizing{%
		\tikzexternalifwritesmakefile{%
			\begingroup
			\pgfutil@ifundefined{w@pgfplots@external@dep@\jobname}{%
				\def\pgfplots@loc@TMPa{\csname newwrite\endcsname}%
				\begingroup
					\globaldefs=1
					\expandafter\pgfplots@loc@TMPa\csname w@pgfplots@external@dep@\jobname\endcsname
				\endgroup
				\expandafter\let\expandafter\w@pgfplots@dep\csname w@pgfplots@external@dep@\jobname\endcsname
				\immediate\openout\w@pgfplots@dep=\jobname.dep
			}{%
				\expandafter\let\expandafter\w@pgfplots@dep\csname w@pgfplots@external@dep@\jobname\endcsname
			}%
			\immediate\write\w@pgfplots@dep{\jobname\tikzexternalimgextension: #1}%
			\endgroup
		}{}%
	}{}%
}%
\def\pgfplots@modify@externalization@makefile{%
	\tikzexternalifwritesmakefile{%
		\tikzexternalwritetomakefile{include $(ALL_FIGURE_NAMES:\pgfplots@PERCENT@TEXT=\pgfplots@PERCENT@TEXT.dep)}%
		\tikzexternalwritetomakefile{}%
		\tikzexternalwritetomakefile{\pgfplots@PERCENT@TEXT.dep:}%
		\tikzexternalwritetomakefile{\pgfplots@TAB touch $@ \pgfplots@ROUTE\space will be filled later.}%
		\tikzexternalwritetomakefile{}%
	}{}
}
...
\pgfutil@ifundefined{AtBeginDocument}{%
	...
	\AtBeginDocument{%
		\pgfutil@ifundefined{tikzexternalifwritesmakefile}{%
			...
			\pgfplots@modify@externalization@makefile
		}%
	}%
}
