...
% Splits '<argument> of <name>*' into (\pgfplotsretval,
% \pgfplotsretvalb).
% 
...
% colormap.
%
% Note that '<name>*' is typically just a colormap name (i.e. without
...
\pgfqkeys{/pgfplots/of colormap}{%
	...
	pos/.initial=,
	selection/.initial=,
	selection/.set if not empty/.code={%
		\edef\pgfplots@loc@TMPa{#1}%
		\ifx\pgfplots@loc@TMPa\pgfutil@empty
		\else
			\pgfkeyslet{/pgfplots/of colormap/selection}\pgfplots@loc@TMPa
		\fi
	},
	%
	@define/.code={%
	...
	%   const colors of colormap
	%
	% All use this implementation:
	% #1: how to interprete results. /pgfplots/of colormap/#1 must be
	% a valid key (like 'color').
	% #2: the argument which can be '<selection> of <name>' or '<selection>'.
	@delegate to@ of colormap/.code 2 args={%
	...
	color/.style={%
		...
		/pgfplots/of colormap/selection/.set if not empty={#1},
	},
	const color/.style={%
	...
	samples/.style={%
		/pgfplots/of colormap/interpolation=map,% can be overridden if needed.
		/pgfplots/of colormap/@define/.code={%
	...
	/pgfplots/of colormap/.unknown/.code={%
		\pgfplotsutilifstringequal{#1}{\pgfkeysnovalue}{%
			...
			\pgfplotscolormapifdefined{\pgfkeyscurrentname}{%
				\edef\pgfplots@loc@TMPa{%
					\noexpand\pgfkeysalso{%
						/pgfplots/of colormap/name={\pgfkeyscurrentname},
					}%
				}%
				\pgfplots@loc@TMPa
			}{%
...
\pgfplotsinvokeforeach{%
	...
	\else
		\pgfkeysdef{/pgfplots/of colormap/#1 of colormap}{%
			\pgfkeysalso{/pgfplots/of colormap/@delegate to@ of colormap={#1}{##1}}%
		}%
	\fi
}%
...
\def\pgfplots@createcolormap@special@process@single@error{%
	\pgfplots@error{Did not find a suitable input selection in a 'of colormap' specification. Did you forget the 'selection'? It can be given as 'colors of colormap=(selection of name)' or 'of colormap=(selection=...,..)'}%
}%
\def\pgfplots@createcolormap@special@process@single{%
	...
	\fi
	%
	\pgfkeysgetvalue{/pgfplots/of colormap/selection}\pgfplots@loc@TMPa
	\edef\pgfplots@loc@TMPa{\pgfplots@loc@TMPa}%
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
...
\def\pgfplots@createcolormap@special@process@list@sanitize@selection{%
	...
	\edef\pgfplots@loc@TMPa{\pgfplots@loc@TMPa}%
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
		% default to a suitable selection.
		\ifnum\pgfplots@pos@size<2
			...
			\edef\pgfplots@loc@TMPa{%
				\noexpand\pgfkeysalso{%
					/pgfplots/of colormap/selection={0,...,\pgfplotscolormaplastindexof{\pgfkeysvalueof{/pgfplots/colormap name}}},%
					/pgfplots/of colormap/indices,
				}%
			}%
			\pgfplots@loc@TMPa
		\else
			% hm... we have no selection, but we have positions! In
			% this case, we _sample_ selection uniformly with one
			% sample for each position!
			\pgfkeyslet{/pgfplots/of colormap/selection}\pgfplots@pos@size
...
\def\pgfplots@createcolormap@special@process@list{%
	\begingroup
	\pgfkeysgetvalue{/pgfplots/of colormap/pos}\pgfplots@loc@TMP
	...
	\expandafter\pgfplots@loc@TMPa\expandafter{\pgfplots@source}{%
		...
		\ifnum\c@pgfplots@elem<\pgfplots@pos@size\relax
			...
			\pgfkeyslet{/pgfplots/of colormap/pos}\pgfplots@loc@TMPa
		\else
			% auto-position:
...
\def\pgfplots@createcolormap@special@process@list@sampled@define{%
	...
	\pgfmath@smuggleone\pgfplotsretval
	\endgroup
	\pgfkeyslet{/pgfplots/of colormap/selection}\pgfplotsretval
	\pgfqkeys{/pgfplots/of colormap}{%
		range=0:1000,%
	}%
}%
