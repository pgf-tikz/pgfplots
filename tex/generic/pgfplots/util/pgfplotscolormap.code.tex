...
%\message{H := \pgfplots@createcolormap@H( from #1).}%
	...
	\else
		...
		\else
			...
			\else
			% FIXME : how can I activate this without breaking old colormaps!?
				% it is NOT uniform!
				\def\b@pgfplotscreatecolormap@isuniform{0}%
				%
				\pgfmathmin@{\pgfplots@createcolormap@H@cur}{\pgfplots@createcolormap@H}%
				\let\pgfplots@createcolormap@H=\pgfmathresult
			\fi
		\fi
	\fi
	% 
	% remember positions:
	\pgfplotsapplistXpushback{#1,}\to\pgfplots@createcolormap@X
	%
		...
		\ifnum\c@pgf@counta>0
			...
			\ifnum\c@pgf@counta<\c@pgf@countb\relax
				\pgfplotscolormapifisuniform{#1}{%
					...
					\pgf@xa=\pgfplots@loc@TMPc\relax
				}%
...
\def\pgfplotscolormaptoshadingspec#1#2#3{%
	...
	\pgfplotsarrayforeachungrouped{pgfpl@cm@#1}\as\pgfplotscolormaptoshadingspec@TMP{%
		\pgfplotscolormapifisuniform{#1}{%
			...
			\pgf@yb=\pgfplots@loc@TMPd\relax
		}%
