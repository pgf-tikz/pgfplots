...
%
% \pgfplotscolornormalizesequencenext{red}
...
% \endpgfplotscolornormalizesequence
%
% #1 : the color space. Can be auto, rgb, or cmyk (will be expanded).
...
%
% #1 some color. It can be either in a format accepted by xcolor OR of
% the form {rgb}{0,1,0}  or {cmyk}{1,1,1,1}
%
...
\def\pgfplotscolornormalizesequencenext@{%
	\pgfutil@ifnextchar\bgroup{%
		\pgfplotscolornormalizesequencenext@bycomps@gobble
	}{%
		\pgfplotscolornormalizesequencenext@xcolor@gobble
	}%
}
\def\pgfplotscolornormalizesequencenext@bycomps@gobble#1\pgfplots@EOI{%
	\pgfplotscolornormalizesequencenextbycomponents#1\relax\relax\relax% NO braces here. We expect them in '#1' !
}
\def\pgfplotscolornormalizesequencenext@xcolor@gobble#1\pgfplots@EOI{%
	...
	% just to be sure - if \pgf@tempcolor is invalid.
	\relax\relax
}

\def\pgfplots@createcolormap@initcolorspace{%
	\pgfplotscolornormalizesequence[colorspace=\pgfkeysvalueof{/pgfplots/colormap default colorspace}]
}%
...
\def\pgfplots@createcolormap@grabcmyk@(#1)=(#2,#3,#4,#5){%
	...
	\pgfplots@createcolormap@nextcolor{#1}{{cmyk}{#2,#3,#4,#5}}%
 	\pgfplots@createcolormap@}
...
\def\pgfplots@createcolormap@grabcmyk@two@five@five@(#1)=(#2,#3,#4,#5){%
	...
	\edef\pgfplots@loc@TMPa{{cmyk}{\pgfplots@loc@TMPa,\pgfplots@loc@TMPb,\pgfplots@loc@TMPc,\pgfmathresult}}%
	\expandafter\pgfplots@loc@TMPd\expandafter{\pgfplots@loc@TMPa}%
 	\pgfplots@createcolormap@}
...
\def\pgfplots@createcolormap@grabcolor@(#1)=(#2){%
	...
	\pgfplots@createcolormap@nextcolor{#1}{#2}%
	% 
	% continue loop:
	\pgfplots@createcolormap@
}%

\def\pgfplots@createcolormap@rangeexception#1{%
	\pgfplots@error{Sorry, the color component value #1 is out of range. The allowed range is 0 <= value  <= 1. The error occured near `\pgfplots@createcolormap@context'}%
}
...
\def\pgfplots@createcolormap@nextRGB#1#2#3#4{%
	\pgfplots@createcolormap@nextcolor{#1}{{rgb}{#2,#3,#4}}%
}%

% #1: h
% #2: a compound element of the form
% '{<colorspace>}{<comma-separated-color-components>}'
%  In fact, it can also be a color specification of xcolor (see
...
\def\pgfplots@createcolormap@nextcolor#1#2{%
	...
	%
	% I only need the components, the colorspace is irrelevant here:
	\edef\pgfplotsretval{\expandafter\pgfutil@secondoftwo\pgfplotsretval}%
	%
	% invoke
	% \pgfplots@createcolormap@nextcolor@{#1}{C1}{C2}{C3}{C4}.
	% We need to compute C1 ... C4 :
	\def\pgf@colmarshal{{#1}}%
	\expandafter\pgf@uncomma\pgfplotsretval,,%
	\if4\pgfplots@createcolormap@col@comps
	\else
		% ok, we have C4={}. Append it:
		\edef\pgf@colmarshal{\pgf@colmarshal{}}%
	\fi
	\expandafter\pgfplots@createcolormap@nextcolor@\pgf@colmarshal%
}%
