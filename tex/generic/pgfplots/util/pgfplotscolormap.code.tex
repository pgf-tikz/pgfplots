...
\def\pgfplots@createcolormap@special@process@list{%
	...
	\ifnum\c@pgfplots@elem<\pgfplots@pos@size\relax
		...
		\ifx\x\pgfutil@empty
			\pgfplotswarning{too many colormap positions}{no \c@pgfplots@elem\space (\pgfkeysvalueof{/pgfplots/of colormap/target pos})}\pgfeov%
		\else
			%
			% simply replicate the last encountered color
			\pgfplots@createcolormap@special@process@list@append
			%
			\pgfkeyslet{/pgfplots/of colormap/source pos}\pgfutil@empty
		\fi
		\pgfplotsutil@advancestringcounter@global\c@pgfplots@elem
		%
	\pgfutil@repeat
...
\def\pgfplots@createcolormap@special@process@list@sampled@define{%
	...
	\if c\pgfplots@ofcolorbar@sample@for@choice%
		...
		\ifnum\pgfplots@pos@size>1
			...
			\ifnum\pgfplots@pos@size>\pgfplots@source\relax
				% ok, but we have so many positions-- the last source
				% sample will be replicated anyway. No need to apply
				% 'sample for=const':
				\def\b@pgfplots@special@interval{0}%
			\else
