...
\def\pgfplotscolornormalizesequence[#1]{%
	...
	\else
		...
		\else
			...
			\else
				\pgfplots@error{Sorry, the choice colorspace=\pgfplots@loc@TMPa\space is unknown to pgfplots, please use one of 'auto,rgb,cmyk'}%
			\fi
		\fi
	\fi
}%
	...
	\else
		\ifx\pgfplotscolornormalizesequence@colspace\pgfutil@empty
			\pgfutil@ifundefined{pgfplots@colspace@name@\pgfplots@loc@TMPa}{%
				\pgfplotscolornormalizesequencenext@color@error{{#1}{#2}}%
			}{%
...
\def\pgfplotscolornormalizesequencenext@color@error#1{%
	% FIXME : do not refer to 'colormap' here:
	\pgfplotsthrow{invalid argument}{\pgfplots@loc@TMPa}{Sorry, the current configuration is to auto-detect the output colorspace. However, the first provided color '#1' has an unsupported output colorspace '\pgfplots@loc@TMPa'. Please provide a color with supported first color or configure the output colorspace 'colormap default colorspace'. Note that conversion to the output colorspace is possible. (near \pgfplotscolornormalizesequence@context@message)}\pgfeov%
}
