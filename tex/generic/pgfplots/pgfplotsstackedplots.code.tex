...
\def\pgfplots@stacked@prepare@value@serialization{%
	...
	%
%	\pgfplotsset{%
%		visualization depends on=value \pgfplots@stacked@zerolevel@current\as\pgfplots@stacked@zerolevel
%	}%
	%
	%
	...
	%
	\let\pgfplots@stacked@deserialize@without@zerolevel=\pgfplotsaxisdeserializedatapointfrom@private
...
\def\pgfplotsaxisdeserializedatapointfrom@private@stacked@ Z#1#2{%
	...
	\pgfplots@stacked@deserialize@without@zerolevel{#2}%
}%
...
\def\pgfplotsgetzerolevelcoordinates{%
	\ifx\pgfplots@stacked@zerolevel\pgfutil@empty
		% FIXME : this is currently unsupported :-(
		% if we want to use this from 'nodes near coords', we have no
		% access to \pgfplots@current@point@x - only to transformed
		% coordinates. maybe that is the problem. Or maybe we need to
		% ensure that we know these values... ?
		\pgfkeyslet{/data point/zero/x}\pgfplots@current@point@x
		\pgfkeyslet{/data point/zero/y}\pgfplots@current@point@y
		\pgfkeyslet{/data point/zero/z}\pgfplots@current@point@z
		\ifpgfplots@stackedmode
			\pgfkeyssetvalue{/data point/zero/\pgfplots@stacked@dir}{0}%
		\fi
		\pgfkeyslet{/data point/zero/meta}\pgfutil@empty
	...
	\else
		...
		\xdef\pgfplots@glob@TMPd{%
			...
			\noexpand\pgfkeyssetvalue{/data point/zero/auto}{0}%
		}%
		\endgroup
		\pgfplots@glob@TMPd
	\fi
	\ifpgfplots@stackedmode
		\pgfkeysgetvalue{/data point/zero/\pgfplots@stacked@dir}\pgfmathresult
		\pgfkeyslet{/data point/zero/dir}{\pgfmathresult}%
	\fi
%
\message{pgfplotsgetzerolevelcoordinates returned x=\pgfkeysvalueof{/data point/zero/x}, y=\pgfkeysvalueof{/data point/zero/y}^^J}%
}%
