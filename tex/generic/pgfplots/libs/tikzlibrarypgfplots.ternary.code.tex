...
%\message{\string\pgfplotspointonorientedsurfaceab{#1}{#2} -----> }%
	...
	\else
		\csname pgfplots@ternary@pre@map@\pgfplotspointonorientedsurfaceA\endcsname{#1}%
		\let\pgfplotspointonorientedsurfaceab@ternary@A=\pgfmathresult
		%
		\csname pgfplots@ternary@pre@map@\pgfplotspointonorientedsurfaceB\endcsname{#2}%
...
\def\pgfplots@ternary@init@pre@map#1{%
	...
	\let\pgfplots@pre@map@scale=\pgfmathresult
	%
	\expandafter\xdef\csname pgfplots@ternary@pre@map@#1\endcsname##1{%
		\noexpand\pgfmath@basic@subtract@{##1}{\csname pgfplots@#1min\endcsname}%
		\noexpand\pgfmath@basic@multiply@{\noexpand\pgfmathresult}{\pgfplots@pre@map@scale}%
	}%
	\endgroup
}
...
\def\pgfplotsqpointxyz@ternary#1#2#3{%
	\pgf@process{%
		\ifpgfplots@ternary@next@is@unitinterval
			...
			\dimen3=#3pt
		\else
			\pgfplots@ternary@pre@map@x{#1}%
			\dimen1=\pgfmathresult pt
			%
			\pgfplots@ternary@pre@map@y{#2}%
			\dimen2=\pgfmathresult pt
			%
			\pgfplots@ternary@pre@map@z{#3}%
