...
\pgfkeys{
    /pgfplots/group/size/.is family,
    /pgfplots/group style/.style={/pgfplots/group/.cd,#1},
...
%    /pgfplots/group/group size/.belongs to family=/pgfplots/group/size,
    /pgfplots/group/columns/.store in=\pgfplots@group@columns,
%    /pgfplots/group/columns/.belongs to family=/pgfplots/group/size,
    /pgfplots/group/rows/.store in=\pgfplots@group@rows,
%    /pgfplots/group/rows/.belongs to family=/pgfplots/group/size,
...
\def\pgfplots@group@nextplot[#1]{%
    ...
    \ifnum\pgfplots@group@current@plot=\pgfplots@group@totalplots\relax
      \pgfplots@warning{You are adding more plots than you specified, current plots=\the\pgfplots@group@current@plot\space vs. total plots=\pgfplots@group@totalplots.}
    \fi
    % 
    ...
    \ifnum1=\pgfplots@group@current@row\relax
      \ifnum1=\pgfplots@group@current@column\relax
        \pgfplots@environment@opt[name={\pgfplots@group@name\space c\the\pgfplots@group@current@column r\the\pgfplots@group@current@row},%
          ...
          #1]
      \else
        %\pgfmathparse{int(\the\pgfplots@group@current@column -1)}
        ...
        \pgfplots@environment@opt[name={\pgfplots@group@name\space c\the\pgfplots@group@current@column r\the\pgfplots@group@current@row},%
          /pgfplots/group/every plot,%
          /pgfplots/group/plot c\the\pgfplots@group@current@column r\the\pgfplots@group@current@row,%
          at={($(\pgfplots@glob@TMPb.east)+(\pgfkeysvalueof{/pgfplots/group/horizontal sep},0)$)},anchor=west,#1]
      \fi
    \else
      %\pgfmathparse{int(\the\pgfplots@group@current@row-1)}
      ...
      \pgfplots@environment@opt[name={\pgfplots@group@name\space c\the\pgfplots@group@current@column r\the\pgfplots@group@current@row},%
        /pgfplots/group/every plot,%
        /pgfplots/group/plot c\the\pgfplots@group@current@column r\the\pgfplots@group@current@row,%
        at={($(\pgfplots@glob@TMPb.south)-(0,\pgfkeysvalueof{/pgfplots/group/vertical sep})$)},anchor=north,#1]%
    \fi%
}
...
\def\pgfplots@group@determine@ticklabels{%
    ...
    \ifx\pgfplots@loc@TMPc\pgfplots@group@xticklabels%
      \foreach \pgfplots@column in {1,...,\pgfplots@group@columns} {%
          ...
          \ifnum1<\pgfplots@group@rows\relax
            %\pgfmathparse{int(\pgfplots@group@rows -1)}
    ...
    \ifx\pgfplots@loc@TMPc\pgfplots@group@yticklabels%
      \foreach \pgfplots@row in {1,...,\pgfplots@group@rows} {%
          ...
          \ifnum1<\pgfplots@group@columns\relax
            %\pgfmathparse{int(\pgfplots@group@columns -1)}
...
\def\pgfplots@environment@groupplot{%
	\pgfutil@ifnextchar[{\pgfplots@@environment@groupplot}{\pgfplots@@environment@groupplot[]}%
}
...
%     \pgfkeyssetfamily{/tikz/variable}{/pgfplots}%<-- unnecessary;  applies only to /tikz/variable
%
...
%     \pgfqkeysactivatesinglefamilyandfilteroptions{/pgfplots/group/size}%
%     {/pgfplots}%
%     {#1}%
%     \let\pgfplots@rmopts=\relax
	...
	\endgroup
    %\pgfmathparse{int((\pgfplots@group@rows)*\pgfplots@group@columns)}%
