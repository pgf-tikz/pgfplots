...
\pgfplotsset{
	/pgfplots/xgrid stop each nth at y/.initial=,
	/pgfplots/xgrid stop each nth at y start/.initial=0,
	/pgfplots/ygrid stop each nth at x/.initial=,
	/pgfplots/ygrid stop each nth at x start/.initial=0,
	%
	/pgfplots/xgrid stop at y/.initial=,
...
\def\pgfplots@drawgridlines@INSTALLCLIP@onorientedsurf@smithchartaxis#1{%
	...
	\edef\pgfplots@loc@TMPa{\pgfplotspointonorientedsurfaceA\pgfplotspointonorientedsurfaceB}%
	\expandafter\pgfplots@gridlines@init@grid@stop@points@for\pgfplots@loc@TMPa
}%

% Initialises the '#1grid stop each nth at #2' and '#1grid stop at #2'
...
\def\pgfplots@gridlines@init@grid@stop@points@for#1#2{%
	...
	%
	\pgfkeysgetvalue{/pgfplots/#1grid stop each nth at #2}\pgfplots@loc@TMPa
	...
	\else
		...
		% normalise the argument for 'ygrid stop each nth at x': each list element
		...
		% Ok. 
		%
		% Now, the 'ygrid stop each nth at x' feature relies *crucially* on grid line
		...
		\c@sortindex=\pgfkeysvalueof{/pgfplots/#1grid stop each nth at #2 start} % this is assigned to the '0.0' gridline (if any)
...
%\message{\pgfplots@ticknum\space(abs(tickpos) = \pgfplots@tick)---> sort index \csname pgfplots@#1tickpos@to@sortidx@\pgfplots@ticknum\endcsname^^J}%
	...
	\edef\pgfplots@loc@TMPa{\pgfplots@loc@TMPa\space}% this inserts the final space and handles any active ':' sign (for french babel)
...
\def\pgfplots@gridlines@init@stop@maps@next#1#2#3:#4 {%
	...
	\expandafter\let\csname pgfplots@#1gridline@stopmap@\pgfmathresult\endcsname\pgfplots@loc@TMPa%
	\if#1y%
		% support symmetry:
...
\def\pgfplots@gridlines@init@grid@stop@points@for@normalise#1\relax#2#3{%
	...
	\edef\entry{\pgfmathresult:\pgfplots@grid@stop@at@eachnth}%
}%
\def\pgfplots@gridlines@stop@at@unpack#1:#2\relax{%
...
%
% The method relies on the 'ygrid stop each nth at x' feature, more specifically the
...
\def\pgfplots@get@current@grid@stop@point#1#2#3{%
	...
	%
	\ifx\pgfplotsretval\pgfutil@empty
		% the individual map failed. Ok, then check for the '#1grid stop each nth at #2'
				...
				\expandafter\pgfplotslistforeachungrouped\csname pgfplots@#1grid@stop@points\endcsname\as\pgfplots@loc@TMPa{%
					\ifx\pgfplotsretval\pgfutil@empty
						...
						\else
							...
							\if#1y%
								\pgfplots@get@current@grid@stop@point@handle@yconstraints{#3}%
							\fi
						\fi
					\fi
				}%
	...
	\if#1x%
		\ifx\pgfplotsretval\pgfutil@empty
		\else
			\pgfplots@get@current@grid@stop@point@handle@xconstraints{#3}%
		\fi
	\fi
}%

% PRECONDITION
%   \pgfplotsretval contains a non-zero contraint for the xgrid at #1.
%
% POSTCONDITION
% 	- the value of \pgfplotsretval is not changed.
%   - several values required to evaluate
%   \pgfplots@get@current@grid@stop@point@handle@yconstraints
%   are initialised.
%
% @see \pgfplots@get@current@grid@stop@point@handle@yconstraints
\def\pgfplots@get@current@grid@stop@point@handle@xconstraints#1{%
	%
	% FIXME : this pollutes the global
	% namespace with inherently local
	% information!
	\expandafter\global\expandafter\let\csname pgfplots@final@stop@point@for@xtick@\pgfplots@tick@prepared\endcsname=\pgfplotsretval
\message{x grid line: storing end value for '#1' END[\pgfplots@tick@prepared] = \pgfplotsretval.^^J}%
	%
	\let\pgfplotsretval@@=\pgfplotsretval
	\pgfplots@gridlines@stopmap@prepare{\pgfplotsretval}%
	\let\pgfplots@loc@TMPa=\pgfmathresult
	\pgfutil@ifundefined{pgfplots@minimal@abslength@for@ytick@\pgfplots@loc@TMPa}{%
		\pgfplotscoordmath{default}{zero}%
	}{%
		\expandafter\let\expandafter\pgfmathresult\csname pgfplots@minimal@abslength@for@ytick@\pgfplots@loc@TMPa\endcsname
	}%
	\let\pgfplots@loc@TMPb=\pgfmathresult
	\pgfplotscoordmath{default}{max}{\pgfplots@loc@TMPb}{#1}%
	\expandafter\global\expandafter\let\csname pgfplots@minimal@abslength@for@ytick@\pgfplots@loc@TMPa\endcsname=\pgfmathresult
\message{y grid line: updating minimal abs length[\pgfplots@loc@TMPa] = \pgfmathresult.^^J}%
	\let\pgfplotsretval=\pgfplotsretval@@
}%
...
%  	  The current solution checks the minimal absolute length of y
%  	  grid lines to avoid these dead-ends.
%
% Uncomment the routine(s) and run the test cases to see what happens.
\def\pgfplots@get@current@grid@stop@point@handle@yconstraints#1{%
	% check for first constraint:
	% FIXME this one turns out to be
	% correct, but it is ugly
	\pgfutil@ifundefined{pgfplots@minimal@abslength@for@ytick@\pgfplots@tick@prepared@abs}{%
	}{%
		\pgfplotscoordmath{default}{if less than}
			{\pgfplotsretval}
			{\csname pgfplots@minimal@abslength@for@ytick@\pgfplots@tick@prepared@abs\endcsname}
			{%
	...
	\else
		...
		\let\pgfplots@loc@TMPa=\pgfmathresult
		%
\message{checking ending 'y=#1' at 'x=\pgfplotsretval'. 'x=\pgfplots@loc@TMPa' stops}%
		\pgfutil@ifundefined{pgfplots@final@stop@point@for@xtick@\pgfplots@loc@TMPa}{%
		% no constraint. Ok.
\message{ nowhere. No contraint.^^J}%
		}{%
\message{ at \pgfplots@loc@TMPa. Feasible: }%
			% Oh, a constraint. That means
			% further work.
			\expandafter\let\expandafter\pgfplots@loc@TMPa\csname pgfplots@final@stop@point@for@xtick@\pgfmathresult\endcsname
			\pgfplotscoordmath{default}{op}{abs}{{#1}}%
...
\def\pgfplots@smithchart@draw@xcircle#1{%
	...
	\let\pgfplots@radius@inverse=\pgfmathresult
	%
	% This here would suffice in case 'xgrid stop each nth at y={}':
%	\pgfpathellipse
%		{\pgfpointxy@orig{\pgfplots@center}{0}}
%		{\pgfpointxy@orig{\pgfplots@radius}{0}}
%		{\pgfpointxy@orig{0}{\pgfplots@radius}}%
	%
	% but we also check for the 'xgrid stop each nth at y' feature:
...
%\message{X grid line \#\csname pgfplots@ticknum\endcsname \space at '#1': center = (\pgfplots@center,0);  radius = \pgfplots@radius\space(start angle \pgfplots@startangle, end angle = \pgfplots@endangle; stops at y = \pgfplots@arc@ends@at@y@arc@value).^^J}%
	...
	\edef\pgfplots@loc@TMPa{%
		{\noexpand\pgfpointxy@orig{\pgfplots@center}{0}}%
		{\pgfplots@startangle}%
		{\pgfplots@endangle}%
		{\noexpand\pgfpointxy@orig{\pgfplots@end}{\pgfplots@endim}}%
		{\the\pgf@xa}%
		{\the\pgf@xb}%
		{\pgfplots@smithchart@axis@ratioxy}%
		{\pgfplots@smithchart@axis@ratioyx}%
	}%
...
%			\let\pgfplots@endangle=\pgfplots@loc@TMPa
		\fi
		\edef\pgfplots@loc@TMPa{%
			{\noexpand\pgfpointxy@orig{1}{\pgfplots@signedradius}}%
			{\pgfplots@startangle}%
			{\pgfplots@endangle}%
			{\noexpand\pgfpointxy@orig{\pgfplots@end}{\pgfplots@endim}}%
			{\the\pgf@xa}%
