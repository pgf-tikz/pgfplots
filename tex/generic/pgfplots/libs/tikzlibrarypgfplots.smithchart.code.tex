...
\pgfplotsset{
	...
	%
	/pgfplots/xgrid stop at y map/.initial=,
	/pgfplots/ygrid stop at x map/.initial=,
	%
...
% map' features. All it does is to prepare the
...
\def\pgfplots@gridlines@init@grid@stop@points@for#1#2{%
	...
	\else
		...
		\expandafter\pgfplotsutilforeachcommasep\pgfplots@loc@TMPa\as\entry{%
			\expandafter\pgfplots@gridlines@init@grid@stop@points@for@normalise\expandafter #1\expandafter #2\entry\relax
...
%\message{\pgfplots@ticknum\space(abs(tickpos) = \pgfplots@tick)---> sort index \csname pgfplots@#1tickpos@to@sortidx@\pgfplots@ticknum\endcsname^^J}%
	...
	\pgfkeysgetvalue{/pgfplots/#1grid stop at #2 map}\pgfplots@loc@TMPa
...
\def\pgfplots@gridlines@init@stop@maps@next#1#2#3:#4 {%
	...
	\expandafter\edef\csname pgfplots@#1gridline@stopmap@\pgfmathresult\endcsname{#4}%
	\if#1y%
		% support symmetry:
		% add -\pgfmathresult to the map.
		% To disable this symmetry, use 'ygrid stop at x map={0.2:4 -0.2:5}
...
\def\pgfplots@gridlines@stopmap@prepare#1{%
	...
	\expandafter\pgfplots@gridlines@stopmap@trunc@\pgfmathresult00\relax
}%
...
\def\pgfplots@gridlines@init@grid@stop@points@for@normalise#1#2#3\relax{%
	\pgfutil@in@:{#3}%
	\ifpgfutil@in@
	\else
		\edef\entry{\entry:2}%
	...
	\pgfutil@in@{[index]}{#3}%
	\ifpgfutil@in@
		\if0\csname b@pgfplots@#2ticknum@to@pos@lookup\endcsname
			\pgfplots@gridlines@init@grid@stop@points@for@init@ticknum@lookup#1#2%
		\fi
		\expandafter\pgfplots@gridlines@stop@at@unpack\entry\relax
		\pgfutil@ifundefined{pgfplots@#2tick@num@to@pos@\pgfplots@grid@stop@at}{%
			\pgfplots@warning{There is no #2tick with index '\pgfplots@grid@stop@at'. Skipping it.}%
			\let\entry=\pgfutil@empty
		}{%
			\edef\entry{\csname pgfplots@#2tick@num@to@pos@\pgfplots@grid@stop@at\endcsname:\pgfplots@grid@stop@at@eachnth}%
		}%
	\fi
}%
...
\def\pgfplots@get@current@grid@stop@point#1#2#3{%
	\def\pgfplotsretval{}%
	%
	% first: check the '#1grid stop at #2 map':
	\expandafter\ifx\csname b@pgfplots@#1gridline@stopmap\endcsname\relax%
		% ok. there is no such thing.
	\else
		% ah - process it!
		\pgfplots@gridlines@stopmap@prepare{#3}%
		\pgfutil@ifundefined{pgfplots@#1gridline@stopmap@\pgfmathresult}{%
		}{%
			\edef\pgfplotsretval{\csname pgfplots@#1gridline@stopmap@\pgfmathresult\endcsname}%
		}%
	\fi
	%
	\ifx\pgfplotsretval\pgfutil@empty
...
%		{\pgfpointxy@orig{0}{\pgfplots@radius}}%
	...
	\pgfplots@get@current@grid@stop@point xy{#1}%
	...
	\else
		...
		\pgfplotscoordmath{default}{parsenumber}{\pgfplots@arc@ends@at@y@arc@value}%
		\let\pgfplots@arc@ends@at@y@arc@value=\pgfmathresult
...
\def\pgfplots@smithchart@draw@yarc#1{%
	\pgfplotsmath@ifapproxequal@dim{#1pt}{0pt}{0.002pt}{%
		...
		%
		% compute end angle.
		\pgfplots@get@current@grid@stop@point yx{#1}%
