...
\pgfplotsset{
	/pgfplots/grid line limits/.initial=,
...
\message{X grid line \#\csname pgfplots@ticknum\endcsname \space at '#1': center = (\pgfplots@center,0); radius = \pgfplots@radius.^^J}%
	...
	\else
		\pgfplotsmath@ifapproxequal@dim{#1pt}{0pt}{0.002pt}{%
			...
			% 
			%
			% compute diff vector from center=(1,\pgfplots@signedradius) to start:
			\pgfmathsubtract@{\pgfplots@start}{1}%
			\let\pgfplots@D\pgfmathresult
			\pgfmathsubtract@{\pgfplots@startim}{\pgfplots@signedradius}%
			...
			%
\message{Y grid line  \#\csname pgfplots@ticknum\endcsname\space at '#1': center = (1,\pgfplots@signedradius); signedradius = \pgfplots@signedradius\space( start angle \pgfplots@startangle, end angle \pgfplots@endangle)^^J}%
			%\pgfpathellipse
			...
			\else
				\pgfpathmoveto{\pgfpointxy@orig{1}{0}}%
				\edef\pgfplots@loc@TMPa{{\pgfplots@endangle}{\pgfplots@startangle}{\the\pgf@xa\space and \the\pgf@xb}}%
			\fi
