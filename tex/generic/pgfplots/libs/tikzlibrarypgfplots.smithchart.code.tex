...
%	\let\pgfplotspointouternormalvectorofaxis@=\pgfplotspointouternormalvectorofaxis@smithchartaxis
	...
	\let\pgfplots@drawtickgridlines@INSTALLCLIP@onorientedsurf=\pgfplots@drawtickgridlines@INSTALLCLIP@onorientedsurf@smithchartaxis
...
\def\pgfplotsqpointxy@smithchartaxis#1#2{%
	\pgf@process{%
		% compute rx + j* ry = (1 - #1 - j * #2) / (1 + #1 +j * #2)
		%
		% I write
		% 1 - #1 - j * #2 = A + j * B
		% 1 + #1 + j * #2 = C + j * D
		...
		%
		\pgfplotscoordmath{default}{op}{subtract}{{\pgfplots@one}{\pgfplots@x}}%
		\let\pgfplots@A=\pgfmathresult
		%
		\pgfplotscoordmath{default}{zero}%
		\pgfplotscoordmath{default}{op}{subtract}{{\pgfmathresult}{\pgfplots@D}}%
		\let\pgfplots@B=\pgfmathresult
		%
		% ok, compute it:
...
\def\pgfplotspointouternormalvectorofaxis@smithchartaxis#1#2#3\relax{%
	\if v#1%
		...
		% FIXME : datascaling!
		% FIXME : aspect ratios!
		% perhaps pgfpointsmithchartxy?
		\pgfpointsmithchart{\pgfplotsretval}{1pt}%
	\else
		\if v#2%
			\if#10%
...
\def\pgfplots@drawtickgridlines@INSTALLCLIP@onorientedsurf@smithchartaxis#1{%
