...
\def\pgfplots@gridlines@init@arc@limits{%
	...
	% What I want is that 
	% 0.0 gets sort index -1
	% 1.0 and -1.0 get sort index 0
	% 3.0 and -3.0 get sort index 1
	% 4.0 and -4.0 get sort index 2
	% and so on. The array contains only absolute values, so that's
	% not too difficult to check.
	%
	% The reason why the '0.0' line gets index -1 is to start the each
	% nth stuff with the first arc -- the '0.0' line is no arc and
	...
	\c@sortindex=-1 % this is assigned to the '0.0' ygridline (if any)
