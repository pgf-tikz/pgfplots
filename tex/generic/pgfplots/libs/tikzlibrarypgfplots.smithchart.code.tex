...
\pgfplotsset{
	...
	/pgfplots/xgrid stop at y start/.initial=0,
	/pgfplots/ygrid stop at x/.initial=,
	/pgfplots/ygrid stop at x start/.initial=0,
	%
...
\def\pgfplots@drawgridlines@INSTALLCLIP@onorientedsurf@smithchartaxis#1{%
	...
	\pgfkeysgetvalue{/pgfplots/ygrid stop at x}\pgfplots@ygrid@stop@at@x@key
	\ifx\pgfplots@ygrid@stop@at@x@key\pgfutil@empty
	\else
		\pgfplots@gridlines@init@grid@stop@points@for
	\fi
}%

% Initialises the 'ygrid stop at x' feature. All it does is to prepare the
% method \pgfplots@get@current@ygrid@stop@at@x. 
%
...
\def\pgfplots@gridlines@init@grid@stop@points@for{%
	%
	% we have no "xticknum -> xtickpos" lookup table yet.
	\def\b@pgfplots@xticknum@to@pos@lookup{0}%
	%
	% normalise the argument for 'ygrid stop at x': each list element
	% should be of the form '<xtickpos>:<n>' where <n> means that each
	% <n>th arc can pass.
	\pgfplotslistnewempty\pgfplots@ygrid@stop@at@x@normalised
	\expandafter\pgfplotsutilforeachcommasep\pgfplots@ygrid@stop@at@x@key\as\entry{%
		\expandafter\pgfplots@gridlines@init@grid@stop@points@for@normalise\entry\relax
		\expandafter\pgfplotslistpushback\entry\to\pgfplots@ygrid@stop@at@x@normalised
	}%
	%
	% Ok. 
	%
	% Now, the 'ygrid stop at x' feature relies *crucially* on grid line
	...
	\c@sortindex=\pgfkeysvalueof{/pgfplots/ygrid stop at x start} % this is assigned to the '0.0' ygridline (if any)
...
% manual).
%
\def\pgfplots@gridlines@init@grid@stop@points@for@init@ticknum@lookup{%
	\c@pgf@counta=0
	\ifpgfplots@ymajorgrids
...
\def\pgfplots@gridlines@init@grid@stop@points@for@normalise#1\relax{%
	\pgfutil@in@:{#1}%
	\ifpgfutil@in@
	\else
		\edef\entry{\entry:2}%
	...
	\ifpgfutil@in@
		\if0\b@pgfplots@xticknum@to@pos@lookup
			\pgfplots@gridlines@init@grid@stop@points@for@init@ticknum@lookup
		\fi
		\expandafter\pgfplots@gridlines@stop@at@unpack\entry\relax
		\pgfutil@ifundefined{pgfplots@xtick@num@to@pos@\pgfplots@ygrid@stop@at@x}{%
			\pgfplots@warning{There is no xtick with index '\pgfplots@ygrid@stop@at@x'. Skipping it.}%
			\let\entry=\pgfutil@empty
		}{%
			\edef\entry{\csname pgfplots@xtick@num@to@pos@\pgfplots@ygrid@stop@at@x\endcsname:\pgfplots@arcskipeachnth}%
		}%
	\fi
}%
\def\pgfplots@gridlines@stop@at@unpack#1:#2\relax{%
	\def\pgfplots@ygrid@stop@at@x{#1}%
...
%
% The method relies on the 'ygrid stop at x' feature, more specifically the
% stuff prepared by \pgfplots@gridlines@init@grid@stop@points@for
\def\pgfplots@get@current@ygrid@stop@at@x#1{%
	\def#1{}%
	\ifx\pgfplots@ygrid@stop@at@x@key\pgfutil@empty
	\else
		...
		\pgfutil@ifundefined{pgfplots@tickpos@to@sortidx@\pgfplots@ticknum}{%
			...
			%
			\pgfplotslistforeachungrouped\pgfplots@ygrid@stop@at@x@normalised\as\pgfplots@loc@TMPa{%
				\ifx#1\pgfutil@empty
					...
					\else
						...
						\let#1=\pgfplots@ygrid@stop@at@x
					\fi
				\fi
			}%
		}%
...
\def\pgfplots@smithchart@draw@yarc#1{%
	\pgfplotsmath@ifapproxequal@dim{#1pt}{0pt}{0.002pt}{%
		...
		%
		% compute end angle.
		\pgfplots@get@current@ygrid@stop@at@x\pgfplots@arc@ends@at@x@circle@value%
		%
