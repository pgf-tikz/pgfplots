...
%\message{\pgfplots@ticknum\space(abs(tickpos) = \pgfplots@tick)---> sort index \csname pgfplots@#1tickpos@to@sortidx@\pgfplots@ticknum\endcsname^^J}%
	...
	\expandafter\pgfplotslistforeachungrouped\csname pgfplots@prepared@tick@positions@major@#1\endcsname\as\elem{%
		...
		\pgfplots@gridlines@preparation@get@current@grid@stop@point #1#2\pgfplots@tick
...
\def\pgfplots@gridlines@preparation@get@current@grid@stop@point#1#2#3{%
	\def\pgfplotsretval{}%
	%
	\pgfplots@gridlines@stopmap@prepare{#3}%
...
\def\pgfplots@get@current@grid@stop@point@handle@xconstraints#1{%
	\expandafter\let\csname pgfplots@final@stop@point@for@xtick@\pgfplots@tick@prepared\endcsname=\pgfplotsretval
\message{x grid line: storing end value for '#1' END[\pgfplots@tick@prepared] = \pgfplotsretval.^^J}%
	...
	\expandafter\let\csname pgfplots@minimal@abslength@for@ytick@\pgfplots@loc@TMPa\endcsname=\pgfmathresult
