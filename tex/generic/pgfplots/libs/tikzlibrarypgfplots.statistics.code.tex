...
\def\pgfplotsplothandlersurveyend@boxplot@computestats@quantile{%
	...
	\ifnum\c@pgf@countd=\c@pgfplots@coordindex\relax
		...
		\if 1\pgfplots@loc@TMPb
			...
			% 
			% we have to reassess the current point for the next search target.
			% Since this index is advanced during the loop, we have to decrement it here:
			\advance\c@pgfplots@coordindex by-1 %
		\else
