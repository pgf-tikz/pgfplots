...
\else
	\pgfplots@error{Library 'contourlua' requires you to run in LUA mode (i.e. lualatex instead of pdflatex)}%
\fi
	...
	contour lua/.style={
		contour external={%
			...
			script={
				...
				levels = PrepcMesh.computeLevels(\thecontournumber, \pgfplotsmetamin,\pgfplotsmetamax);
				\else
