...
\def\pgfplotsfillbetween@ensure@clipping@on@layer#1{%
	\ifpgfplots@clip
...
\def\pgfplotslibraryfillbetweenpreparecurrentlayer{%
	...
	\else
		\if0\b@pgfplotsfillbetween@list@has@set@layers
			\expandafter\pgfplotsfillbetween@ensure@clipping@on@layer\expandafter{\pgfplots@loc@TMPa}%
			%
			\edef\pgfplots@loc@TMPa{fill between/@ensure layers={\pgfkeysvalueof{/tikz/fill between/on layer}}}%
			\expandafter\pgfplotssetlateoptions\expandafter{\pgfplots@loc@TMPa}%
