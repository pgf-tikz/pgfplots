...
\pgfkeys{
	...
	/pgfplots/execute at end axis@@/.add={}{%
		\ifx\pgfplotsfillbetween@list@of@layers@with@clippath\pgfutil@empty
		\else
		\expandafter\pgfplotsutilforeachcommasep\expandafter{\pgfplotsfillbetween@list@of@layers@with@clippath}\as\pgfplots@loc@TMPa{%
			\edef\pgfplots@loc@TMPb{%
				\noexpand\pgfplotsextra{%
					\noexpand\pgfplotsonlayer{\pgfplots@loc@TMPa}%
					\noexpand\pgfsys@endscope
