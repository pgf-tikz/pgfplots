...
\pgfutil@IfUndefined{pgflibraryfillbetween.code.tex}{%
...
\def\pgfplotslibraryfillbetween@path#1#2#3{%
	...
	%
\iffalse
	\path[fill,#1,/pgfplots/fill between={#2}] 
		\pgfextra
...
\else
	\begingroup
	% In some cases, we do not even need to generate an outer path -
	% the result iterator can do that on its own.... which would allow
	% shadings, fadings, decorations and the whole zoo of tikz apps!
	\tikzset{fill,#1,/pgfplots/fill between={#2}}
	\pgfplotslibraryfillbetween@path@generatepath
	\endgroup
\fi
	%
