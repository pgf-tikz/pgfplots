...
\def\pgfplotsplothandlersurveystart@mesh@image{%
	\let\pgfplotsplothandlermesh@image@lastx=\pgfutil@empty
	\let\pgfplotsplothandlermesh@image@lastlastx=\pgfutil@empty
	\let\pgfplotsplothandlermesh@image@lasty=\pgfutil@empty
	\def\b@pgfplotsplothandlermesh@image@updatedlimits@x@next{1}%
	\def\b@pgfplotsplothandlermesh@image@updatedlimits@y@next{0}%
	%
	% FIXME : this will NOT be computed unless there ARE scanlines!
	\let\pgfplotsplothandlernotifyscanlinecomplete=\pgfplotsplothandlersurveyend@mesh@scanlinecomplete
}%
\def\pgfplotsplothandlersurveyend@mesh@image{%
	\if1\pgfplotsaxisplothasjumps
		...
		\pgfplots@error{Sorry, 'matrix input=image' supports no holes (jumps) in the input data. Please implement jumps using an unbounded point meta value (i.e. inf or nan)}%
	\fi
}%
\def\pgfplotsplothandlersurveypoint@mesh@image{%
	...
	%
	\if1\b@pgfplotsplothandlermesh@image@updatedlimits@x@next
		\ifx\pgfplots@current@point@x\pgfutil@empty
		\else
			\ifx\pgfplotsplothandlermesh@image@lastx\pgfutil@empty
			\else
				\pgfplotscoordmath{x}{parse}{\pgfplotsplothandlermesh@image@lastx - 0.5*(\pgfplots@current@point@x - \pgfplotsplothandlermesh@image@lastx)}%
				\let\pgfplots@current@point@xh=\pgfmathresult
				\pgfplotsaxisupdatelimitsforcoordinate\pgfplots@current@point@xh\pgfplots@current@point@y\pgfplots@current@point@z
				%
				\def\b@pgfplotsplothandlermesh@image@updatedlimits@x@next{0}%
			\fi
		\fi
	\fi
	%
	\if1\b@pgfplotsplothandlermesh@image@updatedlimits@y@next
		\ifx\pgfplots@current@point@y\pgfutil@empty
		\else
			\ifx\pgfplotsplothandlermesh@image@lasty\pgfutil@empty
			\else
				\pgfplotscoordmath{y}{parse}{\pgfplotsplothandlermesh@image@lasty - 0.5*(\pgfplots@current@point@y - \pgfplotsplothandlermesh@image@lasty)}%
				\let\pgfplots@current@point@yh=\pgfmathresult
				\pgfplotsaxisupdatelimitsforcoordinate\pgfplots@current@point@x\pgfplots@current@point@yh\pgfplots@current@point@z
				%
				%
				\pgfplotscoordmath{y}{parse}{\pgfplots@current@point@y + 0.5*(\pgfplots@current@point@y - \pgfplotsplothandlermesh@image@lasty)}%
				\let\pgfplots@current@point@yh=\pgfmathresult
				\pgfplotsaxisupdatelimitsforcoordinate\pgfplots@current@point@x\pgfplots@current@point@yh\pgfplots@current@point@z
				%
				\def\b@pgfplotsplothandlermesh@image@updatedlimits@y@next{0}%
		...
		\fi
	\fi
	\let\pgfplotsplothandlermesh@image@lastlastx=\pgfplotsplothandlermesh@image@lastx
	\let\pgfplotsplothandlermesh@image@lastx=\pgfplots@current@point@x
	%
	\let\pgfplotsplothandlermesh@image@lasty=\pgfplots@current@point@y
}%

\def\pgfplotsplothandlersurveyend@mesh@scanlinecomplete{%
	\ifx\pgfplotsplothandlermesh@image@lastlastx\pgfutil@empty
	\else
		\ifx\pgfplotsplothandlermesh@image@lastx\pgfutil@empty
		\else
			\pgfplotscoordmath{x}{parse}{\pgfplotsplothandlermesh@image@lastx + 0.5*(\pgfplotsplothandlermesh@image@lastx - \pgfplotsplothandlermesh@image@lastlastx)}%
			\let\pgfplots@current@point@xh=\pgfmathresult
			\pgfplotsaxisupdatelimitsforcoordinate\pgfplots@current@point@xh\pgfplots@current@point@y\pgfplots@current@point@z
		\fi
	\fi
	%
	\def\b@pgfplotsplothandlermesh@image@updatedlimits@y@next{1}%
	%
	\let\pgfplotsplothandlermesh@image@lastlastx=\pgfutil@empty
	\let\pgfplotsplothandlermesh@image@lastx=\pgfutil@empty
	\def\b@pgfplotsplothandlermesh@image@updatedlimits@x@next{1}%
...
\def\pgfplotsplothandlermesh@PIPE@DECODE@image@sweep@create@cell#1{%
		...
		\pgfplotspatchvertexcopymeta\pgfplotsplothandlermesh@i@j\to\pgfplotsplothandlermesh@bb
		%
		%
		\if1#1%
			...
			\pgfplotspatchvertexcopymeta\pgfplotsplothandlermesh@im@j\to\pgfplotsplothandlermesh@ab
		\else
		...
		\ifnum\c@pgfplots@scanlineindex=1
			...
			\pgfplotspatchvertexcopymeta\pgfplotsplothandlermesh@i@jm\to\pgfplotsplothandlermesh@ba
			%
			\if1#1%
				...
				\pgfplotspatchvertexcopymeta\pgfplotsplothandlermesh@im@jm\to\pgfplotsplothandlermesh@aa
			\else
...
\def\pgfplotsplothandlermesh@PIPE@DECODE@image@sweep@create@cell@lastrow{%
	...
	%\pgfplotspatchvertexcopymeta\pgfplotsplothandlermesh@i@j\to\pgfplotsplothandlermesh@last@bb
	%
	\ifnum\c@pgfplots@scanlineindex=1
		...
		% FIXME : which is the correctly extrapolated cdata!?
		%\pgfplotspatchvertexcopymeta\pgfplotsplothandlermesh@i@jm\to\pgfplotsplothandlermesh@last@ba
	\else
