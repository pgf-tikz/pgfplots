...
\def\pgfplots@initsizes{%
	...
	\ifx\pgfplots@unit@vector@ratio\pgfplots@loc@TMPb
	\else
		...
		\or
			...
			\pgfplots@error{Sorry, the combination 'scale mode=stretch to fill' and 'unit vector ratio=\pgfplots@loc@TMPb' is unsupported. Please use 'scale mode='scale uniformly'}%
			\def\pgfplots@scale@mode@choice{3}% set to 'scale uniformly'
		\fi
	\fi
	%
...
\message{Got W/H scale for all x components: \pgfplots@glob@TMPa; for all y components: \pgfplots@glob@TMPb^^J}%
	...
	%
	% these values will be accumulated. empty means: nothing to do.
	\def\pgfplots@target@limitrescale@x{}%
	\def\pgfplots@target@limitrescale@y{}%
	\def\pgfplots@target@limitrescale@z{}%
	%
...
\def\pgfplots@BB@update@cumulative@limit@compensations{%
	...
	% operations:
	\ifx\pgfplots@target@limitrescale@x\pgfutil@empty
		\let\pgfmathresult=\pgfplots@target@limitrescale@x@
	\else
		\ifx\pgfplots@target@limitrescale@x@\pgfutil@empty
			\let\pgfmathresult=\pgfplots@target@limitrescale@x
		\else
			\pgfplotscoordmath{pgfbasic}{op}{add}{{\pgfplots@target@limitrescale@x@}{\pgfplots@target@limitrescale@x}}%
		\fi
	\fi
	\let\pgfplots@target@limitrescale@x=\pgfmathresult
	%
	\ifx\pgfplots@target@limitrescale@y\pgfutil@empty
		\let\pgfmathresult=\pgfplots@target@limitrescale@y@
	\else
		\ifx\pgfplots@target@limitrescale@y@\pgfutil@empty
			\let\pgfmathresult=\pgfplots@target@limitrescale@y
		\else
			\pgfplotscoordmath{pgfbasic}{op}{add}{{\pgfplots@target@limitrescale@y@}{\pgfplots@target@limitrescale@y}}%
		\fi
	\fi
	\let\pgfplots@target@limitrescale@y=\pgfmathresult
	%
	\ifx\pgfplots@target@limitrescale@z\pgfutil@empty
		\let\pgfmathresult=\pgfplots@target@limitrescale@z@
	\else
		\ifx\pgfplots@target@limitrescale@z@\pgfutil@empty
			\let\pgfmathresult=\pgfplots@target@limitrescale@z
		\else
			\pgfplotscoordmath{pgfbasic}{op}{add}{{\pgfplots@target@limitrescale@z@}{\pgfplots@target@limitrescale@z}}%
		\fi
	\fi
...
\def\pgfplots@BB@for@plotbox@get@unit@scales@compensated@axis@limits#1#2#3#4#5#6{%
	...
	\endgroup
\message{scale mode=scale uniformly: returning \meaning\pgfplots@glob@TMPa^^J}%
	\pgfplots@glob@TMPa
}
