...
% #1: axis
% #2: [input/output] the macro containing the current scale
\def\pgfplots@axis@apply@post@scale#1#2{%
	%
...
\def\pgfplots@rescale@view@dir{%
	...
	\else
		...
		\pgfplotsmathvectorfromstring{%
			\pgfkeysvalueof{/pgfplots/scaling/x unit scale},%
			\pgfkeysvalueof{/pgfplots/scaling/y unit scale},%
			\pgfkeysvalueof{/pgfplots/scaling/z unit scale}%
		}{default}%
...
\def\pgfplots@scaleaxes@to@BB@prepare@plotbox@limits{%
	...
	%
	\ifx\pgfplots@x\pgfutil@empty
		\def\pgfplots@plotbox@xmin{0}%
		\def\pgfplots@plotbox@xmax{1}%
		\def\b@pgfplots@plotbox@xisunit{1}%
	\else
		\let\pgfplots@plotbox@xmin\pgfplots@xmin%
		\let\pgfplots@plotbox@xmax\pgfplots@xmax%
		\def\b@pgfplots@plotbox@xisunit{0}%
		\if2\pgfplots@scale@mode@choice
			% scale mode=stretch to fill
			\def\b@pgfplots@rescale@x{0}%
		\fi
	\fi
	\ifx\pgfplots@y\pgfutil@empty
		\def\pgfplots@plotbox@ymin{0}%
		\def\pgfplots@plotbox@ymax{1}%
		\def\b@pgfplots@plotbox@yisunit{1}%
	\else
		\let\pgfplots@plotbox@ymin\pgfplots@ymin%
		\let\pgfplots@plotbox@ymax\pgfplots@ymax%
		\def\b@pgfplots@plotbox@yisunit{0}%
		\if2\pgfplots@scale@mode@choice
			% scale mode=stretch to fill
			\def\b@pgfplots@rescale@y{0}%
		\fi
	\fi
	\ifx\pgfplots@z\pgfutil@empty
		\def\pgfplots@plotbox@zmin{0}%
		\def\pgfplots@plotbox@zmax{1}%
		\def\b@pgfplots@plotbox@zisunit{1}%
	\else
		\let\pgfplots@plotbox@zmin\pgfplots@zmin%
		\let\pgfplots@plotbox@zmax\pgfplots@zmax%
		\def\b@pgfplots@plotbox@zisunit{0}%
		\if2\pgfplots@scale@mode@choice
			% scale mode=stretch to fill
			\def\b@pgfplots@rescale@z{0}%
		\fi
	\fi
	%
\iffalse
% the following lines restore what was previously done in
% \pgfplotsset{compat=1.3}.
% However, it restores it only partially: the axis equal feature would
% still be broken.
\let\pgfplots@plotbox@xmin\pgfplots@xmin%
\let\pgfplots@plotbox@xmax\pgfplots@xmax%
\def\b@pgfplots@plotbox@xisunit{0}%
...
\def\pgfplots@scaleaxes@to@BB@#1#2{%
	...
	\if1\pgfplots@scale@mode@choice
		...
		\def\xscale{1}%
		\def\yscale{1}%
		\def\pgfplots@target@unit@scale@x{1}%
		\def\pgfplots@target@unit@scale@y{1}%
		\def\pgfplots@target@unit@scale@z{1}%
	\else
		%
		...
		%
\message{got scales to fit limits into BB: x=\pgfplots@target@unit@scale@x, y=\pgfplots@target@unit@scale@y, z=\pgfplots@target@unit@scale@z^^J}%
		%
...
\message{Got W/H scale for all x components: \xscale; for all y components: \xscale; ^^J   axis limit componsation scales x=\pgfplots@target@limitrescale@x, y=\pgfplots@target@limitrescale@y, z=\pgfplots@target@limitrescale@z^^J}%
		...
		\pgfplots@BB@update@cumulative@limit@compensations
		%
\message{adjusted scales for 'unit vector ratio': x=\pgfplots@target@unit@scale@x, y=\pgfplots@target@unit@scale@y, z=\pgfplots@target@unit@scale@z;  ^^J   axis limit componsation scales x=\pgfplots@target@limitrescale@x, y=\pgfplots@target@limitrescale@y, z=\pgfplots@target@limitrescale@z^^J}%
	\fi
	%
	\pgfplots@scaling@adjust@datascaling%
\message{adjusted scales for data scale trafo: x=\pgfplots@target@unit@scale@x, y=\pgfplots@target@unit@scale@y, z=\pgfplots@target@unit@scale@z;  ^^J   axis limit componsation scales x=\pgfplots@target@limitrescale@x, y=\pgfplots@target@limitrescale@y, z=\pgfplots@target@limitrescale@z;^^J  data scale trafo exponents x=\pgfplots@target@datascaletrafo@x@exponent@old -> \pgfplots@target@datascaletrafo@x@exponent, y=\pgfplots@target@datascaletrafo@y@exponent@old -> \pgfplots@target@datascaletrafo@y@exponent, z=\pgfplots@target@datascaletrafo@z@exponent@old -> \pgfplots@target@datascaletrafo@z@exponent^^J}%
	%
	\pgfplots@axis@apply@post@scale{x}{\pgfplots@target@unit@scale@x}%
	\pgfplots@axis@apply@post@scale{y}{\pgfplots@target@unit@scale@y}%
	\ifpgfplots@threedim
		\pgfplots@axis@apply@post@scale{z}{\pgfplots@target@unit@scale@z}%
	\fi
%\message{adjusted scales for post scales: x=\pgfplots@target@unit@scale@x, y=\pgfplots@target@unit@scale@y, z=\pgfplots@target@unit@scale@z;  ^^J   axis limit componsation scales x=\pgfplots@target@limitrescale@x, y=\pgfplots@target@limitrescale@y, z=\pgfplots@target@limitrescale@z^^J}%
	%
...
\def\pgfplots@scaling@compute@final@scales#1#2#3#4#5{%
	\def\pgfplots@loc@TMPa##1##2{%
		\pgfplotscoordmath{\pgfplots@compat@scaling@coordmath}{parsenumber}{##1}%
		\let\pgfplots@loc@TMPd=\pgfmathresult
		\pgfplotscoordmath{\pgfplots@compat@scaling@coordmath}{parsenumber}{##2}%
		\pgfplotscoordmath{\pgfplots@compat@scaling@coordmath}{op}{multiply}{{\pgfplots@loc@TMPd}{\pgfmathresult}}%
		\pgfplotscoordmath{\pgfplots@compat@scaling@coordmath}{tofixed}{\pgfmathresult}%
	}%
	\if1\b@pgfplots@rescale@x
		\pgfplots@loc@TMPa{\xscale}{\pgfplots@target@unit@scale@x}%
		\let\pgfplots@target@unit@scale@xx=\pgfmathresult
		\pgfplots@loc@TMPa{\yscale}{\pgfplots@target@unit@scale@x}%
		\let\pgfplots@target@unit@scale@xy=\pgfmathresult
	\else
		\let\pgfplots@target@unit@scale@xx=\pgfplots@target@unit@scale@x
		\let\pgfplots@target@unit@scale@xy=\pgfplots@target@unit@scale@x
	\fi
	%
	\if1\b@pgfplots@rescale@y
		\pgfplots@loc@TMPa{\xscale}{\pgfplots@target@unit@scale@y}%
		\let\pgfplots@target@unit@scale@yx=\pgfmathresult
		\pgfplots@loc@TMPa{\yscale}{\pgfplots@target@unit@scale@y}%
		\let\pgfplots@target@unit@scale@yy=\pgfmathresult
	\else
		\let\pgfplots@target@unit@scale@yx=\pgfplots@target@unit@scale@y
		\let\pgfplots@target@unit@scale@yy=\pgfplots@target@unit@scale@y
	\fi
	%
	\ifpgfplots@threedim
		\if1\b@pgfplots@rescale@z
			\pgfplots@loc@TMPa{\xscale}{\pgfplots@target@unit@scale@z}%
			\let\pgfplots@target@unit@scale@zx=\pgfmathresult
			\pgfplots@loc@TMPa{\yscale}{\pgfplots@target@unit@scale@z}%
			\let\pgfplots@target@unit@scale@zy=\pgfmathresult
		\else
			\let\pgfplots@target@unit@scale@zx=\pgfplots@target@unit@scale@z
			\let\pgfplots@target@unit@scale@zy=\pgfplots@target@unit@scale@z
		\fi
	\fi
	%
}%
...
%  \pgfplots@target@unit@scale@x and its variants for y and z
%
% OUTPUT:
...
\def\pgfplots@scaling@adjust@datascaling@for#1{%
	\pgfplots@if{pgfplots@apply@datatrafo@#1}{%
		...
		\ifdim\pgf@xa>5pt %
			...
			\pgf@xa=\csname pgfplots@target@unit@scale@#1\endcsname pt
			\multiply\pgf@xa by\pgfplotsretval\relax %
			\expandafter\edef\csname pgfplots@target@unit@scale@#1\endcsname{\pgf@sys@tonumber\pgf@xa}%
			%
...
\def\pgfplots@get@scale@horiz@and@vert@scaleuniformly@of@optimal@strategy{%
	\begingroup
		\pgfplotscoordmath{pgfbasic}{parsenumber}{16000}%
		...
		\def\pgfplots@scalestrategy@compute@cost{%
			...
			\pgfplotscoordmath{pgfbasic}{parsenumber}{\pgfplots@target@limitrescale@x}%
			\let\X=\pgfmathresult
			\pgfplotscoordmath{pgfbasic}{parsenumber}{\pgfplots@target@limitrescale@y}%
			\let\Y=\pgfmathresult
			\ifpgfplots@threedim
				\pgfplotscoordmath{pgfbasic}{parsenumber}{\pgfplots@target@limitrescale@z}%
				\let\Z=\pgfmathresult
			\else
				\let\Z=\pgfplots@target@limitrescale@z
			\fi
			%
			% If one of the resulting limit compensation scales its
			% less than 1, we can immediately skip it - we do not want
			% to risk to clip away image content.
			\pgfplotscoordmath{pgfbasic}{if less than}{\X}{\ONE}{%
				\let\pgfplots@cost@for@choice=\pgfplots@cost@for@choice@superhigh
			}{%
				\pgfplotscoordmath{pgfbasic}{if less than}{\Y}{\ONE}{%
					\let\pgfplots@cost@for@choice=\pgfplots@cost@for@choice@superhigh
				}{%
					\ifpgfplots@threedim
						\pgfplotscoordmath{pgfbasic}{if less than}{\Z}{\ONE}{%
							\let\pgfplots@cost@for@choice=\pgfplots@cost@for@choice@superhigh
						}{%
...
\message{scale uniformly strategy=auto: '\pgfplots@tostring@scaleuniformlystrategy{\pgfplots@scaleuniformly@choice}' has cost \pgfplots@cost@for@choice\space(limit rescaling factors x=\X, y=\Y, z=\Z)^^J}%
		...
		\def\pgfplots@scalestrategy@compute@cost@{%
			\pgfplotscoordmath{pgfbasic}{op}{add}{{\X}{\Y}}%
			\ifpgfplots@threedim
				\pgfplotscoordmath{pgfbasic}{op}{add}{{\pgfmathresult}{\Z}}%
			\fi
		...
		%
		% update minimum:
		\pgfplotscoordmath{pgfbasic}{if less than}{\pgfplots@cost@for@choice}{\pgfplots@cost@for@choice@sofar}{%
			\let\pgfplots@cost@for@choice@arg=\pgfplots@scalestrategy@values
			\let\pgfplots@cost@for@choice@sofar=\pgfplots@cost@for@choice%
		}{%
		}%
...
\def\pgfplots@BB@for@plotbox@get@unit@scales@compensated@axis@limits#1#2#3#4#5#6{%
	...
	% target plot box.
	\def\pgfplots@loc@TMPa##1##2{%
		\if1\pgfplots@scaleuniformly@choice
