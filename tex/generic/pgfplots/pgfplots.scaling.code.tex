...
\def\pgfplots@scale@plotbox@to@widthheight{%
	...
	\let\pgfplots@BB@for@plotbox@getunitheight=\pgfplots@BB@for@plotbox@getunitheight@relative
	\let\pgfplots@BB@for@plotbox@get@unit@scales@for@limits=\pgfplots@BB@for@plotbox@get@unit@scales@for@limits@relative
...
\def\pgfplots@scale@axisbox@to@widthheight{%
	...
	\let\pgfplots@BB@for@plotbox@getunitheight=\pgfplots@BB@for@plotbox@getunitheight@computelimits
	\let\pgfplots@BB@for@plotbox@get@unit@scales@for@limits=\pgfplots@BB@for@plotbox@get@unit@scales@for@limits@NOP
	\expandafter\pgfplots@scaleaxes@to@BB\pgfplots@loc@TMPa
}
...
\let\pgfplots@BB@for@plotbox=\pgfplots@BB@for@plotbox@relative
\def\pgfplots@BB@for@plotbox@get@unit@scales@for@limits@relative#1#2#3{%
...
\def\pgfplots@BB@for@plotbox@get@unit@scales@for@limits@NOP#1#2#3{%
	\def#1{1}%
	\def#2{1}%
	\def#3{1}%
}%
...
\def\pgfplots@target@limitrescale@z{1}%
	\or
		...
		% equations are non-linear. A possible way to solve for R and
		% s is to iterate: choose R arbitrarily (for example R=1),
		% solve for s. Then, fix s and solve for R. Then, fix R and
...
\def\pgfplots@target@limitrescale@z{1}%
		...
		\global\let\pgfplots@glob@TMPc=\relax
		\global\let\pgfplots@glob@TMPd=\relax
		\ifdim\pgf@zy=0pt
			\ifdim\pgf@yx=0pt
				\global\def\pgfplots@glob@TMPd{y}%
		...
		\else
			\ifdim\pgf@zx=0pt
				\global\def\pgfplots@glob@TMPd{z}%
		...
		\fi
\else
\pgfplots@FOO{\pgfplots@target@unit@scale@x}{\pgfplots@target@limitrescale@x}{\pgfplots@target@limitrescale@y}{\pgfplots@target@limitrescale@z}%
\global\let\pgfplots@glob@TMPa=\pgfplots@target@unit@scale@x
\global\let\pgfplots@glob@TMPb=\pgfplots@glob@TMPa
\global\let\pgfplots@glob@TMPc=\relax
\fi
	\fi
\global\let\pgfplots@target@limitrescale@x=\pgfplots@target@limitrescale@x
\global\let\pgfplots@target@limitrescale@y=\pgfplots@target@limitrescale@y
\global\let\pgfplots@target@limitrescale@z=\pgfplots@target@limitrescale@z
	\endgroup
\message{Got W/H scale for all x components: \pgfplots@glob@TMPa; for all y components: \pgfplots@glob@TMPb^^J}%
	...
	%
	% these values will be accumulated.
%	\def\pgfplots@target@limitrescale@x{1}%
%	\def\pgfplots@target@limitrescale@y{1}%
%	\def\pgfplots@target@limitrescale@z{1}%
	%
	\ifx\pgfplots@glob@TMPc\relax
	\else
		% initialize limit compensation in vertical direction:
		\if x\pgfplots@glob@TMPd
			\let\pgfplots@target@limitrescale@x=\pgfplots@glob@TMPc
		\fi
		\if y\pgfplots@glob@TMPd
			\let\pgfplots@target@limitrescale@y=\pgfplots@glob@TMPc
		\fi
		\if z\pgfplots@glob@TMPd
			\let\pgfplots@target@limitrescale@z=\pgfplots@glob@TMPc
		\fi
	\fi
	%
...
\message{got scales to fit limits into BB: scalex=\pgfplots@target@unit@scale@x, scaley=\pgfplots@target@unit@scale@y, scalez=\pgfplots@target@unit@scale@z, axis limit componsation scales x=\pgfplots@target@limitrescale@x, y=\pgfplots@target@limitrescale@y, z=\pgfplots@target@limitrescale@z^^J}%
	%
	...
	\fi
\message{adjusted scales for 'scale mode': scalex=\pgfplots@target@unit@scale@x, scaley=\pgfplots@target@unit@scale@y, scalez=\pgfplots@target@unit@scale@z;  axis limit componsation scales x=\pgfplots@target@limitrescale@x, y=\pgfplots@target@limitrescale@y, z=\pgfplots@target@limitrescale@z^^J}%
	%
...
\def\pgfplots@FOO#1#2#3#4{%
	...
	\let\M=\pgfmathresult
	%
	\pgfplotscoordmath{default}{parsenumber}{\expandafter\pgf@sys@tonumber\csname pgf@xa\endcsname}%
	\let\w=\pgfmathresult
	\pgfplotscoordmath{default}{parsenumber}{\expandafter\pgf@sys@tonumber\csname pgf@ya\endcsname}%
	\let\W=\pgfmathresult
	\pgfplotscoordmath{default}{parsenumber}{\expandafter\pgf@sys@tonumber\csname pgf@xb\endcsname}%
	\let\h=\pgfmathresult
	\pgfplotscoordmath{default}{parsenumber}{\expandafter\pgf@sys@tonumber\csname pgf@yb\endcsname}%
	\let\H=\pgfmathresult
...
\message{updated S = \S\space ( R = \R ) ^^J}%
	...
	\endgroup
\message{returning \meaning\pgfplots@glob@TMPa^^J}%
	%
	\pgfplots@glob@TMPa
}%
...
% #1 : a dimen register
% #2 : x, y, or z
\def\pgfplots@BB@for@plotbox@getunitheight@computelimits#1#2{%
	#1=\csname pgfplots@#2max\endcsname\csname pgf@#2y\endcsname
	\advance#1 by -\csname pgfplots@#2min\endcsname\csname pgf@#2y\endcsname
}%

% #1 : a dimen register
% #2 : x, y, or z
\def\pgfplots@BB@for@plotbox@getunitheight@relative#1#2{%
...
\def\pgfplots@prepare@vertical@rescaling@for@scale@uniformly@in@dir#1\returninto#2{%
	...
	% H = \pgf@yb
	% \pgf@xb = actual height 'h' = r_x * e_xy + r_y * e_yy + r_z * e_zy
	% =>
	% s_z = ( H- s*( \pgf@xb - r_z * e_zy) ) / ( s * r_z * e_zy).
	% 
	\begingroup
	...
	%
	\pgfplotscoordmath{default}{parsenumber}{\pgfplots@glob@TMPa}%
	\let\pgfplots@s=\pgfmathresult
	%
	% this is a precondition of this method:
	\pgfplotscoordmath{default}{parsenumber}{\pgf@sys@tonumber\pgf@xb}%
	\let\pgfplots@xb=\pgfmathresult
	\pgfplotscoordmath{default}{parsenumber}{\pgf@sys@tonumber\pgf@yb}%
	\let\pgfplots@yb=\pgfmathresult
	%
	% compute counter := YB - s * (XB - (max-min))
	\pgfplotscoordmath{default}{op}{subtract}{{\pgfplots@xb}{\pgfplots@diff}}%
	\pgfplotscoordmath{default}{op}{multiply}{{\pgfplots@s}{\pgfmathresult}}%
	\pgfplotscoordmath{default}{op}{subtract}{{\pgfplots@yb}{\pgfmathresult}}%
...
%\message{Setting unitvector(#1) to auto-computed multiple of e_#2 ...}%
		...
		\fi
		\begingroup
			\pgf@xa=\pgfplots@loc@TMPa\relax
