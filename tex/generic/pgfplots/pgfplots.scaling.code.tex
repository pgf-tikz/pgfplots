...
\def\pgfplots@scaling@compute@final@scales#1#2#3#4#5{%
	\ifpgfplots@threedim
		\let\pgfplots@compat@scaling@coordmath=\pgfplots@compat@scaling@coordmath@threedim
	\fi
...
\def\pgfplots@BB@for@plotbox@get@unit@scales@compensated@axis@limits#1#2#3#4#5#6{%
	...
	\ifdim\pgf@zx=0pt
		\ifdim\pgf@zy=0pt
			% set this temporarily to 'false' -- we can ignore the z vector in this method:
	...
	\else
		\ifcase\pgfplots@scale@mode@choice
			...
			\if1\pgfplots@compat@scale@mode@compatible@mode
				...
				\else
					% ... for 2d: 
					\edef\pgfplots@loc@TMPb{\pgfplots@loc@TMPb\space1 1 }%
					%
					\expandafter\pgfplots@unit@vector@ratio@check@nop\pgfplots@loc@TMPb\pgfplots@EOI
					\ifpgfplots@loc@tmp
						% we have a non-trivial unit vector ratio:
						\def\pgfplots@scale@mode@choice{2}% set to 'stretch to fill'
						\pgfplots@compat@scale@mode@compatible@mode@warning
					\else
						% ok; unit vector ratio=1 1 1  is still the
						% same.
					\fi
				\fi
			\fi
...
\def\pgfplots@compat@scale@mode@compatible@mode@warning{%
	\ifpgfplots@compat@scale@mode@compatible@mode@warning
		\pgfplots@warning{You have an axis with 'axis equal' and/or 'unit vector ratio'. In backwards compatibility mode, this suffers from skewed angles. Please consider writing \string\pgfplotsset{compat=1.6} (or newer) in your preamble to enable the upgrade. Use \string\pgfplotsset{compat/scale mode warning=false} to disable this warning.}%
	\fi
}%
