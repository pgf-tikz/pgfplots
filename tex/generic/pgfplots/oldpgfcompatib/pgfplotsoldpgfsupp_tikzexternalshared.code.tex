...
%%%  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Date of this copy: Tue Feb 16 14:45:51 CET 2010 %%%
...
%	system call/.initial={pdflatex -interaction=batchmode -jobname '\image' '\texsource'; pdftops -eps \image.pdf},
	...
	read main aux/.is if=pgfexternalreadsmainaux,
	optimize/.is choice,
	optimize/true/.code={%
...
\def\tikzexternalize@opt[#1]#2{%
	...
	\else
		...
		\let\tikzexternalfiledependsonfile=\tikzexternalfiledependsonfile@INACTIVE
		\let\tikzpicturedependsonfile=\tikzpicturedependsonfile@INACTIVE
	\fi
}
...
% called.
\def\tikzexternalfiledependsonfile@INACTIVE#1#2{%
	\begingroup
...
% This command will be actived as soons as \tikzexternalize has been
% called.
\def\tikzpicturedependsonfile@INACTIVE#1{%
	\begingroup
	\ifx\tikzexternal@curfilename\relax
		% we are *not* inside of the exported file.
		% use next file name.
		\tikzexternalgetnextfilename\tikzpicturedependsonfile@name
	\else
		\let\tikzpicturedependsonfile@name=\tikzexternal@curfilename
	\fi
...
\long\def\tikzexternal@externalizefig@systemcall@@#1{%
	...
	\else
		...
		\fi
		%
		% FIXME : here is a problem with .aux files -- if the \@auxout
		% file is not complete (for example due to write buffers), the
