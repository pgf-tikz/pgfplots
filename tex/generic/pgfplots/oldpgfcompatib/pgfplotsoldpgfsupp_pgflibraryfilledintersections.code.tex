...
%%%  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Date of this copy: Sa 16. Nov 22:03:05 CET 2013 %%%
...
\newif\ifpgfpathfilledintersections@split

\pgfkeys{%
	/pgf/filled intersection/reverse/.is choice,
	/pgf/filled intersection/reverse/false/.code=\def\pgfpathfilledintersections@reverse@mode{0},%
	/pgf/filled intersection/reverse/true/.code=\def\pgfpathfilledintersections@reverse@mode{1},%
	%
	% checks the coordinates of the first two path instructions as heuristics:
	/pgf/filled intersection/reverse/auto/.code=\def\pgfpathfilledintersections@reverse@mode{2},%
	/pgf/filled intersection/reverse/auto,
	/pgf/filled intersection/split/.is if=pgfpathfilledintersections@split,
	/pgf/filled intersection/split/.default=true,
	/pgf/filled intersection/result stream/begin/.code=,%
	/pgf/filled intersection/result stream/next ready/.code={
	...
	/pgf/filled intersection/result stream/end/.code=,%
}%

% \pgfpathfilledintersections[<options>]{<first path>}{<second path>}
%
% Generates a new path consisting of the "filled intersection" of the two arguments.
% #1: optional arguments with /pgf/filled intersection prefix
...
\def\pgfpathfilledintersections{\pgfutil@ifnextchar[{\pgfpathfilledintersections@opt}{\pgfpathfilledintersections@opt[]}}%
\def\pgfpathfilledintersections@opt[#1]#2#3{%
	\begingroup
	\pgfqkeys{/pgf/filled intersection}{#1}%
	%
	% remove any special round-corner-tokens:
	\pgfprocessround{#2}{#2}%
	\pgfprocessround{#3}{#3}%
	%
	\if2\pgfpathfilledintersections@reverse@mode
		% reverse=auto
		\expandafter\pgfpathfilledintersections@heuristics@get@direction#2\pgf@stop
		\let\pgfpath@filled@dir@first=\pgfretval
		\expandafter\pgfpathfilledintersections@heuristics@get@direction#3\pgf@stop
...
\message{reverse=auto: comparing \pgfpath@filled@dir@first\space with \pgfpath@filled@dir@second^^J}%
		\ifx\pgfpath@filled@dir@first\pgfpath@filled@dir@second
			...
			\def\pgfpathfilledintersections@reverse@mode{1}%
		\fi
	\fi
	%
	\pgfpathfilledintersections@invoke{result stream/begin}%
	%
	\ifpgfpathfilledintersections@split
		\pgfpathfilledintersections@compute@splitted{#2}{#3}%
	\else
		\pgfpathfilledintersections@compute{#2}{#3}%
		\pgfpathfilledintersections@invoke{result stream/next ready}%
	\fi
	%
	\pgfpathfilledintersections@invoke{result stream/end}%
	%
	\endgroup
}%

\def\pgfpathfilledintersections@invoke#1{\pgfkeysvalueof{/pgf/filled intersection/#1/.@cmd}\pgfeov}

\def\pgfpathfilledintersections@replace@first@moveto#1#2\pgf@stop{%
...
\def\pgfpathfilledintersections@compute@splitted#1#2{%
	...
	\ifnum\c@pgfpathfilled@counter<\pgfpathfilled@a@segments\relax
		...
		%
		\pgfpathfilledintersections@compute{\pgf@loc@path@a}{\pgf@loc@path@b}%
...
\def\pgfpathfilledintersections@compute#1#2{%
\message{Combination of \meaning#1\space and \meaning#2...^^J}%
	%
	\if1\pgfpathfilledintersections@reverse@mode
		% reverse=true
		%
	...
	\fi
	%
	\expandafter\pgfpathfilledintersections@replace@first@moveto#2\pgf@stop
	\let#2=\pgfretval
	% 
...
\def\pgfpathfilledintersections@heuristics@get@direction#1#2#3{%
	\pgfutil@ifnextchar\pgf@stop{%
		% wow. it has just one path element... we can do nothing.
		\def\pgfretval{++}%
		\pgfutil@gobble
	}{%
		\pgfpathfilledintersections@heuristics@get@direction@{#1}{#2}{#3}%
	}%
}%

% example: \pgfsyssoftpath@linetotoken{#2}{#3}\pgfsyssoftpath@linetotoken{#5}{#6}#7\pgf@stop
\def\pgfpathfilledintersections@heuristics@get@direction@#1#2#3#4#5#6#7\pgf@stop{%
	\edef\pgfretval{%
		\ifdim #2<#5\space
			+%
		\else 
