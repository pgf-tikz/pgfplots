...
%%%  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Date of this copy: Mo 23. Dez 18:09:16 CET 2013 %%%
...
\def\pgfpathfillbetween@compute@splitted#1#2{%
	...
	%
	% Make sure that we compute the time offsets. We *need* them in order to split curveto paths.
	\pgf@intersect@sorttrue
	% BUT: we do NOT want to sort by time; we only need the value:
	\let\pgfintersectionsolutionsortbytime=\relax
...
\def\pgf@insert@intersections@split@segment{%
	...
	\else
		\ifx\pgfinsert@intersect@tok@type\pgfsyssoftpath@curvetotoken
			...
			\edef\pgf@split@it@time{\csname pgf@intersect@solution@\pgf@insert@cursolution @time@\pgf@insert@intersections@src\endcsname}%
...
\csname pgfpoint@intersect@solution@\pgf@insert@cursolution\endcsname%
\message{Splitted curve to segment at t=\pgf@split@it@time; point \the\pgf@x,\the\pgf@y. Evaluation returned point \pgfretval^^J}%
