...
%%%  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Date of this copy: Mi 25. Dez 13:25:13 CET 2013 %%%
...
\def\pgfpathfillbetween@opt[#1]#2#3{%
	...
	\fi
	%
	% clip, if configured (no-op otherwise):
	\pgfpathfillbetween@clip{#2}{\pgfpathfillbetween@clip@A}\let#2=\pgfretval%
	\pgfpathfillbetween@clip{#3}{\pgfpathfillbetween@clip@B}\let#3=\pgfretval%
	%
...
% ---------------------------------------------------------------------------------
%
%  CLIPPING as part of 'fill between'.
%
%  "clip" means to get rid of parts at the beginning and/or end of a
%  path ("poor-mans-clipping").
%
%  The idea is to have a path on input which serves as a kind of clip
%  path. Everything outside of the clip path is cutted away.
%
...
\def\pgfpathfillbetween@clip#1#2{%
	\ifx#2\pgfutil@empty
		\let\pgfretval=#1%
	\else
		\begingroup
		...
		\else
			...
			\expandafter\pgfpathfillbetween@get@first@coord#2\pgf@stop
			\let\pgfpathfilled@b@firstcoord=\pgfretval
			%
			% Answer the question "b@pgffill@is@outside@clip = (is #1 outside of the clip path)
			%
			% I do not want to implement clipping in its entirety - I
			% hope it is sufficient to assume that *if* we have at
			% least one intersection, then we can deduce the answer
			% based on the first coord of '#1' and of '#2'. In
			% general, this will be wrong. But in the special case
			% where #2 is actually a rectangle which has been built to
			% cut off either beginning or end of #1, we can use it.
			% Will need adoptions eventually.
			\pgfpathfillbetween@check@x@less@than{\pgfpathfilled@a@firstcoord}{\pgfpathfilled@b@firstcoord}%
			...
			\ifnum\c@pgfpathfilled@counter<\pgfpathfilled@a@segments\relax
				...
				\pgfutil@advancestringcounter\c@pgfpathfilled@counter
			\repeatpgfmathloop
			\pgfapplistlet#1={pgfretval@tmp}%
		\fi
		%
		\global\let\pgf@glob@TMPa=#1%
		\endgroup
...
\def\pgffillbetweensetclippathfirst#1{%
	\let\pgfpathfillbetween@clip@A=#1%
}%

% #1: a macro containing the clip path. More precisely, it is supposed to be a return value of \pgfgetpath.
\def\pgffillbetweensetclippathsecond#1{%
	\let\pgfpathfillbetween@clip@B=#1%
}%

\pgffillbetweensetclippathfirst{\pgfutil@empty}
\pgffillbetweensetclippathsecond{\pgfutil@empty}
