...
\def\pgfplotstableresolvecolname#1\of#2\to#3{%
	\begingroup
	\def\pgfplotstablegetcolumnbyname@exists##1\of##2\to##3{\edef#3{##1}}%
...
\def\pgfplotstablereadgetptrtocolname@#1#2{%
	\begingroup
	\def\pgfplotstablegetcolumnbyname@exists##1\of##2\to##3{%
		\edef#2{\csname pgfplotstblread@colindex@for@name##1\endcsname}%
	}%
	\def\pgfplotstableifcolumnexists##1\of##2##3##4{%
		\pgfutil@ifundefined{pgfplotstblread@colindex@for@name##1}{##4}{##3}%
	}%
	\pgfplotstablegetcolumnbyname@impl{#1}\of{}\to{#2}\allowcreateonuse0%
	\pgfmath@smuggleone#2%
	\endgroup
}%
