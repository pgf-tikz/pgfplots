...
\pgfkeys{%
	...
	/pgfplots/table/create col/linear regression/x/.initial=[index]0,%
	/pgfplots/table/create col/linear regression/y/.initial=[index]1,%
	...
	/pgfplots/table/sort key/.initial=,%
	/pgfplots/table/sort asc/.code={\def\pgfplotstable@sort@asc{1}},
	/pgfplots/table/sort desc/.code={\def\pgfplotstable@sort@asc{0}},
	/pgfplots/table/sort asc,
...
% Sorts table #2 according to options defined in #1.
%
% \pgfplotstablesort[sort key=<colname>]\loadedtable
% \pgfplotstablesort[sort key=<colname>,sort key
% source=<\table>]\loadedtable
\def\pgfplotstablesort[#1]{%
	\pgfplotstablecollectoneargwithpreparecatcodes{%
		\pgfplotstablesort@[#1]%
	}%
}
\def\pgfplotstablesort@[#1]#2#3{%
	\begingroup
	\pgfplotstableset{#1}%
	...
	\pgfplotstable@isloadedtable{#3}{%
		...
		%
		\pgfplotstabletranspose[input colnames to=,colnames from=]\pgfp@tmp{#3}%
		\def\pgfplotstable@table{#3}%
		\let\pgfplotstable@input@colnames=#3\relax
	}{%
		\pgfplotstableread{#3}\pgfp@tmp@in
	...
	% 'pgfp@tmp@<index>'. So, we can interprete them as arrays!
	\pgfplotsarrayresize{\pgfp@tmp}{\pgfplotstable@numrows}%
	%
	\pgfkeysdefargs{/pgfplots/array/iflessthan}{##1##2##3##4}{%
		\pgfplotsarrayselect{\pgfplotsarraysortiflessthanindexa}\of\pgfplotstable@sort@key@array\to\pgfplotstable@arga
		\pgfplotsarrayselect{\pgfplotsarraysortiflessthanindexb}\of\pgfplotstable@sort@key@array\to\pgfplotstable@argb
		\pgfmathfloatparsenumber{\pgfplotstable@arga}%
		\let\pgfplotstable@arga=\pgfmathresult
		%
		\pgfmathfloatparsenumber{\pgfplotstable@argb}%
		\let\pgfplotstable@argb=\pgfmathresult
		%
		\if1\pgfplotstable@sort@asc
			\pgfmathfloatlessthan@{\pgfplotstable@arga}{\pgfplotstable@argb}%
		\else
			\pgfmathfloatgreaterthan@{\pgfplotstable@arga}{\pgfplotstable@argb}%
		\fi
		\ifpgfmathfloatcomparison
			##3%
		\else
