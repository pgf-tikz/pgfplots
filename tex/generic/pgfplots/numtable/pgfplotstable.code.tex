...
\pgfkeys{%
	...
	/pgfplots/table/create col/linear regression/.code={%
		\pgfplotstable@linear@regression#1\pgfplotstable@EOI
	...
	/pgfplots/table/create col/linear regression/xmode/.initial=linear,%
	/pgfplots/table/create col/linear regression/ymode/.initial=linear,%
...
% TODO : logs and variances.
% for semilogx:
% 	input: compute log(x_i)
% 	output: copy x_i from input
% for semilogy:
%   input: use log(y_i)
% 	output: compute y_i = exp(a*x_i + b)
% for loglog:
%   input: compute log(x_i) and log(y_i)
% 	output: take x_i from input source (which is not log)
% 	output: take y_i = exp( a* log(x_i) + b)
%
% OPEN: which log!?
% -> is to be determined.
%   
% variances: also from table. perhaps from another one?
% -> in any case, it should use 'create on use' things.
%
% #1: table file name or struct
% #2: x col
% #3: y col
% #4: options (or empty)
\def\pgfplotstable@linear@regression#1#2#3#4\pgfplotstable@EOI{%
	\begingroup
	\pgfqkeys{/pgfplots/table/create col/linear regression}{/pgf/fpu,#4}%
	\pgfplotstablegetcolumn{#2}\of{#1}\to\pgfplotstable@X
	\pgfplotstablegetcolumn{#3}\of{#1}\to\pgfplotstable@Y
	...
	\let\pgfplotstable@X@orig=\pgfplotstable@X
	%
	...
	\else
		\ifx\pgfplotstable@variance@table\pgfutil@empty
			\expandafter\pgfplotstablegetcolumn\expandafter{\pgfplotstable@variance@colname}\of{#1}\to\pgfplotstable@VARIANCE
		\else
	...
	\pgfplotslistforeachungrouped\pgfplotstable@X@orig\as\pgfplotstable@x{%
		\pgfplotstableparsex{\pgfplotstable@x}%
		\let\pgfplotstable@x=\pgfmathresult
		...
		\ifx\pgfplotstablelogbasisy\pgfutil@empty
		\else
			\expandafter\pgfmathfloatmultiply@\expandafter{\pgfmathresult}{\pgfplotstableparseylogofbase}%
			\pgfmathfloatexp@{\pgfmathresult}%
		\fi
		\pgfmathfloattosci{\pgfmathresult}%
...
\def\pgfplotstable@linear@regression@prepare@mode#1#2{%
	...
	\expandafter\def\csname pgfplotstablelogbasis#2\endcsname{}%
	\pgfplotsutilifstringequal{linear}{#1}{%
	}{%
		\pgfplotsutilifstringequal{log}{#1}{%
			\pgfkeysgetvalue{/pgfplots/log basis #2}\pgfplots@loc@TMPa
			\ifx\pgfplots@loc@TMPa\pgfutil@empty
				\def\pgfplots@loc@TMPa{2.718281828459}%
			\fi
			\expandafter\edef\csname pgfplotstablelogbasis#2\endcsname{\pgfplots@loc@TMPa}%
			\pgfplotsmathdefinemacrolnbase{pgfplotstableparse#2}{\pgfplots@loc@TMPa}%
			\expandafter\def\csname pgfplotstableparse#2\endcsname##1{%
				\pgfmathfloatparsenumber{##1}%
				\let\pgfmathresultparsed=\pgfmathresult
				\csname pgfplotstableparse#2@\endcsname{\pgfmathresultparsed}%
			}%
		}{%
