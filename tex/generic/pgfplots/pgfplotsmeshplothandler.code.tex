...
\def\pgfplotsplothandlersurveystart@mesh{%
	...
	\pgfplotscolornormalizesequence[colorspace=\pgfkeysvalueof{/pgfplots/mesh/colorspace explicit color input}]%
}%
...
\def\pgfplotsplothandlermesh@definemappedcolor#1{%
	...
	\else
		...
		% i.e. it is a tuple {<colorspace>}{<comma-separate-components>}
		\edef\pgfplots@loc@TMPb{\noexpand\pgfutil@definecolor{mapped color}#1}%
		\pgfplots@loc@TMPb
	\fi
}%
...
\def\pgfplotsplothandlermesh@init@shader{%
	...
	\ifnum\pgfplotsplothandlermesh@shader>1
		...
		\else
			\pgfkeyssetvalue{/pgfplots/surf shading/colorspace}{rgb}% FIXME : communicate from survey phase!
		\fi
		\if2\pgfplotsplothandlermesh@shader
			% shader=interp
