...
\def\pgfplotsplothandlermesh@init@flat@color@getter{%
	\if\pgfplotsplothandlermesh@flatmode c% 'flat corner':
		...
		\def\pgfplotsplothandlermesh@get@flat@color{%
			% ATTENTION : this does not respect potential reordering done
			% by 'z buffer'! That means that 'shader=flat corner' is
			% well defined for 'z buffer=none', otherwise it is
			% undefined (depends on view)
