...
\pgfqkeys{/pgfplots/@declare patch}{%
	...
	%
	% A method which defines \pgfplotsretvalu,\pgfplotsretvalv,\pgfplotsretvalw to be the normal of the current segment.
	% The values will be given in \pgfplotscoordmath{pgfbasic}
	% 
	% This method REQUIRES that 3d coordinates are stored (see \pgfplotsplothandlermesh@serialize@logical@coords).
	compute normal/.code=
		{\expandafter\def\csname pgfpptch\pgfplotsdeclarepatchclass@ compute normal\endcsname##1{#1}},%
}
...
\def\pgfplotsplothandlermesh@compute@normal@default{%
	...
	\c@pgf@countd=0
	\pgfplotspatchclass{\pgfplotspatchclassname}{foreach vertex}{%
		\expandafter\let\csname point\the\c@pgf@countd\endcsname=\pgfplotspatchvertexcoords
		\advance\c@pgf@countd by1 %
	}%
	\pgfplotsmathvectorsubtract{\csname point1\endcsname}{\csname point0\endcsname}{default}%
	\let\diffA=\pgfplotsretval
	\pgfplotsmathvectorsubtract{\csname point2\endcsname}{\csname point0\endcsname}{default}%
	\let\diffB=\pgfplotsretval
	\pgfplotsmathvectorcrossprod{\diffA}{\diffB}{default}%
	\pgfmath@smuggleone\pgfplotsretval
	\endgroup
	\expandafter\pgfplotsplothandlermesh@compute@normal@set@macros\pgfplotsretval\pgfplots@EOI
}%
\def\pgfplotsplothandlermesh@compute@normal@set@macros#1,#2,#3\pgfplots@EOI{%
	\def\pgfmathresult{#1}\let\pgfplotsretvalu=\pgfmathresult
	\def\pgfmathresult{#2}\let\pgfplotsretvalv=\pgfmathresult
	\def\pgfmathresult{#3}\let\pgfplotsretvalw=\pgfmathresult
	% \pgfplotscoordmath{default}{tofixed}{#1}\let\pgfplotsretvalu=\pgfmathresult
	% \pgfplotscoordmath{default}{tofixed}{#2}\let\pgfplotsretvalv=\pgfmathresult
	% \pgfplotscoordmath{default}{tofixed}{#3}\let\pgfplotsretvalw=\pgfmathresult
}%
...
\def\pgfplotsplothandlermesh@show@normals{%
	\pgfplotspatchclass{\pgfplotsplothandlermesh@patchclass}{compute normal}\relax%
	\let\pgfplots@normal@vector=\pgfplotsretval
\message{ normal = \pgfplotsretval}%
	\pgfplotsmathvectorlength{\pgfplotsretval}{default}%
\message{ len = \pgfplotsretval}%
	\pgfplotscoordmath{default}{parsenumber}{20}% FIXME
...
% expands to logical (3d) coordinates 
\def\pgfplotsplothandlermesh@serialize@logical@coords{}% empty by default
...
\def\pgfplotsplothandlermesh@PIPE@DECODE#1{%
	...
	\pgfplotspatchclass{\pgfplotsplothandlermesh@patchclass}{set next vertex}{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval:\pgfplotsplothandlermesh@serialize@logical@coords}%%
}%
...
\let\pgfplotsplothandlermesh@colormap@hook=\relax
\def\pgfplotsplothandlermesh@colormap@hook@depends@on@patch@normal{%
	\begingroup
	\pgfplotspatchclass{\pgfplotsplothandlermesh@patchclass}{compute normal}%
	%
	\pgfplotsmathfloatviewdepthxyz{\pgfplotsretvalu}{\pgfplotsretvalv}{\pgfplotsretvalw}%
%\message{normal DOT view = \pgfmathresult\space(normal = \pgfplotsretvalu,\pgfplotsretvalv,\pgfplotsretvalw)^^J}%
	\pgfmathfloatgetflags\pgfmathresult\c@pgf@counta
	\ifcase\c@pgf@counta
		% 0 :
		\global\let\pgfplots@glob@TMPc\pgfplotsplothandlermesh@colormapname
	\or
		% >0
		\global\let\pgfplots@glob@TMPc\pgfplotsplothandlermesh@colormapname
	\or
		% <0:
		\global\let\pgfplots@glob@TMPc\pgfplotsplothandlermesh@colormapname@bg
	\else
		\global\let\pgfplots@glob@TMPc\pgfplotsplothandlermesh@colormapname
	\fi
	\endgroup
	\pgfkeyslet{/pgfplots/colormap name}\pgfplots@glob@TMPc
}%

\def\pgfplotsplothandlermesh@init@cdata{%
	% Ok, we have color data.
	%
	\pgfkeysgetvalue{/pgfplots/mesh/background colormap name}\pgfplotsplothandlermesh@colormapname@bg
	\pgfkeysgetvalue{/pgfplots/colormap name}\pgfplotsplothandlermesh@colormapname
	\ifx\pgfplotsplothandlermesh@colormapname@bg\pgfutil@empty
	\else
		\let\pgfplotsplothandlermesh@colormap@hook=\pgfplotsplothandlermesh@colormap@hook@depends@on@patch@normal
		% actually store 3d coordinates:
		\let\pgfplotsplothandlermesh@serialize@logical@coords=\pgfplotsplothandlermesh@serialize@logical@coords@doit
	\fi
	%
	...
	\def\pgfplotsplothandlermesh@definecolor{%
		...
		\pgfplotsplothandlermesh@colormap@hook
...
\def\pgfplotsplothandlermesh@PIPE@DECODE@matrix@ONEDIMMODE#1{%
	...
	\pgfplotsplothandlermesh@compute@vertex@depth% <-- this modifies \pgfplotsretval
	\pgf@process{#1}%
	\edef\pgfplotsplothandlermesh@i{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval:\pgfplotsplothandlermesh@serialize@logical@coords}%
...
\def\pgfplotsplothandlermesh@PIPE@DECODE@matrix@fillscanline#1{%
	\pgfplotsplothandlermesh@compute@vertex@depth% <-- this modifies \pgfplotsretval
	\pgf@process{#1}%
	\edef\pgfplots@loc@TMPa{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval:\pgfplotsplothandlermesh@serialize@logical@coords}%
...
\def\pgfplotsplothandlermesh@PIPE@DECODE@matrix#1{%
	...
	\pgfplotsplothandlermesh@compute@vertex@depth% <-- this modifies \pgfplotsretval
	\pgf@process{#1}%
	\edef\pgfplotsplothandlermesh@i@j{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval:\pgfplotsplothandlermesh@serialize@logical@coords}% FIXME relies on pgfplots macros
