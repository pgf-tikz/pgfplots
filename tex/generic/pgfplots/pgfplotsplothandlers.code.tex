...
% Then, provide `quiver/u value' or `quiver/u expr' to 
...
\pgfplotsset{
	...
	quiver/u expr/.code		=\pgfplots@quiver@pgfkeys@src{#1}{u}{1},%
	quiver/v value/.code	=\pgfplots@quiver@pgfkeys@src{#1}{v}{0},%
	quiver/v expr/.code		=\pgfplots@quiver@pgfkeys@src{#1}{v}{1},%
	quiver/w value/.code	=\pgfplots@quiver@pgfkeys@src{#1}{w}{0},%
	quiver/w expr/.code		=\pgfplots@quiver@pgfkeys@src{#1}{w}{1},%
	quiver/before arrow/.code=,
	quiver/after arrow/.code=,
	quiver/colored/.style={%
		set point meta if empty=f(x),%
		quiver/before arrow/.append code={%
			\pgfplotsaxisvisphasetransformpointmeta
			\pgfplotscolormapdefinemappedcolor{\pgfplotspointmetatransformed}%
			\scope[#1]
		},%
		quiver/after arrow/.prefix code={%
			\endscope
		},%
	},%
	quiver/colored/.default={color=mapped color},
}%
...
\def\pgfplotsplothandlerquiver{%
	...
	\pgfkeyslet{/pgf/data point/u}\pgfutil@empty%
	\pgfkeyslet{/pgf/data point/v}\pgfutil@empty%
	\pgfkeyslet{/pgf/data point/w}\pgfutil@empty%
...
\def\pgfplots@quiver@prepare@src#1{%
	...
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
		% assume the '/pgf/data point/#1' is set by some input
		% routine.
		% Invoke math parser in this case.
		\pgfkeyssetvalue{/pgfplots/quiver/#1 is expr}{1}%
	\else
		\pgfkeyslet{/pgf/data point/#1}\pgfplots@loc@TMPa
	\fi
	\if1\pgfkeysvalueof{/pgfplots/quiver/#1 is expr}%
		\t@pgfplots@toka={%
			\pgfmathparse{\pgfkeysvalueof{/pgf/data point/#1}}%
		}%
	\else
		\t@pgfplots@toka={%
			\pgfmathfloatparsenumber{\pgfkeysvalueof{/pgf/data point/#1}}%
		}%
	\fi
	\expandafter\edef\csname pgfplots@quiver@parse@#1\endcsname{%
		\the\t@pgfplots@toka
...
\def\pgfplotsplothandlersurveypoint@quiver{%
	\pgfplots@quiver@parse@u
	\pgfplots@quiver@parse@v
	\pgfplotsifcurplotthreedim{%
		\pgfplots@quiver@parse@w
	}{%
...
\def\pgfplotsplothandlerserializepointto@quiver#1{%
	\edef#1{\pgfplots@current@point@x,\pgfplots@current@point@y,\pgfplots@current@point@z,\pgfplots@current@point@meta>\pgfplots@quiver@u,\pgfplots@quiver@v,\pgfplots@quiver@w}%
}%
\def\pgfplotsplothandlerdeserializepointfrom@quiver#1{%
	\pgfplotsplothandlerdeserializepointfrom@quiver@#1\relax
}%
\def\pgfplotsplothandlerdeserializepointfrom@quiver@#1,#2,#3,#4>#5,#6,#7\relax{%
	...
	\def\pgfplots@quiver@u{#5}%
	\def\pgfplots@quiver@v{#6}%
	\def\pgfplots@quiver@w{#7}%
}%
