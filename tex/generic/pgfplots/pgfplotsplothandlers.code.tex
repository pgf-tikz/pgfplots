...
\def\pgfplotsplothandlername@default{%
	\edef\pgfplotsretval{[unnamed tikz@plot@handler: \meaning\pgf@plotstreamstart]}%
}%
...
\def\pgfplotsplothandlercontourprepared{%
	...
	\else
		\pgfplotsaxisifhaspointmeta{}{%
			\pgfplots@error{Sorry, contour requires \string\addplot3 (or a non-empty `point meta' used as label data)}%
		}%
	\fi
}%
...
\pgfplotsset{
	contour gnuplot/.code={%
		\edef\tikz@plot@handler{\noexpand\pgfplotsplothandlercontourgnuplot{#1}}%
	},
}%
\def\pgfplotsplothandlercontourgnuplot#1{%
	\edef\pgfplotsplothandlercontourgnuplot@file{#1}%
	\pgfplotsresetplothandler
	\def\pgfplotsplothandlername{contour gnuplot}%
	\let\pgfplotsplothandlersurveystart=\pgfplotsplothandlersurveystart@contourgnuplot
	\def\pgf@plotstreamstart{%
		\pgfplotsplothandlercontourprepared
		\pgf@plotstreamstart
	}%
}%
\gdef\c@pgfplotsplothandlersurveystart@contourgnuplot@fileno{0}%
\def\pgfplotsplothandlersurveystart@contourgnuplot{%
	\ifx\pgfplotsplothandlercontourgnuplot@file\pgfutil@empty
		\edef\pgfplotsplothandlercontourgnuplot@file{\jobname-contourtmp\c@pgfplotsplothandlersurveystart@contourgnuplot@fileno.dat}%
		\pgfplotsutil@advancestringcounter@global\c@pgfplotsplothandlersurveystart@contourgnuplot@fileno
	\fi
\message{contour gnuplot: start (file=\pgfplotsplothandlercontourgnuplot@file)}%
	%
	\pgfplotsplothandlertofile{\pgfplotsplothandlercontourgnuplot@file}%
	\pgfplotsplothandlersurveystart
	...
	\let\pgfplotsplothandlersurveypoint=\pgfplotsplothandlersurveypoint@contourgnuplot
	\let\pgfplotsplothandlersurveyend=\pgfplotsplothandlersurveyend@contourgnuplot
}%
\def\pgfplotsplothandlersurveypoint@contourgnuplot{%
	\pgfplotsaxissurveysetpointmeta
...
{
\catcode`\"=12
\gdef\pgfplotsplothandlersurveyend@contourgnuplot{%
\message{contour gnuplot: end survey.}%
	\pgfplotsplothandlersurveyend@tofile
	%
	\immediate\openout\w@pgf@writea=\pgfplotsplothandlercontourgnuplot@file.gnuplot
	\immediate\write\w@pgf@writea{unset surface}%
	\immediate\write\w@pgf@writea{set contour}%
	\immediate\write\w@pgf@writea{set table "\pgfplotsplothandlercontourgnuplot@file.table"}%
	\immediate\write\w@pgf@writea{splot "\pgfplotsplothandlercontourgnuplot@file"}%
	\immediate\closeout\w@pgf@writea
	%
	\immediate\write18{gnuplot \pgfplotsplothandlercontourgnuplot@file.gnuplot}%
	%
	\openin\r@pgfplots@reada=\pgfplotsplothandlercontourgnuplot@file.table
	%
	\pgfplotsplothandlercontourprepared
	\let\pgfplots@coord@stream@start=\relax% has already been done.
	\def\pgfplots@coord@stream@end{\pgfplotsplothandlersurveyend}%
	\pgfplots@addplotimpl@file@streamit{\pgfplotsplothandlercontourgnuplot@file.table}%
...
\def\pgfplotsplothandlernotifyscanlinecomplete@tofile{%
	\immediate\write\w@pgf@writea{}% empty line.
}%
\def\pgfplotsplothandlersurveypoint@tofile{%
	...
	\immediate\write\w@pgf@writea{%
		\pgfplots@current@point@x\pgfplots@TAB
		\pgfplots@current@point@y\pgfplots@TAB
		\ifpgfplots@curplot@threedim\pgfplots@current@point@z\pgfplots@TAB\fi
		\pgfplots@current@point@meta\pgfplots@TAB
	}%
}%
