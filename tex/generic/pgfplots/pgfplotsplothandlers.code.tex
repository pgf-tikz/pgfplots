		...
		\E\ifpgfplotsaxisparsecoordinateok
			...
			\ifpgfplots@apply@datatrafo
				\ifpgfplots@datascaletrafo@initialised
					...
					% FIXME : as far as I remember, this will *never* happen.
...
\pgfplotsset{
	contour/.code={%
		\let\tikz@plot@handler=\pgfplotsplothandlercontour
	...
	contour/every contour label/.style={%
		...
		transform shape,%
		inner sep=2pt,
		every node/.style={mapped color,fill=white},%
		%every node/.style={yshift=10pt},%
	},
}%
\def\pgfplotsplothandlercontour{%
	\pgfplotsresetplothandler
	\ifpgfplots@curplot@threedim
	...
	\fi
	\let\pgf@plotstreamstart=\pgfplotsplothandlercontour@visstart
}%

\def\pgfplotsplothandlercontour@visstart{%
	\pgfplotsresetplothandler
	%
	...
	\let\pgfplots@plothandler@unbounded@visualize@jump=\relax
	%
	\pgfplotsapplistXnewempty[to global]\pgfplotsplothandlercontour@storedlabels
	%
	\pgfplotsset{/pgfplots/contour/handler}%
	\tikz@plot@handler
	\let\pgfplotsplothandlercontour@handler@start=\pgf@plotstreamstart
	\global\let\pgfplotsplothandlercontour@lastcontour=\pgfutil@empty
...
\def\pgfplotsplothandlercontour@streampoint#1{%
	\global\let\pgfplotsplothandlercontour@act@contour=\pgfplots@current@point@meta
	\ifx\pgfplotsplothandlercontour@lastcontour\pgfutil@empty
	...
	%
	%
	% compute difference vector and its squared length:
	...
	\global\let\pgfplotsplothandlercontour@lastcanvas=\pgfplotsplothandlercontour@act@canvas
	\global\let\pgfplotsplothandlercontour@lastcontour=\pgfplotsplothandlercontour@act@contour
	%
	\ifx\pgf@plotstreampoint\pgfplotsplothandlercontour@streampoint
	\else
		\global\let\pgfplotsplothandlercontour@handler@streampoint=\pgf@plotstreampoint
	\fi
	\global\let\pgf@plotstreampoint=\pgfplotsplothandlercontour@streampoint
	\global\let\pgf@plotstreamend=\pgfplotsplothandlercontour@streamend
}%
\def\pgfplotsplothandlercontour@handlesplinesegment#1{%
	#1%
	\pgf@xb=\pgf@x
	\pgf@yb=\pgf@y
	\pgfpointdiff
	...
	\pgf@xa=\pgf@sys@tonumber\pgf@x\pgf@x
	\advance\pgf@xa by\pgf@sys@tonumber\pgf@y\pgf@y
	\edef\pgfplotsplothandlercontour@act@len{\pgf@sys@tonumber\pgf@xa}%
	\pgfmath@basic@sqrt@\pgfplotsplothandlercontour@act@len
	\pgf@xa=\pgfmathresult pt
	%
	\advance\pgf@xa by\pgfplotsplothandlercontour@len\relax
	\xdef\pgfplotsplothandlercontour@len{\the\pgf@xa}%
%\message{contour(\pgfplotsplothandlercontour@act@contour): cur len=\pgfplotsplothandlercontour@len > \pgfplotsplothandlercontour@labeldist ?;}%
	\ifdim\pgf@xa>\pgfplotsplothandlercontour@labeldist\relax
		\pgfplotsplothandlercontour@lastcanvas
		\edef\pgfplots@loc@TMPa{{\the\pgf@x,\the\pgf@y;\the\pgf@xb,\the\pgf@yb;{\pgfplots@current@point@meta}}}%
		...
		\xdef\pgfplotsplothandlercontour@len{\the\pgf@xa\relax}%
	\fi
}%


\def\pgfplotsplothandlercontour@preparenewcontourline{%
%\message{PREPARE NEW CONTOUR (\pgfplots@current@point@meta).}%
	\pgfplotsaxisvisphasetransformpointmeta
	\pgfplotscolormapdefinemappedcolor{\pgfplotspointmetatransformed}%
	\pgfsetstrokecolor{mapped color}%
	\global\let\pgfplotsplothandlercontour@lastcanvas=\pgfutil@empty
	\gdef\pgfplotsplothandlercontour@len{0pt}%
	%
	\pgfplotsplothandlercontour@handler@start
	\global\let\pgfplotsplothandlercontour@handler@end=\pgf@plotstreamend
	\global\let\pgfplotsplothandlercontour@handler@streampoint=\pgf@plotstreampoint
}%
