...
\newif\ifpgfplotsaxispreparecoordinateok
...
%\message{Assembled update-limits \ifpgfplots@curplot@threedim 3D\else 2D\fi macro to {\meaning\pgfplots@update@limits@for@one@point}}%
	...
	%
	%
	% A routine which prepares a coordinate.
	...
	% - limit updates.
	%
	% It might happen that \pgfplotsaxispreparecoordinate is called
	...
	% 	- \ifpgfplotsaxispreparecoordinateok will be true if and only
	...
	% 	data.
	%
	% @see \pgfplotsaxissurveydatapoint
	\xdef\pgfplotsaxispreparecoordinate{%
		...
		\E\pgfplotsaxispreparecoordinateoktrue
		\E\ifx\E\pgfplots@current@point@x\E\pgfutil@empty
			\E\pgfplotsaxispreparecoordinateokfalse
		\E\else
			\E\ifx\E\pgfplots@current@point@y\E\pgfutil@empty
				\E\pgfplotsaxispreparecoordinateokfalse
			\E\else
		...
		%
		%
		\E\ifpgfplotsaxispreparecoordinateok
	...
	% \pgfplotsaxissurveydatapoint is invoked once for the complete
	% set.
	% 
	% @PRECONDITION
	% 	- \pgfplots@current@point@[xyz] contain readily parsed data
	...
	% 	are all processed.
	%
	\xdef\pgfplotsaxissurveydatapoint{%
		\E\ifpgfplotsaxispreparecoordinateok
	...
	\xdef\pgfplots@process@one@point{%
		% this here should be done by the plot handler:
		% it should invoke its 'surveypoint' method:
		\E\pgfplotsplothandlersurveypoint
		%
		\E\ifpgfplotsaxispreparecoordinateok
			\E\pgfplots@acquire@serialized@point@and@store@it
		\E\else
	...
	\else
		...
		\xdef\pgfplots@glob@TMPa{%
			...
			{% precommand(s):
				...
				\noexpand\pgfplotsplothandlerdeserializestatefrom{\the\t@pgfplots@tokb}%
			}%
			{% draw command:
				\noexpand\draw%
			}%
...
% namely those in the key path /pgf/data point. See the pgf manual.
\def\pgfplotsplothandlersurveypoint@genericimpl{%
	\pgfplotsaxispreparecoordinate
	\pgfplotsaxissurveydatapoint
}%
...
\def\pgfplotsplothandlerserializepointto@genericimpl#1{%
	...
	\edef#1{\pgfplots@current@point@x,\pgfplots@current@point@y,\pgfplots@current@point@z,\pgfplots@current@point@meta}%
}%
...
\def\pgfplotsplothandlerdeserializepointfrom@genericimpl#1{%
	\pgfplotsplothandlerdeserializepointfrom@genericimpl@#1\relax
}%
\let\pgfplotsplothandlerdeserializepointfrom=\pgfplotsplothandlerdeserializepointfrom@genericimpl

\def\pgfplotsplothandlerdeserializepointfrom@genericimpl@#1,#2,#3,#4\relax{%
...
\def\pgfplots@acquire@serialized@point@and@store@it{%
	\pgfplotsplothandlerserializepointto\pgfplots@loc@TMPa
	\expandafter\pgfplotsapplistXXpushback\expandafter{\expandafter{\pgfplots@loc@TMPa}}%
}%
...
%   \pgfplotsplothandlerdeserializepointfrom{<encoded data>}
% 	\pgfplots@coord@stream@coord
%
% \pgfplots@coord@stream@end
% -------------
...
\def\pgfplots@apply@zbuffer#1{%
	...
	\or
		...
		\def\pgfplots@coord@stream@coord{%
			\expandafter\pgfplotsprependlistXpushfront\expandafter{\pgfplots@coord@stream@foreach@NORMALIZED@curencoded}\to{reversed}%
		}%
		\def\pgfplots@coord@stream@end{%
...
\def\pgfplots@apply@zbuffer@reversescanline#1{%
	...
	\def\pgfplots@coord@stream@coord{%
		\expandafter\pgfplotsprependlistXpushfront\expandafter{\pgfplots@coord@stream@foreach@NORMALIZED@curencoded}\to{reversedscanline}%
		\advance\c@pgfplots@scanlineindex by1
...
\def\pgfplots@apply@zbuffer@reversetransposed#1{%
	...
	\def\pgfplots@coord@stream@coord{%
		\expandafter\pgfplotsapplistXpushback\expandafter{\pgfplots@coord@stream@foreach@NORMALIZED@curencoded}\to{\scanline}%
		\advance\c@pgfplots@scanlineindex by1
		\ifnum\c@pgfplots@scanlineindex=\pgfplotsscanlinelength\relax
