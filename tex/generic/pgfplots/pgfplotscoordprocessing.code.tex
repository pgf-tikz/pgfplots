...
\message{got scalex=\pgfplots@loc@TMPa, scaley=\pgfplots@loc@TMPb, scalez=\pgfplots@loc@TMPc.^^J}%
	%
	...
	\else
		...
		\let\pgfplotsretvalb=\relax
		\let\pgfplotsretvalc=\relax
	\fi
\message{got scalex=\pgfplots@loc@TMPa, scaley=\pgfplots@loc@TMPb, scalez=\pgfplots@loc@TMPc.^^J}%
...
% #4: [output] some executable code which will be used to rescale axis limits in x
% #5: [output] some executable code which will be used to rescale axis limits in y
% #6: [output] some executable code which will be used to rescale axis limits in z
%
...
\def\pgfplots@BB@for@plotbox@get@unit@scales@compensated@axis@limits#1#2#3#4#5#6{%
	\begingroup
	% FIXME : needs to respect the datascaling trafo as well because
	% it is also a non-uniform scale per component.
	% My idea is to check #1*T(x) here. That should be fine ... except
	% if the limits vary drastically. In this case, one would probably
	% need to change the datascaling trafo (or optimize it for this
	% problem in the first place).
	% Changing the data trafo seems to be out-of-question at this
	% point in the code (it is already applied to user args)
...
%
% POSTCONDITION:
% #2 contains executable code to modify axis limits.
...
%	}%
	...
	% \pgfmathresult:
\message{FIXME: vertical size compensation might fail - deactivated^^J}%
	\xdef#2{%
%		\noexpand\pgfplots@apply@unit@vector@rescale@keep@size{#1}{\pgfmathresult}%
	}%
	\endgroup
