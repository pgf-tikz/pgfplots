...
\message{Got W/H scale for all x components: \pgfplots@glob@TMPa; for all y components: \pgfplots@glob@TMPb^^J}%
	...
	%
	\pgfplots@BB@for@plotbox@get@unit@scales@for@limits{\pgfplots@loc@TMPa}{\pgfplots@loc@TMPb}{\pgfplots@loc@TMPc}%
\message{got scales to fit limits into BB: scalex=\pgfplots@loc@TMPa, scaley=\pgfplots@loc@TMPb, scalez=\pgfplots@loc@TMPc, vertical limit componensation in direction \pgfplots@glob@TMPd=\pgfplots@glob@TMPc.^^J}%
	%
	% ok. Now apply all scalings.
	\if3\pgfplots@scale@mode@choice
		% scale mode=scale uniformly
		\pgfplots@BB@for@plotbox@get@unit@scales@compensated@axis@limits{\pgfplots@loc@TMPa}{\pgfplots@loc@TMPb}{\pgfplots@loc@TMPc}
			{\pgfplotsretval}{\pgfplotsretvalb}{\pgfplotsretvalc}%
	\else
		\pgfplotscoordmath{pgfbasic}{one}%
		\let\pgfplotsretval=\pgfmathresult
		\let\pgfplotsretvalb=\pgfmathresult
		\let\pgfplotsretvalc=\pgfmathresult
	\fi
	...
	% inverse
\message{adjusted scales for 'scale mode': scalex=\pgfplots@loc@TMPa, scaley=\pgfplots@loc@TMPb, scalez=\pgfplots@loc@TMPc;  axis limit componsation scales x=\pgfplotsretval, y=\pgfplotsretvalb, z=\pgfplotsretvalc^^J}%
	...
	%
	%
	% and finally, resize limits appropriately:
	\ifx\pgfplots@glob@TMPc\relax
	\else
		% handle size compensation in vertical direction:
		\if x\pgfplots@glob@TMPd
			\pgfplotscoordmath{pgfbasic}{op}{add}{{\pgfplots@glob@TMPc}{\pgfplotsretval}}%
			\let\pgfplotsretval=\pgfmathresult
		\fi
		\if y\pgfplots@glob@TMPd
			\pgfplotscoordmath{pgfbasic}{op}{add}{{\pgfplots@glob@TMPc}{\pgfplotsretvalb}}%
			\let\pgfplotsretvalb=\pgfmathresult
		\fi
		\if z\pgfplots@glob@TMPd
			\pgfplotscoordmath{pgfbasic}{op}{add}{{\pgfplots@glob@TMPc}{\pgfplotsretvalc}}%
			\let\pgfplotsretvalc=\pgfmathresult
		\fi
	\fi
	% add all cumulative limit compensations:
	\edef\pgfplots@loc@TMPa{%
		\noexpand\pgfplots@apply@unit@vector@rescale@keep@size{x}{\pgfplotsretval}%
		\noexpand\pgfplots@apply@unit@vector@rescale@keep@size{y}{\pgfplotsretvalb}%
		\noexpand\pgfplots@apply@unit@vector@rescale@keep@size{z}{\pgfplotsretvalc}%
	}%
	\pgfplots@loc@TMPa
}%
