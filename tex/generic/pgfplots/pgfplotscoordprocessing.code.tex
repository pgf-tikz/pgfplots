...
\def\pgfplotspoint@initialisation{%
	...
	\edef\pgfplotsplothandlerpointxyz##1##2##3{%
		...
		\else
			\ifpgfplots@threedim
				\noexpand\pgfplotsqpointxyz{##1}{##2}{0.0}%
			\else
...
\def\pgfplotsdeclarecoordmath#1#2{%
	...
	\pgfqkeys{/pgfplots/@declare coord math}{%
		...
		set log basis=\pgfplotscoordmath@log@set@basis{##1},
	...
	\pgfutil@ifundefined{pgfpmth\pgfplotsdeclarecoordmath@ log}{%
		\pgfutil@namelet
			{pgfpmth\pgfplotsdeclarecoordmath@ log}
			{pgfpmth\pgfplotsdeclarecoordmath@ log e}%
	}{}%
	% store old impl of log unsigned int:
	\pgfutil@namelet
		{pgfpmth\pgfplotsdeclarecoordmath@ log unsigned int@orig}
		{pgfpmth\pgfplotsdeclarecoordmath@ log unsigned int}%
	\pgfplotscoordmath@def@log@to@log@ten{#1}{}% empty arg
	%
}%
\pgfqkeys{/pgfplots/@declare coord math}{%
	initialise/.code=
	...
	%
	% 'log' is special in that it accepts a number literal which is
	% OUTSIDE of the accepted number format. The result, however, is
	% then in the accepted number format.
	%
	% The log basis can be set with 'set log basis'
	log/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarecoordmath@ log\endcsname##1{#1}},%
	% similar, but always uses log basis e:
	log e/.code=
	...
	log to display log/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarecoordmath@ log to display log\endcsname##1{#1}},%
	...
	log from display log/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarecoordmath@ log from display log\endcsname##1{#1}},%
	log to log 10/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarecoordmath@ log to log 10\endcsname##1{#1}},%
	% returns log(i) where i \in {1,2,3,...,basis-1}
	% currently, it is only invoked for log basis 10
	log unsigned int/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarecoordmath@ log unsigned int\endcsname##1{#1}},%
	...
	datascaletrafo set params/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarecoordmath@ datascaletrafo set params\endcsname##1##2{#1}},%
	datascaletrafo set shift/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarecoordmath@ datascaletrafo set shift\endcsname##1{#1}},%
	% 
...
% subclass.
\def\pgfplotscoordmath@log@set@basis#1{%
	%
	\pgfplotscoordmath{\pgfplotscoordmathid}{log e}{#1}%
	\let\pgfplots@loc@TMPa=\pgfmathresult% TMPa = log_e(#1)
	\pgfplotscoordmath{\pgfplotscoordmathid}{op}{reciprocal}{{\pgfmathresult}}%
	\let\pgfplots@loc@TMPb=\pgfmathresult% TMPb = 1/log_e(#1)
	%
	\expandafter\edef\csname pgfpmth@\pgfplotscoordmathid @log\endcsname##1{%
	...
	\expandafter\def\csname pgfpmth@\pgfplotscoordmathid @log to display log\endcsname##1{\edef\pgfmathresult{##1}}%
	\expandafter\def\csname pgfpmth@\pgfplotscoordmathid @log from display log\endcsname##1{\edef\pgfmathresult{##1}}%
	%
	...
	% and re-compute i>=10 :
	\begingroup
		\expandafter\let\expandafter\logi\csname pgfpmath@\pgfplotscoordmathid @log unsigned int@orig\endcsname
	...
	\endgroup
	\expandafter\let\csname pgfpmth@\pgfplotscoordmathid @log unsigned int\endcsname=\pgfplots@glob@TMPa
	\pgfplotscoordmath@def@log@to@log@ten\pgfplotscoordmathid\pgfplots@loc@TMPb
}%
% #1: the id of coord math
% #2: either empty (basis e) or 1/ln(basis)
\def\pgfplotscoordmath@def@log@to@log@ten#1#2{%
	\csname pgfpmth@#1@parsenumber\endcsname{0.434294}%
	\edef\pgfplots@loc@TMPa{#2}%
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
		\expandafter\edef\csname pgfpmth@#1@log to log 10\endcsname##1{%
...
\pgfplotsdeclarecoordmath{float}{%
	...
	datascaletrafo noshift inverse={%
		\edef\pgfplots@datascaletrafo@tmp{#1}%
		\pgfmathapproxequalto@{\pgfplots@datascaletrafo@tmp}{0.0}%
		\ifpgfmathcomparison
			\pgfmathfloatcreate{0}{0.0}{0}%
		\else
			\pgfmathfloatparsenumber{\pgfplots@datascaletrafo@tmp}%
			\pgfmathfloatshift@{\pgfmathresult}{-\csname pgfplots@data@scale@trafo@EXPONENT@\pgfplotscoordmathid\endcsname}%
		\fi
	},%
...
\def\pgfplotsscanlinelengthinitzero{%
	...
	\ifx\pgfplots@loc@TMPa\pgfplots@loc@TMPb
		\ifpgfplots@curplot@threedim
			\def\pgfplots@loc@TMPa{scanline}%
		\else
			\def\pgfplots@loc@TMPa{jump}%
		\fi
	\fi
...
\def\pgfplots@PREPARE@COORD@STREAM@start@{%
	...
	\if0\csname pgfpmeta@\pgfplotspointmetainputhandler @issymbolic\endcsname
		...
		\edef\pgfplotsaxisupdatelimitsforpointmeta##1{%
			\noexpand\begingroup
			\noexpand\def\noexpand\pgfplots@current@point@meta{##1}%
			\the\t@pgfplots@tokb
			\noexpand\endgroup
		}%
	\else
	...
	\else
		...
		\pgfplots@if{pgfplots@#1islinear}{%
			...
			\ifcase\csname pgfplots@errorbars@#1mode\endcsname
				...
				%
				% FIXME : I'd prefer to use the \pgfplotcoordmath
				% framework here. But I can't since here, TWO math
				% engines take place: float for non-log coords and
				% basic for log!
			...
			\or% fixed relative, log( x ( 1+-e_x ) ) = log(x) + log(1+-e_x)
				...
				\pgfplotscoordmath{#1}{if is bounded}{\pgfmathresult}{%
					...
					\pgfplotscoordmath{#1}{tostring}{\pgfplots@loc@TMPb}%
				...
				\if\pgfplots@perpointmeta@rel@choice0%
					...
					\def\pgfplots@perpointmeta@trafo##1{%
						\pgfplotscoordmath{meta}{if is}{##1}{u}{%
							...
							\pgfplotscoordmath{meta}{op}{subtract}{{##1}{\pgfplots@axiswide@metamin}}%
					...
					\pgfplotscoordmath{meta}{op}{subtract}{{\pgfplots@axiswide@metamax}{\pgfplots@axiswide@metamin}}%
				\else
					...
					\def\pgfplots@perpointmeta@trafo##1{%
						\pgfplotscoordmath{meta}{if is}{##1}{u}{%
							...
							\pgfplotscoordmath{meta}{op}{subtract}{{##1}{\pgfplots@metamin}}%
							\pgfplotscoordmath{meta}{op}{multiply}{{\pgfmathresult}{\pgfplots@perpointmeta@trafo@factor}}%
							\pgfplotscoordmath{meta}{tofixed}{\pgfmathresult}%
						}%
					}%
					\pgfplotscoordmath{meta}{op}{subtract}{{\pgfplots@metamax}{\pgfplots@metamin}}%
				\fi
				\let\pgfplots@loc@TMPa=\pgfmathresult
...
%	}%
	...
	%
	% enable FPU after setting options of '#1'. Otherwise things like
	...
	\fi
	\expandafter\def\expandafter\pgfplots@addplot@survey@@optionlist\expandafter{\pgfplots@addplot@survey@@optionlist,#1}%
	\pgfplots@validate@plot@domain@arguments
}

\def\pgfplotsplothandlersurveyaddoptions#1{%
...
\def\pgfplots@addplotimpl@graphics@@#1#2#3#4#5{%
	...
	\else
		\let\pgfplots@coord@stream@start=\relax
		\let\pgfplots@addplotimpl@graphics@@end=\pgfplots@coord@stream@end
		\let\pgfplots@coord@stream@end=\relax
		\ifpgfplots@curplot@threedim
			\expandafter\pgfplots@coord@stream@foreach@threedim\expandafter{\pgfplots@current@point@points}%
		\else
			\expandafter\pgfplots@coord@stream@foreach\expandafter{\pgfplots@current@point@points}%
		\fi
		\let\pgfplots@coord@stream@end=\pgfplots@addplotimpl@graphics@@end
...
%		\scope
		...
		\else
			...
			\pgfplotsplothandlermesh@init@plottype
		...
		\if\pgfplotsplothandlermesh@ONEDIMMODE1%
			...
			\pgfplotsplothandlermesh@init@ONEDIMMESH
		\else
			...
			%
			\pgfplotsplothandlermesh@init@mesh@process@element
			%
			%
			% handling of jumps.
			...
			%
			\pgfplotsplothandlermesh@init@mesh@mem
			%
			\pgfplotsplothandlermesh@init@mesh@stream
		\fi% endif ONEDIM mode
		\c@pgfplots@scanlineindex=0
	}%
}%

\def\pgfplotsplothandlermesh@init@meshkeys{%
	\if5\pgfplotsplothandlermesh@type
		% mesh/type=lattice
...
\def\pgfplotsplothandlermesh@init@plottype{%
	% 
	...
	\if\pgfplotsplothandlermesh@zbuffer@choice4% 'z buffer=sort':
		\if5\pgfplotsplothandlermesh@type
			% mesh/type=lattice
			\ifnum\pgfplotsplothandlermesh@shader>1% 'shader=interp'
				\pgfplots@error{Sorry, 'shader=interp' and 'shader=faceted interp' do not support 'z buffer=sort'. If you continue now, 'shader=flat' will be used.}%
				\def\pgfplotsplothandlermesh@shader{0}% 'flat'
			\fi
		\fi
	\fi
	...
	\ifnum\pgfplotsplothandlermesh@shader>1
		...
		% a mix of 'faceted' and 'interp'. Very inefficient!
		%
		...
		\if5\pgfplotsplothandlermesh@type
			\pgfkeys{/pgfplots/surf shading/shading type=lattice}%
		\else
			\pgfkeys{/pgfplots/surf shading/shading type=triangle}%
		\fi
		...
		\if2\pgfplotsplothandlermesh@shader
			...
			\else
				% [this happens only for mesh/type=lattice]
				% colwise:
				%
	...
	\if1\pgfplotsplothandlermesh@needs@flat@color
		...
		\else
			...
			\if\pgfplotsplothandlermesh@ONEDIMMODE0
				...
				\else
					% mesh/type=triangle
					\t@pgfplots@tokb={\global\divide\pgfplots@tmpa by3 }%
				\fi
			\else
				% its just a line:
...
\def\pgfplotsplothandlermesh@init@ONEDIMMESH{%
...
% This appends code to the \pgfplotsplothandlermesh@process@element
% routine.
%
% Note that \pgfplotsplothandlermesh@process@element will be
% *overwritten* in case of z buffering.
\def\pgfplotsplothandlermesh@init@mesh@process@element{%
	\if5\pgfplotsplothandlermesh@type
		% mesh/type=lattice
		\expandafter\def\expandafter\pgfplotsplothandlermesh@process@element\expandafter{%
			\pgfplotsplothandlermesh@process@element
			\pgfplotsplothandlermesh@draw@element
	...
	\else
		...
		\expandafter\def\expandafter\pgfplotsplothandlermesh@process@element\expandafter{%
			\pgfplotsplothandlermesh@process@element
			\pgfplotsplothandlermesh@draw@element
				\pgfplotsplothandlermesh@triangle@A
				\pgfplotsplothandlermesh@triangle@B
				\pgfplotsplothandlermesh@triangle@C
		}%
	\fi
}%
...
\def\pgfplotsplothandlermesh@init@jumphandling{%
	\if2\pgfplotsplothandlermesh@shader
		% shader=interp does not support jumps.
		\def\pgfplotsplothandlervisualizejump{%
	...
	\else
		\if\pgfplots@unbounded@handler j%
			...
			\else
				...
				\t@pgfplots@tokb={%
					\pgfplots@loc@tmptrue
					\expandafter\pgfplotsplothandlermesh@check@unbounded\pgfplotsplothandlermesh@triangle@A\relax
					\expandafter\pgfplotsplothandlermesh@check@unbounded\pgfplotsplothandlermesh@triangle@B\relax
					\expandafter\pgfplotsplothandlermesh@check@unbounded\pgfplotsplothandlermesh@triangle@C\relax
				}%
			\fi
			\t@pgfplots@toka=\expandafter{\pgfplotsplothandlermesh@process@element}%
			\edef\pgfplotsplothandlermesh@process@element{%
...
\def\pgfplotsplothandlermesh@init@zbuffer{%
	\def\pgfplotsplothandlermesh@checkfor@zbuffer{1}%
	\if5\pgfplotsplothandlermesh@type
		% mesh/type=lattice
		\if2\pgfplotsplothandlermesh@shader
			% shader=interp  does not support z buffer=sort.
			\def\pgfplotsplothandlermesh@checkfor@zbuffer{0}%
		\fi
	\fi
	\if1\pgfplotsplothandlermesh@checkfor@zbuffer
		\if\pgfplotsplothandlermesh@zbuffer@choice4%
			% z buffer=sort:
			% create a z buffer:
			% the z buffer contains elements {<view depth>}{{<pt1>}{<pt2>}{<pt3>}{<pt4>}}
			\pgfplotsarraynewempty\pgfplots@zbuffer@local
			%
			\def\pgfplotsplothandlermesh@determine@streamdata{%
				% The idea: while the coordinate stream visits every
				% coordinate, we compute the view depth.
				%
				% We store it along with
				% {<canvasx>}{<canvasy>}{<cdata>}{<depth>}
				% and extract it once all points of one patch element
				% are together.
				\pgfplotsmathviewdepthxyz{\pgfplots@current@point@x}{\pgfplots@current@point@y}{\pgfplots@current@point@z}% FIXME relies on pgfplots macros
			...
			\if5\pgfplotsplothandlermesh@type
				...
				\def\pgfplotsplothandlermesh@process@element{%
					...
					\pgfplotsplothandlermesh@unpack@zbuffer@pointdepth\pgfplotsplothandlermesh@im@jm
					\let\pgfmathresult\pgfplotsretval
					\pgfplotsplothandlermesh@unpack@zbuffer@pointdepth\pgfplotsplothandlermesh@im@j
					\pgfplotsplothandlermesh@accum@element@depth\pgfplotsretval\pgfmathresult
					\pgfplotsplothandlermesh@unpack@zbuffer@pointdepth\pgfplotsplothandlermesh@i@j
					\pgfplotsplothandlermesh@accum@element@depth\pgfplotsretval\pgfmathresult
					\pgfplotsplothandlermesh@unpack@zbuffer@pointdepth\pgfplotsplothandlermesh@i@jm
					\pgfplotsplothandlermesh@accum@element@depth\pgfplotsretval\pgfmathresult
					%
					% assemble array element:
					\edef\pgfplots@loc@TMPa{%
			...
			\else
				% mesh/type=triangle
				\def\pgfplotsplothandlermesh@process@element{%
					% compute DEPTH(triangle element):= min{DEPTH(A),DEPTH(B),DEPTH(C)}
					\pgfplotsplothandlermesh@unpack@zbuffer@pointdepth\pgfplotsplothandlermesh@triangle@A
					\let\pgfmathresult\pgfplotsretval
					\pgfplotsplothandlermesh@unpack@zbuffer@pointdepth\pgfplotsplothandlermesh@triangle@B
					\pgfplotsplothandlermesh@accum@element@depth\pgfplotsretval\pgfmathresult
					\pgfplotsplothandlermesh@unpack@zbuffer@pointdepth\pgfplotsplothandlermesh@triangle@C
					\pgfplotsplothandlermesh@accum@element@depth\pgfplotsretval\pgfmathresult
					%
					% assemble array element:
					\edef\pgfplots@loc@TMPa{%
...
\def\pgfplotsplothandlermesh@init@mesh@mem{%
	\if5\pgfplotsplothandlermesh@type
		% mesh/type=lattice
		% Init memory:
...
\def\pgfplotsplothandlermesh@init@mesh@stream{%
	...
	\if5\pgfplotsplothandlermesh@type
		...
		\if2\pgfplotsplothandlermesh@shader
			% shader=interp :
			\let\pgfplotsplothandlermesh@draw@element=\relax% unused here.
			\global\let\pgf@plotstreampoint=\pgfplotsplothandlermesh@shader@interp%
		\else
			\if3\pgfplotsplothandlermesh@shader
				% shader=faceted interp:
				\let\pgfplotsplothandlermesh@draw@element=\pgfplotsplothandlermesh@draw@element@rectangle@faceted@interp
			\else
	...
	\else
		...
		\if2\pgfplotsplothandlermesh@shader
			% shader=interp :
			\let\pgfplotsplothandlermesh@draw@element=\pgfplotsplothandlermesh@draw@element@triangle@interp
		\else
			\if3\pgfplotsplothandlermesh@shader
				% shader=faceted interp:
				\let\pgfplotsplothandlermesh@draw@element=\pgfplotsplothandlermesh@draw@element@triangle@faceted@interp
			\else
				\let\pgfplotsplothandlermesh@draw@element=\pgfplotsplothandlermesh@draw@element@triangle
			\fi
		\fi
		\global\let\pgf@plotstreampoint=\pgfplotsplothandlermesh@collecttriangle%
	\fi
	%
	%
}
...
\def\pgfplotsplothandlermesh@determine@streamdata{\let\pgfplotsretval\pgfutil@empty}%

% the pgf@plotstreampoint routine for the first scan line:
\def\pgfplotsplothandlermesh@fillscanline#1{%
	\pgfplotsplothandlermesh@determine@streamdata% <-- this modifies \pgfplotsretval
	\pgf@process{#1}%
	\edef\pgfplotsplothandlermesh@i@j{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval}%
...
% the pgf@plotstreampoint routine for patch plots
\def\pgfplotsplothandlermesh@collecttriangle#1{%
	\pgfplotsplothandlermesh@determine@streamdata% <-- this modifies \pgfplotsretval
	\pgf@process{#1}%
	\ifcase\c@pgfplots@scanlineindex
		\edef\pgfplotsplothandlermesh@triangle@A{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval}%
		\c@pgfplots@scanlineindex=1
	\or
		\edef\pgfplotsplothandlermesh@triangle@B{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval}%
		\c@pgfplots@scanlineindex=2
	\or
		\edef\pgfplotsplothandlermesh@triangle@C{{\the\pgf@x}{\the\pgf@y}{\pgfplots@current@point@meta}\pgfplotsretval}%
		\c@pgfplots@scanlineindex=0
		\pgfplotsplothandlermesh@process@element
	\fi
}%
...
\def\pgfplotsplothandlermesh@draw@element@line#1#2{%
	\pgfpathmoveto{\expandafter\pgfplotsplothandlermesh@unpack#1\relax}% the \relax is *CRUCIAL*!
	\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack#2\relax}%
...
\def\pgfplotsplothandlermesh@draw@element@rectangle#1#2#3#4{%
	\pgfpathmoveto{\expandafter\pgfplotsplothandlermesh@unpack#1\relax}%
	\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack#2\relax}%
	\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack#3\relax}%
	\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack#4\relax}%
	\pgfplotsplothandlermesh@preparedrawforcurrent
	\pgfpathclose
	\pgfplotsplothandlermesh@usepath
}%
% a possible value for \pgfplotsplothandlermesh@draw@element
\def\pgfplotsplothandlermesh@draw@element@triangle#1#2#3{%
	\pgfpathmoveto{\expandafter\pgfplotsplothandlermesh@unpack#1\relax}%
	\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack#2\relax}%
	\pgfpathlineto{\expandafter\pgfplotsplothandlermesh@unpack#3\relax}%
	\pgfplotsplothandlermesh@preparedrawforcurrent
...
\def\pgfplotsplothandlermesh@draw@element@triangle@interp#1#2#3{%
	\expandafter\pgfplotsplothandlermesh@unpack@simple#1\relax%
	\pgfplotsplothandlermesh@shader@interp{}%
	\expandafter\pgfplotsplothandlermesh@unpack@simple#2\relax%
	\pgfplotsplothandlermesh@shader@interp{}%
	\expandafter\pgfplotsplothandlermesh@unpack@simple#3\relax%
	\pgfplotsplothandlermesh@shader@interp{}%
}%
\def\pgfplotsplothandlermesh@draw@element@rectangle@interp#1#2#3#4{%
	\expandafter\pgfplotsplothandlermesh@unpack@simple#1\relax%
	\pgfplotsplothandlermesh@shader@interp{}%
	\expandafter\pgfplotsplothandlermesh@unpack@simple#2\relax%
	\pgfplotsplothandlermesh@shader@interp{}%
	\expandafter\pgfplotsplothandlermesh@unpack@simple#3\relax%
	\pgfplotsplothandlermesh@shader@interp{}%
	\expandafter\pgfplotsplothandlermesh@unpack@simple#4\relax%
	\pgfplotsplothandlermesh@shader@interp{}%
}%

\def\pgfplotsplothandlermesh@draw@element@rectangle@faceted@interp#1#2#3#4{%
	% shader=faceted interp
	%
	\pgfplotslibrarysurfstreamstart
		\pgfplotsplothandlermesh@draw@element@rectangle@interp{#1}{#2}{#4}{#3}%
	\pgfplotslibrarysurfstreamend
	\pgfplotslibrarysurfdrawinpicture
	%
	\pgfplotsplothandlermesh@draw@element@rectangle{#1}{#2}{#3}{#4}%
}%
\def\pgfplotsplothandlermesh@draw@element@triangle@faceted@interp#1#2#3{%
	% shader=faceted interp
	%
	\pgfplotslibrarysurfstreamstart
		\pgfplotsplothandlermesh@draw@element@triangle@interp{#1}{#2}{#3}%
	\pgfplotslibrarysurfstreamend
	\pgfplotslibrarysurfdrawinpicture
	%
	\pgfplotsplothandlermesh@draw@element@triangle{#1}{#2}{#3}%
}%
...
% #4: usually nothing.
\def\pgfplotsplothandlermesh@unpack@corner#1#2#3#4{%
	\global\pgf@x=#1\relax
	\global\pgf@y=#2\relax
}%
\def\pgfplotsplothandlermesh@unpack@simple#1#2#3#4{%
	\global\pgf@x=#1\relax
	\global\pgf@y=#2\relax
	\def\pgfplots@current@point@meta{#3}%
}%
\def\pgfplotsplothandlermesh@unpack@mean@accum#1#2#3#4{%
	\global\pgf@x=#1\relax
	\global\pgf@y=#2\relax
	\def\pgfplots@current@point@meta{#3}%
	\pgfplotsaxisvisphasetransformpointmeta
