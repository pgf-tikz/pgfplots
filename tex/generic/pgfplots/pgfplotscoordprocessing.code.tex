...
\def\pgfplots@LUA@survey@log@deactivation#1{%
	\pgfplots@log3{Using 'lua backend=false' for plot \the\pgfplots@numplots\space (type '\pgfplotsplothandlername'): #1.}%
}%
...
\def\pgfplots@LUA@survey@start{%
	\ifpgfplots@LUA@backend@supported
		...
		\pgfkeysgetvalue{/pgfplots/x filter/.@cmd}\pgfplots@loc@TMPc\ifx\pgfplots@loc@TMPc\pgfplots@empty@command@key\else \pgfplots@LUA@backend@supportedfalse\pgfplots@LUA@survey@log@deactivation{LUA does not support filter (yet)}\fi
		\pgfkeysgetvalue{/pgfplots/y filter/.@cmd}\pgfplots@loc@TMPc\ifx\pgfplots@loc@TMPc\pgfplots@empty@command@key\else \pgfplots@LUA@backend@supportedfalse\pgfplots@LUA@survey@log@deactivation{LUA does not support filter (yet)}\fi
		\pgfkeysgetvalue{/pgfplots/z filter/.@cmd}\pgfplots@loc@TMPc\ifx\pgfplots@loc@TMPc\pgfplots@empty@command@key\else \pgfplots@LUA@backend@supportedfalse\pgfplots@LUA@survey@log@deactivation{LUA does not support filter (yet)}\fi
		...
		\ifpgfplots@LUA@backend@supported
			...
			\pgfplots@log3{lua backend=true: Activating LUA backend for plot \the\pgfplots@numplots\space (type '\pgfplotsplothandlername').}%
			\edef\pgfplots@loc@TMPa{%
				local gca = pgfplots.gca;^^J%
				gca.min = {^^J%
					pgfplots.pgfplotsmath.tonumber("\pgfplots@xmin"), ^^J%
					pgfplots.pgfplotsmath.tonumber("\pgfplots@ymin"), ^^J%
					pgfplots.pgfplotsmath.tonumber("\pgfplots@zmin")}; ^^J%
				gca.max = {^^J%
					pgfplots.pgfplotsmath.tonumber("\pgfplots@xmax"), ^^J%
					pgfplots.pgfplotsmath.tonumber("\pgfplots@ymax"), ^^J%
					pgfplots.pgfplotsmath.tonumber("\pgfplots@zmax")}; ^^J%
				...
				\else
					...
					plothandler.metamin = pgfplots.pgfplotsmath.tonumber("\pgfplots@loc@metamin");^^J%
				\fi
				\ifx\pgfplots@loc@metamax\pgfutil@empty
				\else
					plothandler.autocomputeMetaMax = false;^^J%
					plothandler.metamax = pgfplots.pgfplotsmath.tonumber("\pgfplots@loc@metamax");^^J%
				\fi
...
\def\pgfplots@LUA@survey@point{%
	\directlua{%
		local pt = pgfplots.Coord.new()^^J
		pt.x[1] = "\pgfplots@current@point@x";^^J
		pt.x[2] = "\pgfplots@current@point@y";^^J
		pt.x[3] = "\pgfplots@current@point@z";^^J
		pt.meta = "\pgfplots@current@point@meta";^^J
		^^J
		pgfplots.gca.currentPlotHandler:surveypoint(pt)^^J
	}%
}%
...
%\message{LUA backend: got \pgfplots@loc@TMPa^^J}%
	%
	\begingroup
	\gdef\pgfplots@marshal{}%
	\pgfkeyslet{/pgfplots/@xmin}\pgfutil@empty%
	\pgfkeyslet{/pgfplots/@xmax}\pgfutil@empty%
	\pgfkeyslet{/pgfplots/@ymin}\pgfutil@empty%
	\pgfkeyslet{/pgfplots/@ymax}\pgfutil@empty%
	\pgfkeyslet{/pgfplots/@zmin}\pgfutil@empty%
	\pgfkeyslet{/pgfplots/@zmax}\pgfutil@empty%
	\pgfkeyslet{/pgfplots/@is3d}\pgfutil@empty%
	\pgfkeysdef{/pgfplots/@plot has jumps}{%
		\expandafter\gdef\expandafter\pgfplots@marshal\expandafter{%
			\pgfplots@marshal
			\def\pgfplotsaxisplothasjumps{##1}%
		}%
	}%
	\pgfkeysdef{/pgfplots/@filtered coords away}{%
		\expandafter\gdef\expandafter\pgfplots@marshal\expandafter{%
			\pgfplots@marshal
			\def\pgfplotsaxisfilteredcoordsaway{##1}%
		}%
	}%
	\pgfkeysdefargs{/pgfplots/@first coord}{##1,##2,##3}{%
		\gdef\pgfplots@currentplot@firstcoord@x{##1}%
		\gdef\pgfplots@currentplot@firstcoord@y{##2}%
		\gdef\pgfplots@currentplot@firstcoord@z{##3}%
	}%
	\pgfkeysdefargs{/pgfplots/@last coord}{##1,##2,##3}{%
		\gdef\pgfplots@currentplot@lastcoord@x{##1}%
		\gdef\pgfplots@currentplot@lastcoord@y{##2}%
		\gdef\pgfplots@currentplot@lastcoord@z{##3}%
	}%
	\pgfkeysdef{/pgfplots/@surveyed coords}{%
		\expandafter\gdef\expandafter\pgfplots@marshal\expandafter{%
			\pgfplots@marshal
			\pgfplotsapplistXXpushback{##1}%
		}%
	}%
	\pgfkeysdef{/pgfplots/@surveyed coordindex}{%
		\expandafter\gdef\expandafter\pgfplots@marshal\expandafter{%
			\pgfplots@marshal
			\c@pgfplots@coordindex=##1 %
		}%
	}%
	\expandafter\pgfplotsset\expandafter{\pgfplots@loc@TMPa}%
	%
	%
	\pgfkeysgetvalue{/pgfplots/@xmin}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@xmin=\pgfplots@loc@TMPb\fi
	\pgfkeysgetvalue{/pgfplots/@xmax}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@xmax=\pgfplots@loc@TMPb\fi
	\pgfkeysgetvalue{/pgfplots/@ymin}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@ymin=\pgfplots@loc@TMPb\fi
	\pgfkeysgetvalue{/pgfplots/@ymax}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@ymax=\pgfplots@loc@TMPb\fi
	\ifpgfplots@threedim
		\pgfkeysgetvalue{/pgfplots/@zmin}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@zmin=\pgfplots@loc@TMPb\fi
		\pgfkeysgetvalue{/pgfplots/@zmax}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@zmax=\pgfplots@loc@TMPb\fi
	\fi
	\pgfkeysgetvalue{/pgfplots/point meta min}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@metamin=\pgfplots@loc@TMPb\fi
	\pgfkeysgetvalue{/pgfplots/point meta max}\pgfplots@loc@TMPb\ifx\pgfplots@loc@TMPb\pgfutil@empty\else\global\let\pgfplots@metamax=\pgfplots@loc@TMPb\fi
	%
	\pgfkeysgetvalue{/pgfplots/@is3d}\pgfplots@loc@TMPb
	\def\pgfplots@loc@TMPc{true}%
	\ifx\pgfplots@loc@TMPc\pgfplots@loc@TMPb
		\global\pgfplots@threedimtrue
	\fi
	%
	\endgroup
...
\def\pgfplots@LUA@visualization@of@current@plot{%
	\ifpgfplots@LUA@backend@supported
		...
		\else
			% FIXME : there are currently >1 places where the coords are
			% serialized to TeX.
			%
			% Ensure that this call is the ONLY one which does it and move
			% this to some suitable place!
...
\long\def\pgfplots@addplotimpl@expression@@#1#2#3{%
	...
	\fi
	%
	\def\b@pgfplots@LUA@survey@expression{0}%
	\ifpgfplots@LUA@backend@supported
		\def\b@pgfplots@LUA@survey@expression{1}%
		\ifpgfplotsplothandlermesh@patch@type@sampling
			\def\b@pgfplots@LUA@survey@expression{0}%
			\pgfplots@log3{Deactivating LUA version of plot expression for plot \the\pgfplots@numplots\space (type '\pgfplotsplothandlername'): patch type sampling unsupported (yet).}%
			\pgfplots@LUA@backend@supportedfalse
		\fi
	\fi
	%
	...
	\pgfplots@coord@stream@start
	%
	\if1\b@pgfplots@LUA@survey@expression
		\pgfplots@log3{lua backend=true: Activating LUA version of plot expression for plot \the\pgfplots@numplots\space (type '\pgfplotsplothandlername').}%
		...
		\edef\pgfplotsretval{%
			...
			\directlua{%
				pgfplots.texAddplotExpressionCoordinateGenerator(^^J%
					...
					\ifpgfplots@LUA@backend@debugmode true\else false\fi)^^J%
			}%
		}%
		\if0\pgfplotsretval
			\pgfplots@log3{lua backend=true: LUA version of plot expression failed. Using TeX version. (plot \the\pgfplots@numplots).}%
...
\def\pgfplots@LUA@visualizationlog@deactivation#1{%
	\pgfplots@log3{Using 'lua backend=false' during visualization of plot \plotnum: #1.}%
}%
\def\pgfplots@LUA@visualization@init{%
	\ifpgfplots@LUA@backend@supported
		...
		\if m\pgfplots@colormap@access
		\else
			\pgfplots@LUA@visualizationlog@deactivation{color map access=direct not supported (yet)}%
			\pgfplots@LUA@backend@supportedfalse
		\fi
		% FIXME : add further conditions under which LUA has to be
		% switched off!
		%
		\ifpgfplots@LUA@backend@supported
			\edef\pgfplots@loc@TMPa{%
				\directlua{pgfplots.texVisualizationInit(\plotnum)}%
			}%
			\if1\pgfplots@loc@TMPa
				\pgfplots@log3{lua backend=true: Activating partial LUA backend for visualization of plot \plotnum.}%
			\else
				\pgfplots@LUA@visualizationlog@deactivation{deactivated during survey phase}%
				\pgfplots@LUA@backend@supportedfalse
			\fi
		\fi
	\fi
...
\long\def\pgfplots@coord@stream@finalize@storedcoords@START{%
	...
	%
	\let\pgfplots@plot@timer@args=\pgfplots@stored@current@data
	\pgfplotspointplotattimeclearcache
...
\def\pgfplots@apply@zbuffer{%
	...
	\or
		% reverse xy seq:
		\ifpgfplots@LUA@backend@supported
			\expandafter\edef\expandafter\pgfplots@stored@current@data\expandafter{%
