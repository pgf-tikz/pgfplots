...
\message{EDGE INIT^^J}%
		...
		\else
			...
			\edef\PD{\pgfplotspatchvertexextra pt D}%
			\pgfplotsutilsortfour\PA\PB\PC\PD
			\def\toCHAR##1pt ##2{##2}% get original index
			%
\message{DEPTH SEQUENCE: \PA, \PB, \PC, \PD^^J}%
			%
			\edef\PA{\expandafter\toCHAR\PA}%%
			\edef\Pcur{\expandafter\toCHAR\PB}%%
			\edef\Pnext{\csname pgfplots@rect@next@\Pcur\endcsname}%
			\if\Pnext\PA
				% ok, next(BB) = CC.
				% that means we have to advance forward.
				\def\Pnextseq{}%
				\edef\Pcur{\csname pgfplots@rect@rnext@\Pcur\endcsname}%
			\else
				\edef\Pcur{\csname pgfplots@rect@next@\Pcur\endcsname}%
				\def\Pnextseq{r}%
			\fi
		\fi
