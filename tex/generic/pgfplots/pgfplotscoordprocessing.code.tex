...
\def\pgfplots@LUA@visualization@of@current@plot{%
	...
	\ifpgfplots@LUA@backend@supported
		\ifx\pgfplotsplothandlerLUAvisualizerfactory\pgfutil@empty
			...
			\pgfplots@log3{Using 'lua backend=false' for visualization of plot \the\pgfplots@numplots\space (type '\pgfplotsplothandlername'): it has no LUA visualizer.}%
...
\def\pgfplots@LUA@plotexpression@log@deactivation#1{%
	\pgfplots@log3{Deactivating LUA version of plot expression for plot \the\pgfplots@numplots\space (type '\pgfplotsplothandlername'): #1.}%
}
...
\def\pgfplots@coord@stream@INIT@finalize@storedcoords{%
	...
	\pgfplotstreamstart
	%
	% Must be invoked AFTER the stream started... (but only because of
	% strange dependencies in \pgfplotsplothandlerLUAvisualizerfactory
	% in plot handler mesh...)
