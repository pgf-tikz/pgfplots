...
\def\pgfplotspointonorientedsurfaceabsetupfor#1#2#3{%
	\pgfutil@ifundefined{pgfplotspointonorientedsurfaceabsetupfor@@#1#2#3}{%
		\edef\pgfplots@loc@TMPa{#1#2#3}%
		\expandafter\pgfplotspointonorientedsurfaceabsetupfor@@generic\pgfplots@loc@TMPa
...
\def\pgfplotspointonorientedsurfaceabsetupfor@@generic#1#2#3{%
	\def\pgfplotspointonorientedsurfaceab##1##2{%
		\pgfplotsqpointxyz
			{\if#1x##1\else\if#2x##2\else\csname pgfplotspointonorientedsurfaceabsetupfor@fixed#3\endcsname\fi\fi}
			{\if#1y##1\else\if#2y##2\else\csname pgfplotspointonorientedsurfaceabsetupfor@fixed#3\endcsname\fi\fi}
			{\if#1z##1\else\if#2z##2\else\csname pgfplotspointonorientedsurfaceabsetupfor@fixed#3\endcsname\fi\fi}%
	}%
	\def\pgfplotspointonorientedsurfaceA{#1}%
	\def\pgfplotspointonorientedsurfaceB{#2}%
	\def\pgfplotspointonorientedsurfaceN{#3}%
	\edef\pgfplotspointonorientedsurfacespec{%
		\if#1x%
			a%
		\else
			\if#2x%
				b%
			\else
				\pgfplotspointonorientedsurfaceabsetupfor@fixedsymbol
			\fi
		\fi
		\if#1y%
			a%
	...
	\edef\pgfplotspointonorientedsurfacespecunordered{%
		\if#1x%
			v%
		\else
			\if#2x%
	...
	%\edef\pgfplotspointonorientedsurfacespec{ba0}% FIXME 
	%\edef\pgfplotspointonorientedsurfacespecunordered{vv0}%
...
\def\pgfplotsscanlinelengthinitzero{%
	...
	\expandafter\let\expandafter\pgfplotsscanlinecomplete			\csname pgfplotsscanlinelength@\pgfplots@loc@TMPa @complete\endcsname
...
%\message{Prepared macro \string\pgfplots@process@one@point: {\meaning\pgfplots@process@one@point}}%
	...
	\def\pgfplots@coord@stream@coord@{\pgfplotsplothandlersurveypoint}% don't use \let here.
