...
\def\pgfplots@PREPARE@COORD@STREAM@end@#1{%
	...
	\else
		...
		%
		\pgfplotssurveyphase@set@visphase@names
...
%
\def\pgfplotssurveyphase@set@visphase@names{%
...
% Error bars have a strange implementation: they are COLLECTED during
% the survey phase. The result of the collection is stored into macro '#1'. 
% Later, the macro '#1' is *executable* and draws the error bars.
\def\pgfplots@streamerrorbar@recordto#1{%
	\def\pgfplots@streamerrorbarstart{%
		...
		\to
			\pgfplots@streamerrorbar@recordto@@
	}%
	\def\pgfplots@streamerrorbarend{%
	...
	% ##2 of the form (<x>,<y>,<z>)
	\def\pgfplots@streamerrorbarcoords##1##2{%
		% It turns out that we ALSO want to access the original point
		% data, including its point meta and any visualization depends
		% on aspects.
		%
		% In order to keep it simple, I add this information here
		...
		\edef\pgfplots@loc@TMPa{
			% deserialize the current point ...
			\noexpand\pgfplotsaxisdeserializedatapointfrom\pgfplotsretval%
			%
			% ... ensure that point meta is set up ...
			\noexpand\pgfplotsaxisvisphasetransformpointmetaifany
			%
			% ... and draw the error bar.
			\the\t@pgfplots@tokb
		}%
		\expandafter\pgfplotsapplistXpushback\expandafter{\pgfplots@loc@TMPa}%
		\to
			\pgfplots@streamerrorbar@recordto@@
	}%
}
...
\def\pgfplotsaxisvisphase@get@afterpath#1{%
	\ifx\pgfplots@visphase@names\pgfutil@empty
		% ah - there are NO standard visualization phases. Consequently, we
		% MUST be in the marker visualization phase. Draw them.
