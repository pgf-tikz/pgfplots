...
\message{PIPE@DECODE initialised to patch output (from matrix).}
		\else
			% SPECIAL HANDLING!
			\let\pgfplotsplothandlermesh@PIPE@DECODE=\pgfplotsplothandlermesh@PIPE@DECODE@to@shader
\message{PIPE@DECODE initialised to matrix output (from matrix).}
		\fi
	\else
\message{PIPE@DECODE initialised to patch output (from patch).}
	\fi
}%
