...
%		\scope
		...
		\pgfplotspatchclass{\pgfplotsplothandlermesh@patchclass}{allow matrix}%
		\if0\pgfplotsretval
...
\def\pgfplotsplothandlermesh@PIPE@DECODE{%
...
\def\pgfplotsplothandlermesh@PIPE@CHECKJUMP{%
	...
	\ifpgfplots@loc@tmp
		\pgfplotsplothandlermesh@PIPE@REFINE
	\fi
}%
\def\pgfplotsplothandlermesh@PIPE@REFINE{%
...
\def\pgfplotsplothandlermesh@VISUALIZE{%
...
\def\pgfplotspatchvertex#1#2#3#4\endvertex{%
	...
	\def\pgfplotspatchvertexy{#1}%
	\def\pgfplotspatchvertexmeta{#3}%
	\def\pgfplotspatchvertexextra{#4}%
}%
...
\pgfqkeys{/pgfplots/@declare patch}{%
	allow matrix/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarepatchclass@\pgfkeyscurrentname\endcsname{#1}},%
	%
	% sets the next vertex value of the patch and invokes \pgfplotspatchready once
	% the patch is complete.
	% #1 encodes the current vertex such that
	% \pgfplotspatchvertex#1\endvertex
	% is accepted.
	set next vertex/.code=
		{\expandafter\def\csname pgfpmth\pgfplotsdeclarepatchclass@\pgfkeyscurrentname\endcsname##1{#1}},%
	%
...
\pgfplotsdeclarepatchclass{triangle}{%
	...
	set next vertex={%
		\ifcase\c@pgfplots@scanlineindex
			\edef\pgfplotsplothandlermesh@triangle@A{#1}%
			\c@pgfplots@scanlineindex=1
		\or
			\edef\pgfplotsplothandlermesh@triangle@B{#1}%
			\c@pgfplots@scanlineindex=2
		\or
			\edef\pgfplotsplothandlermesh@triangle@C{#1}%
			\c@pgfplots@scanlineindex=0
			\pgfplotspatchready%
		\fi
	},
	first vertex=\expandafter\pgfplotspatchvertex\pgfplotsplothandlermesh@triangle@A\endvertex,
	foreach vertex={%
		\expandafter\pgfplotspatchvertex\pgfplotsplothandlermesh@triangle@A\endvertex #1%
		\expandafter\pgfplotspatchvertex\pgfplotsplothandlermesh@triangle@B\endvertex #1%
		\expandafter\pgfplotspatchvertex\pgfplotsplothandlermesh@triangle@C\endvertex #1%
	}%,
	fill path={%
		\pgfpathmoveto{\expandafter\pgfplotspointpatchvertex\pgfplotsplothandlermesh@triangle@A\relax}%
		\pgfpathlineto{\expandafter\pgfplotspointpatchvertex\pgfplotsplothandlermesh@triangle@B\relax}%
		\pgfpathlineto{\expandafter\pgfplotspointpatchvertex\pgfplotsplothandlermesh@triangle@C\relax}%
		\pgfpathclose
	}%,
	num vertices=3,
}%
		...
		\else
			...
			\def\pgfplotsplothandlermesh@get@flat@color{%
				\pgfplotspatchclass{\pgfplotsplothandlermesh@patchclass}{foreach vertex}{%
					\let\pgfplots@current@point@meta=\pgfplotspatchvertexmeta
					\pgfplotsaxisvisphasetransformpointmeta
					\global\advance\pgfplots@tmpa by\pgfplotspointmetatransformed
				}%
				\global\divide\pgfplots@tmpa by\pgfplotspatchclass{\pgfplotsplothandlermesh@patchclass}{num vertices} %
	...
	\if1\pgfplotsplothandlermesh@checkfor@zbuffer
		\if\pgfplotsplothandlermesh@zbuffer@choice4%
			...
			\def\pgfplotsplothandlermesh@PIPE@FINISH{%
				...
				\def\pgfmathresult{0}%
				\pgfplotspatchclass{\pgfplotsplothandlermesh@patchclass}{foreach vertex}{%
					\pgfplotsplothandlermesh@accum@element@depth\pgfplotspatchvertexextra\pgfmathresult
				}%
				%
...
\def\pgfplotsplothandlermesh@check@unbounded{%
	\ifx\pgfplotspatchvertexx\pgfutil@empty
		\pgfplots@loc@tmpfalse
	\fi
	\ifx\pgfplotspatchvertexy\pgfutil@empty
		\pgfplots@loc@tmpfalse
	\fi
}%
