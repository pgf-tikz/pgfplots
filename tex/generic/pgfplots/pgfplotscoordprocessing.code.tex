...
\def\pgfplots@coord@stream@INIT@finalize@storedcoords{%
	...
	\gdef\pgfplots@coord@stream@finalize@currentpt{%
		%
		% this here seems like a no-op. However, PGF remembers this
		% value as "\pgfplotlastpoint" - and it needs coordinates
		% inside of it.
		\edef\pgfplots@loc@TMPa{\pgf@x=\the\pgf@x\space\pgf@y=\the\pgf@y\space}%
		\expandafter\pgfplotstreampoint\expandafter{\pgfplots@loc@TMPa}%
		%
