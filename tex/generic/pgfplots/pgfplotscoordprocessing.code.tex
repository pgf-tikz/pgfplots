...
\def\pgfplotscoordmath@log@set@basis#1#2{%
	%
	\pgfplotscoordmath{#1}{log e}{#2}%
	\let\pgfplots@loc@TMPa=\pgfmathresult% TMPa = log_e(#2)
	\pgfplotscoordmath{#1}{op}{reciprocal}{{\pgfmathresult}}%
	\let\pgfplots@loc@TMPb=\pgfmathresult% TMPb = 1/log_e(#2)
	%
	\expandafter\edef\csname pgfpmth@#1@log\endcsname##1{%
		\noexpand\pgfplotscoordmath{#1}{log e}{##1}%
		\noexpand\ifx\noexpand\pgfmathresult\noexpand\pgfutil@empty
		\noexpand\else
			\noexpand\pgfplotscoordmath{#1}{op}{multiply}{%
				{\noexpand\pgfmathresult}%
				{\pgfplots@loc@TMPb}%
			}%
		\noexpand\fi
	}%
	\expandafter\def\csname pgfpmth@#1@log to display log\endcsname##1{\edef\pgfmathresult{##1}}%
	\expandafter\def\csname pgfpmth@#1@log from display log\endcsname##1{\edef\pgfmathresult{##1}}%
	%
	...
	% and re-compute i>=10 :
	\begingroup
		\expandafter\let\expandafter\logi\csname pgfpmath@#1@log unsigned int@orig\endcsname
		\let\logscale=\pgfplots@loc@TMPb
		%
		\logi{1}%
		\pgfplotscoordmath{#1}{op}{multiply}{{\pgfmathresult}{\logscale}}%
		...
		\xdef\pgfplots@glob@TMPa##1{%
			...
			\noexpand\else
				\noexpand\pgfplotscoordmath{#1}{log}{##1}%
			\noexpand\fi
		}%
	\endgroup
	\expandafter\let\csname pgfpmth@#1@log unsigned int\endcsname=\pgfplots@glob@TMPa
	\pgfplotscoordmath@def@log@to@log@ten{#1}\pgfplots@loc@TMPa
}%
