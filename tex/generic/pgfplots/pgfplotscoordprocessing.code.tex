...
\def\pgfplots@prepare@visualization@depends@on@preparetype@value value#1\as#2{%
	...
	\ifx\pgfplots@loc@TMPa\pgfplots@loc@TMPb
		...
		% this is special; we do not need to EXECUTE #2 to arrive at its value.
...
\def\pgfplots@foreach@plot@coord@NEXT@WITH@ERRORRANGE@#1,#2){%
	\ifpgfplots@plot@coords@mathparser
		\pgfmathparse{#1}%
		\let\pgfplots@current@point@error@x@plus=\pgfmathresult
		\pgfmathparse{#2}%
		\let\pgfplots@current@point@error@y@plus=\pgfmathresult
	\else
		\def\pgfplots@current@point@error@x@plus{#1}%
		\def\pgfplots@current@point@error@y@plus{#2}%
	\fi
	\let\pgfplots@current@point@error@x@minus=\pgfplots@current@point@error@x@plus
	\let\pgfplots@current@point@error@y@minus=\pgfplots@current@point@error@y@plus
	\pgfutil@ifnextchar[{%
		\pgfplots@foreach@plot@coord@NEXT@meta
	}{%
		\let\pgfplots@current@point@meta=\pgfutil@empty
...
\def\pgfplots@foreach@plot@coord@threedim@NEXT@WITH@ERRORRANGE@#1,#2,#3){%
	\ifpgfplots@plot@coords@mathparser
		\pgfmathparse{#1}\let\pgfplots@current@point@error@x@plus=\pgfmathresult
		\pgfmathparse{#2}\let\pgfplots@current@point@error@y@plus=\pgfmathresult
		\pgfmathparse{#3}\let\pgfplots@current@point@error@z@plus=\pgfmathresult
	\else
		\def\pgfplots@current@point@error@x@plus{#1}%
		\def\pgfplots@current@point@error@y@plus{#2}%
		\def\pgfplots@current@point@error@z@plus{#3}%
	\fi
	\let\pgfplots@current@point@error@x@minus=\pgfplots@current@point@error@x@plus
	\let\pgfplots@current@point@error@y@minus=\pgfplots@current@point@error@y@plus
	\let\pgfplots@current@point@error@z@minus=\pgfplots@current@point@error@z@plus
	\pgfutil@ifnextchar[{%
		\pgfplots@foreach@plot@coord@threedim@NEXT@meta
	}{%
		\let\pgfplots@current@point@meta=\pgfutil@empty
...
\long\def\pgfplots@addplotimpl@table@fromstructure@#1#2#3#4{%
	...
	%
	%
	\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@init x{#3}%
	\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@init y{#3}%
	\ifpgfplots@curplot@threedim
		\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@init z{#3}%
	\fi
	%
	\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@load x{#3}%
	\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@load y{#3}%
	\ifpgfplots@curplot@threedim
		\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@load z{#3}%
	\fi
	%
	...
	\ifpgfplots@loop@CONTINUE
		...
		\fi
		\ifpgfplots@errorbars@enabled
			\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@getnext x%
			\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@getnext y%
			\ifpgfplots@curplot@threedim
				\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@getnext z%
			\fi
		\fi
		\pgfplots@coord@stream@coord
	\pgfutil@repeat
...
% #1: x, y, or z
% #2: the table name
\def\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@init#1#2{%
	\expandafter\let\csname addplot@tbl@error@#1\endcsname=\pgfutil@empty
	...
	\ifpgfplots@errorbars@enabled
		...
		\def\pgfplots@loc@TMPa{%
			\pgfplots@addplot@table@fromstructure@preparecolname{#1 error}{#2}}%
		\edef\pgfplots@loc@TMPb{%
			\expandafter\noexpand\csname pgfplots@plot@tbl@error@#1\endcsname
			\expandafter\noexpand\csname columnerror#1\endcsname
		}%
		% execute prepared statement:
		% = \pgfplots@addplot@table@fromstructure@preparecolname
		% 		{x error}{#2}{\pgfplots@plot@tbl@error@x}{\columnerrorx}
		\expandafter\pgfplots@loc@TMPa\pgfplots@loc@TMPb
		%
	\fi
}%
...
% #1: x, y, or z
% #2: the table name
\def\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@load#1#2{%
	\ifpgfplots@errorbars@enabled
		\expandafter\ifx\csname pgfplots@plot@tbl@error@#1\endcsname\pgfutil@empty
			% ok, we do not have this error kind.
		\else
			% assemble the statement
			%  \expandafter\pgfplotstablegetcolumnbyname\expandafter{\pgfplots@plot@tbl@error@x}\of#2\to\addplot@tbl@error@x
			...
			\expandafter\pgfplots@loc@TMPb\csname addplot@tbl@error@#1\endcsname
		\fi
	\fi
}%

% #1: x, y, or z
\def\pgfplots@addplotimpl@table@fromstructure@prepare@errorbar@getnext#1{%
	\expandafter\ifx\csname addplot@tbl@error@#1\endcsname\pgfutil@empty
		\expandafter\let\csname pgfplots@current@point@error@#1@plus\endcsname=\pgfutil@empty
	\else
		\expandafter\pgfplotslistpopfront\csname addplot@tbl@error@#1\endcsname\to\pgfplots@loc@TMPa
		\expandafter\let\csname pgfplots@current@point@error@#1@plus\endcsname=\pgfplots@loc@TMPa
	\fi
	\pgfutil@namelet{pgfplots@current@point@error@#1@minus}{pgfplots@current@point@error@#1@plus}%
}%
...
% #1: x, y, or z
\def\pgfplots@addplotimpl@table@fromfile@listener@witherrors@COLLECTHIGHLEVEL@prepare@#1{%
	\pgfkeysgetvalue{/pgfplots/table/#1 error index}\pgfplots@plot@tbl@error@index
	\pgfkeysgetvalue{/pgfplots/table/#1 error}\pgfplots@plot@tbl@error
	%
	\expandafter\def\csname pgfplots@table@ERRPTR@#1\endcsname{#1 error}%
	%
	\expandafter\def\csname pgfplots@dereferencepointer@and@ASSIGN@error@#1\endcsname{%
		\expandafter\pgfplotstablereadevalptr\csname pgfplots@table@ERRPTR@#1\endcsname\pgfmathresult
		\expandafter\let\csname pgfplots@current@point@error@#1@plus\endcsname=\pgfmathresult
		\expandafter\let\csname pgfplots@current@point@error@#1@minus\endcsname=\pgfmathresult
	}%
	%
	\ifx\pgfplots@plot@tbl@error\pgfutil@empty
		\ifx\pgfplots@plot@tbl@error@index\pgfutil@empty
			\let\pgfplotsretval=\relax
			\expandafter\let\csname pgfplots@dereferencepointer@and@ASSIGN@error@#1\endcsname=\relax
		\else
	...
	\fi
	\expandafter\let\csname pgfplots@table@ERRPTR@#1\endcsname=\pgfplotsretval
	%
	\pgfkeysgetvalue{/pgfplots/table/#1 error expr}{\pgfplots@loc@TMPa}%
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
	\else
		\expandafter\def\csname pgfplots@dereferencepointer@and@ASSIGN@error@#1\endcsname{%
			\pgfmathparse{\pgfkeysvalueof{/pgfplots/table/#1 error expr}}%
			\expandafter\let\csname pgfplots@current@point@error@#1@plus\endcsname=\pgfmathresult
			\expandafter\let\csname pgfplots@current@point@error@#1@minus\endcsname=\pgfmathresult
		}%
	\fi
...
\def\pgfplots@addplotimpl@table@fromfile@listener@witherrors@COLLECTHIGHLEVEL@prepare{%
	% this here is only evaluated ONCE.
	\pgfplots@addplotimpl@table@fromfile@listener@witherrors@COLLECTHIGHLEVEL@prepare@ x%
	\pgfplots@addplotimpl@table@fromfile@listener@witherrors@COLLECTHIGHLEVEL@prepare@ y%
	%
	\ifpgfplots@curplot@threedim
		\pgfplots@addplotimpl@table@fromfile@listener@witherrors@COLLECTHIGHLEVEL@prepare@ z%
	\else
