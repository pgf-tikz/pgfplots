...
%\message{STREAMING COLLAPSED EDGE AT START POINT^^J}%
	...
	refine={%
		\let\pgfplotspatchclass@patchready@afterrefine=\pgfplotspatchready
		\pgfplotspatchclass@qtri@recursiverefine{#1}%
	},%
	set edge visible={%
		\ifcase#1
...
\def\pgfplotspatchclass@qtri@recursiverefine#1{%
	\c@pgf@counta=#1
	\ifnum\c@pgf@counta=1
		\let\pgfplotspatchready=\pgfplotspatchclass@patchready@afterrefine
	\else
		\advance\c@pgf@counta by-1
		\edef\pgfplotspatchready{\noexpand\pgfplotspatchclass@qtri@recursiverefine{\the\c@pgf@counta}}%
	\fi
	%
	\t@pgfplots@tokc=\expandafter{\pgfplotspatchready}%
	\edef\pgfplots@loc@TMPa{%
		\noexpand\def\noexpand\pgfplotspatchclass@qtri@A{\pgfplotspatchclass@qtri@A}%
		\noexpand\def\noexpand\pgfplotspatchclass@qtri@B{\pgfplotspatchclass@qtri@B}%
		\noexpand\def\noexpand\pgfplotspatchclass@qtri@C{\pgfplotspatchclass@qtri@C}%
		\noexpand\def\noexpand\pgfplotspatchclass@qtri@D{\pgfplotspatchclass@qtri@D}%
		\noexpand\def\noexpand\pgfplotspatchclass@qtri@E{\pgfplotspatchclass@qtri@E}%
		\noexpand\def\noexpand\pgfplotspatchclass@qtri@F{\pgfplotspatchclass@qtri@F}%
		\noexpand\def\noexpand\pgfplotspatchready{\the\t@pgfplots@tokc}%
		\noexpand\def\noexpand\pgfplotspatchclassname{triangle quadr}%
	}%
	\expandafter\pgfplotspatchclass@qtri@recursiverefine@\expandafter{\pgfplots@loc@TMPa}{#1}%
}%


% #1: commands to restore the variables of the actual patch.
% #2: refine depth.
\def\pgfplotspatchclass@qtri@recursiverefine@#1#2{%
...
\def\pgfplotsplothandlermesh@init@REFINE{%
	\pgfkeysgetvalue{/pgfplots/mesh/refines}\pgfplotsplothandlermesh@refinecount
	\if0\pgfplotsplothandlermesh@triangulate
