...
\def\pgfplots@coord@stream@INIT@finalize@storedcoords#1{%
	...
	\fi
	%
	% record marker streams == true 
	% :<=>
	%  (mark != none) &&
	%  (mark layer != empty 
	%   ||  (!only marks  && !clip marker paths)
	%  )
	%
	% perhaps better negated:
	% record marker streams = false
	% :<=>
	% (mark == none) || (mark layer == empty && only marks && clip marker paths)
	%
...
\long\def\pgfplots@coord@stream@finalize@storedcoords@START normalized coordinates #1#2;\pgfplots@EOI{%
	...
	\pgfplotspointplotattimeclearcache
	%
	\pgfplots@markers@visphase@directlyafterpath
	%
}%
