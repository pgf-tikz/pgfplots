...
% \pgfplotstransformplotattime{#1} as shift and using the slope of the
...
%
% As a side-effect, it defines the (global) macros \pgfplotspointplotattimefirst and \pgfplotspointplotattimesecond
% in order to allow the computation of gradients. These macros can be decoded using
...
\def\pgfplotspointplotattime#1{%
	...
	\def\pgfplots@coord@stream@coord@{%
		...
		\else
%\message{\pgfplots@len@total^^J}%
			\pgfplotsplothandlerserializepointto{\pgfplots@cur}%
			\pgfplotsplothandlersurveydifflen{\pgfplots@last}{\pgfplots@cur}%
			\pgfplotscoordmath{default}{op}{add}{{\pgfmathresult}{\pgfplots@len@total}}%
			\let\pgfplots@len@total=\pgfmathresult
			\pgfplotspointattime@do
			%
		\fi
		\pgfplotsplothandlerserializepointto{\pgfplots@last}%
	}%
	\def\pgfplots@coord@stream@end@{}%
	...
	%
	\let\pgfplots@len@total@=\pgfplots@len@total
	%
...
%\message{total length=\pgfplots@len@total@; target len \pgfplots@len@thresh\space (#1)^^J}
	%
	\def\pgfplots@HIT{0}%
	...
	\def\pgfplotspointattime@do{%
		...
		\let\pgfplots@last@last=\pgfplots@last
	}%
	\expandafter\pgfplots@coord@stream@foreach@NORMALIZED\expandafter{\pgfplots@plot@timer@args}%
	\if\pgfplots@HIT0%
		\let\pgfplots@last=\pgfplots@last@last
...
%\message{HIT between \pgfplots@len@last\space and \pgfplots@len@total\space (\pgfplots@time@between@two)^^J}%
	...
	\else
		\pgfplotsplothandlerserializepointto{\pgfplots@cur}%
		\pgfplotsplothandlersurveypointattime{\pgfplots@time@between@two}{\pgfplots@last}{\pgfplots@cur}%
	\fi
	\pgfplotsaxisvisphasegetpoint
	\global\let\pgfplotspointplotattimefirst=\pgfplots@last
