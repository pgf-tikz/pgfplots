...
% Changes \pgfplots@current@point@[xyz] to the new coordinate system
% (cs) designated by '#2'.
%
% #1: the actual coordinate system's name
% #2: the desired coordinate system's name
%
% PRECONDITION: \pgfplots@current@point@[xyz] contain the current
...
%
% POSTCONDITION: \pgfplots@current@point@[xyz] contain same point as
...
%
% Example:
% \def\pgfplots@current@point@x{90}
% \def\pgfplots@current@point@y{1}
% \pgfplotsaxistransformcs{polar}{cart}
% -->
%  \pgfplots@current@point@x = 0
%  \pgfplots@current@point@y = 1
...
\pgfplotsdefinecstransform{polar}{cart}{%
	\pgfplotscoordmath{default}{parsenumber}{\pgfplots@current@point@x}%
	...
	\else
		...
		\else
			...
			\ifpgfplots@loop@CONTINUE
				...
				%
				% this is nonsense, I just keep it because the @scale
				% variable will change eventually:
				\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotszlen}{\pgfplots@scale}}%
				\let\pgfplotszlenratio@float=\pgfmathresult
				\pgfplotscoordmath{default}{tofixed}{\pgfmathresult}%
				\let\pgfplotszlenratio=\pgfmathresult
				%
		...
		%\pgfplotscoordmath{default}{parsenumber}{1}\let\pgfplots@scale=\pgfmathresult
					...
					\else
						...
						% '\pgfplotszlenratio' (which is usually 1)
						\pgfplotscoordmath{default}{parsenumber}{\pgfplots@loc@TMPa}%
						\pgfplotscoordmath{default}{op}{multiply}{{\pgfmathresult}{\pgfplotszlenratio@float}}%
						\let\pgfplots@loc@TMPa=\pgfmathresult
						%
