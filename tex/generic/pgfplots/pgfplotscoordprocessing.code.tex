...
\long\def\pgfplots@PREPARE@COORD@STREAM#1{%
	...
	% particular plot.
	%
	% 1.\pgfplotsaxisupdatelimitsforcoordinate
...
\def\pgfplots@PREPARE@COORD@STREAM@start@{%
	...
	%
	\pgfkeyssetvalue{/pgf/data point/x}{\pgfplots@current@point@x}%
	\pgfkeyssetvalue{/pgf/data point/y}{\pgfplots@current@point@y}%
	\pgfkeyssetvalue{/pgf/data point/z}{\pgfplots@current@point@z}%
	\pgfkeyssetvalue{/pgf/data point/meta}{\pgfplots@current@point@meta}%
	%
...
\def\pgfplots@prepare@visualization@dependencies{%
	\pgfkeysgetvalue{/pgfplots/visualization depends on}\pgfplots@loc@TMPa
	\ifx\pgfplots@loc@TMPa\pgfutil@empty
	\else
...
\def\pgfplots@prepare@visualization@depends@on#1\as#2\pgfplots@EOI{%
	\pgfplots@prepare@visualization@depends@on@preparetype{#2}#1\pgfplots@EOI
	% prepare the serialization:
	\t@pgfplots@tokb=\expandafter{\the\t@pgfplots@tokb\noexpand#2{\csname\string#2@value\endcsname}}%
	\t@pgfplots@tokc=\expandafter{\the\t@pgfplots@tokc#2}%
}%
\def\pgfplots@prepare@visualization@depends@on@preparetype#1{%
	\pgfutil@ifnextchar\bgroup{%
		\pgfplots@prepare@visualization@depends@on@preparetype@expr{#1}expr%
	}{%
		\pgfutil@ifnextchar v{%
			\pgfplots@prepare@visualization@depends@on@preparetype@value{#1}%
		}{%
			\pgfutil@ifnextchar e{%
				\pgfplots@prepare@visualization@depends@on@preparetype@expr{#1}%
			}{%
				\pgfplots@prepare@visualization@depends@on@preparetype@error{#1}%
			}%
		}%
	}%
}%
\def\pgfplots@prepare@visualization@depends@on@preparetype@error#1#2\pgfplots@EOI{%
	\t@pgfplots@tokc={#2 \as #1}%
	\pgfplotsthrow{invalid argument}{\pgfplots@loc@TMPa}{Sorry, but `visualization depends on=\{\the\t@pgfplots@tokc\}' is not what I expected. I expected something like 'expr\{<math stuff>\}\string\as\string#1 or value\{<fixed value>\}\string\as\string#1}\pgfeov%
}
\def\pgfplots@prepare@visualization@depends@on@preparetype@expr#1expr#2{%
	\pgflibraryfpuifactive{%
		\def#1{%
			\pgfmathparse{#2}%
			\pgfmathfloattofixed{\pgfmathresult}%
			\expandafter\let\csname \string#1@value\endcsname=\pgfmathresult
		}%
	}{%
		\def#1{%
			\pgfmathparse{#2}%
	...
	\pgfutil@ifnextchar\pgfplots@EOI{%
		...
		\pgfplots@prepare@visualization@depends@on@preparetype@error{#1}expr#2\pgfplots@EOI
	}%
}
\def\pgfplots@prepare@visualization@depends@on@preparetype@value#1value#2{%
	\def#1{%
		\expandafter\def\csname \string#1@value\endcsname{#2}%
	}%
	\pgfutil@ifnextchar\pgfplots@EOI{%
		\pgfutil@gobble
	}{%
		\pgfplots@prepare@visualization@depends@on@preparetype@error{#1}value#2\pgfplots@EOI
	}%
}
...
\def\pgfplots@coord@stream@INIT@finalize@storedcoords@prepare@scaletrafomacro{%
	...
	\xdef\pgfplotsaxisvisphasetransformcoordinate##1##2##3{%
		\ifpgfplots@apply@datatrafo@x
			\E\pgfplots@datascaletrafo@x{##1}%
			\E\let##1=\E\pgfmathresult
		\fi
		\ifpgfplots@apply@datatrafo@y
			\E\pgfplots@datascaletrafo@y{##2}%
			\E\let##2=\E\pgfmathresult
		\fi
		\ifpgfplots@curplot@threedim
			\ifpgfplots@apply@datatrafo@z
				\E\pgfplots@datascaletrafo@z{##3}%
