...
\def\pgfplots@borderanchor@for@axis#1#2#3{%
	...
	\else
\message{^^JPGFPlots warning: The ticklabel anchor cannot be determined, the normal vector and the unit #1 vector are almost parallel (abs(cos(angle)) = \the\pgf@xc)!^^J}%
...
\def\pgfplotssetaxesfromazel#1#2#3{%
	...
	\endgroup
%\message{Setting x,y and z from {#1}{#2} to x = \meaning\pgfplots@glob@TMPa, y = \meaning\pgfplots@glob@TMPb, z = \meaning\pgfplots@glob@TMPc...n = \pgfplots@glob@TMPd.^^J}%
	...
	\def#3{0}%
	\pgfplots@scaleaxes@to@widthheight
}%

\def\pgfplots@scaleaxes@to@widthheight{%
...
\def\pgfplots@scaleaxes@to@BB#1#2{%
	\if0\pgfplots@scale@mode@choice
		\def\pgfplots@scale@mode@choice{2}% auto--> stretch to fill
	\fi
...
\message{After scaling: x = (\the\pgf@xx,\the\pgf@xy), y =(\the\pgf@yx,\the\pgf@yy), z = (\the\pgf@zx,\the\pgf@zy).^^J}%
}%
\def\pgfplots@scaleaxes@to@BB@#1#2{%
	\begingroup
...
%\message{PGFPLOTS: the current unit vectors result in a UNIT BB of (\the\pgf@xa,\the\pgf@xb). Scaling it to (\the\pgf@ya,\the\pgf@yb)...}%
	...
	\or
		...
		\if\pgfplots@user@provided@height\pgfplots@user@provided@width
			\pgfplots@warning{scale mode=scale uniformly  expected EITHER 'width' OR 'height'... it will now scale to fit the widths.}%
			\if0\pgfplots@user@provided@height
				\def\pgfplots@user@provided@width{1}%
			\fi
		\fi
		\if1\pgfplots@user@provided@width
			\pgfplotsutil@edef@invoke\pgfmathdivide@{%
				{\pgf@sys@tonumber\pgf@ya}%
				{\pgf@sys@tonumber\pgf@xa}%
			}%
...
% configuration of x,y,z (2d) unit vectors.
%
% The idea is as follows:
% I suppose the actual view is actually a transformed XZ plane and the
% view direction is the transformed Y axis of the underlying axis. In
% other words, I try to restore the case as it is documented for
% \pgfplotssetaxesfromazel.
%
% Consequently, I assume that the actual transformation from 3D points
% q \in \R^3 to 2d CANVAS is given by
% P(q) = (q^T X, q^T Z)
% where X,Z \in \R^3 (!) are the 3d transformed axes. Once I have
% these two vectors, I know Y = Z crossproduct X.
%
% That's what I do. I can recover X and Z (which have 6 components)
% using the 6 equations for
% P([1 0 0]), P([0 1 0]), P([0 0 1]).
% That yields
% X = [ e_xx e_yy e_xy ]
% Z = [ e_zx e_yx e_zy ].
%
% Then, I use Y = Z crossprod X  (plus a normalization) as view
% direction.
%
\def\pgfplotsgetnormalforcurrentview{%
	\begingroup	
		\pgf@xa=\pgf@sys@tonumber\pgf@xy\pgf@yx
		\pgf@xb=\pgf@sys@tonumber\pgf@yy\pgf@zy
		\advance\pgf@xa by -\pgf@xb
		\pgf@ya=\pgf@xa
		\xdef\pgfplots@view@dir@threedim@x{\pgf@sys@tonumber\pgf@xa}%
		%
		\pgf@xa=\pgf@sys@tonumber\pgf@xx\pgf@zy
		\pgf@xb=\pgf@sys@tonumber\pgf@xy\pgf@zx
		\advance\pgf@xa by -\pgf@xb
		\pgf@yb=\pgf@xa
		\xdef\pgfplots@view@dir@threedim@y{\pgf@sys@tonumber\pgf@xa}%
		%
		\pgf@xa=\pgf@sys@tonumber\pgf@yy\pgf@zx
		\pgf@xb=\pgf@sys@tonumber\pgf@xx\pgf@yx
		\advance\pgf@xa by -\pgf@xb
		\pgf@xc=\pgf@xa
		\xdef\pgfplots@view@dir@threedim@z{\pgf@sys@tonumber\pgf@xa}%
		%
		% L_infty normalization:  (the normalization is more or less
		% irrelevant, so I choose the simplest one)
		\ifdim\pgf@ya<0sp \multiply\pgf@ya by-1\fi
		\ifdim\pgf@yb<0sp \multiply\pgf@yb by-1\fi
		\ifdim\pgf@xc<0sp \multiply\pgf@xc by-1\fi
		\pgf@xa=\pgf@ya
		\ifdim\pgf@xa<\pgf@yb \pgf@xa=\pgf@yb	\fi
		\ifdim\pgf@xa<\pgf@xc \pgf@xa=\pgf@xc	\fi
		\expandafter\pgfmathreciprocal@\expandafter{\pgf@sys@tonumber\pgf@xa}%
		\let\pgfplots@normalization=\pgfmathresult
		%
		\pgfmathmultiply@{\pgfplots@view@dir@threedim@x}{\pgfplots@normalization}%
		\global\let\pgfplots@view@dir@threedim@x=\pgfmathresult
		\pgfmathmultiply@{\pgfplots@view@dir@threedim@y}{\pgfplots@normalization}%
		\global\let\pgfplots@view@dir@threedim@y=\pgfmathresult
		\pgfmathmultiply@{\pgfplots@view@dir@threedim@z}{\pgfplots@normalization}%
...
\def\pgfplotsplothandlergraphicspointmapcomputerequiredview{%
	...
	\else
		...
		%
		%
		% The actual implementation is dump; it requires 4 (!) points for
		...
		%
		% Then, I solve for o,e_x,e_y,e_z and use
		% unit vector ratio={||e_x||/ ||e_z||  ||e_y||/||e_z|| 1}.
		%
		%
		...
		\else
			...
			\pgfplotscoordmath{default}{op}{veclen}{%
				{\pgfplotsarrayvalueofelem0\of\pgfplotsE}%
				{\pgfplotsarrayvalueofelem1\of\pgfplotsE}%
			}%
			\let\pgfplotsxlen=\pgfmathresult
			%
			\pgfplotscoordmath{default}{op}{veclen}{%
				{\pgfplotsarrayvalueofelem2\of\pgfplotsE}%
				{\pgfplotsarrayvalueofelem3\of\pgfplotsE}%
			}%
			\let\pgfplotsylen=\pgfmathresult
			%
			\pgfplotscoordmath{default}{op}{veclen}{%
			...
			%
			\pgfplots@loop@CONTINUEtrue
			\pgfplotscoordmath{default}{if is}{\pgfplotsxlen}{0}{%
				\pgfplots@loop@CONTINUEfalse
			}{}%
			\pgfplotscoordmath{default}{if is}{\pgfplotsylen}{0}{%
				\pgfplots@loop@CONTINUEfalse
			}{}%
			\pgfplotscoordmath{default}{if is}{\pgfplotszlen}{0}{%
				\pgfplots@loop@CONTINUEfalse
			}{}%
			%
			\ifpgfplots@loop@CONTINUE
				\pgfplotscoordmath{default}{op}{reciprocal}{{\pgfplotszlen}}%
				\let\pgfplots@scale=\pgfmathresult
				%
				\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsxlen}{\pgfplots@scale}}%
				\pgfplotscoordmath{default}{tofixed}{\pgfmathresult}%
				\let\pgfplotsxlenratio=\pgfmathresult
				\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsylen}{\pgfplots@scale}}%
				\pgfplotscoordmath{default}{tofixed}{\pgfmathresult}%
				\let\pgfplotsylenratio=\pgfmathresult
				%
				\pgfmathapproxequalto@{\pgfplotsxlenratio}{0.0}%
				\ifpgfmathcomparison
					\pgfplots@loop@CONTINUEfalse
				\fi
				\pgfmathapproxequalto@{\pgfplotsylenratio}{0.0}%
				\ifpgfmathcomparison
					\pgfplots@loop@CONTINUEfalse
				\fi
				\ifpgfplots@loop@CONTINUE
					\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsarrayvalueofelem0\of\pgfplotsE}{\pgfplots@scale}}%
					\pgfplotscoordmath{default}{tofixed}\pgfmathresult%
					\let\pgfplots@xx=\pgfmathresult
					\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsarrayvalueofelem1\of\pgfplotsE}{\pgfplots@scale}}%
					\pgfplotscoordmath{default}{tofixed}\pgfmathresult%
					\let\pgfplots@xy=\pgfmathresult
					%
					\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsarrayvalueofelem2\of\pgfplotsE}{\pgfplots@scale}}%
					\pgfplotscoordmath{default}{tofixed}\pgfmathresult%
					\let\pgfplots@yx=\pgfmathresult
					\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsarrayvalueofelem3\of\pgfplotsE}{\pgfplots@scale}}%
					\pgfplotscoordmath{default}{tofixed}\pgfmathresult%
					\let\pgfplots@yy=\pgfmathresult
					%
					\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsarrayvalueofelem4\of\pgfplotsE}{\pgfplots@scale}}%
					\pgfplotscoordmath{default}{tofixed}\pgfmathresult%
					\let\pgfplots@zx=\pgfmathresult
					\pgfplotscoordmath{default}{op}{multiply}{{\pgfplotsarrayvalueofelem5\of\pgfplotsE}{\pgfplots@scale}}%
					\pgfplotscoordmath{default}{tofixed}\pgfmathresult%
					...
					%	z={(\pgfplots@zx,\pgfplots@zy)},%
						scale mode=scale uniformly,%
						unit vector ratio={\pgfplotsxlenratio\space\pgfplotsylenratio\space1}%
					}%
				\fi
			\fi
			%
			\ifx\pgfplotsretval\pgfutil@empty
				\pgfplotscoordmath{default}{tofixed}{\pgfplotsxlen}\let\pgfplotsxlen@=\pgfmathresult
				\pgfplotscoordmath{default}{tofixed}{\pgfplotsylen}\let\pgfplotsylen@=\pgfmathresult
				\pgfplotscoordmath{default}{tofixed}{\pgfplotszlen}\let\pgfplotszlen@=\pgfmathresult
				\pgfplots@warning{plot graphics failed to compute the 'unit vector ratio'\csname on@line\endcsname\space (it found ||ex|| =\pgfplotsxlen@; ||ey||= \pgfplotsylen@; ||ez|| =\pgfplotszlen@). The graphics might be scaled incorrectly. Perhaps the provided 'points' are not linearly independent?}%
			\else
				\begingroup
				\pgfkeysgetvalue{/pgfplots/plot graphics/src}\pgfplots@loc@TMPa
				\t@pgfplots@tokc=\expandafter{\pgfplots@loc@TMPa}%
				\immediate\write-1{PGFPlots plot graphics[auto adjust axis=true] {\the\t@pgfplots@tokc} \csname on@line\endcsname: using '\pgfplotsretval'. Use 'plot graphics[debug] to generate debug output files.^^J}% 
				\endgroup
			\fi
			%
			\if1\pgfplotsplothandlergraphicspointmapcomputerequiredview@debug
...
\def\pgfplotsplothandlergraphicspointmapcomputerequiredview@debug@output{%
	...
	\immediate\write\w@pgf@writea{img x unit=\pgfplots@loc@TMPa\space\pgfplots@loc@TMPb,}%
	...
	\immediate\write\w@pgf@writea{img y unit=\pgfplots@loc@TMPa\space\pgfplots@loc@TMPb,}%
	...
	\immediate\write\w@pgf@writea{img origin=\pgfplots@loc@TMPa\space\pgfplots@loc@TMPb,}%
	\pgfplotscoordmath{default}{tofixed}{\pgfplotsxlen}\let\pgfplotsxlen@=\pgfmathresult
	\pgfplotscoordmath{default}{tofixed}{\pgfplotsylen}\let\pgfplotsylen@=\pgfmathresult
	\pgfplotscoordmath{default}{tofixed}{\pgfplotszlen}\let\pgfplotszlen@=\pgfmathresult
	\immediate\write\w@pgf@writea{unit length=\pgfplotsxlen@\space\pgfplotsylen@\space\pgfplotszlen@,}%
	\begingroup
	\def\n{^^J}%
	\def\t{^^I}%
...
\def\pgfplots@plot@handler@graphics@process@pointmap{%
	...
	\ifnum\pgf@x=0pt
		\pgfplots@error{Sorry, the first two points with '=>' in plot graphics[points={}] are expected to have different image X coordinates. Please reorder the sequence.}%
	\fi
	\ifnum\pgf@y=0pt
	...
	\pgfpointadd
		{\pgfplots@plot@handler@graphics@pointmap@A@canvas}% the canvas coordinate corresponding to 'A'
		{\pgfqpointscale{-1}
			{\pgfpoint\pgfplots@vx\pgfplots@vy}%
		}%
	...
	\ifx\pgfplots@plot@handler@graphics@pointmap@C@canvas\pgfutil@empty
		...
		\fi
	\else
		\pgfplots@plot@handler@graphics@pointmap@sanitize@scaling
	\fi
	\endgroup
}%

\def\pgfplots@plot@handler@graphics@pointmap@sanitize@scaling{%
	\expandafter\pgfplots@extractimgaspoint\pgfplots@plot@handler@graphics@pointmap@C@img% anchor 3 in image
	...
	\edef\pgfplots@plot@handler@graphics@pointmap@C@canvas@check{\pgf@x=\the\pgf@x\space\pgf@y=\the\pgf@y\space}%
	\pgfpointdiff
		\pgfplots@plot@handler@graphics@pointmap@C@canvas@check
		\pgfplots@plot@handler@graphics@pointmap@C@canvas
	...
	\if0\pgfplots@is@the@same@point
		...
		\pgfplots@error{sorry, I can't fix the scaling of 'plot graphics {\the\t@pgfplots@tokc}'.
			The points (\expandafter\pgfplots@extractcoord\pgfplots@plot@handler@graphics@pointmap@A@logical) and (\expandafter\pgfplots@extractcoord\pgfplots@plot@handler@graphics@pointmap@B@logical) are correct, but the point (\expandafter\pgfplots@extractcoord\pgfplots@plot@handler@graphics@pointmap@C@logical) is wrong (its position vector has an error of (\the\pgf@x,\the\pgf@y) which is larger than 'squeeze tol=\pgfkeysvalueof{/pgfplots/plot graphics/squeeze tol}'). This is probably caused by improper relations between the axis' unit vectors because the view is incorrect.^^J
			- Is the 'view' argument correct (matlab: [h,v] = view)? ^^J
			- Does your image have a non-trivial 'plot box ratio' (matlab: get(gca,'PlotBoxAspectRatio') )?^^J
		...
		\pgfsetfillcolor{red}%
		\pgfpathcircle{\pgfplots@plot@handler@graphics@pointmap@C@canvas@check}{2pt}%
		\pgfusepath{stroke,fill}%
		\pgfsetfillcolor{green}%
		\pgfpathcircle{\pgfplots@plot@handler@graphics@pointmap@C@canvas}{2pt}%
		\pgfusepath{stroke,fill}%
		\draw[->,red] 
			\pgfextra{
				\pgfpathmoveto{\pgfplots@plot@handler@graphics@pointmap@C@canvas@check}%
				\pgfpathlineto{\pgfplots@plot@handler@graphics@pointmap@C@canvas}};
		\endscope
	\fi
}
