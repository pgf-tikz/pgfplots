...
\def\pgfplots@scaleaxes@to@BB@#1#2{%
	...
	%  "1" == upper axis limit
	%
	%  FIXME : these here are no relative limits if unit vectors have
	%  been provided explicitly! They are UNITS and have nothing todo
	%  with axis limits! The algorithm here applies to 'plot box
	%  ratio', not to 'unit vector ratio'!
	\ifpgfplots@threedim
...
%\message{PGFPLOTS: the current unit vectors result in a UNIT BB of (\the\pgf@xa,\the\pgf@xb). Scaling it to (\the\pgf@ya,\the\pgf@yb)...}%
	...
	\or
		...
		%
		% FIXME : that's not correct. What do I want? 
		% - keep the 'unit vector ratio'? then, this here is not enough.
		% - keep the 'plot box ratio'? Yes, that should work.
		\if\pgfplots@user@provided@height\pgfplots@user@provided@width
			\pgfplots@warning{scale mode=scale uniformly  expected EITHER 'width' OR 'height'... it will now scale to fit the widths '#1'.}%
		...
		\else
			...
			\else
				...
				\immediate\write
					...
					{PGFPlots plot graphics[auto adjust axis=true] {\the\t@pgfplots@tokc} \csname on@line\endcsname: using '\pgfplotsretval'. Use 'plot graphics[debug] to generate debug output files.^^J}% 
				\endgroup
			\fi
			%
			\if1\pgfplotsplothandlergraphicspointmapcomputerequiredview@debug
...
\def\pgfplots@plot@handler@graphics@process@pointmap{%
	...
	\pgfpointadd
		{\pgfplots@plot@handler@graphics@pointmap@A@canvas}% the canvas coordinate corresponding to 'A'
		{\pgfqpointscale{-1}
			{\pgfqpoint\pgfplots@vx\pgfplots@vy}%
		}%
