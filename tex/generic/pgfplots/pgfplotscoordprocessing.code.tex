...
%\message{and 1/(xmax-xmin) = 1/(\pgfplots@xmax-\pgfplots@xmin) = \pgfmathresult.}%
	...
	%
	\pgfkeysgetvalue{/pgfplots/x post scale}\pgfmathresult
	\pgf@xx=\pgfmathresult\pgf@xx
	\pgf@xy=\pgfmathresult\pgf@xy
	%
...
%\message{and 1/(ymay-ymin) = 1/(\pgfplots@ymax-\pgfplots@ymin) = \pgfmathresult.}%
	...
	%
	\pgfkeysgetvalue{/pgfplots/y post scale}\pgfmathresult
	\pgf@yx=\pgfmathresult\pgf@yx
	\pgf@yy=\pgfmathresult\pgf@yy
	%
	\ifpgfplots@threedim
...
%\message{and 1/(zmaz-zmin) = 1/(\pgfplots@zmax-\pgfplots@zmin) = \pgfmathresult.}%
		...
		%
		\pgfkeysgetvalue{/pgfplots/z post scale}\pgfmathresult
		\pgf@zx=\pgfmathresult\pgf@zx
		\pgf@zy=\pgfmathresult\pgf@zy
	\fi
...
\def\pgfplots@plot@handler@graphics@parsepointmap@loop@@@(#1,#2)(#3,#4){%
	...
	\else
		\pgfplotsplothandlergraphicspointmappoint(#1,#2,)(#2,#4)
	\fi
	\pgfplots@plot@handler@graphics@parsepointmap@loop
}
...
\def\pgfplots@plot@handler@graphics@pointmap@sanitize@scaling{%
	...
	\if0\pgfplots@is@the@same@point
		...
		\pgfusepath{fill}%
		%
		% idea: use 'z post scale' to fix this !?
		%\pgfplots@plot@handler@graphics@pointmap@C@canvas
		%\edef\pgfplots@Y{\pgf@sys@tonumber\pgf@y}%
		%\pgfplots@plot@handler@graphics@pointmap@C@canvas@check
		%\edef\pgfplots@YY{\pgf@sys@tonumber\pgf@y}%
		%\pgfmath@basic@divide@\pgfplots@Y\pgfplots@YY
		\pgfpointdiff
			\pgfplots@plot@handler@graphics@pointmap@C@canvas@check
			\pgfplots@plot@handler@graphics@pointmap@C@canvas
		\edef\pgfmathresult{\pgf@sys@tonumber\pgf@y}%
		\pgfmath@basic@multiply@\pgfmathresult\pgfplots@z@inverseveclength
		%\pgfmath@basic@add@{1}{\pgfmathresult}%
		%
		\pgfplots@warning{Plot graphics: sorry, I failed to provide the correct scale for the axis. The points '(\pgfplots@plot@handler@graphics@pointmap@A@logical)' and '(\pgfplots@plot@handler@graphics@pointmap@B@logical)' are correct, but the point '(\pgfplots@plot@handler@graphics@pointmap@C@logical)' is wrong (its position vector has an error of (\the\pgf@x,\the\pgf@y)). This can probably be fixed by using the correct zmin/zmax values for the AXIS. Use z post scale=\pgfmathresult.}%
	\fi
}
