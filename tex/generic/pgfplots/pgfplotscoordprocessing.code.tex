...
\long\def\pgfplots@addplotimpl@graphics@@#1#2#3#4#5{%
	...
	\else
		...
		\expandafter\pgfplots@plot@handler@graphics@parsepointmap\pgfplots@current@point@points\pgfplots@EOI
		%
...
\def\pgfplots@plot@handler@graphics@finish{%
	...
	\else
		\expandafter\pgfplots@plot@handler@graphics@parsepointmap\pgfplots@plot@handler@graphics@pointmap\pgfplots@EOI
	\fi
	%
	%
\message{plot graphics: B@canvas is \meaning\pgfplots@plot@handler@graphics@pointmap@B@canvas.^^J}%
...
% Idea: re-use the 'points' syntax with option '=> '.
% This allows to update the bounding box without too much overhead.
%
\def\pgfplots@plot@handler@graphics@parsepointmap{%
...
\def\pgfplotsplothandlergraphicspointmappoint(#1,#2,#3)(#4,#5){%
	...
	\else
		...
		\fi
		\ifx\pgfplots@plot@handler@graphics@pointmap@A@canvas\pgfutil@empty
			\def\pgfplots@loc@TMPa{A}%
		\else
			\ifx\pgfplots@plot@handler@graphics@pointmap@B@canvas\pgfutil@empty
				\def\pgfplots@loc@TMPa{B}%
			\else
				\ifx\pgfplots@plot@handler@graphics@pointmap@C@canvas\pgfutil@empty
					\def\pgfplots@loc@TMPa{C}%
				\else
			...
			\fi
		\fi
		\expandafter\edef\csname pgfplots@plot@handler@graphics@pointmap@\pgfplots@loc@TMPa @logical\endcsname{#1,#2,#3}%
		%
		\expandafter\edef\csname pgfplots@plot@handler@graphics@pointmap@\pgfplots@loc@TMPa @canvas\endcsname{\pgf@x=\the\pgf@x\space\pgf@y=\the\pgf@y\space}%
		%
		\pgfmathparse{#4}%
		\let\pgfplots@loc@TMPb=\pgfmathresult
		%
		\pgfmathparse{#5}%
		\expandafter\edef\csname pgfplots@plot@handler@graphics@pointmap@\pgfplots@loc@TMPa @img\endcsname{\pgf@x=\pgfplots@loc@TMPb pt \pgf@y=\pgfmathresult pt }%
	\fi
}%
...
\def\pgfplots@plot@handler@graphics@process@pointmap{%
	...
	%
	\pgfplots@plot@handler@graphics@pointmap@A@img% anchor 1 in image
	...
	%   DD := B - A
	\pgfpointdiff
		\pgfplots@plot@handler@graphics@pointmap@A@img
		\pgfplots@plot@handler@graphics@pointmap@B@img
	\edef\pgfplots@DDx{\pgf@sys@tonumber\pgf@x}%
	\edef\pgfplots@DDy{\pgf@sys@tonumber\pgf@y}%
	%
...
\def\pgfplots@plot@handler@graphics@pointmap@sanitize@scaling{%
	\pgfplots@plot@handler@graphics@pointmap@C@img% anchor 3 in image
...
\def\pgfplots@imgunit@x@len{261pt}%
\def\pgfplots@imgunit@y@len{215pt}%
\def\pgfplots@imgunit@z@len{65pt}%
%
\pgfmath@basic@reciprocal@\pgfplots@imgunit@z@len
\let\pgfplots@loc@TMPa=\pgfmathresult
%
\pgfmath@basic@multiply@\pgfplots@loc@TMPa\pgfplots@imgunit@x@len
\let\pgfplots@imgunit@x@len=\pgfmathresult
\pgfmath@basic@multiply@\pgfplots@loc@TMPa\pgfplots@imgunit@y@len
\let\pgfplots@imgunit@y@len=\pgfmathresult
\edef\pgfplots@loc@TMPb{unit vector ratio={\pgfplots@imgunit@x@len\space\pgfplots@imgunit@y@len\space1}}%
		%
		%
		\begingroup
		\pgfkeysgetvalue{/pgfplots/plot graphics/src}\pgfplots@loc@TMPa
		\t@pgfplots@tokc=\expandafter{\pgfplots@loc@TMPa}%
		\pgfplots@warning{sorry, I can't fix the scaling of 'plot graphics {\the\t@pgfplots@tokc}'. The points (\pgfplots@plot@handler@graphics@pointmap@A@logical) and (\pgfplots@plot@handler@graphics@pointmap@B@logical) are correct, but the point (\pgfplots@plot@handler@graphics@pointmap@C@logical) is wrong (its position vector has an error of (\the\pgf@x,\the\pgf@y) which is larger than 'squeeze tol=\pgfkeysvalueof{/pgfplots/plot graphics/squeeze tol}'). This is probably caused by improper relations between the axis' limits: the image has probably different min/max values. Try enlargelimits=false or try to acquire the picture's axis limits. AAH - use '\pgfplots@loc@TMPb'!}%
		\endgroup
		%
		%
		\scope
