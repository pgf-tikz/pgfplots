...
\def\pgfplots@qtri@next@A{B}%
\def\pgfplots@qtri@next@B{C}%
\def\pgfplots@qtri@next@C{D}%
\def\pgfplots@qtri@next@D{E}%
\def\pgfplots@qtri@next@E{F}%
\def\pgfplots@qtri@next@F{A}%

\def\pgfplots@qtri@rnext@A{F}%
\def\pgfplots@qtri@rnext@B{A}%
\def\pgfplots@qtri@rnext@C{B}%
\def\pgfplots@qtri@rnext@D{C}%
\def\pgfplots@qtri@rnext@E{D}%
\def\pgfplots@qtri@rnext@F{E}%
...
%\message{EDGE INIT^^J}%
		...
		\ifx\pgfplotspatchvertexextra\pgfutil@empty
			\def\Pcur{A}%
			\def\Pnextseq{}%
		\else
			...
			%  or backward.
			%
			\pgf@xa=\pgfplotspatchvertexextra pt
			\expandafter\pgfplotspatchvertex\pgfplotspatchclass@qtri@C\endvertex
			\pgf@xb=\pgfplotspatchvertexextra pt
			\expandafter\pgfplotspatchvertex\pgfplotspatchclass@qtri@E\endvertex
			\pgf@xc=\pgfplotspatchvertexextra pt
			%
			\def\minsofarP{A}%
			\pgf@ya=\pgf@xa
			\ifdim\pgf@xb<\pgf@ya
			...
			\else
				...
				\fi
			\fi
%\message{FOREGROUND VERTEX = \minsofarP.^^J}%
%\message{SECOND NEAREST VERTEX = \secondnearest.^^J}%
			\let\Pcur=\secondnearest%
...
\pgfplotsdeclarepatchclass{bilinear}{%
	...
	set next vertex={%
		...
		\or
			\edef\pgfplotspatchclass@rect@D{#1}%
			\def\pgfplotspatchclass@rect@no{0}%
\message{bilinear ready:^^J\pgfplotspatchclass@rect@A^^J\pgfplotspatchclass@rect@B^^J\pgfplotspatchclass@rect@C^^J\pgfplotspatchclass@rect@D^^J.^^J}%
			\pgfplotspatchready%
		\fi
	},
	first vertex=\expandafter\pgfplotspatchvertex\pgfplotspatchclass@rect@A\endvertex,
	...
	stream to shader={%
		...
		\begingroup
		\def\pgfplots@coonsedge{%
			\ifx\PA\pgfplotspatchclass@rect@A
				\expandafter\pgfplotspatchvertex\PA\endvertex
					\let\pgfplots@current@point@meta=\pgfplotspatchvertexmeta
					\pgfplotsaxisvisphasetransformpointmeta
				\pgfplotslibrarysurfstreamcoord{\pgfqpoint\pgfplotspatchvertexx\pgfplotspatchvertexy}{\pgfplotspointmetatransformed}%
			...
			\ifx\PB\pgfplotspatchclass@rect@A
			\else
		...
		%
		\let\PA=\pgfplotspatchclass@rect@A
		\let\PB=\pgfplotspatchclass@rect@B
		\pgfplots@coonsedge
		%
		\let\PA=\pgfplotspatchclass@rect@B
		\let\PB=\pgfplotspatchclass@rect@C
		\pgfplots@coonsedge
		%
		\let\PA=\pgfplotspatchclass@rect@C
		\let\PB=\pgfplotspatchclass@rect@D
		\pgfplots@coonsedge
		%
		\let\PA=\pgfplotspatchclass@rect@D
		\let\PB=\pgfplotspatchclass@rect@A
		\pgfplots@coonsedge
		%
		\endgroup
	},%
...
% If \pgfplotsplothandlermesh@determine@streamdata is set to this
% particular implementation, the \pgfplotspatchvertexextra macro will
% contain the view depth.
%
\def\pgfplotsplothandlermesh@determine@streamdata@depth{%
...
\def\pgfplotsplothandlermesh@init@zbuffer{%
	\ifpgfplots@curplot@threedim
		...
		\if1\pgfplotsretval
			\let\pgfplotsplothandlermesh@determine@streamdata=\pgfplotsplothandlermesh@determine@streamdata@depth%
		\fi
		%
