...
\def\pgfplots@plot@expression@preparekeys{%
	...
	\pgfkeyssetvalue{/pgfplots/mesh/cols}{\pgfkeysvalueof{/pgfplots/samples y}}%
...
\def\pgfplots@addplotimpl@function@opt@#1#2#3#4#5{%
	\pgfplots@start@plot@with@behavioroptions{#1,#2,#3,/pgfplots/mesh/ordering/colwise}%
	%
...
\def\pgfplots@apply@zbuffer#1{%
	...
	\or
		...
		\else
			...
			\fi
		\fi
%\message{z buffer=auto mode chose  z buffer= \ifcase\pgfplotsplothandlermesh@zbuffer@choice NONE \or reverse x seq \or reverse y seq \or reverse xy seq \or sort\or default \fi. mesh ordering = \ifcase\pgfplots@plot@mesh@ordering rowwise\or colwise\fi}%
