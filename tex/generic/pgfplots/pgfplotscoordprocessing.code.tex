		...
		\else
			...
			\else
				...
				\else
					...
					\else
						\def\pgfplotsplothandlergraphicspointmappointindex{*}%
						%\pgfplots@error{Sorry, the argument '(#1,#2) => (#3,#4)' of plot graphics/pointmap is superfluous; ignoring it.}%
					\fi
				\fi
			\fi
		\fi
...
\def\pgfplots@zbuffer@error{%
	\pgfplots@error{An internal error occurred during z buffer reorderings: the rows/cols where not balanced! I have rows= \pgfkeysvalueof{/pgfplots/mesh/rows}, cols=\pgfkeysvalueof{/pgfplots/mesh/cols}. If this happens to be wrong, you might want to provide rows and cols manually.}%
}
