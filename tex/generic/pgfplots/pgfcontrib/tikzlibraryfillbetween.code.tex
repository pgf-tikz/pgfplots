...
\pgfkeys{%
	/tikz/fill between/of/.code=\tikzlibraryfillbetween@parse#1\pgfplots@EOI,
	%
...
\def\tikzlibraryfillbetween@parse#1 and #2\pgfplots@EOI{%
...
\def\tikzfillbetween@opt[#1]#2{%
	...
	%
	\pgfplotsonlayer{\tikzlibraryfillbetween@path@layer@name}%
	%
...
\def\tikzlibraryfillbetween@path@warn@layer{%
	\pgfplots@warning{'fill between': Could not activate graphics layer '\tikzlibraryfillbetween@path@layer@name'. Filled path will be on top of the other ones. Please ensure that '/pgfplots/activate layers=true' and '\tikzlibraryfillbetween@path@layer@name' is somewhere in the layer list (or set '/tikz/fill between/on layer=').}%
}%
